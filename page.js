...
function airlineFromMawb(mawb=''){
  ...
  return { name:prefix?`Airline prefix ${prefix}`:'Unknown airline', iata:'', official:prefix?`https://www.google.com/search?q=${encodeURIComponent('AWB prefix '+prefix+' official airline cargo tracking')}`:'' };
}
...
export default function Home(){
  ...
  function saveManual(e){ e.preventDefault(); if(!form.mawb) return setNotice('Enter a MAWB number.'); upsertMawbOnly(form.mawb); setForm(EMPTY); setManualOpen(false); setNotice('MAWB saved. Airline detected from AWB prefix.'); }
  ...
  function saveBags(id){
    ...
    setBagEditId(''); setBagEditValue(''); setNotice('Number of bags saved manually.');
  }

  async function trackOne(id, forceRefresh=false){
    const s=shipments.find(x=>x.id===id); if(!s) return;
    const prefix=String(s.mawb).replace(/\D/g,'').slice(0,3);
    const airline=airlineFromMawb(s.mawb);
    setBusy(true); setNotice(`Checking ${s.mawb} — ${airline.name} first…`);
    try{
      ...
      const p=localParts(live.actualArrival||live.eta);
      const liveBags=live.bags ?? live.pieces ?? live.pcs ?? '';
      const liveWeight=live.weight ?? live.grossWeight ?? '';
      const isDirect=String(d.source||live.source||'').includes('Emirates SkyCargo');
      setShipments(v=>v.map(x=>x.id===id?{
        ...
        origin: prefix==='176' ? (isDirect ? (live.origin||'') : '') : (live.origin||x.origin),
        arrivalDate:p.date||x.arrivalDate,
        arrivalTime:p.time||x.arrivalTime,
        flightNo:live.flightNo||x.flightNo,
        updatedAt:new Date().toISOString(),
        remarks:`LIVE — ${d.source||live.source||'tracking response'}`
      }:x));
      if(p.date&&p.time) setNotice(`${airline.name} live ETA received for ${s.mawb}: ${p.date} ${p.time}.`);
      else setNotice(`${airline.name}: shipment found, but no machine-readable ETA was returned. Track123 fallback was checked without inventing a time.`);
  ...
  async function uploadFile(e){
    ...
    try{
      let text;
      if(file.type==='application/pdf'||file.name.toLowerCase().endsWith('.pdf')) text=await readPdf(file); else text=await runOcr(file);
  ...
  return <main>
    <header>
      ...
      <div className="right"><div>{new Date().toLocaleDateString('en-IN')}</div><span className={apiConfigured?'live':'offline'}>{apiConfigured?'● TRACK123 FALLBACK CONNECTED':'● TRACK123 FALLBACK MISSING'}</span></div>
    </header>
    ...
    <section className="tableWrap">
      <table><thead><tr><th>MAWB</th><th>Airline</th><th>Bags</th><th>Weight</th><th>Origin</th><th>Arrival Date</th><th>Arrival Time</th><th>Mail Due</th><th>Mail Alert</th><th>Status</th><th>Remarks</th><th></th></tr></thead>
      <tbody>{!shipments.length?<tr><td colSpan="12" className="empty">No shipments yet. Add a MAWB or upload a MAWB PDF/photo.</td></tr>:shipments.map(s=>{const airline=airlineFromMawb(s.mawb);return <tr key={s.id} className={statusClass(s.status)}>
        ...
        <td>{s.weight?`${s.weight} kg`:'—'}</td><td>{s.origin||'—'}</td><td>{s.arrivalDate||'—'}</td><td>{s.arrivalTime||'—'}</td><td>{mailDue(s)||'—'}</td><td><b>{mailAlert(s)}</b></td><td><b>{s.status||'—'}</b><small>{s.updatedAt?`Updated ${new Date(s.updatedAt).toLocaleTimeString()}`:''}</small></td><td>{s.remarks||'—'}</td><td className="actions"><button onClick={()=>trackOne(s.id,false)} disabled={busy}>Track Live</button><button className="x" onClick={()=>remove(s.id)}>×</button></td>
      </tr>})}</tbody></table>
    </section>
    <section className="help"><b>Airline-first tracking.</b> For Emirates 176 AWBs, Mayavi attempts Emirates SkyCargo first and uses Track123 only as fallback. Unverified origin/status text is discarded, and Mayavi never invents ETA.</section>

    {manualOpen&&<div className="modal"><form onSubmit={saveManual}><h2>Add Shipment</h2><div className="grid"><label>MAWB*<input value={form.mawb} onChange={e=>setForm({...form,mawb:e.target.value})} placeholder="020-12345678" required/></label></div><div className="modalBtns"><button type="button" onClick={()=>setManualOpen(false)}>Cancel</button><button className="primary" type="submit">Save MAWB</button></div></form></div>}
  </main>
}
