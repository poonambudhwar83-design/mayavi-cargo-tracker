...
function openOfficial(row={}){
  ...
  if(n.startsWith('514-')){
    const awb=digits(n);
    try{navigator.clipboard?.writeText(awb).catch(()=>{})}catch{}
  }
...
export default function Page(){
 ...
 return <main>
   <section className="hero"><div><div className="eyebrow">MAYAVI CARGO • V3.8.4</div><h1>Global MAWB Live Tracker</h1><p>Shared cloud database + automatic airline tracking. Add or refresh a MAWB once and the same saved row is visible to everyone opening this tracker.</p></div><div className="version">{shared?'SHARED DATABASE ✓':'LOCAL BACKUP'}</div></section>
