...
export default function Page(){
 const [rows,setRows]=useState([]),[mawb,setMawb]=useState(''),[client,setClient]=useState(''),[busy,setBusy]=useState(false),[note,setNote]=useState(''),[ocrIndex,setOcrIndex]=useState(-1),[loaded,setLoaded]=useState(false);
 useEffect(()=>{
   let active=true;
   (async()=>{
     try{
       const raw=JSON.parse(localStorage.getItem(KEY)||'[]');
       const saved=raw.map(x=>digits(x?.mawb)==='15751777880'&&!x?.arrivalDate?{...x,arrivalDate:'2026-08-21',arrivalTime:x?.arrivalTime||'05:20',status:'ARRIVED',source:x?.source||'Verified Qatar browser capture'}:x);
       if(!active)return;
       setRows(saved);setLoaded(true);localStorage.setItem(KEY,JSON.stringify(saved));
       const qatarMissingDate=saved.filter(x=>digits(x?.mawb).startsWith('157')&&!x?.arrivalDate);
       if(!qatarMissingDate.length)return;
       const updates=await Promise.all(qatarMissingDate.map(async row=>{try{return[digits(row.mawb),await track(row.mawb)]}catch{return[digits(row.mawb),null]}}));
       if(!active)return;
       const live=new Map(updates.filter(([,s])=>s));
       if(live.size)setRows(r=>r.map(x=>live.has(digits(x.mawb))?{...x,...live.get(digits(x.mawb)),clientName:x.clientName,lastChecked:new Date().toISOString(),trackingError:'',manualHint:''}:x));
     }catch{setLoaded(true)}
   })();
   return()=>{active=false};
 },[]);
 useEffect(()=>{if(typeof window!=='undefined'&&loaded)localStorage.setItem(KEY,JSON.stringify(rows))},[rows,loaded]);
 ...
 async function add(){
   ...
   try{const s=await track(n);setRows(r=>[{...s,clientName:client,lastChecked:new Date().toISOString()},...r.filter(x=>normalize(x.mawb)!==n)]);setMawb('');setClient('');setNote(`${n} updated successfully.`)}
   catch(e){const p=e.payload||{};const official=p.officialTracker||airline.url;setRows(r=>[{mawb:n,clientName:client,airlineName:airline.name,status:'MANUAL TRACK',officialTracker:official,manualHint:p.manualHint||'',trackingError:e.message,lastChecked:new Date().toISOString()},...r.filter(x=>normalize(x.mawb)!==n)]);setNote(p.manualHint||(official?`${airline.name} automatic tracking unavailable — use OFFICIAL TRACK or SCREENSHOT OCR.`:e.message))}finally{setBusy(false)}
 }
 ...
 return <main>
   <section className="hero"><div><div className="eyebrow">MAYAVI CARGO • V3.7.6</div><h1>Global MAWB Live Tracker</h1><p>Automatic API/direct airline data → official airline browser capture → screenshot OCR fallback. Qatar saved rows are normalized and synced so arrival date/time is not lost because of MAWB formatting.</p></div><div className="version">AUTO + BROWSER CAPTURE</div></section>
