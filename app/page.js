...
export default function Page(){
 ...
 async function track(one){
   ...
   const data=await res.json();if(!data.ok)throw new Error(data.trackingError||data.apiFallbackError||data.error||'Tracking failed');
 ...
 async function add(){
   ...
   catch(e){setRows(r=>[{mawb:n,clientName:client,airlineName:airline.name,status:'CHECKING',trackingError:e.message,lastChecked:new Date().toISOString()},...r.filter(x=>x.mawb!==n)]);setNote(e.message)}finally{setBusy(false)}
 }
 ...
 return <main>
   <section className="hero"><div><div className="eyebrow">MAYAVI CARGO • V3</div><h1>Global MAWB Live Tracker</h1><p>Enter one MAWB. Mayavi identifies the airline from its prefix, uses the global air-cargo API first, then the official airline tracker as fallback.</p></div><div className="version">API FIRST</div></section>
   ...
   <footer>{CONFIGURED_PREFIXES.length} airline prefixes mapped • Germany • UK • USA • Japan • Bangkok • Dubai/UAE • Saudi/Jeddah • Qatar • Bahrain • Canada • Europe/Asia connections</footer>
 </main>
}
