...
export default function Page(){
 ...
 async function add(){
   ...
   catch(e){const p=e.payload||{};const official=p.officialTracker||airline.url;setRows(r=>[{mawb:n,clientName:client,airlineName:airline.name,status:'MANUAL TRACK',officialTracker:official,trackingError:e.message,lastChecked:new Date().toISOString()},...r.filter(x=>x.mawb!==n)]);setNote(official?`${airline.name} automatic tracking unavailable — use OFFICIAL TRACK button.`:e.message)}finally{setBusy(false)}
 }
 async function refresh(index){const row=rows[index];setNote(`Refreshing ${row.mawb}…`);try{const s=await track(row.mawb);setRows(r=>r.map((x,i)=>i===index?{...x,...s,clientName:x.clientName,lastChecked:new Date().toISOString(),trackingError:''}:x));setNote(`${row.mawb} refreshed.`)}catch(e){const p=e.payload||{};setRows(r=>r.map((x,i)=>i===index?{...x,status:'MANUAL TRACK',officialTracker:p.officialTracker||x.officialTracker,trackingError:e.message,lastChecked:new Date().toISOString()}:x));setNote((p.officialTracker||row.officialTracker)?'Automatic tracking unavailable — use OFFICIAL TRACK button.':e.message)}}
