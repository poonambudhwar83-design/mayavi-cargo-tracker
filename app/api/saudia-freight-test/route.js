...
export async function GET(req){
  ...
  if(full.length!==11||!full.startsWith('065'))return Response.json({ok:false,error:'valid 065 mawb required'},{status:400});
  const serial=full.slice(3);
  const pageUrl=`${PAGE}?Carrier=SV&Pfx=065&Portlet=yes&Shipment=${serial}&Site=CargoWeb`;
  try{
    ...
    const body={portalId,awbInfo:[{awbNumber:full,carrierId:'SV',trackingAirline:''}],captchaResponse:'FREIGHT_LOGGED_CAPTCHA'};
    ...
    return Response.json({ok:response.ok,pageStatus:page.status,trackStatus:response.status,portalId,uuid:Boolean(uuid),payload,preview:text.slice(0,3000)});
