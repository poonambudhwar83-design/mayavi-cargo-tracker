...
async function detectCarrier(apiKey, trackingNo, supplied = '') {
  ...
  try {
    const r = await fetch(CARRIERS_URL, {
      headers: { 'Track123-Api-Secret': apiKey, Accept: 'application/json' },
      cache: 'no-store'
    });
    if (!r.ok) return '';
...
const EVENT_TIME_FIELDS = [
  'actualDateTime','eventDateTime','dateTime','timestamp','occurredAt',
  'actualArrivalDateTime','arrivalActualDateTime','flightActualArrivalDateTime',
  'estimatedDateTime','estimatedArrivalDateTime','flightEstimatedArrivalDateTime',
  'scheduledDateTime','scheduledArrivalDateTime','flightScheduledArrivalDateTime',
  'actualArrival','arrivedAt','estimatedArrival','scheduledArrival','eta','etaDateTime'
];

const EVENT_LABEL_FIELDS = [
  'eventCode','milestoneCode','code','eventDetail','eventName','milestone',
  'statusName','status','description','remark','activity'
];

const ARRIVAL_RE = /(^|\W)ARR($|\W)|ARRIVED|ACTUAL ARRIVAL|FLIGHT ARRIVAL|RECEIVED FROM FLIGHT|(^|\W)RCF($|\W)|ARRIVAL COMPLETE|FLIGHT LANDED|LANDED/i;

function eventLabel(node) {
  return EVENT_LABEL_FIELDS
    .map(k => node && node[k] != null ? String(node[k]) : '')
    .filter(Boolean)
    .join(' ')
    .toUpperCase();
}
...
function splitDateTime(node, mode = 'event') {
  if (!node || typeof node !== 'object') return null;
  const actualDates = ['actualArrivalDate','arrivalActualDate','flightActualArrivalDate','actualDate','eventDate','date','arrivalDate','flightArrivalDate','arrDate'];
  const actualTimes = ['actualArrivalTime','arrivalActualTime','flightActualArrivalTime','actualTime','eventTime','time','arrivalTime','flightArrivalTime','arrTime'];
  const estimateDates = ['estimatedArrivalDate','scheduledArrivalDate','etaDate','plannedArrivalDate','arrivalDate','flightArrivalDate'];
  const estimateTimes = ['estimatedArrivalTime','scheduledArrivalTime','etaTime','plannedArrivalTime','arrivalTime','flightArrivalTime'];
  const dates = mode === 'estimate' ? estimateDates : actualDates;
  const times = mode === 'estimate' ? estimateTimes : actualTimes;
  return combineDateAndTime(pickFirst(node, dates), pickFirst(node, times));
}

function eventTime(node) {
  const direct = pickFirst(node, EVENT_TIME_FIELDS);
  if (direct && validDateValue(direct)) return direct;
  return splitDateTime(node, 'event');
}

function siblingArrivalDateTime(node) {
  if (!node || typeof node !== 'object') return null;
  const combined = splitDateTime(node, 'estimate');
...
function findArrivalMilestone(raw) {
  const nodes = walkObjects(raw).filter(n => eventLabel(n) && ARRIVAL_RE.test(eventLabel(n)));
...
function findActualArrival(raw) {
  const candidates = [];
  for (const node of walkObjects(raw)) {
    const label = eventLabel(node);
    const hasActualArrivalKey = Object.keys(node || {}).some(k => /actual.*arrival|arrival.*actual|arrivedAt/i.test(k));
    if (!ARRIVAL_RE.test(label) && !hasActualArrivalKey) continue;

    for (const key of ['actualArrivalDateTime','arrivalActualDateTime','flightActualArrivalDateTime','actualArrival','arrivedAt']) {
      if (node[key] && validDateValue(node[key])) {
        candidates.push({ value: node[key], time: toDateValue(node[key]).getTime() });
      }
    }

    const split = splitDateTime(node, 'event');
    if (split && validDateValue(split)) {
      candidates.push({ value: split, time: toDateValue(split).getTime() });
    }

    const t = eventTime(node);
    if (t && validDateValue(t) && ARRIVAL_RE.test(label)) {
      candidates.push({ value: t, time: toDateValue(t).getTime() });
    }
  }
...
function findArrivalEstimate(raw, destination = '') {
  ...
  for (const node of walkObjects(raw)) {
    ...
    if (combined && destMatch) {
      ...
      if (d) candidates.push({ value: combined, time: d.getTime(), score: 7 });
    }
    ...
    if (t && /ARRIVAL|ARRIVED|(^|\W)ARR($|\W)/i.test(label) && destMatch) {
      ...
      if (d) candidates.push({ value: t, time: d.getTime(), score: /ESTIMAT|SCHEDUL|PLANNED/.test(label) ? 8 : 3 });
    }
  }
...
function normalizeResponse(raw, fallbackMawb, detectedCarrier = '') {
  ...
  const actualArrival = findActualArrival(best) || findActualArrival(raw) ||
    (arrivalEvent && validDateValue(eventTime(arrivalEvent)) ? eventTime(arrivalEvent) : null);
  const eta = actualArrival || findArrivalEstimate(best, destination) || findArrivalEstimate(raw, destination) || null;
  const statusText = String(summaryStatus || '');
  const arrivedByStatus = ARRIVAL_RE.test(statusText) || ARRIVAL_RE.test(eventLabel(arrivalEvent));
  const status = actualArrival || arrivalEvent || arrivedByStatus ? 'ARRIVED' : (summaryStatus ? String(summaryStatus) : 'Tracking record found');
...
function parseEmiratesHtml(html, trackingNo) {
  ...
  for (const re of etaPatterns) {
    const m = text.match(re);
    if (m && toDateValue(m[1])) {
      eta = toDateValue(m[1]).toISOString();
      break;
    }
  }
  ...
  for (const re of actualPatterns) {
    const m = text.match(re);
    if (m && toDateValue(m[1])) {
      actualArrival = toDateValue(m[1]).toISOString();
      break;
    }
  }
...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({
      error: prefix === '176'
        ? 'Emirates SkyCargo did not expose machine-readable shipment details, and Track123 fallback was unavailable.'
        : 'No live tracking source returned shipment details.'
    }, { status: 502 });
  } catch (e) {
