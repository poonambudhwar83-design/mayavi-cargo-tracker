...
function parseVisibleText(text='',mawb=''){
  ...
  return{mawb,carrierCode:'EK',airlineName:'Emirates SkyCargo',origin:route?.[1]||'',destination:route?.[2]||'',bags:pieces,pieces,weight,flightNo,arrivalDate,arrivalTime,arrivalIsActual,status,officialTracker:HOME,source:'Emirates eSkyCargo visible tracking data'};
}
...
async function trackingDetailsScreenshot(page){
  const clip=await page.evaluate(()=>{
    ...
    for(let i=0;i<5&&el;i++,el=el.parentElement){const r=el.getBoundingClientRect();if(r.width>500&&r.height>220&&r.height<1800)return{x:Math.max(0,r.x-20),y:Math.max(0,r.y-20),width:Math.min(window.innerWidth-Math.max(0,r.x-20),r.width+40),height:Math.min(window.innerHeight-Math.max(0,r.y-20),r.height+40)};}
    return null;
  }).catch(()=>null);
...
export async function trackEmirates(mawb){
  ...
  try{
    ...
    const showDetails=await clickByText(page,/^Show Details$|^Details$/i);if(showDetails)await sleep(2500);
    const trackingDetails=await clickByText(page,/^Tracking Details$/i);if(!trackingDetails)return{ok:false,reason:'EMIRATES TRACKING DETAILS TAB NOT FOUND',officialTracker:page.url(),debug:{url:page.url(),filled,showDetails}};
    await sleep(3000);
    ...
    if(!screenshotBase64)return{ok:false,reason:'EMIRATES TRACKING DETAILS SCREENSHOT FAILED',officialTracker:page.url()};
    ...
    shipment.source=ocrResult?.ok?'Emirates eSkyCargo Tracking Details screenshot OCR':'Emirates eSkyCargo visible tracking data';

    if(useful(shipment))return{ok:true,shipment,screenshotCaptured:true,screenshotVerified:true,screenshotOcrUsed:Boolean(ocrResult?.ok),debug:{url:page.url(),filled,showDetails,trackingDetails,ocrOk:Boolean(ocrResult?.ok),ocrSample:ocrResult?.debug?.textSample||'',ocrReason:ocrResult?.reason||'',bodySample:body.slice(0,2500),networkUrls:network.slice(-25).map(x=>x.url)}};
    return{ok:false,reason:ocrResult?.reason||'EMIRATES TRACKING DETAILS OCR FOUND NO SHIPMENT FIELDS',officialTracker:page.url(),screenshotCaptured:true,debug:{url:page.url(),filled,showDetails,trackingDetails,ocr:ocrResult?.debug||null,bodySample:body.slice(0,2500)}};
