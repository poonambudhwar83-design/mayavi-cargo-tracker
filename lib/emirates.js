...
function normalizeFromPanel(result){
  ...
  if(text){
    ...
    if(actualArrival){
      ...
      shipment.status=/\bDEP\b|Departed\s+to|Departed\s+from|Flight\s+departed/i.test(tracking)?'DEPARTED':'BOOKED';
    }else if(/\bDEP\b|Departed\s+to|Departed\s+from|Flight\s+departed/i.test(tracking)){
      shipment.status='DEPARTED';shipment.arrivalIsActual=false;delete shipment.arrivalDate;delete shipment.arrivalTime;
