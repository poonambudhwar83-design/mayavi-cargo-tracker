...
export async function trackEmirates(mawb){
  ...
  try{
    ...
    let shipment={mawb,carrierCode:'EK',airlineName:'Emirates SkyCargo',officialTracker:page.url(),source:'Emirates eSkyCargo visible tracking data'};
    ...
    shipment.source='Emirates eSkyCargo visible tracking data';
    ...
    return{ok:false,reason:'EMIRATES TRACKING DETAILS FOUND NO SHIPMENT FIELDS',officialTracker:HOME,screenshotCaptured:Boolean(screenshotBase64),debug:{url:page.url(),filled,showDetails,trackingDetails,bodySample:body.slice(0,3000),networkUrls:network.slice(-30).map(x=>x.url)}};
