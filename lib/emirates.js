...
function normalizeFromPanel(result){
  ...
  if(text){
    ...
    if(finalLeg&&!finalArrived){
      ...
      if(arrived){shipment.arrivalDate=parseDate(arrived[1]);shipment.arrivalTime=arrived[2].padStart(5,'0');shipment.arrivalIsActual=true;shipment.status='ARRIVED';shipment.arrivalEvidence=`Emirates final destination ${finalDest} arrival headline`;strongArrivalEvidence=true;}
      if(!strongArrivalEvidence){const parsed=parseArrivalEvidence(tracking,text,finalDest);if(parsed){shipment.arrivalDate=parsed.date;shipment.arrivalTime=parsed.time;shipment.arrivalIsActual=parsed.actual;if(parsed.actual){shipment.status='ARRIVED';strongArrivalEvidence=true;}}}
    }

    if(finalArrived){shipment.status='ARRIVED';shipment.arrivalIsActual=true;strongArrivalEvidence=true;shipment.arrivalEvidence=shipment.arrivalEvidence||`Emirates final destination ${finalDest} arrival confirmed`;}
