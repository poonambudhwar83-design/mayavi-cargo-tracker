...
function normalizeFromPanel(result){
  ...
  if(text){
    ...
    const latestMatch=tracking.match(/Latest\s+status(?:\s+at\s+[A-Z]{3})?.{0,180}?\b(Departed|DEP|Arrived|ARR|Received|RCF|Booked|BKD)\b/i)
      ||text.match(/Latest\s+status(?:\s+at\s+[A-Z]{3})?.{0,180}?\b(Departed|DEP|Arrived|ARR|Received|RCF|Booked|BKD)\b/i);
    const latestState=String(latestMatch?.[1]||'').toUpperCase();
    ...
    if(latestState==='DEPARTED'||latestState==='DEP'){
      ...
      shipment.arrivalIsActual=false;
      shipment.arrivalEvidence='Emirates latest status is Departed';
      strongArrivalEvidence=false;
    }else if(latestState==='ARRIVED'||latestState==='ARR'||latestState==='RECEIVED'||latestState==='RCF'){
      shipment.status='ARRIVED';
      shipment.arrivalEvidence='Emirates latest status confirms arrival';
      strongArrivalEvidence=true;
    }else if(latestState==='BOOKED'||latestState==='BKD'){
      shipment.status='BOOKED';
      strongArrivalEvidence=false;
    }else if(!strongArrivalEvidence){
