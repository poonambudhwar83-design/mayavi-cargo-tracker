...
function parseVisibleText(text='',mawb=''){
  ...
  else if(/ACTUAL\s+ARRIVAL|\bARRIVED\b|\bLANDED\b|RECEIVED\s+FROM\s+FLIGHT|\bRCF\b|NOTIFIED\s+OF\s+ARRIVAL|ARRIVAL\s+OF\s+SHIPMENT/i.test(actual||flat))status='ARRIVED';
  ...
  return{mawb,carrierCode:'EK',airlineName:'Emirates SkyCargo',origin:route?.[1]||'',destination:route?.[2]||'',bags:pieces,pieces,weight,flightNo,arrivalDate,arrivalTime,arrivalIsActual,status,officialTracker:HOME,source:'Emirates eSkyCargo live page'};
}
...
async function launch(){chromium.setGraphicsMode=false;return puppeteer.launch({args:[...chromium.args,'--no-sandbox','--disable-setuid-sandbox'],defaultViewport:{width:1280,height:900,deviceScaleFactor:1},executablePath:await chromium.executablePath(),headless:'shell'});}
...
export async function trackEmirates(mawb){
  ...
  try{
    ...
    const showDetails=await clickByText(page,/^Show Details$/i);if(showDetails)await sleep(3500);
    const trackingDetails=await clickByText(page,/^Tracking Details$/i);if(trackingDetails)await sleep(2800);
    const body=clean(await page.evaluate(()=>document.body?.innerText||'').catch(()=>''));
    const networkText=network.map(x=>x.text).join(' ');
    let shipment=merge({mawb,carrierCode:'EK',airlineName:'Emirates SkyCargo',officialTracker:page.url()},parseVisibleText(networkText,mawb));
    ...
    if(useful(shipment))return{ok:true,shipment,screenshotCaptured:Boolean(screenshotBase64),screenshotVerified:Boolean(screenshotBase64),screenshotOcrUsed:false,debug:{url:page.url(),filled,showDetails,trackingDetails,bodySample:body.slice(0,3000),networkUrls:network.slice(-30).map(x=>x.url)}};
    return{ok:false,reason:'EMIRATES LIVE RESULT FOUND NO SHIPMENT FIELDS',officialTracker:HOME,screenshotCaptured:Boolean(screenshotBase64),debug:{url:page.url(),filled,showDetails,trackingDetails,bodySample:body.slice(0,3000),networkUrls:network.slice(-30).map(x=>x.url)}};
