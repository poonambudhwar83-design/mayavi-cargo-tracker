...
async function launch(){
  ...
  return puppeteer.launch({args:[...chromium.args,'--no-sandbox','--disable-setuid-sandbox'],executablePath:await chromium.executablePath(),headless:'shell',defaultViewport:{width:1440,height:1100,deviceScaleFactor:1}});
}
async function rootsEval(frame,fn,...args){return frame.evaluate(fn,...args).catch(()=>null);}

async function acceptCookies(page){
  for(const frame of page.frames())await rootsEval(frame,()=>{
    ...
    for(const root of roots){const b=[...(root.querySelectorAll?.('button,a,[role="button"]')||[])].find(e=>/accept all cookies|^accept$|^allow all$/i.test(text(e)));if(b){b.click();return true;}}
    return false;
  });
}
...
async function findMawbInput(frame){
  ...
  for(const input of inputs){
    try{
      const meta=await input.evaluate(el=>{const r=el.getBoundingClientRect(),s=getComputedStyle(el);const text=`${el.placeholder||''} ${el.name||''} ${el.id||''} ${el.getAttribute('aria-label')||''} ${el.getAttribute('formcontrolname')||''}`;let score=0;if(/doc\.?\s*no|document|awb|air\s*way|waybill/i.test(text))score+=120;if(/176\d|e\.g\.?\s*176/i.test(text))score+=80;if(/email|password|login|username/i.test(text))score-=200;return{visible:r.width>60&&r.height>18&&s.display!=='none'&&s.visibility!=='hidden'&&!el.disabled,score,text};});
      if(meta.visible&&(!best||meta.score>best.meta.score))best={input,meta};
    }catch{}
  }
...
async function fillMawb(page,digits){
  for(let attempt=0;attempt<10;attempt++){
    for(const frame of page.frames()){
      ...
      try{
        await found.input.click({clickCount:3});
        await page.keyboard.press('Backspace');
        await found.input.type(digits,{delay:45});
        await sleep(250);
        const value=await found.input.evaluate(el=>String(el.value||''));
        if(digitsOnly(value)===digits)return{ok:true,value,meta:found.meta.text,frameUrl:frame.url()};
      }catch{}
    }
    await sleep(700);
  }
...
async function clickNativeExact(page,rx){
    ...
    for(const frame of page.frames()){
      ...
      for(const handle of handles){
        try{
          ...
          if(meta.visible&&!meta.disabled&&rx.test(meta.text)){await handle.click({delay:80});return{label:meta.text,frameUrl:frame.url()};}
        }catch{}
      }
    }
    await sleep(650);
  ...
  for(let attempt=0;attempt<8;attempt++){
    for(const frame of page.frames()){
      const r=await rootsEval(frame,value=>{
        ...
        for(const root of roots){const input=[...(root.querySelectorAll?.('input,textarea')||[])].find(e=>visible(e)&&ds(e.value)===value);if(!input)continue;let node=input;for(let depth=0;depth<8&&node;depth++,node=node.parentElement){const buttons=[...(node.querySelectorAll?.('button,input[type="submit"],input[type="button"],a,[role="button"]')||[])].filter(visible);const b=buttons.find(x=>/^Search$/i.test(text(x)));if(b){b.click();return{ok:true,label:text(b),depth};}}}
        return null;
      },digits);
  ...
  for(let attempt=0;attempt<12;attempt++){
    for(const frame of page.frames()){
      const r=await rootsEval(frame,(source,flags)=>{
        const re=new RegExp(source,flags),roots=[document];for(let i=0;i<roots.length;i++)for(const el of roots[i].querySelectorAll?.('*')||[])if(el.shadowRoot)roots.push(el.shadowRoot);
        const visible=e=>{try{const b=e.getBoundingClientRect(),s=getComputedStyle(e);return b.width>0&&b.height>0&&s.display!=='none'&&s.visibility!=='hidden';}catch{return false;}};const text=e=>String(e.innerText||e.value||e.textContent||e.getAttribute?.('aria-label')||e.getAttribute?.('title')||'').replace(/\s+/g,' ').trim();
        for(const root of roots){const els=[...(root.querySelectorAll?.('button,a,[role="button"],[role="tab"],input[type="submit"],input[type="button"],span,div,p')||[])].filter(visible);const exact=els.find(e=>re.test(text(e)));if(!exact)continue;const clickable=exact.closest?.('button,a,[role="button"],[role="tab"],[tabindex]')||exact;if(clickable.disabled||clickable.getAttribute?.('aria-disabled')==='true')continue;clickable.click();return{text:text(exact)};}
        return null;
      },rx.source,rx.flags);
...
function noMatchingRecord(text='',network=[],digits=''){
  const count=text.match(/Search\s+Results\s*\((\d+)\)/i);if(count&&Number(count[1])>0)return false;
  if(/Search\s+Results\s*\(0\)|No matching records? found/i.test(text))return true;
  const serial=digits.slice(3);const relevant=network.filter(n=>n.body.includes(digits)||n.body.includes(serial));
  return relevant.some(n=>/BOOKING_NOT_FOUND|There is no matching result found|no matching result found for your shipment reference/i.test(n.body));
}
...
export async function trackEmirates(mawb){
  const digits=digitsOnly(mawb);if(!/^176\d{8}$/.test(digits))return{ok:false,reason:'INVALID EMIRATES MAWB',officialTracker:HOME};
  let browser;
  try{
    ...
    let searched=await clickNativeExact(page,/^Search$/i);if(!searched)searched=await clickSearchNearMawb(page,digits);if(!searched){for(const frame of page.frames()){const found=await findMawbInput(frame);if(found){try{await found.input.press('Enter');searched={label:'Enter',frameUrl:frame.url()};break;}catch{}}}}if(!searched)searched=await clickDeepText(page,/^Search$/i);
    ...
    await sleep(7000);
    let panelText=await combinedText(page);if(noMatchingRecord(panelText,network,digits))return{ok:false,notFound:true,reason:'EMIRATES NO MATCHING RECORD',officialTracker:page.url(),debug:{stage:'no-record',filled,searched,url:page.url(),bodySample:panelText.slice(0,3400),networkUrls:[...new Set(network.map(n=>n.url))].slice(-12)}};
    const trackingClick=await clickDeepText(page,/^Tracking\s+Details$/i);if(!trackingClick)return{ok:false,reason:'EMIRATES TRACKING DETAILS BUTTON NOT FOUND',officialTracker:page.url(),debug:{stage:'tracking-details-button',filled,searched,url:page.url(),bodySample:panelText.slice(0,4600),networkUrls:[...new Set(network.map(n=>n.url))].slice(-12)}};
    await sleep(3500);panelText=await combinedText(page);
    const serial=digits.slice(3),awbMatched=panelText.includes(digits)||panelText.includes(serial)||panelText.includes(`176-${serial}`);if(!awbMatched&&!trackingDetailsVisible(panelText))return{ok:false,reason:'EMIRATES TRACKING DETAILS DID NOT OPEN',officialTracker:page.url(),debug:{stage:'tracking-details-open',filled,searched,trackingClick,url:page.url(),bodySample:panelText.slice(0,5200)}};
