...
export async function trackEmirates(mawb){
  ...
  try{
    ...
    const trackingClick=await clickDeepText(page,/^Tracking\s+Details$/i);if(!trackingClick)return{ok:false,reason:'EMIRATES TRACKING DETAILS BUTTON NOT FOUND',officialTracker:page.url(),debug:{stage:'tracking-details-button',filled,searched,url:page.url(),bodySample:panelText.slice(0,5600),networkUrls:[...new Set(network.map(n=>n.url))].slice(-16)}};
    await sleep(3500);panelText=await combinedText(page);const serial=digits.slice(3),awbMatched=panelText.includes(digits)||panelText.includes(serial)||panelText.includes(`176-${serial}`);if(!awbMatched&&!trackingDetailsVisible(panelText))return{ok:false,reason:'EMIRATES TRACKING DETAILS DID NOT OPEN',officialTracker:page.url(),debug:{stage:'tracking-details-open',filled,searched,trackingClick,url:page.url(),bodySample:panelText.slice(0,6500)}};
    ...
    const shipment={mawb,carrierCode:'EK',airlineName:'Emirates SkyCargo',officialTracker:page.url(),source:'Emirates eSkyCargo Tracking Details screenshot + rendered text'};return{ok:true,shipment,screenshotBase64,screenshotCaptured:true,screenshotVerified:true,screenshotOcrUsed:false,debug:{stage:'done-search-tracking-details',filled,searched,trackingClick,url:page.url(),panelSample:panelText.slice(0,18000),awbMatched,networkUrls:[...new Set(network.map(n=>n.url))].slice(-16)}};
