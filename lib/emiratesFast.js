...
export async function trackEmirates(mawb){
  ...
  try{
    ...
    let searched=await clickSearchNearMawb(page,digits);if(!searched){for(const frame of page.frames()){const found=await findMawbInput(frame);if(found){try{await found.input.press('Enter');searched={label:'Enter',frameUrl:frame.url()};break;}catch{}}}}if(!searched)searched=await clickDeepText(page,/^Search$/i);
