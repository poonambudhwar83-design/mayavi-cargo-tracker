...
function parseTimeline(text='',mawb='',base={}){
  ...
  const cardRx=/\b(CX\s*\d{2,4})\b[\s\S]{0,360}?(\d{1,2}\s+(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|SEPT|OCT|NOV|DEC)[A-Z]*(?:\s+20\d{2})?\s+\d{1,2}:\d{2})[\s\S]{0,320}?(\d{1,2}\s+(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|SEPT|OCT|NOV|DEC)[A-Z]*(?:\s+20\d{2})?\s+\d{1,2}:\d{2})/gi;
...
export async function trackCathayBrowserFallback(mawb,base={}){
  const serial=String(mawb||'').replace(/\D/g,'').slice(3);let browser;const network=[];
  try{
    ...
    page.on('response',async response=>{
      try{
        ...
        if(!body||body.length>350000)return;
    ...
    const prefixEntry=await fillField(page,'prefix','160');
    ...
    const awbEntry=await fillField(page,'awb',serial);
    if(awbEntry.ok){await page.keyboard.press('Enter').catch(()=>{});}await sleep(900);
    const clicked=await clickText(page,'^track\\s*(now)?$');
    await Promise.race([page.waitForNetworkIdle({idleTime:900,timeout:10000}).catch(()=>{}),sleep(10000)]);
    let pageText='';for(let i=0;i<20;i++){pageText=await deepText(page);if(/Current\s+status\s*:/i.test(pageText)||/\b[A-Z]{3}\s+(?:Accepted|Departed|Arrived)\b/i.test(pageText))break;await sleep(350);}
    await clickText(page,'^(show all details|show details|view details|expand)$');for(let i=0;i<12;i++){await page.evaluate(y=>window.scrollTo(0,y),i*500).catch(()=>{});await sleep(220);}pageText=await deepText(page);
    const usefulNetwork=network.filter(x=>!/static\/js|clientlibs|qualtrics|chatbot|akam/i.test(x.url));
    const networkText=usefulNetwork.map(x=>x.body).join('\n');
    const combined=`${pageText}\n${networkText}`;
    const shipment=parseTimeline(combined,mawb,base);
    const networkDebug=usefulNetwork.slice(-15).map(x=>({url:x.url,status:x.status,sample:x.body.slice(0,1800)}));
    return shipment?{ok:true,shipment,debug:{prefixEntry,awbEntry,clicked,pageSample:pageText.slice(0,3500),network:networkDebug}}:{ok:false,reason:'CATHAY BROWSER RESULT NOT PARSED',debug:{prefixEntry,awbEntry,clicked,pageSample:pageText.slice(0,3500),network:networkDebug}};
  }catch(e){return{ok:false,reason:e?.message||String(e),debug:{network:network.slice(-10).map(x=>({url:x.url,status:x.status,sample:x.body.slice(0,1000)}))}};}finally{try{if(browser)await browser.close();}catch{}}
}
