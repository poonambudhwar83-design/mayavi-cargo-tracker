...
async function deepText(page){
  ...
  for(const frame of page.frames()){
    try{out+='\n'+await frame.evaluate(()=>{const parts=[document.body?.innerText||''];for(const el of document.querySelectorAll('*')){try{if(el.shadowRoot){const t=el.shadowRoot.innerText||el.shadowRoot.textContent||'';if(t)parts.push(t);}}catch{}}return parts.join('\n');});}catch{}
  }
  return out;
}
async function clickText(page,source){
  return page.evaluate(source=>{
    ...
    for(const root of roots){const nodes=[...root.querySelectorAll('button,[role="button"],a,input[type="button"],input[type="submit"]')];const hit=nodes.find(el=>visible(el)&&rx.test((el.innerText||el.value||el.textContent||el.getAttribute('aria-label')||el.getAttribute('title')||'').trim()));if(hit){hit.click();return true;}}
    return false;
  },source).catch(()=>false);
}
async function fillField(page,kind,value){
  return page.evaluate(({kind,value})=>{
    ...
    const candidates=all.filter(visible),meta=el=>`${el.id||''} ${el.name||''} ${el.placeholder||''} ${el.getAttribute('aria-label')||''}`;
    ...
    if(kind==='prefix')el=prefixEl;
      ...
      el=candidates.find(x=>x!==prefixEl&&/air\s*waybill|airway\s*bill|airwaybill|\bawb\b|waybill/i.test(meta(x))&&!/airline\s*code|prefix/i.test(meta(x)))||
        ...
        candidates.find(x=>x!==prefixEl&&!['hidden','checkbox','radio','submit','button'].includes(String(x.type||'text').toLowerCase()));
    }
    if(!el)return{ok:false,found:candidates.map(x=>({meta:meta(x),type:x.type||'',maxLength:x.maxLength||0})).slice(0,12)};
    el.focus();
...
export async function trackCathayBrowserFallback(mawb,base={}){
  ...
  try{
    ...
    const prefixEntry=await fillField(page,'prefix','160');await sleep(500);const awbEntry=await fillField(page,'awb',serial);await sleep(650);const clicked=await clickText(page,'^track\\s*(now)?$');
    let text='';for(let i=0;i<45;i++){text=await deepText(page);if(/Current\s+status\s*:/i.test(text)&&(/\bCX\s*\d{2,4}\b/i.test(text)||text.replace(/\D/g,'').includes(serial)))break;await sleep(500);}
    await clickText(page,'^(show all details|show details|view details|expand)$');for(let i=0;i<10;i++){await page.evaluate(y=>window.scrollTo(0,y),i*500).catch(()=>{});await sleep(250);}text=await deepText(page);
    const shipment=parseTimeline(text,mawb,base);return shipment?{ok:true,shipment,debug:{prefixEntry,awbEntry,clicked,sample:text.slice(0,3000)}}:{ok:false,reason:'CATHAY BROWSER RESULT NOT PARSED',debug:{prefixEntry,awbEntry,clicked,sample:text.slice(0,3000)}};
