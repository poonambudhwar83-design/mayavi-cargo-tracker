...
function parseCathayApiDate(value=''){
  const m=String(value||'').toUpperCase().match(/^(d{2})(JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|SEPT|OCT|NOV|DEC)(d{2})$/);
  if(!m)return'';
  const mon=m[2]==='SEPT'?'SEP':m[2],year=Number(m[3])>=70?`19${m[3]}`:`20${m[3]}`;
  return`${year}-${MONTH[mon]}-${m[1]}`;
}
function parseCathayTrackingApi(body='',mawb='',base={}){
  ...
  if(!data||String(data.AWBNumber||'').replace(/D/g,'')!==serial)return null;
  ...
  const flightNo=String(b.flight||base.flightNo||'').split('/')[0].replace(/s+/g,'').toUpperCase();
  ...
  const arrived=Boolean(arrActual.date&&arrActual.time)||/ARRIVED|RCF/.test(latest);
  const delivered=/DELIVER/.test(latest);
  const departed=Boolean(depActual.date&&depActual.time)||/DEPART/.test(latest);
  ...
  const weight=String(data.weight?.value??b.weight??base.weight??'').replace(/,/g,'');
  const shipment={
