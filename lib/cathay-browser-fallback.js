...
function parseBookingStatus(text='',mawb='',base={}){
  ...
  const tail=flat.slice(marker),end=tail.search(/Shipment\s+History/i),block=end>0?tail.slice(0,end):tail.slice(0,3000);
  ...
  const actuals=[...block.matchAll(/\b([A-Z]{3})\b[\s\S]{0,90}?(\d{1,2}\s+(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|SEPT|OCT|NOV|DEC)[A-Z]*(?:\s+20\d{2})?\s+\d{1,2}:\d{2})\s+Actual\b/gi)].map(m=>({airport:m[1].toUpperCase(),dt:parseShortDate(m[2],fallbackYear)}));
  const dep=actuals.find(x=>x.airport===origin)?.dt||{date:'',time:''},arr=actuals.find(x=>x.airport===destination)?.dt||{date:'',time:''};
  const summary=block.match(/\b(\d{1,5})\s*pc\(s\)\s*[|｜]\s*([\d,.]+)\s*kg\b/i)||block.match(/\b(\d{1,5})\s*(?:pcs?|pieces?)\b[\s\S]{0,40}?\b([\d,.]+)\s*kg\b/i);
  const pieces=summary?.[1]||'',weight=(summary?.[2]||'').replace(/,/g,'');
  if(!arr.date||!arr.time)return null;
  return{mawb,carrierCode:'CX',airlineName:'Cathay Cargo',origin,destination,bags:pieces,pieces,weight,flightNo,departureDate:dep.date||base.departureDate||'',departureTime:dep.time||base.departureTime||'',arrivalDate:arr.date,arrivalTime:arr.time,arrivalIsActual:true,status:'ARRIVED',officialTracker:CATHAY,source:'Cathay Cargo Booking Status actual arrival',arrivalTimeSource:'Cathay Booking Status destination Actual time'};
}
