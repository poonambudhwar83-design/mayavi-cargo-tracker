...
async function screenshotShipment(page,mawb){
  const shot=await page.screenshot({type:'png',fullPage:true,encoding:'base64'}).catch(()=>null);
  if(!shot)return null;
  const ocr=await readSaudiaScreenshot({mawb,screenshotBase64:shot,timeoutMs:30000}).catch(()=>null);
  return ocr?.ok?ocr.shipment:null;
}
async function tryDeepLink(page,base,mawb,network){
  const target=`${base}?awbNumber=${encodeURIComponent(digits(mawb))}`;
  network.length=0;
  await page.goto(target,{waitUntil:'domcontentloaded',timeout:30000});
  for(let i=0;i<6;i++){
    await sleep(i?700:1800);
    const text=`${await pageText(page)}\n${network.join('\n')}`;
    const parsed=parseResultCard(text,mawb);
    if(parsed)return{shipment:parsed,stage:'DEEPLINK_RESULT',target,text:clean(text).slice(0,1800)};
  }
  const shotShipment=await screenshotShipment(page,mawb);
  if(shotShipment)return{shipment:{...shotShipment,arrivalDate:'',arrivalTime:'',arrivalIsActual:false,bookingDate:''},stage:'DEEPLINK_SCREENSHOT',target,text:clean(await pageText(page)).slice(0,1800)};
  return{shipment:null,stage:'DEEPLINK_NO_RESULT',target,text:clean(await pageText(page)).slice(0,1800)};
}
...
export async function trackSaudiaDirect(input){
  ...
  let browser;const debug={stage:'OPEN',submit:null,deepLinks:[]},network=[];
  try{
    ...
    page.on('response',async r=>{try{const u=r.url(),ct=String(r.headers()['content-type']||'');if(!/saudiacargo/i.test(u)||!/json|text|javascript/i.test(ct))return;const body=await r.text();if(body&&body.length<500000&&(body.includes(digits(mawb))||body.includes(digits(mawb).slice(3))))network.push(body)}catch{}});

    for(const base of DEEP_LINKS){
      const deep=await tryDeepLink(page,base,mawb,network).catch(e=>({shipment:null,stage:'DEEPLINK_ERROR',target:base,text:String(e?.message||e)}));
      debug.deepLinks.push({stage:deep.stage,target:deep.target,sample:deep.text});
      if(deep.shipment)return{ok:true,shipment:deep.shipment,officialTracker:URL,debug:{...debug,stage:deep.stage}};
    }

    network.length=0;await page.goto(URL,{waitUntil:'domcontentloaded',timeout:30000});await sleep(1800);
    if(!await markAwbInput(page))return{ok:false,reason:'SAUDIA AWB INPUT NOT FOUND',officialTracker:URL,debug:{...debug,stage:'AWB_INPUT_NOT_FOUND'}};
    ...
    const submit=await clickSubmit(page);debug.submit=submit;if(!submit)return{ok:false,reason:'SAUDIA SUBMIT BUTTON NOT FOUND',officialTracker:URL,debug:{...debug,stage:'SUBMIT_NOT_FOUND'}};
    const text=await waitForResult(page,mawb,network,22000);debug.resultSample=clean(text).slice(0,5000);
    let shipment=parseResultCard(text,mawb);
    if(!shipment)shipment=await screenshotShipment(page,mawb);
    ...
    shipment={...shipment,arrivalDate:'',arrivalTime:'',arrivalIsActual:false,bookingDate:''};
