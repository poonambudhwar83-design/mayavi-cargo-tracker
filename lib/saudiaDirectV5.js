...
async function markAwbInput(page){return page.evaluate(()=>{const visible=e=>{const s=getComputedStyle(e),r=e.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&r.width>40&&r.height>18&&!e.disabled&&!e.readOnly};const inputs=[...document.querySelectorAll('input')].filter(visible).filter(e=>!['hidden','checkbox','radio','submit','button','password','email'].includes(String(e.type||'text').toLowerCase()));const found=inputs.find(e=>/awb|air\s*waybill|shipment/i.test(`${e.placeholder||''} ${e.name||''} ${e.id||''} ${e.getAttribute('aria-label')||''} ${e.closest('section,form,div')?.innerText||''}`))||inputs[0];if(!found)return false;found.dataset.mayaviSaudiaAwb='1';found.scrollIntoView({block:'center'});return true}).catch(()=>false)}
async function fillMarkedAwb(page,mawb){
  ...
  await page.type('[data-mayavi-saudia-awb="1"]',digits(mawb),{delay:35});
  await page.evaluate(()=>{const e=document.querySelector('[data-mayavi-saudia-awb="1"]');if(!e)return;e.dispatchEvent(new Event('input',{bubbles:true}));e.dispatchEvent(new Event('change',{bubbles:true}));e.blur();}).catch(()=>{});
}
async function clickAction(page,words=['Submit','Search','Track','Track Shipment'],timeout=12000){
  const end=Date.now()+timeout;
  while(Date.now()<end){
    const hit=await page.evaluate((wanted)=>{const visible=e=>{const s=getComputedStyle(e),r=e.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&r.width>6&&r.height>6&&!e.disabled};const txt=e=>String(e.innerText||e.textContent||e.value||e.getAttribute('aria-label')||'').replace(/\s+/g,' ').trim();const xs=[...document.querySelectorAll('button,a,[role="button"],input[type="submit"],input[type="button"]')].filter(visible);for(const w of wanted){const x=xs.find(e=>txt(e).toLowerCase()===String(w).toLowerCase());if(x){x.dataset.mayaviSaudiaAction='1';x.scrollIntoView({block:'center'});return{mode:'EXACT_ACTION',text:txt(x)}}}return null;},words).catch(()=>null);
    if(hit){try{await page.click('[data-mayavi-saudia-action="1"]')}catch{await page.evaluate(()=>document.querySelector('[data-mayavi-saudia-action="1"]')?.click()).catch(()=>{})}return hit}
    await sleep(300);
  }
  return null;
}
...
async function tryConnect(page,mawb,network){
  ...
  try{
    network.length=0;
    await page.goto(CONNECT_LOGIN,{waitUntil:'domcontentloaded',timeout:30000});await sleep(1800);
    const marked=await page.evaluate(()=>{const visible=e=>{const s=getComputedStyle(e),r=e.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&r.width>40&&r.height>18&&!e.disabled};const inputs=[...document.querySelectorAll('input')].filter(visible);const email=inputs.find(e=>String(e.type||'').toLowerCase()==='email'||/email/i.test(`${e.name||''} ${e.id||''} ${e.placeholder||''} ${e.getAttribute('aria-label')||''}`));const pass=inputs.find(e=>String(e.type||'').toLowerCase()==='password'||/password/i.test(`${e.name||''} ${e.id||''} ${e.placeholder||''}`));if(!email||!pass)return false;email.dataset.mayaviConnectEmail='1';pass.dataset.mayaviConnectPassword='1';return true;}).catch(()=>false);
    if(!marked)return{shipment:null,stage:'CONNECT_LOGIN_FIELDS_NOT_FOUND'};
    await page.click('[data-mayavi-connect-email="1"]',{clickCount:3}).catch(()=>{});await page.type('[data-mayavi-connect-email="1"]',email,{delay:20});
    await page.click('[data-mayavi-connect-password="1"]',{clickCount:3}).catch(()=>{});await page.type('[data-mayavi-connect-password="1"]',password,{delay:20});
    const signIn=await clickAction(page,['Sign in','Login'],12000);if(!signIn)return{shipment:null,stage:'CONNECT_SIGNIN_BUTTON_NOT_FOUND'};
    await sleep(3500);
    await page.goto(CONNECT_TRACK,{waitUntil:'domcontentloaded',timeout:30000});await sleep(1800);
    const body=clean(await pageText(page));
    if(/please register|sign in if you already have an account|login to your account/i.test(body))return{shipment:null,stage:'CONNECT_LOGIN_FAILED',sample:body.slice(0,1200)};
    if(!await markAwbInput(page))return{shipment:null,stage:'CONNECT_AWB_INPUT_NOT_FOUND',sample:body.slice(0,1200)};
    await fillMarkedAwb(page,mawb);
    const action=await clickAction(page,['Search','Track','Track Shipment','Submit'],12000);if(!action)return{shipment:null,stage:'CONNECT_TRACK_BUTTON_NOT_FOUND'};
    const text=await waitForResult(page,mawb,network,25000);
    let shipment=parseResultCard(text,mawb);if(!shipment)shipment=await screenshotShipment(page,mawb);
    if(!shipment)return{shipment:null,stage:'CONNECT_RESULT_NOT_READABLE',sample:clean(text).slice(0,1600)};
    return{shipment:{...shipment,source:'Saudia Cargo Connect official tracking'},stage:'CONNECT_RESULT'};
...
export async function trackSaudiaDirect(input){
  ...
  let browser;const debug={stage:'OPEN',submit:null,deepLinks:[],networkMeta:[],connect:null},network=[];
  try{
    ...
    network.length=0;debug.networkMeta=[];await page.goto(URL,{waitUntil:'domcontentloaded',timeout:30000});await sleep(2200);
    ...
    await fillMarkedAwb(page,mawb);
    const submit=await clickAction(page,['Submit','Search','Track Shipment','Track'],12000);debug.submit=submit;if(!submit)return{ok:false,reason:'SAUDIA SUBMIT BUTTON NOT FOUND',officialTracker:URL,debug:{...debug,stage:'SUBMIT_NOT_FOUND',diag:await diag(page)}};
    ...
    let shipment=parseResultCard(text,mawb);if(!shipment)shipment=await screenshotShipment(page,mawb);
    if(shipment){shipment={...shipment,arrivalDate:'',arrivalTime:'',arrivalIsActual:false,bookingDate:''};return{ok:true,shipment,officialTracker:URL,debug:{...debug,stage:'RESULT_CARD_SUCCESS'}};}

    const connect=await tryConnect(page,mawb,network);debug.connect={stage:connect.stage,sample:connect.sample||'',error:connect.error||''};
    if(connect.shipment)return{ok:true,shipment:connect.shipment,officialTracker:CONNECT_TRACK,debug:{...debug,stage:connect.stage}};
    const reason=connect.stage==='CONNECT_NOT_CONFIGURED'?'SAUDIA PUBLIC TRACKING BLOCKED; CONNECT ACCOUNT NOT CONFIGURED':connect.stage||'SAUDIA SUBMIT RESULT CARD NOT READABLE';
    return{ok:false,reason,officialTracker:URL,debug:{...debug,stage:'RESULT_NOT_READABLE'}};
