...
async function findInputHandles(page){
  ...
  for(const h of handles){
    const meta=await h.evaluate(e=>{
      ...
      const nearby=`${e.parentElement?.innerText||''} ${e.parentElement?.previousElementSibling?.innerText||''}`;
      return{visible:s.display!=='none'&&s.visibility!=='hidden'&&r.width>30&&r.height>12&&!e.disabled,type:(e.type||'text').toLowerCase(),desc:`${e.name||''} ${e.id||''} ${e.placeholder||''} ${e.getAttribute('aria-label')||''} ${e.getAttribute('title')||''} ${labels} ${nearby}`.replace(/\s+/g,' ').toLowerCase()};
    }).catch(()=>null);
    if(meta?.visible&&!['hidden','checkbox','radio','submit','button'].includes(meta.type))rows.push({h,meta});
  }
  const scoreAir=x=>(/airline\s*code|carrier\s*code|awb\s*prefix|prefix/.test(x.meta.desc)?20:0)+(/airline|carrier/.test(x.meta.desc)?8:0)-(/search|email|chat/.test(x.meta.desc)?20:0);
  const scoreAwb=x=>(/air\s*waybill|airway\s*bill|airwaybill|\bawb\b|waybill/.test(x.meta.desc)?20:0)+(/shipment|tracking/.test(x.meta.desc)?5:0)-(/search|email|chat/.test(x.meta.desc)?20:0);
...
async function typeValue(page,handle,value){
  ...
  try{await handle.click({clickCount:3});await page.keyboard.press('Backspace');await handle.type(String(value),{delay:70});const actual=await handle.evaluate(e=>String(e.value||''));return actual.replace(/\s/g,'').includes(String(value).replace(/\s/g,''));}catch{return false;}
}

async function clickTrack(page,awbHandle){
  const clicked=await page.evaluate(()=>{
    ...
    const btn=els.find(e=>/^track$/i.test((e.innerText||e.value||e.textContent||e.getAttribute('aria-label')||'').trim()))||els.find(e=>/track\s+(shipment|cargo|awb)|^search$/i.test((e.innerText||e.value||e.textContent||e.getAttribute('aria-label')||'').trim()));
    if(!btn)return'';const t=(btn.innerText||btn.value||btn.textContent||'').trim();btn.click();return t;
  }).catch(()=> '');
  if(!clicked&&awbHandle)await awbHandle.press('Enter').catch(()=>{});
  return clicked;
}

async function mainTimeline(mawb){
  const digits=mawb.replace(/\D/g,''),prefix=digits.slice(0,3),serial=digits.slice(3);let browser;
  try{
    ...
    if((!airline||!awb)&&rows.length>=2){const useful=rows.filter(x=>!/search|email|chat/.test(x.meta.desc));if(useful.length>=2){airline=airline||useful[0].h;awb=awb||useful[1].h;}}
    const prefixOk=await typeValue(page,airline,prefix);
    const awbOk=await typeValue(page,awb,serial)||await typeValue(page,awb,digits);
    const clicked=await clickTrack(page,awb);
    await Promise.race([page.waitForNetworkIdle({idleTime:900,timeout:9000}).catch(()=>{}),sleep(9000)]);await sleep(1500);
    const text=await page.evaluate(()=>document.body?.innerText||'').catch(()=> '');
    const shipment=parseTimeline(text,mawb);
    return shipment?{ok:true,shipment,debug:{stage:'TIMELINE',prefixOk,awbOk,clicked,pageText:text.slice(0,7000)}}:{ok:false,reason:'CATHAY MAIN TIMELINE NOT PARSED',debug:{stage:'NO_TIMELINE',prefixOk,awbOk,clicked,pageText:text.slice(0,7000)}};
  }catch(e){return{ok:false,reason:e?.message||'Cathay main page failed',debug:{stage:'ERROR'}};}finally{try{if(browser)await browser.close();}catch{}}
}
