  ...
  try{
    ...
    if(focusResult.mode==='deep-control'){
      ...
      for(let i=0;i<5;i++){
        ...
        const a=await page.evaluate(()=>{
          ...
          const tag=(e?.tagName||'').toLowerCase(),type=(e?.type||'').toLowerCase(),role=(e?.getAttribute?.('role')||'').toLowerCase(),d=`${e?.id||''} ${e?.name||''} ${e?.placeholder||''} ${e?.getAttribute?.('aria-label')||''} ${role}`.toLowerCase(),tabIndex=Number(e?.tabIndex??-1);
          const custom=tag==='div'&&tabIndex>=0;
          const safe=Boolean(e)&&!['submit','button','checkbox','radio','hidden'].includes(type)&&!/airlinecodefield|search/.test(d)&&((tag==='input'||tag==='textarea'||e?.isContentEditable||role==='textbox'||role==='combobox')||custom);
          return{tag,type,role,desc:d,tabIndex,className:String(e?.className||'').slice(0,220),text:String(e?.innerText||e?.textContent||'').slice(0,180),safe,value:'value'in(e||{})?String(e.value||''):''};
        }).catch(()=>({safe:false}));
    ...
    if(!typed){
      ...
      for(const p of points){
        ...
        const a=await page.evaluate(()=>{let e=document.activeElement;while(e?.shadowRoot?.activeElement)e=e.shadowRoot.activeElement;const tag=(e?.tagName||'').toLowerCase(),type=(e?.type||'').toLowerCase(),role=(e?.getAttribute?.('role')||'').toLowerCase(),d=`${e?.id||''} ${e?.name||''} ${e?.placeholder||''} ${e?.getAttribute?.('aria-label')||''} ${role}`.toLowerCase(),tabIndex=Number(e?.tabIndex??-1);const custom=tag==='div'&&tabIndex>=0;const safe=Boolean(e)&&!['submit','button','checkbox','radio','hidden'].includes(type)&&!/airlinecodefield|search/.test(d)&&((tag==='input'||tag==='textarea'||e?.isContentEditable||role==='textbox'||role==='combobox')||custom);return{tag,type,role,desc:d,tabIndex,className:String(e?.className||'').slice(0,220),text:String(e?.innerText||e?.textContent||'').slice(0,180),safe};}).catch(()=>({safe:false}));
    ...
    const reg=await page.evaluate(s=>{
      ...
      const walk=root=>{for(const e of root.querySelectorAll('*')){const tag=(e.tagName||'').toLowerCase(),role=(e.getAttribute?.('role')||'').toLowerCase();if(tag==='input'||tag==='textarea'||e.isContentEditable||role==='textbox'||role==='combobox'||(tag==='div'&&Number(e.tabIndex)>=0))vals.push(String('value'in e?e.value:e.textContent||''));if(e.shadowRoot)walk(e.shadowRoot);}};walk(document);
      const body=norm(document.body?.innerText||'');return{serialInBody:body.includes(s),serialInValue:vals.map(norm).some(v=>v.includes(s)),values:vals.slice(0,30)};
