...
function parseTimeline(text='',mawb=''){
  ...
  const cardRx=/\b(CX\s*\d{2,4})\b[\s\S]{0,260}?(\d{1,2}\s+(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|SEPT|OCT|NOV|DEC)[A-Z]*)\s+(\d{1,2}:\d{2})[\s\S]{0,220}?(\d{1,2}\s+(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|SEPT|OCT|NOV|DEC)[A-Z]*)\s+(\d{1,2}:\d{2})/gi;
  const cards=[...flat.matchAll(cardRx)];
  ...
  const summary=flat.match(/\b(\d{1,5})\s*pc\(s\)\s*[|｜]\s*([\d,.]+)\s*kg\b/i)
    ||flat.match(/\b(\d{1,5})\s*(?:pcs?|pieces?)\b[\s\S]{0,40}?\b([\d,.]+)\s*kg\b/i);
  ...
  return {
    ...
    arrivalIsActual:Boolean(atDestination&&arrival.date&&arrival.time),status,
    officialTracker:CATHAY,source:'Cathay Cargo Track & Trace timeline',
    arrivalTimeSource:'Cathay flight card right-side arrival time',
...
function terminalParse(html='',mawb=''){
  ...
  const route=text.match(/AWB Type\s+(?:Import|Export)\s+([A-Z]{3})\s+([A-Z]{3})/i)
  ...
  const actual=[...text.matchAll(/(?:Actual Arrival|Arrived|ATA)[\s\S]{0,420}?(\d{1,2}\s+(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|SEPT|OCT|NOV|DEC)[A-Z]*\s+20\d{2}\s+\d{1,2}:\d{2})/gi)]
    .map(m=>parseDateTime(m[1])).filter(x=>x.date&&x.time).at(-1)||{date:'',time:''};
...
async function fetchHtml(url){
  try{const r=await fetch(url,{headers:{'user-agent':'Mozilla/5.0','accept':'text/html,application/xhtml+xml'},redirect:'follow',cache:'no-store',signal:AbortSignal.timeout(9000)});return r.ok?await r.text():null;}catch{return null;}
}
...
async function terminalFallback(mawb){
  ...
  for(const url of [`${TERMINAL}/AWBPrefix/160/AWBSuffix/${suffix}`,`${TERMINAL}?AWBPrefix=160&AWBSuffix=${suffix}`]){
    ...
    const shipment=terminalParse(html,mawb);if(shipment)return{shipment,url};
  }
  return null;
}
    ...
    try{
      merged+='\n'+await frame.evaluate(()=>{
        ...
        for(const el of document.querySelectorAll('*')){
          if(el.shadowRoot){const t=el.shadowRoot.innerText||el.shadowRoot.textContent||'';if(t)out.push(t);}
        }
        return out.join('\n');
      });
    }catch{}
...
async function expandAndScroll(page){
  await sleep(1600);
  await page.evaluate(()=>{
    const visible=n=>{const s=getComputedStyle(n),r=n.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&r.width>15&&r.height>10&&!n.disabled;};
    const els=[...document.querySelectorAll('button,[role="button"],a')].filter(visible);
    const b=els.find(n=>/show all details|show details|view details|expand/i.test((n.innerText||n.textContent||n.getAttribute('aria-label')||n.getAttribute('title')||'').trim()));if(b)b.click();
  }).catch(()=>{});
  for(let i=0;i<10;i++){
    await page.evaluate(y=>window.scrollTo(0,y),i*520).catch(()=>{});
    await sleep(500);
    const t=await deepText(page);
    if(/Current\s+status|\b[A-Z]{3}\s+(?:Accepted|Departed|Arrived)\b|\bCX\s*\d{2,4}\b[\s\S]{0,600}\bArrived\b/i.test(t))return t;
  }
  return deepText(page);
}
...
async function enterAwb(page,serial){
  ...
  const current=await airline.evaluate(e=>String(e.value||''));
  if(current.replace(/\D/g,'')!=='160'){
    await airline.click({clickCount:3});await page.keyboard.press('Backspace');await airline.type('160',{delay:60});
  }
  ...
  await page.keyboard.type(serial,{delay:65});await sleep(250);
  ...
  const body=(await deepText(page)).replace(/\s/g,'');
  return{ok:body.includes(serial),mode:'airline-tab-awb-enter',beforeCommit};
}
...
async function mainTimeline(mawb){
  ...
  try{
    ...
    await sleep(2200);await clickOverlays(page);await sleep(700);

    const entry=await enterAwb(page,serial);
    const clicked=await clickTrackNow(page);
    await Promise.race([page.waitForNetworkIdle({idleTime:800,timeout:8000}).catch(()=>{}),sleep(8000)]);
    const text=await expandAndScroll(page);
    const shipment=parseTimeline(text,mawb);
    return shipment
      ?{ok:true,shipment,debug:{stage:'TIMELINE',entry,clicked,pageText:text.slice(0,10000)}}
      :{ok:false,reason:'CATHAY MAIN TIMELINE NOT PARSED',debug:{stage:'NO_TIMELINE',entry,clicked,pageText:text.slice(0,10000)}};
  }catch(e){return{ok:false,reason:e?.message||'Cathay main page failed',debug:{stage:'ERROR',message:e?.message||''}};}
  finally{try{if(browser)await browser.close();}catch{}}
}

function mergeMain(main={},fallback={}){
  const out={...fallback};
  ...
  if(main.arrivalDate&&main.arrivalTime)out.arrivalIsActual=main.arrivalIsActual===true;
  return out;
}
...
export async function trackCathay(input){
  ...
  const [mainResult,terminalResult]=await Promise.all([
    Promise.race([mainTimeline(mawb),new Promise(r=>setTimeout(()=>r({ok:false,reason:'CATHAY MAIN PAGE TIMEOUT',debug:{stage:'TIMEOUT'}}),40000))]),
    terminalFallback(mawb)
  ]);
  const fallback=terminalResult?.shipment||{};

  if(mainResult?.ok){
    const shipment=mergeMain(mainResult.shipment,fallback);
    shipment.source='Cathay Cargo Track & Trace timeline';
    return{ok:true,airline:AIRLINE,shipment,debug:{source:'cathay-main-timeline',main:mainResult.debug,terminalUrl:terminalResult?.url||''}};
  }
