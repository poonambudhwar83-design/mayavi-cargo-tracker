...
async function clickText(page,rx){
  for(const f of page.frames()){
    try{const hit=await f.evaluate(src=>{const re=new RegExp(src,'i'),els=[...document.querySelectorAll('button,input[type="submit"],input[type="button"],a,[role="button"]')],vis=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>8&&r.height>8&&s.display!=='none'&&s.visibility!=='hidden'&&!e.disabled;},e=els.find(x=>vis(x)&&re.test((x.innerText||x.value||x.textContent||x.getAttribute('aria-label')||'').trim()));if(!e)return'';const t=(e.innerText||e.value||e.textContent||'').trim();e.click();return t;},rx.source);if(hit)return hit;}catch{}
  }return'';
}
async function registered(page,serial){
  return page.evaluate(s=>{const norm=v=>String(v||'').replace(/\D/g,'');const t=document.body?.innerText||'',vals=[...document.querySelectorAll('input,textarea,[contenteditable="true"],[tabindex="0"]')].map(e=>String(('value'in e?e.value:'')||e.textContent||''));return{ok:norm(t).includes(s)||vals.map(norm).some(v=>v.includes(s)),body:t.slice(0,3200),values:vals.slice(0,18)};},serial).catch(()=>({ok:false,body:'',values:[]}));
}
...
async function officialCathay(mawb,referenceYear=''){
  ...
  try{
    ...
    await page.goto(CATHAY,{waitUntil:'domcontentloaded',timeout:14000});await sleep(1700);await clickText(page,/^(Reject all|Accept all)$/i);await sleep(300);
    await page.waitForFunction(()=>[...document.querySelectorAll('input')].some(e=>/airlinecodefield/i.test(`${e.id||''} ${e.name||''}`)),{timeout:6000}).catch(()=>{});

    const code=await page.$('input[id*="airlinecodefield"],input[name*="airlinecodefield"]');
    if(!code)return{ok:false,reason:'CATHAY FIXED 160 FIELD NOT FOUND',debug};
    await code.focus();await page.keyboard.press('Tab');await sleep(250);
    const wrap=await page.evaluate(()=>{const e=document.activeElement,r=e?.getBoundingClientRect?.();return{tag:e?.tagName||'',className:String(e?.className||''),text:String(e?.innerText||e?.textContent||''),tabIndex:e?.tabIndex??-1,x:r?r.left+r.width/2:0,y:r?r.top+r.height/2:0,w:r?.width||0,h:r?.height||0};}).catch(()=>null);
    debug.awbWrapper=wrap;
    ...
    await page.mouse.click(wrap.x,wrap.y).catch(()=>{});await sleep(250);
    await page.evaluate(s=>{const e=document.activeElement;try{const d=new DataTransfer();d.setData('text/plain',s);e?.dispatchEvent(new ClipboardEvent('paste',{bubbles:true,cancelable:true,clipboardData:d}));}catch{}},serial).catch(()=>{});await sleep(450);
    let reg=await registered(page,serial);
    if(!reg.ok){await page.keyboard.type(serial,{delay:60}).catch(()=>{});await sleep(250);await page.keyboard.press('Enter').catch(()=>{});await sleep(500);reg=await registered(page,serial);}
    debug.registration=reg;debug.registered=reg.ok;
