...
function parseDateTime(value='',fallbackYear=''){
  ...
  const m=s.match(/(\d{1,2})\s+(JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|SEPT|OCT|NOV|DEC)[A-Z]*(?:\s+(20\d{2}))?(?:\s+(\d{1,2}):(\d{2}))?/);
...
function parseTimeline(text='',mawb=''){
  ...
  const cardRx=/\b(CX\s*\d{2,4})\b[\s\S]{0,320}?(\d{1,2}\s+(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|SEPT|OCT|NOV|DEC)[A-Z]*)\s+(\d{1,2}:\d{2})[\s\S]{0,260}?(\d{1,2}\s+(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|SEPT|OCT|NOV|DEC)[A-Z]*)\s+(\d{1,2}:\d{2})/gi;
  ...
  const departure=card?parseDateTime(`${card[2]} ${card[3]}`,year):{date:'',time:''};
  const arrival=card?parseDateTime(`${card[4]} ${card[5]}`,year):{date:'',time:''};
  ...
  const atDestination=Boolean(destination&&new RegExp(`\\b${destination}\\s+Arrived\\b`,'i').test(flat));
  const delivered=/Current\s+status\s*:\s*Delivered/i.test(flat)||/\bDelivered\b[\s\S]{0,35}?\b([1-9]\d*)\s*\/\s*\1\b/i.test(flat);
  const status=delivered?'DELIVERED':atDestination?'ARRIVED':departed.length?'IN TRANSIT':accepted.length?'BOOKED':'TRACKING';
...
function terminalParse(html='',mawb=''){
  ...
  const actual=[...text.matchAll(/(?:Actual Arrival|Arrived|ATA)[\s\S]{0,420}?(\d{1,2}\s+(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|SEPT|OCT|NOV|DEC)[A-Z]*\s+20\d{2}\s+\d{1,2}:\d{2})/gi)].map(m=>parseDateTime(m[1])).filter(x=>x.date&&x.time).at(-1)||{date:'',time:''};
  const booking=rcs?parseDateTime(rcs[1]):{date:'',time:''},pieces=rcs?.[2]||'',weight=(rcs?.[3]||'').replace(/,/g,'');
...
async function clickOverlays(page){await page.evaluate(()=>{const visible=e=>{const s=getComputedStyle(e),r=e.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&r.width>10&&r.height>10;};const els=[...document.querySelectorAll('button,[role="button"],a')].filter(visible);const cookie=els.find(e=>/accept all|accept cookies|allow all|agree/i.test((e.innerText||e.getAttribute('aria-label')||'').trim()));if(cookie)cookie.click();const close=els.find(e=>/^(close|dismiss)$/i.test((e.innerText||e.getAttribute('aria-label')||e.getAttribute('title')||'').trim()));if(close)close.click();}).catch(()=>{});}
...
async function clickTrackNow(page){return page.evaluate(()=>{const visible=n=>{const s=getComputedStyle(n),r=n.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&r.width>16&&r.height>10&&!n.disabled;};const els=[...document.querySelectorAll('button,input[type="submit"],input[type="button"],[role="button"]')].filter(visible);const btn=els.find(n=>/^track\s*now$/i.test((n.innerText||n.value||n.textContent||n.getAttribute('aria-label')||'').trim()));if(!btn)return'';const t=(btn.innerText||btn.value||btn.textContent||'').trim();btn.click();return t||'Track now';}).catch(()=> '');}

async function expandAndScroll(page){
  await sleep(1800);
  await page.evaluate(()=>{const visible=n=>{const s=getComputedStyle(n),r=n.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&r.width>15&&r.height>10&&!n.disabled;};const els=[...document.querySelectorAll('button,[role="button"],a')].filter(visible);const b=els.find(n=>/show all details|show details|view details|expand/i.test((n.innerText||n.textContent||n.getAttribute('aria-label')||n.getAttribute('title')||'').trim()));if(b)b.click();}).catch(()=>{});
  let latest='';for(let i=0;i<12;i++){await page.evaluate(y=>window.scrollTo(0,y),i*520).catch(()=>{});await sleep(450);latest=await deepText(page);if(/Current\s+status|\b[A-Z]{3}\s+(?:Accepted|Departed|Arrived)\b|\bCX\s*\d{2,4}\b[\s\S]{0,700}\bArrived\b/i.test(latest))break;}return latest||deepText(page);
}
...
async function enterAwb(page,serial){
  ...
  await airline.click({clickCount:3});await page.keyboard.down('Control');await page.keyboard.press('A');await page.keyboard.up('Control');await page.keyboard.press('Backspace');await airline.type('160',{delay:70});await sleep(250);
  await airline.click();await page.keyboard.press('Tab');await sleep(250);
  await page.keyboard.down('Control');await page.keyboard.press('A');await page.keyboard.up('Control');await page.keyboard.press('Backspace');await page.keyboard.type(serial,{delay:65});await sleep(300);
  const beforeCommit=await page.evaluate(()=>String(document.activeElement?.value||document.activeElement?.textContent||''));
  await page.keyboard.press('Enter');await sleep(650);
  const body=(await deepText(page)).replace(/\s/g,'');return{ok:body.includes(serial),mode:'retype-160-tab-awb-enter',beforeCommit};
}
...
async function mainTimeline(mawb){
  ...
  try{
    ...
    page.on('response',async response=>{try{const u=response.url(),ct=(response.headers()['content-type']||'').toLowerCase();if(!/(json|text|javascript|xml)/.test(ct)&&!/track|trace|shipment|awb|cargo/i.test(u))return;const body=await response.text();if(!body||body.length>300000)return;if(body.includes(serial)||/CX\s*\d{2,4}|shipment|awb|arrival|depart/i.test(body))network.push({url:u,status:response.status(),body:body.slice(0,180000)});}catch{}});
    await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/140.0.0.0 Safari/537.36');await page.setExtraHTTPHeaders({'Accept-Language':'en-US,en;q=0.9'});
    await page.goto(CATHAY,{waitUntil:'domcontentloaded',timeout:22000});await sleep(2200);await clickOverlays(page);await sleep(700);
    const entry=await enterAwb(page,serial),clicked=await clickTrackNow(page);await Promise.race([page.waitForNetworkIdle({idleTime:800,timeout:9000}).catch(()=>{}),sleep(9000)]);
    const pageText=await expandAndScroll(page),networkText=network.map(x=>x.body).join('\n'),combined=`${pageText}\n${networkText}`;
    const shipment=parseTimeline(combined,mawb);
    return shipment?{ok:true,shipment,debug:{stage:'TIMELINE',entry,clicked,pageText:pageText.slice(0,9000),network:network.slice(-12).map(x=>({url:x.url,status:x.status,sample:x.body.slice(0,1500)}))}}:{ok:false,reason:'CATHAY MAIN TIMELINE NOT PARSED',debug:{stage:'NO_TIMELINE',entry,clicked,pageText:pageText.slice(0,9000),network:network.slice(-12).map(x=>({url:x.url,status:x.status,sample:x.body.slice(0,1500)}))}};
  }catch(e){return{ok:false,reason:e?.message||'Cathay main page failed',debug:{stage:'ERROR',message:e?.message||'',network:network.slice(-8).map(x=>({url:x.url,status:x.status,sample:x.body.slice(0,1000)}))}};}finally{try{if(browser)await browser.close();}catch{}}
}
...
export async function trackCathay(input){
  ...
  const [mainResult,terminalResult]=await Promise.all([Promise.race([mainTimeline(mawb),new Promise(r=>setTimeout(()=>r({ok:false,reason:'CATHAY MAIN PAGE TIMEOUT',debug:{stage:'TIMEOUT'}}),42000))]),terminalFallback(mawb)]);const fallback=terminalResult?.shipment||{};
  if(mainResult?.ok){const shipment=mergeMain(mainResult.shipment,fallback);shipment.source='Cathay Cargo Track & Trace timeline';return{ok:true,airline:AIRLINE,shipment,debug:{source:'cathay-main-timeline',main:mainResult.debug,terminalUrl:terminalResult?.url||''}};}
