...
async function officialCathay(mawb,referenceYear=''){
  ...
  const debug={fillMode:'',trackClicked:false,showDetails:false,registered:false,fieldsBefore:[],fieldsAfter:[]};
  try{
    ...
    await page.waitForFunction(()=>{const e=[...document.querySelectorAll('input')].find(x=>/airlinecodefield/i.test(`${x.id||''} ${x.name||''}`));return !e||String(e.value||'').replace(/\D/g,'')==='160';},{timeout:4500}).catch(()=>{});
    ...
    const awbPoint=await page.evaluate(()=>{
      ...
      const els=[...document.querySelectorAll('label,div,span,p')].filter(vis),matches=els.filter(e=>/^Air Waybill number\(s\)$/i.test((e.innerText||e.textContent||'').trim()));
      const e=matches.sort((a,b)=>{const ar=a.getBoundingClientRect(),br=b.getBoundingClientRect();return ar.width*ar.height-br.width*br.height;})[0]||els.find(e=>/Air Waybill number\(s\)/i.test((e.innerText||e.textContent||'').trim()));
      if(!e)return null;const r=e.getBoundingClientRect();return{x:r.left+r.width/2,y:r.top+r.height/2};
    }).catch(()=>null);
    if(!awbPoint)return{ok:false,reason:'CATHAY AWB LABEL NOT FOUND',debug};
    await page.mouse.click(awbPoint.x,awbPoint.y);await sleep(600);

    debug.fieldsAfter=await page.$$eval('input,textarea,[contenteditable="true"],[role="textbox"]',els=>els.map((e,i)=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return{i,tag:e.tagName.toLowerCase(),type:(e.type||'').toLowerCase(),value:'value'in e?String(e.value||''):String(e.textContent||''),desc:`${e.name||''} ${e.id||''} ${e.placeholder||''} ${e.getAttribute('aria-label')||''} ${e.getAttribute('role')||''}`.toLowerCase(),visible:r.width>5&&r.height>5&&s.display!=='none'&&s.visibility!=='hidden'&&!e.disabled};})).catch(()=>[]);
    ...
    const target=await page.evaluate(()=>{
      ...
      const okay=e=>{const type=(e.type||'').toLowerCase(),desc=`${e.id||''} ${e.name||''} ${e.placeholder||''} ${e.getAttribute('aria-label')||''}`.toLowerCase();return visible(e)&&!['submit','button','checkbox','radio','hidden'].includes(type)&&!/search|airlinecodefield/.test(desc)&&norm('value'in e?e.value:'')!=='160';};
      let e=document.activeElement;if(!okay(e))e=nodes.find(x=>okay(x)&&/awb|waybill|number/.test(`${x.id||''} ${x.name||''} ${x.placeholder||''} ${x.getAttribute('aria-label')||''}`.toLowerCase()))||nodes.find(x=>okay(x));
      if(!e)return null;e.focus();return{index:nodes.indexOf(e),tag:e.tagName,value:'value'in e?String(e.value||''):String(e.textContent||'')};
    }).catch(()=>null);
    if(!target)return{ok:false,reason:'CATHAY AWB NUMBER CONTROL NOT FOUND AFTER LABEL CLICK',debug};

    const hs=await page.$$('input,textarea,[contenteditable="true"],[role="textbox"]'),h=hs[target.index];if(!h)return{ok:false,reason:'CATHAY AWB HANDLE MISSING',debug};
    await h.click({clickCount:3}).catch(()=>{});await page.keyboard.press('Control+A').catch(()=>{});await page.keyboard.press('Backspace').catch(()=>{});await h.type(serial,{delay:55}).catch(()=>{});debug.fillMode=`${target.tag}:${target.index}:suffix-only`;await page.keyboard.press('Tab').catch(()=>{});await sleep(700);

    const reg=await page.evaluate(s=>{const norm=v=>String(v||'').replace(/\D/g,'');const vals=[...document.querySelectorAll('input,textarea,[contenteditable="true"],[role="textbox"]')].map(e=>'value'in e?String(e.value||''):String(e.textContent||''));const body=norm(document.body?.innerText||'');return{values:vals.slice(0,18),serialInValue:vals.map(norm).some(v=>v.includes(s)),serialInBody:body.includes(s)};},serial).catch(()=>({values:[],serialInValue:false,serialInBody:false}));debug.registration=reg;debug.registered=Boolean(reg.serialInValue||reg.serialInBody);
    ...
    for(let i=0;i<14;i++){await sleep(650);const t=await allFrameText(page);if(/Current status:|Shipment In Progress|Booking Status|Accepted\s+\d+\/\d+|Departed\s+\d+\/\d+|Arrived\s+\d+\/\d+/i.test(t))break;}
    ...
    const text=await allFrameText(page);debug.text=clean(text).slice(0,8500);const shipment=parseOfficial(text,mawb,referenceYear);
