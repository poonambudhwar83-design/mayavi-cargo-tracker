...
async function findControls(page){
  return page.evaluate(()=>{const vis=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>15&&r.height>15&&s.display!=='none'&&s.visibility!=='hidden'&&!e.disabled;};const controls=[...document.querySelectorAll('input,textarea,[contenteditable="true"],[role="textbox"]')].filter(vis);const desc=e=>`${e.getAttribute('aria-label')||''} ${e.getAttribute('placeholder')||''} ${e.getAttribute('name')||''} ${e.id||''} ${e.parentElement?.innerText||''}`.trim();const code=controls.findIndex(e=>String('value'in e?e.value:e.textContent||'').replace(/\D/g,'')==='160'||/airline\s*code/i.test(desc(e)));let awb=controls.findIndex((e,i)=>i!==code&&/air\s*waybill|waybill|\bawb\b/i.test(desc(e))&&!/airline\s*code/i.test(desc(e)));if(awb<0)awb=controls.findIndex((e,i)=>i!==code&&!/search/i.test(desc(e)));return{code,awb,controls:controls.map((e,i)=>({i,tag:e.tagName,value:String('value'in e?e.value:e.textContent||''),desc:desc(e).slice(0,220),tabIndex:e.tabIndex}))};});
}
async function focusControl(page,index){return page.evaluate(i=>{const vis=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>15&&r.height>15&&s.display!=='none'&&s.visibility!=='hidden'&&!e.disabled;};const controls=[...document.querySelectorAll('input,textarea,[contenteditable="true"],[role="textbox"]')].filter(vis),e=controls[i];if(!e)return false;e.click();e.focus();return true;},index).catch(()=>false);}
async function clickAwbWrapper(page){return page.evaluate(()=>{const vis=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>15&&r.height>15&&s.display!=='none'&&s.visibility!=='hidden';};const candidates=[...document.querySelectorAll('label,div,section,fieldset')].filter(vis);const e=candidates.find(x=>/Air Waybill number\(s\)|Air Waybill number|AWB/i.test(String(x.innerText||x.textContent||''))&&x.querySelector('input,textarea,[contenteditable="true"],[role="textbox"]'));if(!e)return false;const t=e.querySelector('input,textarea,[contenteditable="true"],[role="textbox"]');(t||e).click();(t||e).focus?.();return true;}).catch(()=>false);}
...
async function officialCathay(mawb,referenceYear=''){
  ...
  try{
    ...
    let controls=await findControls(page);debug.controls=controls.controls;if(controls.code<0)return{ok:false,reason:'CATHAY AIRLINE CODE FIELD NOT FOUND',debug};
    const codeValue=String(controls.controls[controls.code]?.value||'').replace(/\D/g,'');debug.airlineCode=codeValue;if(codeValue!=='160')return{ok:false,reason:'CATHAY FIXED AIRLINE CODE IS NOT 160',debug};
    const attempt=async(mode,value,commit,focusMode='direct')=>{
      controls=await findControls(page);let focused=false;
      if(focusMode==='wrapper')focused=await clickAwbWrapper(page);else if(focusMode==='tab'){focused=await focusControl(page,controls.code);if(focused){await page.keyboard.press('Tab').catch(()=>{});await sleep(120);}}else focused=controls.awb>=0&&await focusControl(page,controls.awb);
