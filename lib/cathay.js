...
async function keyboardAwbEntry(page,airline,serial){
  ...
  await page.keyboard.type(serial,{delay:65}).catch(()=>{});await sleep(250);
...
async function mainTimeline(mawb){
  ...
  try{
    ...
    if(awb){airlineOk=airline?await typeValue(page,airline,'160'):true;awbOk=await typeValue(page,awb,serial);await awb.press('Enter').catch(()=>{});await sleep(500);mode='direct-awb-handle';}
