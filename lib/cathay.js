...
async function cathayOfficial(mawb,referenceYear=''){
  ...
  try{
    ...
    const awbAreaClicked=await page.evaluate(()=>{
      const visible=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>20&&r.height>15&&s.display!=='none'&&s.visibility!=='hidden';};
      const els=[...document.querySelectorAll('label,div,span,p')].filter(visible);
      const exact=els.filter(e=>/^Air Waybill number\(s\)$/i.test((e.innerText||e.textContent||'').trim()));
      const target=(exact.sort((a,b)=>a.getBoundingClientRect().width*b.getBoundingClientRect().height-b.getBoundingClientRect().width*a.getBoundingClientRect().height)[0])||els.find(e=>/Air Waybill number\(s\)/i.test((e.innerText||e.textContent||'').trim()));
      if(!target)return false;
      target.click();
      const box=target.closest('label,[class*="field"],[class*="input"],[class*="waybill"]')||target.parentElement;
      if(box&&box!==target)box.click();
      return true;
    });
    await new Promise(r=>setTimeout(r,700));

    let fields=await scanInputs(page);
    let usable=fields.filter(x=>x.visible&&!['hidden','checkbox','radio','submit','button'].includes(x.type));
    let awbMeta=usable.find(x=>/airwaybill|air.?waybill|awb/.test(x.desc));
    if(!awbMeta)awbMeta=usable.find(x=>String(x.value||'')!=='160'&&!/search/.test(x.desc));
    ...
    if(awbMeta){
      ...
      if(awb){
        ...
        await awb.type(serial,{delay:70}).catch(()=>{});
      }
    }else if(awbAreaClicked){
      await page.keyboard.type(serial,{delay:70}).catch(()=>{});
    }

    await page.keyboard.press('Enter').catch(()=>{});
    await new Promise(r=>setTimeout(r,700));

    const tokenized=await page.evaluate((full,serial)=>{
      const text=(document.body?.innerText||'').replace(/\s+/g,' ');
      const active=document.activeElement;
      const value=active&&'value' in active?String(active.value||''):'';
      const inputs=[...document.querySelectorAll('input,textarea')].map(e=>String(e.value||''));
      return {fullSeen:text.includes(full),serialSeen:text.includes(serial),valueSeen:value===serial||inputs.includes(serial)};
    },full,serial);
    if(!(tokenized.fullSeen||tokenized.serialSeen||tokenized.valueSeen)){
      fields=await scanInputs(page);
      return{ok:false,reason:'CATHAY AWB DID NOT REGISTER',debug:{awbAreaClicked,serial,fields,tokenized}};
    }
    ...
    if(!clicked)return{ok:false,reason:'TRACK NOW BUTTON NOT FOUND',debug:{awbAreaClicked,tokenized,fields}};
    ...
    if(!hasResult)return{ok:false,reason:'CATHAY RESULT DETAILS NOT VISIBLE',debug:{awbAreaClicked,tokenized,clicked,showDetailsClicked,fields,text:text.slice(0,6500)}};
    return{ok:true,shipment:result,debug:{awbAreaClicked,tokenized,clicked,showDetailsClicked,fields,text:text.slice(0,6500)}};
  }catch(e){
