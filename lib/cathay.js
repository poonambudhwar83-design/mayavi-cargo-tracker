...
async function officialCathay(mawb,referenceYear=''){
  ...
  try{
    ...
    if(awbFocused){const active=await activeDescription(page);debug.awbFocus=active;if(/air\s*waybill|waybill|awb|warpper/i.test(`${active.text} ${active.id} ${active.name} ${active.className} ${active.role}`))===false){awbFocused=await focusAwbWrapper(page);}}
