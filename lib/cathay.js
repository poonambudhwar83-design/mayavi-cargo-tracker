...
async function officialCathay(mawb,referenceYear=''){
  ...
  try{
    ...
    await focusAwbByAX(page,serial,debug);let reg=await registered(page,serial);debug.attempts.push({mode:'ax-suffix-enter',ok:reg.ok});
    if(!reg.ok){
      ...
      const inner=await page.evaluate(()=>{const visible=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>=0&&r.height>=0&&s.display!=='none'&&s.visibility!=='hidden'&&!e.disabled;};const editors=[];const walk=root=>{for(const e of root.querySelectorAll('*')){const tag=(e.tagName||'').toLowerCase(),role=(e.getAttribute?.('role')||'').toLowerCase();if((tag==='input'||tag==='textarea'||e.isContentEditable||role==='textbox'||role==='combobox')&&visible(e))editors.push(e);if(e.shadowRoot)walk(e.shadowRoot);}};const wraps=[...document.querySelectorAll('div[tabindex="0"]')],w=wraps.find(e=>/Air Waybill number\(s\)/i.test(e.innerText||e.textContent||'')||/warpper/i.test(String(e.className||'')));if(!w)return{focused:false,children:[]};walk(w);if(!editors.length)walk(document);const children=editors.slice(0,15).map((e,i)=>({i,tag:e.tagName,type:e.type||'',className:String(e.className||''),id:e.id||'',name:e.name||'',value:'value'in e?String(e.value||''):'',contenteditable:e.isContentEditable,role:e.getAttribute('role')||'',w:e.getBoundingClientRect().width,h:e.getBoundingClientRect().height}));const e=editors.find(x=>!['submit','button','checkbox','radio','hidden'].includes((x.type||'').toLowerCase())&&!/search/i.test(`${x.id||''} ${x.name||''} ${x.getAttribute?.('aria-label')||''}`));if(e){e.focus();e.click();}return{focused:Boolean(e),children};}).catch(()=>({focused:false,children:[]}));debug.innerEditor=inner;
      const tryValue=async(mode,value,commit='Enter')=>{await page.mouse.click(wrap.x,wrap.y).catch(()=>{});await sleep(120);await page.keyboard.press('Control+A').catch(()=>{});await page.keyboard.press('Backspace').catch(()=>{});await page.keyboard.type(value,{delay:45}).catch(()=>{});if(commit==='comma')await page.keyboard.type(',').catch(()=>{});else await page.keyboard.press(commit).catch(()=>{});await sleep(420);const r=await registered(page,serial);debug.attempts.push({mode,value,commit,ok:r.ok});return r;};
      if(inner.focused)reg=await tryValue('deep-editor-suffix',serial,'Enter');
      ...
      if(!reg.ok){await page.evaluate(s=>{const e=document.activeElement;try{const d=new DataTransfer();d.setData('text/plain',s);e?.dispatchEvent(new ClipboardEvent('paste',{bubbles:true,cancelable:true,clipboardData:d}));}catch{}},serial).catch(()=>{});await sleep(350);reg=await registered(page,serial);debug.attempts.push({mode:'paste-suffix',ok:reg.ok});}
    }
