...
async function officialCathay(mawb,referenceYear=''){
  ...
  try{
    ...
    if(!reg.ok){
      ...
      const tryValue=async(mode,value,commit='Enter')=>{const focused=await focusWrapper();await sleep(100);await page.keyboard.press('Control+A').catch(()=>{});await page.keyboard.press('Backspace').catch(()=>{});await page.keyboard.type(value,{delay:45}).catch(()=>{});if(commit==='comma')await page.keyboard.type(',').catch(()=>{});else await page.keyboard.press(commit).catch(()=>{});await sleep(450);const r=await registered(page,serial);debug.attempts.push({mode,value,commit,focused,ok:r.ok});return r;};
