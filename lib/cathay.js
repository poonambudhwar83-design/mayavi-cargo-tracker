...
async function cathayCargoPage(mawb,referenceYear=''){
  ...
  try{
    ...
    const focus=await page.evaluate(()=>{
      ...
      if(awb){const r=awb.getBoundingClientRect();return{mode:'input',x:r.left+r.width/2,y:r.top+r.height/2};}
      ...
      if(label){let p=label;for(let i=0;i<5&&p;i++,p=p.parentElement){const r=p.getBoundingClientRect();if(r.width>180&&r.height>30&&r.height<180){return{mode:'label-box',x:Math.min(r.right-30,Math.max(r.left+120,r.left+r.width*.62)),y:r.top+r.height/2};}}const r=label.getBoundingClientRect();return{mode:'label',x:r.left+r.width/2,y:r.top+r.height/2};}
      return{mode:'none'};
    });
    ...
    const activeBefore=await page.evaluate(()=>{const e=document.activeElement;return{tag:e?.tagName||'',type:e?.type||'',value:'value'in(e||{})?String(e.value||''):''};}).catch(()=>({}));
    if(!/INPUT|TEXTAREA/i.test(activeBefore.tag||'')){
      const fallbackIndex=await page.evaluate(()=>{
        const visible=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>10&&r.height>8&&s.display!=='none'&&s.visibility!=='hidden'&&!e.disabled;};
        const all=[...document.querySelectorAll('input,textarea')];
        for(let i=0;i<all.length;i++){
          const e=all[i],type=(e.type||'text').toLowerCase(),desc=`${e.id||''} ${e.name||''} ${e.placeholder||''} ${e.getAttribute('aria-label')||''}`.toLowerCase();
          if(visible(e)&&!['submit','button','checkbox','radio','hidden'].includes(type)&&/awb|air.?waybill|waybill|shipment|number|12345675/.test(desc))return i;
        }
        return-1;
      });
      if(fallbackIndex>=0){const hs=await page.$$('input,textarea');await hs[fallbackIndex]?.click().catch(()=>{});await new Promise(r=>setTimeout(r,150));}
    }
    ...
    if(!entered.registered)return{ok:false,reason:'CATHAY FORM DID NOT ACCEPT FULL MAWB',debug:{cookieClicked,focus,activeBefore,entered,digits,requests,responses,failed}};

    let clicked=await clickText(page,/^track\s*now$/i);
    if(!clicked)clicked=await clickText(page,/^track$/i);
    if(!clicked)return{ok:false,reason:'TRACK NOW BUTTON NOT FOUND',debug:{cookieClicked,focus,activeBefore,entered,requests,responses,failed}};
    ...
    const snapshot=await page.evaluate(()=>{
      ...
      return{text,bookingText:i>=0?text.slice(i,i+4500):''};
    });
    const text=snapshot.text,bookingText=snapshot.bookingText,networkText=network.map(x=>x.text).join('\n'),combined=`${text}\n${networkText}`;
    const official=parseOfficial(combined,mawb,referenceYear),bookingEta=parseBookingEta(bookingText,referenceYear),a=actualArrival(combined);
    if(!official.arrivalDate&&bookingEta.date){official.arrivalDate=bookingEta.date;official.arrivalTime=bookingEta.time;}
    else if(official.arrivalDate&&!official.arrivalTime&&bookingEta.time)official.arrivalTime=bookingEta.time;
    ...
    const debug={cookieClicked,focus,activeBefore,entered,button:clicked,showDetails:Boolean(showDetails),bookingScrolled,bookingEta,bookingText:bookingText.slice(0,3000),requests:requests.slice(-15),responses:responses.slice(-15),failed:failed.slice(-10),network:network.map(x=>({url:x.url,sample:x.text.slice(0,1800)})).slice(-8),text:text.slice(0,7000)};
