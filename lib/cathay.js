  ...
  try{
    ...
    const focus=await page.evaluate(()=>{
      ...
      const usable=e=>{
        ...
        const s=getComputedStyle(e),type=(e.type||'').toLowerCase(),desc=`${e.id||''} ${e.name||''} ${e.placeholder||''} ${e.getAttribute?.('aria-label')||''}`;
        return s.display!=='none'&&s.visibility!=='hidden'&&!['submit','button','checkbox','radio','hidden'].includes(type)&&!/search/i.test(desc);
      };
      const all=[...document.querySelectorAll('input,textarea,[contenteditable="true"],[role="textbox"],[role="combobox"]')];
      const codeIndex=all.findIndex(e=>usable(e)&&norm(('value'in e?e.value:'')||e.textContent)==='160');
      let target=null;
      if(codeIndex>=0)target=all.slice(codeIndex+1).find(e=>usable(e)&&norm(('value'in e?e.value:'')||e.textContent)!=='160')||null;
      if(!target)target=all.find(e=>usable(e)&&/awb|air.?waybill|waybill|shipment|12345675/i.test(`${e.id||''} ${e.name||''} ${e.placeholder||''} ${e.getAttribute?.('aria-label')||''}`))||null;
      if(target){target.focus();const r=target.getBoundingClientRect();return{mode:'target',index:all.indexOf(target),tag:target.tagName,value:'value'in target?String(target.value||''):String(target.textContent||''),x:r.left+r.width/2,y:r.top+r.height/2,w:r.width,h:r.height};}
      const labels=[...document.querySelectorAll('label,div,span,p')],label=labels.find(e=>/^Air Waybill number\(s\)$/i.test((e.innerText||e.textContent||'').trim()))||labels.find(e=>/Air Waybill number\(s\)/i.test((e.innerText||e.textContent||'').trim()));
      if(label){const r=label.getBoundingClientRect();return{mode:'label',x:r.right+120,y:r.top+r.height/2,w:r.width,h:r.height};}
      return{mode:'none'};
    });
    if(focus.mode==='none')return{ok:false,reason:'CATHAY AWB FIELD NOT FOUND',debug:{cookieClicked,focus,requests,responses,failed}};

    const handles=await page.$$('input,textarea,[contenteditable="true"],[role="textbox"],[role="combobox"]');
    if(focus.mode==='target'&&handles[focus.index])await handles[focus.index].focus().catch(()=>{});
    else{await page.mouse.click(focus.x,focus.y).catch(()=>{});await new Promise(r=>setTimeout(r,350));}

    const active=await page.evaluate(()=>{const e=document.activeElement;return{tag:e?.tagName||'',value:'value'in(e||{})?String(e.value||''):String(e?.textContent||''),role:e?.getAttribute?.('role')||''};}).catch(()=>({}));
    if(String(active.value||'').replace(/\D/g,'')==='160'){
      const switched=await page.evaluate(()=>{
        const norm=s=>String(s||'').replace(/\D/g,'');
        const all=[...document.querySelectorAll('input,textarea,[contenteditable="true"],[role="textbox"],[role="combobox"]')];
        const code=all.findIndex(e=>norm(('value'in e?e.value:'')||e.textContent)==='160');
        const t=code>=0?all.slice(code+1).find(e=>!e.disabled&&getComputedStyle(e).display!=='none'&&getComputedStyle(e).visibility!=='hidden'&&!['submit','button','checkbox','radio','hidden'].includes((e.type||'').toLowerCase())):null;
        if(!t)return false;t.focus();return true;
      }).catch(()=>false);
      if(switched)await new Promise(r=>setTimeout(r,100));
    }
    ...
    const entered=await page.evaluate((digits,serial)=>{
      ...
      return{registered,values:values.slice(0,16),normalized:normalized.slice(0,16),bodyHasFull:bodyDigits.includes(digits),active:{tag:document.activeElement?.tagName||'',value:'value'in(document.activeElement||{})?String(document.activeElement.value||''):String(document.activeElement?.textContent||'')}};
    },digits,serial);
    if(!entered.registered)return{ok:false,reason:'CATHAY FORM DID NOT ACCEPT FULL MAWB',debug:{cookieClicked,focus,active,entered,digits,requests,responses,failed}};

    let clicked=await clickText(page,/^track\s*now$/i);
    if(!clicked)clicked=await clickText(page,/^track$/i);
    if(!clicked)return{ok:false,reason:'TRACK NOW BUTTON NOT FOUND',debug:{cookieClicked,focus,active,entered,requests,responses,failed}};
    ...
    const bookingScrolled=await page.evaluate(()=>{
      const els=[...document.querySelectorAll('h1,h2,h3,h4,h5,h6,div,span,p,button')];
    ...
    const debug={cookieClicked,focus,active,entered,button:clicked,bookingScrolled,booking,bookingText:bookingText.slice(0,3500),requests:requests.slice(-15),responses:responses.slice(-15),failed:failed.slice(-10),network:network.map(x=>({url:x.url,sample:x.text.slice(0,1800)})).slice(-8),text:text.slice(0,7000)};
...
export async function trackCathay(input){
  ...
  if(live.ok){
    ...
    return{ok:true,airline:AIRLINE,shipment,debug:{stage:'SUCCESS',source:'cathay-official-booking-status',live:live.debug}};
  }
