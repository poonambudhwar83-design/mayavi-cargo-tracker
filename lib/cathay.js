...
async function cathayCargoPage(mawb,referenceYear=''){
  ...
  try{
    ...
    if(!entered.registered)return{ok:false,reason:'CATHAY FORM DID NOT ACCEPT FULL MAWB',debug:{cookieClicked,focus,entered,digits,requests,responses,failed}};

    let clicked=await clickText(page,/^track\s*now$/i);
    if(!clicked)clicked=await clickText(page,/^track$/i);
    if(!clicked)return{ok:false,reason:'TRACK NOW BUTTON NOT FOUND',debug:{cookieClicked,focus,entered,requests,responses,failed}};
    ...
    const debug={cookieClicked,focus,entered,button:clicked,showDetails:Boolean(showDetails),bookingScrolled,bookingEta,bookingText:bookingText.slice(0,3000),requests:requests.slice(-15),responses:responses.slice(-15),failed:failed.slice(-10),network:network.map(x=>({url:x.url,sample:x.text.slice(0,1800)})).slice(-8),text:text.slice(0,7000)};
