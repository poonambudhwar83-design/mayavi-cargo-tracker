...
async function cathayCargoPage(mawb,referenceYear=''){
  ...
  try{
    ...
    const debug={cookieClicked,focus,entered,button:clicked,showDetails:Boolean(showDetails),requests:requests.slice(-15),responses:responses.slice(-15),failed:failed.slice(-10),network:network.map(x=>({url:x.url,sample:x.text.slice(0,1800)})).slice(-8),text:text.slice(0,7000)};
