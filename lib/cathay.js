...
async function clickAwbWrapper(page){return page.evaluate(()=>{const vis=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>15&&r.height>15&&s.display!=='none'&&s.visibility!=='hidden';};const rx=/Air Waybill number\(s\)|Air Waybill number|AWB/i;const labels=[...document.querySelectorAll('label')].filter(x=>vis(x)&&rx.test(String(x.innerText||x.textContent||'')));for(const label of labels){const forId=label.getAttribute('for'),byFor=forId&&document.getElementById(forId);const root=label.closest('div,section,fieldset')||label.parentElement;const target=(byFor&&vis(byFor)?byFor:null)||root?.querySelector('input:not([type="submit"]):not([type="button"]):not([type="reset"]):not([type="image"]):not([type="hidden"]),textarea,[contenteditable="true"],[role="textbox"]');if(target&&vis(target)){target.click();target.focus?.();return true;}const sibling=label.nextElementSibling;if(sibling&&vis(sibling)){const nested=sibling.matches?.('input:not([type="submit"]):not([type="button"]):not([type="reset"]):not([type="image"]):not([type="hidden"]),textarea,[contenteditable="true"],[role="textbox"]')?sibling:sibling.querySelector?.('input:not([type="submit"]):not([type="button"]):not([type="reset"]):not([type="image"]):not([type="hidden"]),textarea,[contenteditable="true"],[role="textbox"]');if(nested&&vis(nested)){nested.click();nested.focus?.();return true;}sibling.click();sibling.focus?.();if(document.activeElement&&document.activeElement!==document.body)return true;}}
    const candidates=[...document.querySelectorAll('div,section,fieldset,[role="group"]')].filter(x=>vis(x)&&rx.test(String(x.innerText||x.textContent||'')));for(const e of candidates){const t=e.querySelector('input:not([type="submit"]):not([type="button"]):not([type="reset"]):not([type="image"]):not([type="hidden"]),textarea,[contenteditable="true"],[role="textbox"]');if(t&&vis(t)){t.click();t.focus?.();return true;}e.click();e.focus?.();if(document.activeElement&&document.activeElement!==document.body)return true;}return false;}).catch(()=>false);}
