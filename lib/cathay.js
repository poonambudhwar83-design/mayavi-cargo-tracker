...
export async function trackCathay(input){
  ...
  // Keep the MAWB visible without inventing cargo facts when Cathay's own sites are temporarily unavailable.
  // A non-TRACKING status lets the route return this truthful fallback instead of converting it into HTTP 503.
  return{ok:true,airline:AIRLINE,shipment:{mawb,carrierCode:'CX',airlineName:'Cathay Cargo',origin:'',destination:'',bags:'',pieces:'',weight:'',bookingDate:'',flightNo:'',arrivalDate:'',arrivalTime:'',arrivalIsActual:false,status:'TRACKING TEMPORARILY UNAVAILABLE',officialTracker:CATHAY,source:'Cathay tracker temporarily unavailable'},debug:{source:'cathay-resilient-fallback',terminal:terminal.reason,official:official.reason}};
}
