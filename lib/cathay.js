...
async function submitNearAwb(page,awbHandle){
  if(awbHandle){
    await awbHandle.press('Enter').catch(()=>{});await sleep(1400);
  ...
  const clicked=await awbHandle.evaluate(e=>{
    ...
    if(!root){let p=e.parentElement;for(let i=0;i<7&&p;i++,p=p.parentElement){const t=p.innerText||'';if(/air\s*waybill/i.test(t)&&/add another airline code|clear all|track your shipment/i.test(t)){root=p;break;}}}
    root=root||document;
    const els=[...root.querySelectorAll('button,input[type="submit"],input[type="button"],[role="button"],a')].filter(visible);
    let btn=els.find(n=>n.matches('button[type="submit"],input[type="submit"]'));
    if(!btn)btn=els.find(n=>/^(track|submit|go)$/i.test((n.innerText||n.value||n.textContent||n.getAttribute('aria-label')||'').trim()));
    if(!btn)btn=els.find(n=>/track\s+(shipment|cargo|awb)|track\s+and\s+trace/i.test((n.innerText||n.value||n.textContent||n.getAttribute('aria-label')||'').trim()));
    if(!btn)btn=els.find(n=>{const t=(n.innerText||n.value||n.textContent||n.getAttribute('aria-label')||n.getAttribute('title')||'').trim();return !/reset|clear|add another|cookie|search/i.test(t)&&t.length<40;});
    if(!btn)return'';const t=(btn.innerText||btn.value||btn.textContent||btn.getAttribute('aria-label')||btn.getAttribute('title')||'').trim();btn.click();return t||'scoped-submit';
  }).catch(()=> '');
...
async function mainTimeline(mawb){
  ...
  try{
    ...
    if(!awb&&rows.length){const useful=rows.filter(x=>!/search|email|chat|cookie/.test(x.meta.desc));awb=useful.find(x=>x.meta.type==='contenteditable')?.h||useful.at(-1)?.h||null;airline=airline||useful.find(x=>/airline\s*code/.test(x.meta.desc))?.h||null;}
    const attempts=[];
    const plans=[
      {keepDefaultAirline:true,awbValue:serial,label:'default-160-plus-serial'},
      {keepDefaultAirline:true,awbValue:digits,label:'default-160-plus-full-awb'},
      {airlineValue:'160',awbValue:serial,label:'explicit-160-plus-serial'}
    ];
    for(const plan of plans){
      if(!plan.keepDefaultAirline&&airline){await typeValue(page,airline,plan.airlineValue||'160');}
      ...
      attempts.push({label:plan.label,awbOk,submitted,visible});
      if(visible)break;
    }
    const text=await page.evaluate(()=>document.body?.innerText||'').catch(()=> '');
    ...
    const buttonDebug=await page.evaluate(()=>[...document.querySelectorAll('button,input[type="submit"],input[type="button"],[role="button"],a')].map(n=>{const r=n.getBoundingClientRect(),s=getComputedStyle(n);return{t:(n.innerText||n.value||n.textContent||n.getAttribute('aria-label')||n.getAttribute('title')||'').trim().slice(0,80),type:n.getAttribute('type')||'',visible:s.display!=='none'&&s.visibility!=='hidden'&&r.width>10&&r.height>10,x:Math.round(r.x),y:Math.round(r.y)};}).filter(x=>x.visible&&x.y>200&&x.y<700).slice(0,30)).catch(()=>[]);
    return shipment?{ok:true,shipment,debug:{stage:'TIMELINE',attempts,inputs:inputDebug,buttons:buttonDebug,pageText:text.slice(0,7000)}}:{ok:false,reason:'CATHAY MAIN TIMELINE NOT PARSED',debug:{stage:'NO_TIMELINE',attempts,inputs:inputDebug,buttons:buttonDebug,pageText:text.slice(0,7000)}};
