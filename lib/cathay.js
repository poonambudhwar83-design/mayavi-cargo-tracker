...
async function cathayOfficial(mawb,referenceYear=''){
  ...
  try{
    ...
    await page.waitForFunction(()=>{
      ...
      return [...document.querySelectorAll('input')].some(e=>visible(e)&&/airwaybill|air.?waybill|awb/i.test(`${e.id||''} ${e.name||''} ${e.placeholder||''} ${e.getAttribute('aria-label')||''}`));
    },{timeout:15000}).catch(()=>{});

    const fields=await scanInputs(page);
    const awbMeta=fields.find(x=>x.visible&&!['hidden','checkbox','radio','submit','button'].includes(x.type)&&/airwaybill|air.?waybill|awb/.test(x.desc));
    ...
    const afterFill=await scanInputs(page);
    const awbAfter=afterFill.find(x=>x.visible&&!['hidden','checkbox','radio','submit','button'].includes(x.type)&&/airwaybill|air.?waybill|awb/.test(x.desc));
    const number=String(awbAfter?.value||'')===serial;
    if(!number)return{ok:false,reason:'CATHAY AWB DID NOT STAY FILLED',debug:{serial,fields,afterFill}};
    ...
    if(!clicked)return{ok:false,reason:'TRACK NOW BUTTON NOT FOUND',debug:{number,fields,afterFill}};
    ...
    if(!hasResult)return{ok:false,reason:'CATHAY RESULT DETAILS NOT VISIBLE',debug:{number,clicked,showDetailsClicked,fields,afterFill,text:text.slice(0,6500)}};
    return{ok:true,shipment:result,debug:{number,clicked,showDetailsClicked,fields,afterFill,text:text.slice(0,6500)}};
  }catch(e){
