...
async function officialCathay(mawb,referenceYear=''){
  ...
  try{
    ...
    debug.codeIndex=codeIndex;
    if(codeIndex<0)return{ok:false,reason:'CATHAY FIXED 160 FIELD NOT FOUND',debug};
    const inputs=await page.$$('input'),code=inputs[codeIndex];if(!code)return{ok:false,reason:'CATHAY FIXED 160 HANDLE NOT FOUND',debug};
