...
async function cathayCargoPage(mawb){
 ...
 try{
  ...
  const inputMeta=await page.$$eval('input',els=>els.map((e,i)=>{
   const r=e.getBoundingClientRect(),s=getComputedStyle(e);
   const visible=r.width>10&&r.height>8&&s.display!=='none'&&s.visibility!=='hidden'&&!e.disabled;
   return{i,visible,type:(e.type||'text').toLowerCase(),desc:`${e.name||''} ${e.id||''} ${e.placeholder||''} ${e.getAttribute('aria-label')||''}`.toLowerCase()};
  }));
  const usable=inputMeta.filter(x=>x.visible&&!['hidden','checkbox','radio','submit','button'].includes(x.type));
  const prefixMeta=usable.find(x=>/airline|prefix|code/.test(x.desc));
  const numberMeta=usable.find(x=>/awb|air.?waybill|waybill|shipment|number/.test(x.desc)&&x.i!==prefixMeta?.i)||usable.find(x=>x.i!==prefixMeta?.i);
  const handles=await page.$$('input');
  const typeInto=async(meta,value)=>{
   if(!meta||!handles[meta.i])return false;
   const h=handles[meta.i];
   await h.click({clickCount:3}).catch(()=>{});
   await page.keyboard.press('Backspace').catch(()=>{});
   await h.type(value,{delay:70});
   await page.keyboard.press('Tab').catch(()=>{});
   return true;
  };
  const prefix=await typeInto(prefixMeta,'160');
  const number=await typeInto(numberMeta,serial);
  await new Promise(r=>setTimeout(r,700));

  const entered=await page.$$eval('input',els=>els.map(e=>({value:e.value||'',name:e.name||'',id:e.id||''})).filter(x=>x.value).slice(0,12));
  ...
  for(const h of buttons){
   ...
   if(info?.visible&&/track|search|submit/i.test(info.text)){await h.click().catch(()=>{});button=true;break;}
  }
  await new Promise(r=>setTimeout(r,6500));
  const text=await page.evaluate(()=>document.body?.innerText||'');
  const debug={prefix,number,button,entered,inputs:usable.map(x=>x.desc).slice(0,12)};
  ...
  if(/Please input an air waybill number to start tracking/i.test(text)&&!text.includes(mawb)&&!text.includes(serial))return{ok:false,reason:'CATHAY FORM DID NOT ACCEPT AWB',debug:{...debug,text:text.slice(0,2600)}};
