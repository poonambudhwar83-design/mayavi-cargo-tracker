...
async function fillMeta(page,meta,value){
  if(!meta)return false;
  const handles=await page.$$('input,textarea');
  const h=handles[meta.i];
  if(!h)return false;
...
async function cathayOfficial(mawb,referenceYear=''){
  ...
  try{
    ...
    let prefixMeta=usable.find(x=>/airline|prefix|code/.test(x.desc))||usable[0];
    const prefix=await fillMeta(page,prefixMeta,'160');
    await new Promise(r=>setTimeout(r,700));

    let prefixChoice=await page.evaluate(()=>{
      ...
      const opts=[...document.querySelectorAll('[role="option"],[role="listbox"] li,[class*="autocomplete"] li,[class*="suggest"] li')].filter(visible);
      const o=opts.find(e=>/(^|\D)160(\D|$)/.test((e.innerText||e.textContent||'').trim()));
      if(!o)return'';
      const t=(o.innerText||o.textContent||'').trim();
      o.click();
      return t;
    });
    if(!prefixChoice&&prefixMeta){
      const handles=await page.$$('input,textarea'),h=handles[prefixMeta.i];
      if(h){await h.focus().catch(()=>{});await page.keyboard.press('ArrowDown').catch(()=>{});await page.keyboard.press('Enter').catch(()=>{});prefixChoice='keyboard';}
    }
    await new Promise(r=>setTimeout(r,1200));

    fields=await scanInputs(page);
    usable=fields.filter(x=>x.visible&&!['hidden','checkbox','radio','submit','button'].includes(x.type));
    const awbMeta=usable.find(x=>/air.?waybill|waybill|awb|shipment|number/.test(x.desc)&&x.i!==prefixMeta?.i)||usable.find(x=>x.i!==prefixMeta?.i);
    const number=await fillMeta(page,awbMeta,serial);
    await new Promise(r=>setTimeout(r,500));
    const beforeSubmit=await scanInputs(page);

    const clicked=await page.evaluate(()=>{
      const all=[...document.querySelectorAll('input[type="submit"],input[type="button"],button')];
      const el=all.find(e=>/track\s*now/i.test((e.value||e.innerText||e.textContent||'').trim()))||all.find(e=>/^track$/i.test((e.value||e.innerText||e.textContent||'').trim()));
      if(!el)return false;
      el.click();
      return true;
    });
    if(!clicked)return{ok:false,reason:'TRACK NOW BUTTON NOT FOUND',debug:{prefix,number,prefixChoice,fields,beforeSubmit}};
    ...
    if(!hasResult)return{ok:false,reason:'CATHAY RESULT DETAILS NOT VISIBLE',debug:{prefix,number,prefixChoice,showDetailsClicked,fields,beforeSubmit,text:text.slice(0,6500)}};
    return{ok:true,shipment:result,debug:{prefix,number,prefixChoice,showDetailsClicked,fields,beforeSubmit,text:text.slice(0,6500)}};
  }catch(e){
