...
function parseMilestones(plain){
  const out={pieces:'',weight:'',bookingDate:'',flightNo:'',flightDate:'',departureActual:'',arrival:{date:'',time:''},arrivalIsActual:false};
  ...
  if(rcs){
    ...
    if(m){out.bookingDate=dateTime(m[1]).date;out.pieces=m[2];out.weight=m[3].replace(/,/g,'');}
  }
  if(dep){
...
async function flightStatusFallback(shipment,flightDate){
  ...
  try{
    ...
    const landed=/LANDED|ARRIVED/i.test(statusText);
    return {
...
async function cathaySnapshotFallback(shipment,flightDate){
  try{
    const snap=await trackFlightStatusSnapshot({flightNo:shipment.flightNo,origin:shipment.origin,destination:shipment.destination,date:flightDate});
    if(!snap?.ok)return null;
    return {status:snap.status,arrivalDate:snap.arrivalDate,arrivalTime:snap.arrivalTime,arrivalIsActual:snap.arrivalIsActual,source:snap.source||'Cathay flight-status snapshot',url:snap.url||''};
  }catch{return null;}
}
    ...
    try{
      ...
      if(p?.useful){
        ...
        if(shipment.status==='IN TRANSIT'&&shipment.flightNo&&p.flightDate){
          let f=await flightStatusFallback(shipment,p.flightDate);
          if(!f)f=await cathaySnapshotFallback(shipment,p.flightDate);
          if(f){shipment.status=f.arrivalIsActual?'ARRIVED':'IN TRANSIT';shipment.arrivalDate=f.arrivalDate;shipment.arrivalTime=f.arrivalTime;shipment.arrivalIsActual=Boolean(f.arrivalIsActual);shipment.timing=f.timing||'';shipment.source=`${shipment.source} + ${f.source}`;}
        }
