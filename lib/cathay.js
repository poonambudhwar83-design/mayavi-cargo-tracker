...
function parseTerminal(html,mawb){
  ...
  const depDate=dep?dt(`${dep[2]} ${atd[2]||dep[3]}`):{date:'',time:''};
  const rcfSection=(text.match(/Received from Flight[\s\S]{0,2400}?(?=Cargo Delivered|Last Update|$)/i)||[])[0]||'';
...
function parseOfficial(raw='',mawb='',referenceYear=''){
  ...
  const actualPatterns=[
    /(?:Arrived|Actual arrival|ATA)[\s\S]{0,180}?(\d{1,2}\s+(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)[A-Z]*(?:\s+20\d{2})?\s+\d{1,2}:\d{2})/i,
    /(\d{1,2}\s+(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)[A-Z]*(?:\s+20\d{2})?\s+\d{1,2}:\d{2})[\s\S]{0,100}?Arrived/i
  ];
  if(arrived){
    for(const rx of actualPatterns){
      const m=text.match(rx);if(!m)continue;
      const x=/20\d{2}/.test(m[1])?dt(m[1]):dm(m[1],year);if(x.date){arrivalDate=x.date;arrivalTime=x.time;break;}
    }
  }

  if(!arrivalDate&&flightNo){
    const esc=flightNo.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
    let m=text.match(new RegExp(`(\\d{1,2}\\s+(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)[A-Z]*\\s+\\d{1,2}:\\d{2})[\\s\\S]{0,180}?${esc}[\\s\\S]{0,320}?(\\d{1,2}\\s+(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)[A-Z]*\\s+\\d{1,2}:\\d{2})`,'i'));
    if(!m)m=text.match(new RegExp(`${esc}[\\s\\S]{0,320}?(\\d{1,2}\\s+(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)[A-Z]*\\s+\\d{1,2}:\\d{2})[\\s\\S]{0,320}?(\\d{1,2}\\s+(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)[A-Z]*\\s+\\d{1,2}:\\d{2})`,'i'));
    if(m){const x=dm(m[2],year);arrivalDate=x.date;arrivalTime=x.time;}
  }
  if(!arrivalDate){
    const m=text.match(/Estimated(?:\s+arrival)?[\s\S]{0,160}?(\d{1,2}\s+(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)[A-Z]*(?:\s+20\d{2})?\s+\d{1,2}:\d{2})/i);
...
async function getTerminal(mawb){
  ...
  for(const url of [`${BASE}/AWBPrefix/160/AWBSuffix/${suffix}`,`${BASE}?AWBPrefix=160&AWBSuffix=${suffix}`]){
    try{
      const r=await fetch(url,{headers:{'user-agent':'Mozilla/5.0','accept-language':'en-US,en;q=0.9'},cache:'no-store',signal:AbortSignal.timeout(6000)});
...
async function clickText(page,rx){
  for(const f of page.frames()){
    try{
      const hit=await f.evaluate(src=>{const re=new RegExp(src,'i'),els=[...document.querySelectorAll('button,input[type="submit"],input[type="button"],a,[role="button"]')],vis=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>8&&r.height>8&&s.display!=='none'&&s.visibility!=='hidden'&&!e.disabled;},e=els.find(x=>vis(x)&&re.test((x.innerText||x.value||x.textContent||x.getAttribute('aria-label')||'').trim()));if(!e)return'';const t=(e.innerText||e.value||e.textContent||'').trim();e.click();return t;},rx.source);
      if(hit)return hit;
    }catch{}
  }
  return'';
...
async function registered(page,serial){
  for(let attempt=0;attempt<3;attempt++){
    try{
      return await page.evaluate(s=>{const norm=v=>String(v||'').replace(/\D/g,'');const vals=[],texts=[];const walk=root=>{try{texts.push(root===document?(document.body?.innerText||''):(root.textContent||''));for(const e of root.querySelectorAll('*')){if('value'in e)vals.push(String(e.value||''));else if(e.isContentEditable||e.getAttribute?.('role')==='textbox')vals.push(String(e.textContent||''));if(e.shadowRoot)walk(e.shadowRoot);}}catch{}};walk(document);return{ok:texts.map(norm).some(v=>v.includes(s))||vals.map(norm).some(v=>v.includes(s)),body:texts.join('\n').slice(0,4200),values:vals.slice(0,35)};},serial);
    }catch{await sleep(250);}
  }
  return{ok:false,body:'',values:[]};
}
async function activeDescription(page){
  try{return await page.evaluate(()=>{const e=document.activeElement,r=e?.getBoundingClientRect?.();return{tag:e?.tagName||'',type:e?.type||'',id:e?.id||'',name:e?.name||'',className:String(e?.className||''),role:e?.getAttribute?.('role')||'',text:String(e?.innerText||e?.textContent||'').slice(0,160),x:r?r.left:0,y:r?r.top:0,w:r?r.width:0,h:r?r.height:0,tabIndex:e?.tabIndex??-1};});}catch{return{};}
}
async function findCodeIndex(page){
  for(let attempt=0;attempt<4;attempt++){
    try{
      const i=await page.evaluate(()=>{const all=[...document.querySelectorAll('input')],vis=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>20&&r.height>15&&s.display!=='none'&&s.visibility!=='hidden'&&!e.disabled;};let x=all.findIndex(e=>vis(e)&&/airlinecodefield/i.test(`${e.id||''} ${e.name||''}`));if(x<0)x=all.findIndex(e=>vis(e)&&(e.type||'text').toLowerCase()==='text'&&!/search/i.test(`${e.id||''} ${e.name||''} ${e.placeholder||''} ${e.getAttribute('aria-label')||''}`));return x;});
      if(i>=0)return i;
    }catch{}
    await sleep(300);
  }
...
async function getInputAt(page,index){
  const inputs=await page.$$('input').catch(()=>[]);return inputs[index]||null;
}
async function focusAwbByAX(page,serial,debug){
  try{
    ...
    await client.send('Runtime.callFunctionOn',{objectId,functionDeclaration:'function(){try{this.click()}catch(e){};try{this.focus()}catch(e){};return true}',returnByValue:true});
    await page.keyboard.press('Control+A').catch(()=>{});await page.keyboard.press('Backspace').catch(()=>{});await page.keyboard.type(serial,{delay:55}).catch(()=>{});await page.keyboard.press('Enter').catch(()=>{});await sleep(500);
...
async function officialCathay(mawb,referenceYear=''){
  ...
  try{
    ...
    const proc=browser.process?.();killTimer=setTimeout(()=>{try{proc?.kill('SIGKILL');}catch{}},42000);
    ...
    if(String(debug.airlineCodeBefore).replace(/\D/g,'')!=='160'){
      ...
      if(!debug.airlineCodeSuggestion){await page.keyboard.press('ArrowDown').catch(()=>{});await page.keyboard.press('Enter').catch(()=>{});}await sleep(650);
    }
    debug.airlineCodeAfter=await page.evaluate((i)=>document.querySelectorAll('input')[i]?.value||'',codeIndex).catch(()=>'');

    // Important: Cathay re-renders this field after selecting 160. Re-query it instead of reusing the stale handle.
    code=await getInputAt(page,codeIndex);if(!code)return{ok:false,reason:'CATHAY AIRLINE CODE FIELD DETACHED AFTER SELECTION',debug};
    await code.focus();await page.keyboard.press('Tab').catch(()=>{});await sleep(300);
    debug.awbFocus=await activeDescription(page);

    let reg={ok:false,body:'',values:[]};
    const typeIntoFocused=async(mode,value,commit='Enter')=>{
      await page.keyboard.press('Control+A').catch(()=>{});await page.keyboard.press('Backspace').catch(()=>{});await page.keyboard.type(value,{delay:50}).catch(()=>{});
      if(commit==='comma')await page.keyboard.type(',').catch(()=>{});else await page.keyboard.press(commit).catch(()=>{});
      await sleep(550);const r=await registered(page,serial);debug.attempts.push({mode,value,commit,active:await activeDescription(page),ok:r.ok});return r;
    };
    reg=await typeIntoFocused('tab-suffix-enter',serial,'Enter');
    if(!reg.ok)reg=await typeIntoFocused('tab-suffix-comma',serial,'comma');
    if(!reg.ok){await focusAwbByAX(page,serial,debug);reg=await registered(page,serial);debug.attempts.push({mode:'ax-suffix-enter',ok:reg.ok});}
    ...
    let text='';
    for(let i=0;i<20;i++){
      await sleep(650);text=await frameText(page);
      if(/Booking Status|Current status:|Shipment In Progress|Accepted\s+\d+\/\d+|Departed\s+\d+\/\d+|Arrived\s+\d+\/\d+/i.test(text))break;
    }
    const flat=clean(text),idx=flat.search(/Booking Status/i),booking=idx>=0?flat.slice(idx,idx+6500):flat;debug.bookingText=booking.slice(0,6500);
    const shipment=parseOfficial(booking,mawb,referenceYear);
