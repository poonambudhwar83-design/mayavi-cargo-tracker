...
function terminalParse(html,mawb){
  ...
  // Terminal RCF is warehouse receipt, not aircraft arrival. It is retained only as a fallback
  // when the Cathay Track & Trace flight card cannot be read.
...
function parseMainTimeline(text,mawb){
  ...
  const delivered=/\bDelivered\b[\s\S]{0,40}?\b([1-9]\d*)\s*\/\s*\1\b/i.test(flat)||/Current\s+status\s*:\s*Delivered/i.test(flat);
...
async function get(url){
  try{const r=await fetch(url,{headers:{'user-agent':'Mozilla/5.0','accept':'text/html,application/xhtml+xml'},redirect:'follow',cache:'no-store',signal:AbortSignal.timeout(10000)});if(!r.ok)return null;return await r.text();}catch{return null;}
}
...
async function trackMainPage(mawb){
  ...
  try{
    ...
    await page.goto(CATHAY,{waitUntil:'domcontentloaded',timeout:30000});
    await sleep(3500);await acceptCookies(page);await sleep(700);
    ...
    const fill=await page.evaluate(({prefix,serial,digits,mawb})=>{
      ...
      const numberInput=inputs.find(e=>/(awb|airway|document|tracking|shipment).*(number|no|serial|suffix)|awbnumber|documentnumber/.test(desc(e))&&!/prefix/.test(desc(e)));
    ...
    if(fill.filled){
      ...
      await Promise.race([page.waitForNetworkIdle({idleTime:1000,timeout:12000}).catch(()=>{}),sleep(12000)]);
      await sleep(2200);
    }
