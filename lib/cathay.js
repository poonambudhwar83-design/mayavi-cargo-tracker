...
async function registrationSnapshot(page,serial,digits){
  const merged={ok:false,chip:'',values:[],body:'',frameIndex:-1,frameUrl:''};
  const frames=page.frames();
  for(let frameIndex=0;frameIndex<frames.length;frameIndex++){
    const f=frames[frameIndex];
    try{
      const snap=await f.evaluate(({serial,digits})=>{const norm=v=>String(v||'').replace(/\D/g,'');const visible=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>2&&r.height>2&&s.display!=='none'&&s.visibility!=='hidden';};const chipSelectors='[role="listitem"],[role="option"],[class*="chip" i],[class*="tag" i],[class*="token" i],[class*="pill" i]';const chip=[...document.querySelectorAll(chipSelectors)].find(e=>visible(e)&&[serial,digits].some(x=>norm(e.innerText||e.textContent||'').includes(x)));const values=[...document.querySelectorAll('input,textarea,[contenteditable="true"],[role="textbox"]')].map(e=>({value:String('value'in e?e.value:e.textContent||''),label:String(e.getAttribute('aria-label')||e.getAttribute('placeholder')||e.getAttribute('name')||e.id||'')}));const body=document.body?.innerText||'';return{ok:Boolean(chip)||norm(body).includes(digits)||norm(body).includes(serial),chip:chip?String(chip.innerText||chip.textContent||'').slice(0,180):'',values:values.slice(0,30),body:body.slice(0,4200)};},{serial,digits});
      if(snap.values?.length)merged.values.push(...snap.values.map(x=>({...x,frameIndex,frameUrl:f.url()})));
      if(snap.body&&!merged.body)merged.body=snap.body;
      if(snap.ok)return{...snap,frameIndex,frameUrl:f.url()};
    }catch{}
  }
  return merged;
}
async function findControls(page){
  const result={code:null,awb:null,controls:[]},frames=page.frames();
  for(let frameIndex=0;frameIndex<frames.length;frameIndex++){
    const f=frames[frameIndex];
    try{
      const found=await f.evaluate(()=>{const vis=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>15&&r.height>15&&s.display!=='none'&&s.visibility!=='hidden'&&!e.disabled;};const editable=e=>{const type=String(e.getAttribute('type')||'').toLowerCase(),value=String('value'in e?e.value:e.textContent||'').trim();return !['submit','button','reset','image','hidden'].includes(type)&&!/^track\s*now$/i.test(value);};const controls=[...document.querySelectorAll('input,textarea,[contenteditable="true"],[role="textbox"]')].filter(e=>vis(e)&&editable(e));const desc=e=>{const id=e.id||'',label=id?document.querySelector(`label[for="${CSS.escape(id)}"]`):null;return `${e.getAttribute('aria-label')||''} ${e.getAttribute('placeholder')||''} ${e.getAttribute('name')||''} ${id} ${label?.innerText||''} ${e.parentElement?.innerText||''}`.trim();};const code=controls.findIndex(e=>String('value'in e?e.value:e.textContent||'').replace(/\D/g,'')==='160'||/airline\s*code/i.test(desc(e)));let awb=controls.findIndex((e,i)=>i!==code&&/air\s*waybill|waybill|\bawb\b/i.test(desc(e))&&!/airline\s*code/i.test(desc(e)));if(awb<0)awb=controls.findIndex((e,i)=>i!==code&&!/search|track\s*now/i.test(desc(e)));return{code,awb,controls:controls.map((e,i)=>({i,tag:e.tagName,value:String('value'in e?e.value:e.textContent||''),desc:desc(e).slice(0,220),tabIndex:e.tabIndex}))};});
      const annotated=found.controls.map(x=>({...x,frameIndex,frameUrl:f.url()}));result.controls.push(...annotated);
      if(!result.code&&found.code>=0)result.code={frameIndex,index:found.code,...annotated[found.code]};
      if(!result.awb&&found.awb>=0)result.awb={frameIndex,index:found.awb,...annotated[found.awb]};
    }catch{}
  }
  return result;
}
async function focusControl(page,target){
  if(!target||target.frameIndex==null||target.index==null)return false;
  const f=page.frames()[target.frameIndex];if(!f)return false;
  try{return await f.evaluate(i=>{const vis=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>15&&r.height>15&&s.display!=='none'&&s.visibility!=='hidden'&&!e.disabled;};const editable=e=>{const type=String(e.getAttribute('type')||'').toLowerCase(),value=String('value'in e?e.value:e.textContent||'').trim();return !['submit','button','reset','image','hidden'].includes(type)&&!/^track\s*now$/i.test(value);};const controls=[...document.querySelectorAll('input,textarea,[contenteditable="true"],[role="textbox"]')].filter(e=>vis(e)&&editable(e)),e=controls[i];if(!e)return false;e.click();e.focus();return document.activeElement===e||e.contains(document.activeElement);},target.index);}catch{return false;}
}
async function clickAwbWrapper(page){
  for(const f of page.frames()){
    try{
      const hit=await f.evaluate(()=>{const vis=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>15&&r.height>15&&s.display!=='none'&&s.visibility!=='hidden';};const rx=/Air Waybill number\(s\)|Air Waybill number|AWB/i;const labels=[...document.querySelectorAll('label')].filter(x=>vis(x)&&rx.test(String(x.innerText||x.textContent||'')));for(const label of labels){const forId=label.getAttribute('for'),byFor=forId&&document.getElementById(forId);const root=label.closest('div,section,fieldset')||label.parentElement;const target=(byFor&&vis(byFor)?byFor:null)||root?.querySelector('input:not([type="submit"]):not([type="button"]):not([type="reset"]):not([type="image"]):not([type="hidden"]),textarea,[contenteditable="true"],[role="textbox"]');if(target&&vis(target)){target.click();target.focus?.();return true;}const sibling=label.nextElementSibling;if(sibling&&vis(sibling)){const nested=sibling.matches?.('input:not([type="submit"]):not([type="button"]):not([type="reset"]):not([type="image"]):not([type="hidden"]),textarea,[contenteditable="true"],[role="textbox"]')?sibling:sibling.querySelector?.('input:not([type="submit"]):not([type="button"]):not([type="reset"]):not([type="image"]):not([type="hidden"]),textarea,[contenteditable="true"],[role="textbox"]');if(nested&&vis(nested)){nested.click();nested.focus?.();return true;}sibling.click();sibling.focus?.();if(document.activeElement&&document.activeElement!==document.body)return true;}}const candidates=[...document.querySelectorAll('div,section,fieldset,[role="group"]')].filter(x=>vis(x)&&rx.test(String(x.innerText||x.textContent||'')));for(const e of candidates){const t=e.querySelector('input:not([type="submit"]):not([type="button"]):not([type="reset"]):not([type="image"]):not([type="hidden"]),textarea,[contenteditable="true"],[role="textbox"]');if(t&&vis(t)){t.click();t.focus?.();return true;}e.click();e.focus?.();if(document.activeElement&&document.activeElement!==document.body)return true;}return false;});
      if(hit)return true;
    }catch{}
  }
  return false;
...
async function commitFocused(page,commit){if(commit==='comma')await page.keyboard.type(',').catch(()=>{});else if(commit==='blur'){for(const f of page.frames()){try{const blurred=await f.evaluate(()=>{const e=document.activeElement;if(!e||e===document.body)return false;e.blur?.();return true;});if(blurred)break;}catch{}}}else await page.keyboard.press(commit).catch(()=>{});}
...
async function officialCathay(mawb,referenceYear=''){
  ...
  try{
    ...
    const codeValue=controls.code?String(controls.code.value||'').replace(/\D/g,''):'';debug.airlineCode=codeValue;debug.airlineCodeControlFound=Boolean(controls.code);if(codeValue&&codeValue!=='160')return{ok:false,reason:'CATHAY AIRLINE CODE CONTROL HAS UNEXPECTED VALUE',debug};
    const attempt=async(mode,value,commit,focusMode='direct')=>{
      controls=await findControls(page);let focused=false;
      if(focusMode==='wrapper')focused=await clickAwbWrapper(page);else if(focusMode==='tab'&&controls.code){focused=await focusControl(page,controls.code);if(focused){await page.keyboard.press('Tab').catch(()=>{});await sleep(120);}}else focused=Boolean(controls.awb)&&await focusControl(page,controls.awb);
      if(!focused){debug.attempts.push({mode,value,commit,focusMode,focused:false,ok:false});return{ok:false};}
      await clearFocused(page);await page.keyboard.type(value,{delay:45}).catch(()=>{});await commitFocused(page,commit);await sleep(650);const snap=await registrationSnapshot(page,serial,digits);debug.attempts.push({mode,value,commit,focusMode,focused:true,ok:snap.ok,chip:snap.chip,frameIndex:snap.frameIndex,frameUrl:snap.frameUrl});return snap;
    };
    const permutations=[
      ['tab-from-160-suffix-enter',serial,'Enter','tab'],['tab-from-160-suffix-tab',serial,'Tab','tab'],['tab-from-160-suffix-comma',serial,'comma','tab'],['tab-from-160-suffix-blur',serial,'blur','tab'],
      ...
      ['full-digits-enter',digits,'Enter','direct'],['full-hyphen-enter',hyphen,'Enter','direct'],['tab-from-160-full-enter',digits,'Enter','tab']
    ];
    ...
    debug.registered=true;debug.registration={chip:reg.chip,values:reg.values,body:reg.body,frameIndex:reg.frameIndex,frameUrl:reg.frameUrl};
    ...
    for(const f of page.frames()){try{const did=await f.evaluate(()=>{const els=[...document.querySelectorAll('*')],e=els.find(x=>/Booking Status/i.test(String(x.textContent||''))&&x.children.length<8);if(e){e.scrollIntoView({block:'center'});return true;}return false;});if(did)break;}catch{}}await sleep(250);
