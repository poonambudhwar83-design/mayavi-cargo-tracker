...
import { needsCathayBrowserFallback, mergeCathayBrowserFallback, trackCathayBrowserFallback } from './cathay-browser-fallback.js';
...
const RETAIN_KEYS=['origin','destination','flightNo','bags','pieces','weight','bookingDate','bookingTime','departureDate','departureTime','arrivalDate','arrivalTime','scheduledArrivalDate','scheduledArrivalTime','arrivalTimeSource','provider','source'];
...
function statusRank(value=''){
  ...
  if(s.includes('ARRIVED')||s.includes('RCF')||s.includes('LANDED'))return 5;
  if(s.includes('IN TRANSIT')||s.includes('DEPART')||s.includes('AIRBORNE')||s.includes('IN FLIGHT'))return 4;
  if(s.includes('DELAY'))return 3;
  if(s.includes('BOOK'))return 2;
  if(s.includes('TRACK'))return 1;
  return 0;
}
async function retainSavedCathayData(mawb,incoming={}){
  const url=trackingDbUrl();
  if(!url)return incoming;
  try{
    const awb=String(mawb||'').replace(/\D/g,'');
    const sql=neon(url);
    const rows=await sql`SELECT data FROM mayavi_shipments WHERE awb=${awb} LIMIT 1`;
    ...
    for(const key of RETAIN_KEYS){
      ...
      const old=saved?.[key];
      if((current===''||current===null||current===undefined)&&(old!==''&&old!==null&&old!==undefined))out[key]=old;
    }
    if(incoming?.arrivalIsActual!==true&&saved?.arrivalIsActual===true&&out.arrivalDate&&out.arrivalTime)out.arrivalIsActual=true;
    if(statusRank(saved?.status)>statusRank(incoming?.status))out.status=saved.status;
    return out;
  }catch{
    return incoming;
  }
...
async function enrichWithOfficialFlight(base,flights=[]){
  ...
  if(base.status!=='DELIVERED'){
    ...
    else if(depActual.date||/DEPART|AIRBORNE|IN.?FLIGHT/.test(flightStatus))status='DEPARTED';
...
export async function trackCathay(input){
  ...
  if(!terminal){
    const browser=await trackCathayBrowserFallback(mawb,{});
    if(browser.ok){
      const shipment=await retainSavedCathayData(mawb,browser.shipment);
      return{ok:true,airline:AIRLINE,shipment,debug:{source:'cathay-browser-only-fallback',browser:browser.debug}};
    }
  ...
  if(needsCathayBrowserFallback(shipment)){
    browser=await trackCathayBrowserFallback(mawb,shipment);
    if(browser.ok)shipment=mergeCathayBrowserFallback(shipment,browser.shipment);
  }
  // If Cathay's cargo/flight APIs expose the active flight but omit ETA/ATA,
  // use a dated flight-status fallback for that same CX flight. This is only
  // allowed to fill arrival fields; it must not invent route or overwrite an actual arrival.
  let flightFallback=null;
  if(shipment.flightNo&&shipment.origin&&shipment.destination&&(!shipment.arrivalDate||!shipment.arrivalTime)){
    const probeDates=[];
    for(const raw of [shipment.departureDate,terminal.flights?.at(-1)?.date,shipment.bookingDate]){
      if(/^20\d{2}-\d{2}-\d{2}$/.test(String(raw||''))&&!probeDates.includes(raw))probeDates.push(raw);
    }
    for(const date of probeDates.slice(0,3)){
      ...
      if(probe?.ok&&(probe.arrivalDate||probe.arrivalTime)){
        flightFallback=probe;
        if(!shipment.arrivalDate&&probe.arrivalDate)shipment.arrivalDate=probe.arrivalDate;
        if(!shipment.arrivalTime&&probe.arrivalTime)shipment.arrivalTime=probe.arrivalTime;
        if(probe.arrivalIsActual===true)shipment.arrivalIsActual=true;
        if(probe.status&&shipment.status!=='DELIVERED')shipment.status=probe.status;
        shipment.arrivalTimeSource=probe.source||'CX dated flight-status fallback';
        shipment.source=`${shipment.source||'Cathay Cargo'} + ${probe.source||'CX dated flight-status fallback'}`;
        break;
      }
    }
  }
  shipment=await retainSavedCathayData(mawb,shipment);
  return{ok:true,airline:AIRLINE,shipment,debug:{source:browser?.ok?'cathay-existing-plus-browser-fallback':'cathay-terminal-plus-official-flight-status',terminalUrl:terminal.url,terminalFlights:terminal.flights,flightStatus:enriched.flightStatus,flightFallback,browser:browser?.debug||browser?.reason||null}};
}