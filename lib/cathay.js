...
async function cathayCargoPage(mawb,referenceYear=''){
  let browser;const serial=mawb.replace(/\D/g,'').slice(3),full=`160-${serial}`,network=[];
  try{
    ...
    page.on('response',async r=>{try{const u=r.url(),ct=r.headers()['content-type']||'';if(!/json|text/i.test(ct)||!/track|shipment|awb|cargo|waybill|trace|api/i.test(u))return;const t=await r.text();if(t&&(t.includes(serial)||/CX\d{2,4}|RCF|arriv/i.test(t)))network.push({url:u,text:t.slice(0,50000)});}catch{}});
    ...
    const focus=await page.evaluate(()=>{
      ...
      const awb=inputs.find(e=>e!==prefix&&!['submit','button','checkbox','radio','hidden'].includes((e.type||'text').toLowerCase())&&/awb|air.?waybill|waybill|shipment|number|12345675/i.test(`${e.id||''} ${e.name||''} ${e.placeholder||''} ${e.getAttribute('aria-label')||''}`));
      ...
      if(label){let p=label;for(let i=0;i<5&&p;i++,p=p.parentElement){const r=p.getBoundingClientRect();if(r.width>180&&r.height>30&&r.height<180){const x=Math.min(r.right-30,Math.max(r.left+120,r.left+r.width*.62)),y=r.top+r.height/2,hit=document.elementFromPoint(x,y);if(hit){hit.click();if(hit.focus)hit.focus();return{mode:'label-box',x,y,tag:hit.tagName,prefixValue:prefix?.value||''};}}}label.click();return{mode:'label',prefixValue:prefix?.value||''};}
      return{mode:'none',prefixValue:prefix?.value||''};
    });
    await new Promise(r=>setTimeout(r,250));await page.keyboard.type(serial,{delay:45}).catch(()=>{});await page.keyboard.press('Tab').catch(()=>{});await new Promise(r=>setTimeout(r,400));
    const entered=await page.evaluate((serial,full)=>{const norm=s=>String(s||'').replace(/\D/g,''),values=[...document.querySelectorAll('input,textarea')].map(e=>e.value||'').filter(Boolean),body=document.body?.innerText||'',wanted=norm(serial);return{registered:body.includes(full)||values.some(v=>norm(v)===wanted),values:values.slice(0,12),bodyHasFull:body.includes(full)};},serial,full);
    if(!entered.registered)return{ok:false,reason:'CATHAY FORM DID NOT ACCEPT AWB',debug:{cookieClicked,focus,entered}};
    ...
    await page.waitForFunction((serial)=>{const t=document.body?.innerText||'';return t.includes(`160-${serial}`)||/Current status:|Shipment In Progress|Accepted\s+\d+\/\d+|Departed\s+\d+\/\d+|Arrived\s+\d+\/\d+/i.test(t);},{timeout:9000},serial).catch(()=>{});
