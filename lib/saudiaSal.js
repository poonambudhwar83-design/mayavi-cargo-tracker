...
export async function trackSaudiaViaSal(input){
  ...
  // Safe mode: SAL browser automation is disabled by default. Automated access
  // can be rejected by SAL security/WAF even when using the public tracking page.
  // Re-enable only if SAL explicitly authorizes automated access for this use.
  if(process.env.SAL_AUTOMATION_ENABLED!=='true'){
    return{
      ok:false,
      reason:'SAL AUTOMATION DISABLED - USE OFFICIAL TRACKER / AUTHORIZED API',
      officialTracker:URL,
      safeMode:true
    };
  }
