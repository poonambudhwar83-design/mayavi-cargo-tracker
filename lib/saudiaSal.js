...
function bookingDateFromSal(text=''){
  ...
  let m=s.match(new RegExp(code+'[\\s\\S]{0,160}?'+date,'i'));
  if(!m)m=s.match(new RegExp(date+'[\\s\\S]{0,100}?'+code,'i'));
...
function parseSal(text='',mawb=''){
  ...
  const flightNo=(
    first(fowBlock,/\b(?:Flight\s*(?:Number|No\.?)\s*[:\-]?\s*)?(SV\s*\d{1,4})\b/i)||
    first(t,/\bFlight\s+Number\b\s*[:\-]?\s*([A-Z]{2}\s*\d{1,4})\b/i)
  ...
  const arrivalDate=first(fowBlock,/\b(?:Expected|Estimated|Scheduled)?\s*Arrival\s+Date\b\s*[:\-]?\s*([^]*?)(?=\bArrival\s+Time\b|\bDestination\b|\bNo\.\s*of\s*Pieces\b|\bWeight\b|\bPrint\b)/i)||
  ...
  const bookingDate=bookingDateFromSal(t);
  ...
  return {
    ...
    arrivalDate,
    arrivalTime,
    arrivalIsActual:Boolean(arrivalDate&&arrivalTime&&/ARR|DLV|ARRIVED|DELIVERED|RECEIVED FROM FLIGHT/i.test(latestEventCode||statusRaw)),
    segmentNo,
    airport,
    sourceStatus:latestEventCode||statusRaw,
    latestEventCode:latestEventCode||'',
    status:mapStatus(latestEventCode||statusRaw),
...
async function expandAndScrollSal(page){
  ...
  for(let step=0;step<12;step++){
    await page.evaluate(()=>{
      ...
      const candidates=[...document.querySelectorAll('button,a,[role="button"],[aria-expanded="false"],summary')].filter(visible);
      for(const el of candidates){
        const t=txt(el);
        if(/more\s+(shipment\s+)?details|more\s+information|show\s+more|view\s+details|expand/i.test(t)){
          try{el.click()}catch{}
        }
      }
...
export async function trackSaudiaViaSal(input){
  ...
  try{
    ...
    if(shipment){
      const scrolled=await expandAndScrollSal(page);
      combined=`${combined}\n${scrolled}\n${network.join('\n')}`;
      shipment=parseSal(combined,mawb)||shipment;
      return{ok:true,shipment,officialTracker:URL,debug:{...debug,stage:'SAL_RESULT_ALREADY_PRESENT_SCROLLED',scrolled:true,sample:clean(combined).slice(0,5000)}};
    }
    ...
    while(Date.now()<end){
      ...
      if(shipment){
        ...
        combined=`${combined}\n${scrolled}\n${network.join('\n')}`;
        shipment=parseSal(combined,mawb)||shipment;
        return{ok:true,shipment,officialTracker:URL,debug:{...debug,stage:'SAL_RESULT_SUCCESS_SCROLLED',scrolled:true,sample:clean(combined).slice(0,5000)}};
      }
