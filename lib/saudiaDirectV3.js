...
const departed=t=>/\bDEPARTED\b|\bDEP\b|IN\s+TRANSIT|AIRBORNE/i.test(english(t));
const delayed=t=>/DISCREPANCY|DELAY|LATE|OFFLOAD|EXCEPTION/i.test(english(t));
const booked=t=>/\bBOOKED\b|\bRCS\b|RECEIVED\s+FROM\s+SHIPPER|MANIFESTED\s+ON\s+FLIGHT|PLANNED\s+FOR\s+FLIGHT/i.test(english(t));
...
function parse(raw,mawb){
  ...
  let status=(text.match(/\bState\s*[:\-]?\s*(Booked|Delivered|Arrived|Delayed|Departed|In Transit)\b/i)||[])[1]?.toUpperCase()||'TRACKING';
  if(lastEvent){if(delivered(lastEvent.text))status='DELIVERED';else if(arrived(lastEvent.text))status='ARRIVED';else if(departed(lastEvent.text))status='IN TRANSIT';else if(delayed(lastEvent.text))status='DELAYED';else if(booked(lastEvent.text))status='BOOKED'}
  ...
  return (segs.length||pieces||shipment.destination||shipment.bookingDate||shipment.arrivalDate)?{shipment,segs,firstEvent,lastEvent}:null;
}
...
export async function trackSaudiaDirect(input){
  ...
  try{
    ...
    return{ok:true,shipment:parsed.shipment,debug:{stage:'SUCCESS',moreClicked,segments:parsed.segs.length,segment1:parsed.segs[0]?.text?.slice(0,500)||'',lastSegment:parsed.segs.at(-1)?.text?.slice(0,650)||''}};
