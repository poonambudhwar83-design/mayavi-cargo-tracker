...
function parseMessage(outer,mawb){
  ...
  if(latestFinalIsActive){
    ...
    const historicalSameFlight=[...finalLegEvents].reverse().find(e=>/^(ARR|DLV)$/i.test(clean(e?.Status))&&normalizeSvFlight(e?.Flight_Number)===flightNo&&eventEpoch(e?.DateTime_LT)<eventEpoch(selectedFlight?.DateTime_LT));
