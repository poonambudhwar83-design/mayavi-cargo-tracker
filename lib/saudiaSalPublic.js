...
export async function trackSaudiaViaSal(input){
  ...
  try{
    ...
    if(shipment.flightNo&&shipment.flightDate&&!shipment.arrivalIsActual&&(/^(RCS|DEP|FOW|MAN)$/i.test(clean(shipment.sourceStatus))||shipment.status==='IN TRANSIT')){
      const live=await lookupFlightStatsArrival(shipment.flightNo,shipment.flightDate,shipment.destination);
      if(live?.arrivalTime){shipment.arrivalTime=live.arrivalTime;shipment.arrivalTimeSource=live.source;shipment.flightStatusUrl=live.url;}
    }
