...
function parseMessage(outer,mawb){
  ...
  const pieces=num(data.TotalPieces??pick(flat,['TotalPieces','Pieces','NoOfPieces','PieceCount']));
  const weight=num(data.TotalWeight??pick(flat,['TotalWeight','Weight','GrossWeight']));
  ...
  const finalArrival=[...finalLegEvents].reverse().find(e=>/^(ARR|DLV)$/i.test(clean(e?.Status))&&clean(e?.Airport).toUpperCase()===destination)||null;
  ...
  const flightNo=clean(selectedFlight?.Flight_Number||pick(flat,['FlightNumber','FlightNo','Flight'])).replace(/\s+/g,'').toUpperCase();
  ...
  const status=finalArrival?'ARRIVED':mapStatus(sourceStatus,airport,destination);

  const meaningful=Boolean(origin||destination||pieces||weight||volume||events.length||flightNo||flightDate||arrivalDate||arrivalTime||sourceStatus||bookingDate);
  if(!meaningful)return null;

  return {
    mawb,carrierCode:'SV',airlineName:'Saudia Cargo',officialTracker:URL,
    origin,destination,pieces:pieces||'',bags:pieces||'',weight:weight||'',volume:volume||'',
