...
function parseMessage(outer,mawb){
  ...
  const finalFlightEvent=[...finalFlightEvents].reverse().find(e=>/^(ARR|DLV|DEP|RCF|FIW|FOW|MAN)$/i.test(clean(e?.Status)))||finalFlightEvents.at(-1)||null;
  const selectedFlight=(finalArrival&&normalizeSvFlight(finalArrival?.Flight_Number)?finalArrival:null)||finalFlightEvent||(!destination?([...ordered].reverse().find(e=>normalizeSvFlight(e?.Flight_Number))||null):null);
