...
function parseMessage(outer,mawb){
  ...
  const scheduledArrivalRaw=clean(
    selectedFlight?.Scheduled_Arrival||selectedFlight?.ScheduledArrival||selectedFlight?.Estimated_Arrival||selectedFlight?.EstimatedArrival||selectedFlight?.ETA||
    pick(flat,['ScheduledArrivalDateTime','ScheduledArrival','EstimatedArrivalDateTime','EstimatedArrival','ETA'])
  );
  const scheduledArrivalDateRaw=clean(
    selectedFlight?.Arrival_Date||selectedFlight?.ArrivalDate||pick(flat,['ScheduledArrivalDate','EstimatedArrivalDate','ArrivalDate'])
  );
  const scheduledArrivalTimeRaw=clean(
    selectedFlight?.Arrival_Time||selectedFlight?.ArrivalTime||pick(flat,['ScheduledArrivalTime','EstimatedArrivalTime','ArrivalTime'])
  );
  ...
  if(latestFinalIsActive){
    ...
    arrivalDate=normalizeDateOnly(scheduledArrivalRaw)||normalizeDateOnly(scheduledArrivalDateRaw);
    arrivalTime=normalizeTimeOnly(scheduledArrivalRaw)||normalizeTimeOnly(scheduledArrivalTimeRaw);
    if(arrivalDate||arrivalTime)arrivalTimeSource='SAL scheduled/estimated arrival';
  }
