...
function mapStatus(raw='',airport='',destination=''){
  ...
  const atDest=clean(airport).toUpperCase()===clean(destination).toUpperCase()&&Boolean(destination);
  if(/\bDLV\b|DELIVER|\bARR\b|ARRIVED|RECEIVED FROM FLIGHT/.test(s))return'ARRIVED';
...
function parseMessage(outer,mawb){
  ...
  const destinationArrivals=finalLegEvents.filter(e=>/^(ARR|DLV)$/i.test(clean(e?.Status))&&clean(e?.Airport).toUpperCase()===destination);
  ...
  const finalFlightEvent=[...finalLegEvents].reverse().find(e=>clean(e?.Flight_Number))||null;
  const selectedFlight=finalArrival||finalFlightEvent||latestEvent;
  ...
  const rawFlightNo=clean(selectedFlight?.Flight_Number||pick(flat,['FlightNumber','FlightNo','Flight'])).replace(/\s+/g,'').toUpperCase();
  const flightNo=rawFlightNo?(rawFlightNo.startsWith('SV')?rawFlightNo:`SV${rawFlightNo}`):'';
  const flightDate=clean(selectedFlight?.Flight_Date||pick(flat,['FlightDate','DepartureDate']));
  const arrivalDate=finalArrival?normalizeDateOnly(finalArrival.DateTime_LT):'';
  const arrivalTime=finalArrival?normalizeTimeOnly(finalArrival.DateTime_LT):'';
  const sourceStatus=clean(latestEvent?.Status||pick(flat,['Status','StatusDescription','Event','EventDescription','MovementStatus']));
  const airport=clean(latestEvent?.Airport||pick(flat,['Airport','Station'])).toUpperCase();
  ...
  const pieces=isPartArrived?`${arrivedPieces}/${totalPieces}`:(totalPieces||'');
  const weight=isPartArrived&&totalWeight?`${arrivedWeight}/${totalWeight}`:(totalWeight||'');
  const status=isPartArrived?'PART ARRIVED':isFullyArrived?'ARRIVED':mapStatus(sourceStatus,airport,destination);
  ...
  return {
    ...
    totalPieces:totalPieces||'',arrivedPieces:arrivedPieces||'',pendingPieces:totalPieces?Math.max(totalPieces-arrivedPieces,0):'',
    totalWeight:totalWeight||'',arrivedWeight:arrivedWeight||'',pendingWeight:totalWeight?Math.max(totalWeight-arrivedWeight,0):'',
    ...
    arrivalDate,arrivalTime,arrivalIsActual:Boolean(finalArrival),
