...
function secret(){
  const explicit=process.env.MAYAVI_SESSION_SECRET||process.env.MAYAVI_ADMIN_KEY||'';
  if(explicit)return explicit;
  const databaseSecret=process.env.DATABASE_URL||process.env.POSTGRES_URL||process.env.NEON_DATABASE_URL||process.env.DATABASE_URL_UNPOOLED||'';
  if(!databaseSecret)return'';
  return crypto.createHash('sha256').update(`mayavi-session:${databaseSecret}`).digest('hex');
}
...
function sign(body){
  const key=secret();
  if(!key)throw new Error('Mayavi secure session is not configured.');
