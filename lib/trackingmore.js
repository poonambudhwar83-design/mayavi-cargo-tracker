import { trackEtihad } from './etihad.js';
import { trackVietnamFreight } from './vietnamFreight.js';
...
function chooseBookingDate(returnData={}){
  const events=Array.isArray(returnData.track_info)?returnData.track_info:[];
  if(!events.length)return'';
  const ranked=[];
  for(const event of events){
    const label=clean(event?.status||event?.event||event?.description||event?.event_name||'').toUpperCase();
    let rank=0;
    if(/\bBOOKED\b|\bBOOKING\b|BOOKING CONFIRMED/.test(label))rank=3;
    else if(/\bRCS\b|RECEIVED FROM SHIPPER|\bACCEPTED\b|\bACCEPT\b/.test(label))rank=2;
    if(!rank)continue;
    const raw=event.actual_date||event.actualDate||event.plan_date||event.planDate||'';
    const dt=dateTime(raw);
    if(!dt.date)continue;
    const ts=Date.parse(`${dt.date}T${dt.time||'00:00'}:00`)||0;
    ranked.push({rank,ts,date:dt.date});
  }
  if(!ranked.length)return'';
  ranked.sort((a,b)=>b.rank-a.rank||a.ts-b.ts);
...
function parsePayload(json,mawb,airline){
  ...
  const bookingDate=chooseBookingDate(rd);
  return{
    mawb,
...
export async function trackWithTrackingMore(mawb,airline){
  if(String(mawb).startsWith('607-'))return trackEtihad(mawb);
  if(String(mawb).startsWith('738-'))return trackVietnamFreight(mawb);
  ...
  try{
    ...
    return{ok:true,shipment,debug:{provider:'trackingmore-aircargo',bookingDate:shipment.bookingDate||''}};
