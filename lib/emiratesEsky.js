...
export async function trackEmirates(mawb){
  ...
  try{
    ...
    if(useful(domShipment)){
      const shipment=merge({mawb,carrierCode:'EK',airlineName:'Emirates SkyCargo',officialTracker:page.url(),source:'Emirates eSkyCargo Tracking Details screenshot + rendered text'},domShipment);
      shipment.officialTracker=page.url();shipment.source='Emirates eSkyCargo Tracking Details screenshot + rendered text';
      return{ok:true,shipment,screenshotCaptured:true,screenshotVerified:true,screenshotOcrUsed:false,debug:{stage:'done-rendered-text',shipmentId:discovered.id,shipmentIdSource:discovered.source,url:page.url(),opened,panelSample:panelText.slice(0,3000)}};
    }

    const ocrResult=await readTrackingScreenshot({mawb,screenshotBase64,timeoutMs:22000});
    if(!ocrResult?.ok)return{ok:false,reason:ocrResult?.reason||'EMIRATES TRACKING DETAILS FOUND NO VERIFIED FIELDS',officialTracker:page.url(),screenshotCaptured:true,screenshotVerified:true,screenshotOcrUsed:false,debug:{stage:'ocr-fallback',shipmentId:discovered.id,shipmentIdSource:discovered.source,opened,url:page.url(),panelSample:panelText.slice(0,3000),ocr:ocrResult?.debug||null}};

    let shipment={mawb,carrierCode:'EK',airlineName:'Emirates SkyCargo',officialTracker:page.url(),source:'Emirates eSkyCargo Tracking Details screenshot OCR'};shipment=merge(shipment,ocrResult.shipment);shipment.officialTracker=page.url();shipment.source='Emirates eSkyCargo Tracking Details screenshot OCR';
    if(!useful(shipment))return{ok:false,reason:'EMIRATES OCR RETURNED NO TRACKER FIELDS',officialTracker:page.url(),screenshotCaptured:true,screenshotVerified:true,screenshotOcrUsed:true,debug:{stage:'ocr-fields',shipmentId:discovered.id,shipmentIdSource:discovered.source,opened,ocr:ocrResult?.debug||null}};
    return{ok:true,shipment,screenshotCaptured:true,screenshotVerified:true,screenshotOcrUsed:true,debug:{stage:'done-ocr',shipmentId:discovered.id,shipmentIdSource:discovered.source,url:page.url(),opened,ocrSample:ocrResult?.debug?.textSample||''}};
