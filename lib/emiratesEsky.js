...
async function openShipmentResult(page,digits,network){
  ...
  const result=await page.evaluate(({digits,suffix})=>{
    ...
    const anchors=[...document.querySelectorAll('a[href*="shipments/list"],a[href*="/shipments/"]')].filter(visible);
    let a=anchors.find(x=>text(x).replace(/\D/g,'').includes(digits)||text(x).replace(/\D/g,'').includes(suffix))||anchors[0];
    if(a){const href=a.href||a.getAttribute('href')||'';a.click();return{clicked:'shipment-link',href,text:text(a).slice(0,300)};}
    const candidates=[...document.querySelectorAll('tr,a,button,[role="button"],[tabindex],td,div')].filter(visible);
    const e=candidates.find(x=>{const d=text(x).replace(/\D/g,'');return d.includes(digits)||d.includes(suffix);});
    if(e){const target=e.closest('a,button,[role="button"],[tabindex],tr')||e;target.click();return{clicked:'mawb-result',href:target.href||target.getAttribute?.('href')||'',text:text(target).slice(0,500)};}
    return{clicked:'',href:'',text:''};
  },{digits,suffix}).catch(()=>({clicked:'',href:'',text:''}));
  if(result.clicked)await sleep(4000);
  ...
  const onShipment=/shipments\/list\//i.test(current);
  if(shipmentId&&!onShipment){
    ...
    return{...result,shipmentId,deepLink:true,url:page.url()};
  }
...
export async function trackEmirates(mawb){
  ...
  try{
    ...
    let trackingDetails='';
    if(!/[?&]openedTab=tracking-details/i.test(page.url())){
      trackingDetails=await clickByText(page,/^Tracking Details$/i);
      if(trackingDetails)await sleep(3500);
    }else trackingDetails='deep-link';

    if(!trackingDetails){
      const id=opened.shipmentId;
      if(id){
        const deep=`${APP}#/shipments/list/${id}?openedTab=tracking-details`;
        await page.goto(deep,{waitUntil:'domcontentloaded',timeout:20000});await sleep(4500);trackingDetails='deep-link-fallback';
      }
    }
