...
const OFFICIAL='https://www.turkishcargo.com/en/cargo-tracking';
...
function chooseFlight(events){
  const candidates=events.filter(e=>/^TK\d{2,4}$/i.test(clean(e?.flightNumber)));
...
function parseShipment(payload,mawb){
  ...
  return{
    ...
    origin:airportCode(item.origin),
    destination:airportCode(item.destination),
    bags:pieces,
    pieces,
    ...
    source:'CHAMP Freight.aero Turkish Cargo tracking',
    provider:'CHAMP Freight.aero'
  };
}

export async function trackTurkishFreight(input){
  const mawb=normalize(input);
  if(!mawb||!mawb.startsWith('235-'))return{ok:false,reason:'INVALID TURKISH CARGO MAWB',officialTracker:OFFICIAL};
  const serial=mawb.slice(4),full=digits(mawb);
  const pageUrl=`${FREIGHT_PAGE}?Carrier=TK&Pfx=235&Portlet=yes&Shipment=${serial}&Site=CargoWeb`;
  try{
    ...
    const body={portalId,awbInfo:[{awbNumber:full,carrierId:'TK',trackingAirline:''}],captchaResponse:'FREIGHT_LOGGED_CAPTCHA'};
    ...
    if(!shipment)return{ok:false,reason:'FREIGHT.AERO RETURNED NO TURKISH SHIPMENT DATA',officialTracker:OFFICIAL,debug:{preview:text.slice(0,1000)}};
  ...
  }catch(error){return{ok:false,reason:error?.message||'FREIGHT.AERO TURKISH TRACKING FAILED',officialTracker:OFFICIAL};}
}
