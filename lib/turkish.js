...
async function waitForCargoTrackingInformation(page, timeout=20000) {
  ...
  while (Date.now() < end) {
    ...
    if (isHumanVerificationText(text)) return false;
...
export async function trackTurkish(input) {
  ...
  try {
    ...
    if (isHumanVerificationText(verificationText)) return { ok:false, reason:'TURKISH HUMAN VERIFICATION BLOCKED LIVE READ', airline:AIRLINE, debug:{ ...debug, ...flow, stage:'HUMAN_VERIFICATION', verificationSample:clean(verificationText).slice(0,1200) } };
