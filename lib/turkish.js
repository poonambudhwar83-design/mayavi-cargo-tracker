...
async function visibleButtons(frame){
  const out=[];
  for(const b of await frame.$$('button,input[type="submit"],input[type="button"],[role="button"]')){
    try{
      const m=await b.evaluate(el=>{const r=el.getBoundingClientRect();return{text:String(el.innerText||el.value||el.getAttribute('aria-label')||'').replace(/\s+/g,' ').trim(),disabled:Boolean(el.disabled),visible:r.width>3&&r.height>3}});
      if(m.visible&&!m.disabled)out.push({b,text:m.text});
    }catch{}
  }
  return out;
}
async function clickNamed(frame,rx){
  for(const x of await visibleButtons(frame))if(rx.test(x.text)){await x.b.click({delay:60});return x.text;}
  return'';
}
async function fillAddSearch(page,mawb){
  ...
  for(const frame of page.frames()){
    ...
    const addText=await clickNamed(frame,/^\s*add\s*$/i);
    if(!addText)return{ok:false,stage:'ADD_NOT_FOUND'};
    await sleep(500);
    const searchText=await clickNamed(frame,/^\s*(?:search|track|sorgula)\s*$/i);
    if(!searchText)return{ok:false,stage:'SEARCH_NOT_FOUND',addText};
    return{ok:true,stage:'ADD_THEN_SEARCH',addText,searchText};
  }
...
export async function trackTurkish(input){
  ...
  try{
    ...
    if(!flow.ok)return{ok:false,reason:flow.stage==='ADD_NOT_FOUND'?'TURKISH ADD BUTTON NOT FOUND':flow.stage==='SEARCH_NOT_FOUND'?'TURKISH SEARCH BUTTON NOT FOUND':'TURKISH TRACKING FORM NOT FOUND',airline:AIRLINE,debug:{...debug,...flow}};
    for(let i=0;i<16;i++){
      await sleep(i?700:1000);
      for(let n=network.length-1;n>=0;n--){const p=parseStructured(network[n].body,mawb);if(p?.notFound)return{ok:false,notFound:true,reason:'NO SHIPMENT RECORD',airline:AIRLINE,debug:{...debug,...flow,stage:'NO_RECORD',source:'network'}};if(p?.useful)return{ok:true,airline:AIRLINE,shipment:p.shipment,debug:{...debug,...flow,stage:'SUCCESS',source:'network',networkUrl:network[n].url}};}
      const p=parsePage(await pageText(page),mawb);if(p?.notFound)return{ok:false,notFound:true,reason:'NO SHIPMENT RECORD',airline:AIRLINE,debug:{...debug,...flow,stage:'NO_RECORD',source:'page'}};if(p?.useful)return{ok:true,airline:AIRLINE,shipment:p.shipment,debug:{...debug,...flow,stage:'SUCCESS',source:'page'}};
    }
    return{ok:false,reason:'TURKISH RESULT NOT MACHINE READABLE',airline:AIRLINE,debug:{...debug,...flow,stage:'UNREADABLE',networkUrls:network.slice(-8).map(x=>x.url),preview:clean(await pageText(page)).slice(0,500)}};
