...
function parseDateTime(v=''){
  ...
  m=s.match(/(20\d{2})[-\/]([01]\d)[-\/]([0-3]\d)[T\s](\d{1,2}):(\d{2})/);if(m)return{date:`${m[1]}-${m[2]}-${m[3]}`,time:`${String(m[4]).padStart(2,'0')}:${m[5]}`};
...
function shipmentStatus(text='',destination=''){
  ...
  if(/DELAY|LATE|EXCEPTION/.test(s))return'DELAYED';return'BOOKED';
}
function parseTkSmart(text,mawb){
  ...
  if(status==='ARRIVED'){const block=(s.match(/(?:DELIVERED\s*-?\s*[A-Z]{3}|SHIPMENT DELIVERED|\bDLV\b)[\s\S]{0,420}/i)||[])[0]||s;arrival=parseDateTime(block);if(!arrival.date)arrival=parseDateTime(s);arrivalIsActual=Boolean(arrival.date);}else{const eta=(s.match(/(?:ETA|ESTIMATED ARRIVAL|EXPECTED ARRIVAL|SCHEDULED ARRIVAL)[\s\S]{0,300}/i)||[])[0]||'';arrival=parseDateTime(eta);}
  const bookingBlock=(s.match(/(?:BOOKED|BOOKING|ACCEPTED)[\s\S]{0,260}/i)||[])[0]||'',bookingDate=parseDateTime(bookingBlock).date;
  const useful=Boolean((origin&&destination)||pieces||weight||volume||flightNo||arrival.date||/TK SMART|\bDLV\b|DELIVERED/i.test(s));if(!useful)return null;
...
function flattenResponse(raw=''){
  try{
    const json=JSON.parse(raw),out=[];
    const walk=(v,k='')=>{if(k)out.push(k);if(v===null||v===undefined)return;if(Array.isArray(v))return v.forEach(x=>walk(x));if(typeof v==='object')return Object.entries(v).forEach(([x,y])=>walk(y,x));out.push(String(v));};
    walk(json);return clean(out.join(' '));
  }catch{return clean(raw.replace(/[{}\[\]"',:]+/g,' '));}
}
...
async function submitCargoSearch(frame){
  if(await clickText(frame,'Search'))return'CLICK_SEARCH';
  try{return await frame.evaluate(()=>{
    const norm=s=>String(s||'').replace(/\s+/g,' ').trim().toLowerCase();
    const roots=[...document.querySelectorAll('form,section,article,div')].filter(el=>{const t=norm(el.innerText||el.textContent);return t.includes('cargo tracking information')&&t.includes('awb');}).sort((a,b)=>String(a.innerText||'').length-String(b.innerText||'').length);
    for(const root of roots){const form=root.matches('form')?root:root.querySelector('form');if(form){if(typeof form.requestSubmit==='function')form.requestSubmit();else form.submit();return'FORM_SUBMIT';}}return'';
  })}catch{return''}
}
async function tkSmartText(page,serial){
  for(const frame of page.frames())try{const result=await frame.evaluate((awb)=>{
    ...
    const scan=root=>{for(const el of root.querySelectorAll('section,article,div')){const t=norm(el.innerText||el.textContent);if(t.length>=40&&t.length<=16000&&/TK SMART/i.test(t)&&t.includes(awb)&&(/PIECE\(S\)|PIECES?|\bFROM\b|\bTO\b/i.test(t)))candidates.push({el,text:t,len:t.length});if(el.shadowRoot)scan(el.shadowRoot);}};
...
async function fillAddSearch(page,mawb){
  ...
  for(const frame of page.frames()){
    ...
    await nf.input.click({clickCount:3});await page.keyboard.press('Backspace');await nf.input.type(nf.meta.max===8?serial:(pf?serial:digits),{delay:40});await sleep(1200);
    ...
    await sleep(900);let searchMode=await submitCargoSearch(frame);if(!searchMode){await nf.input.click();await nf.input.press('Enter');await sleep(450);searchMode='ENTER_SEARCH_FALLBACK';}
    return{ok:true,stage:'ADD_THEN_CARGO_SEARCH',addMode,searchMode,serialVisible:await bodyHas(frame,serial)};
  }
...
export async function trackTurkish(input){
  ...
  let browser;const serial=mawb.replace(/\D/g,'').slice(3),debug={prefix:'235',airline:'Turkish Cargo',url:URL,stage:'OPEN'},network=[];
  try{
    const mod=await import('puppeteer-core');const puppeteer=mod.default||mod,launch=await browserConfig();browser=await puppeteer.launch({...launch,headless:true,defaultViewport:{width:1440,height:1000}});const page=await browser.newPage();
    ...
    page.on('response',async res=>{try{const u=res.url(),ct=res.headers()['content-type']||'';if(!/turkishcargo|thy|cargo|track|shipment|awb|api/i.test(u)&&!/json|text/i.test(ct))return;const body=await res.text();if(body&&body.length<2000000){network.push({url:u,status:res.status(),ct,body});if(network.length>80)network.shift();}}catch{}});
    ...
    for(let i=0;i<28;i++){
      await sleep(i?700:1200);
      const card=await tkSmartText(page,serial);if(card){const parsed=parseTkSmart(card,mawb);if(parsed?.notFound)return{ok:false,notFound:true,reason:'NO SHIPMENT RECORD',airline:AIRLINE,debug:{...debug,...flow,stage:'NO_RECORD'}};if(parsed?.useful)return{ok:true,airline:AIRLINE,shipment:parsed.shipment,debug:{...debug,...flow,stage:'TK_SMART_SUCCESS',tkSmartSample:clean(card).slice(0,8000)}};}
      for(let n=network.length-1;n>=0;n--){const flat=flattenResponse(network[n].body);if(!flat.includes(serial))continue;const parsed=parseTkSmart(flat,mawb);if(parsed?.useful)return{ok:true,airline:AIRLINE,shipment:{...parsed.shipment,source:'Turkish Cargo official tracking response'},debug:{...debug,...flow,stage:'OFFICIAL_RESPONSE_SUCCESS',networkUrl:network[n].url,networkSample:flat.slice(0,8000)}};}
      if(i===6||i===15)for(const frame of page.frames())try{await frame.evaluate(()=>window.scrollBy({top:520,left:0,behavior:'auto'}))}catch{}
    }
    const finalText=await pageText(page),parsed=parseTkSmart(finalText,mawb);if(parsed?.useful)return{ok:true,airline:AIRLINE,shipment:parsed.shipment,debug:{...debug,...flow,stage:'FULL_PAGE_FALLBACK_SUCCESS'}};
    const matches=network.filter(x=>x.body.includes(serial)).slice(-8).map(x=>({url:x.url,status:x.status,ct:x.ct,sample:flattenResponse(x.body).slice(0,1600)}));
    return{ok:false,reason:'TURKISH TK SMART RESULT NOT MACHINE READABLE',airline:AIRLINE,debug:{...debug,...flow,stage:'TK_SMART_UNREADABLE',preview:clean(finalText).slice(0,5000),networkMatches:matches,networkUrls:network.slice(-20).map(x=>x.url)}};
