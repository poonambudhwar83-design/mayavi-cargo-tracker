...
export async function trackTurkish(input) {
  ...
  // Prefer Turkish Cargo's own shipmentTracking endpoint when it is openly
  // available. If the carrier blocks the server request, respect that response
  // and fall back to the normal official page flow below.
  ...
  const debug = {
    ...
    stage:'OPEN',
    tkSmartClicked:false,
    restoredWorkingFlow:true,
    directApiReason:directApi?.reason || '',
    directApiHttpStatus:directApi?.httpStatus || 0
  };
  try {
    ...
    // The same official API can become available inside a normal Turkish Cargo
    // browser session even when a server-side request is rejected. This uses
    // only the page's ordinary session; it does not solve or bypass any human
    ...
    // Existing working page flow: fill AWB -> Add -> Search directly on the official page.
    ...
    if (debug.humanVerificationPresent) {
      return {
        ok:false,
        reason:'TURKISH HUMAN VERIFICATION REQUIRED',
        manualHint:'Open OFFICIAL TRACK, complete the Turkish Cargo Press & Hold verification, show the Cargo Tracking Information result, then take a screenshot and tap TK SCREENSHOT. Mayavi will read and save the fields automatically.',
        airline:AIRLINE,
        officialTracker:URL,
        screenshotCaptured:Boolean(shot?.screenshotCaptured),
        screenshotOcrUsed:Boolean(shot?.ocrUsed),
        debug:{ ...debug, ...flow, stage:'HUMAN_VERIFICATION_REQUIRED', ocrSample:shot?.sample || '', ocrError:shot?.error || '' }
      };
    }
    return { ok:false, reason:'TURKISH TK SMART RESULT NOT READABLE', airline:AIRLINE, officialTracker:URL, screenshotCaptured:Boolean(shot?.screenshotCaptured), screenshotOcrUsed:Boolean(shot?.ocrUsed), debug:{ ...debug, ...flow, stage:'TK_SMART_UNREADABLE', ocrSample:shot?.sample || '', ocrError:shot?.error || '' } };
  } catch (error) {
