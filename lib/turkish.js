...
function parseTkSmart(text,mawb){
  ...
  if(status==='ARRIVED'){
    const block=(s.match(/(?:DELIVERED\s*-?\s*[A-Z]{3}|SHIPMENT DELIVERED|\bDLV\b)[\s\S]{0,420}/i)||[])[0]||s;
    arrival=parseDateTime(block);if(!arrival.date)arrival=parseDateTime(s);arrivalIsActual=Boolean(arrival.date);
  }else{
...
async function pageText(page){
  const out=[];for(const f of page.frames())try{const t=await f.evaluate(()=>document.body?.innerText||'');if(t)out.push(t)}catch{}
  return out.join('\n');
}
  ...
  try{return await frame.evaluate((label)=>{
    ...
    const scan=root=>{
      for(const el of root.querySelectorAll('button,input[type="submit"],input[type="button"],[role="button"],a,[onclick],li,[role="option"],[role="menuitem"],div,span')){
        const r=el.getBoundingClientRect(),text=norm(el.innerText||el.value||el.textContent||'');
        if(r.width>3&&r.height>3&&text===target)found.push({el,area:r.width*r.height});
        if(el.shadowRoot)scan(el.shadowRoot);
      }
    };
    scan(document);found.sort((a,b)=>a.area-b.area);
    if(!found.length)return false;found[0].el.click();return true;
  },wanted)}catch{return false}
}

async function bodyHas(frame,needle){
  try{return await frame.evaluate(n=>String(document.body?.innerText||'').includes(n),needle)}catch{return false}
}
  ...
  try{
    const mode=await frame.evaluate(()=>{
      ...
      for(const root of roots){
        ...
        if(form){if(typeof form.requestSubmit==='function')form.requestSubmit();else form.submit();return'FORM_SUBMIT';}
      }
      return'';
    });
    return mode;
  ...
  for(const frame of page.frames()){
    try{
      const result=await frame.evaluate((awb)=>{
        const norm=s=>String(s||'').replace(/\s+/g,' ').trim();const candidates=[];
        const scan=root=>{
          for(const el of root.querySelectorAll('section,article,div')){
            const t=norm(el.innerText||el.textContent);
            if(t.length>=40&&t.length<=16000&&/TK SMART/i.test(t)&&t.includes(awb)&&(/PIECE\(S\)|PIECES?|\bFROM\b|\bTO\b/i.test(t)))candidates.push({el,text:t,len:t.length});
            if(el.shadowRoot)scan(el.shadowRoot);
          }
        };
        scan(document);candidates.sort((a,b)=>a.len-b.len);const hit=candidates[0];
        if(!hit)return'';hit.el.scrollIntoView({block:'center',behavior:'auto'});return hit.text;
      },serial);
...
async function fillAddSearch(page,mawb){
  ...
  for(const frame of page.frames()){
    ...
    await nf.input.click({clickCount:3});await page.keyboard.press('Backspace');await nf.input.type(nf.meta.max===8?serial:(pf?serial:digits),{delay:40});
    await sleep(1200);

    let addMode='CLICK_ADD';
    if(!await clickText(frame,`Add: ${serial}`)){
      await nf.input.press('ArrowDown');await nf.input.press('Enter');addMode='ARROWDOWN_ENTER_ADD';await sleep(500);
      if(!await bodyHas(frame,serial)){await nf.input.click();await nf.input.press('Enter');addMode='ENTER_ADD';}
    }
    await sleep(900);

    let searchMode=await submitCargoSearch(frame);
    if(!searchMode){
      await nf.input.click();await nf.input.press('Enter');await sleep(450);searchMode='ENTER_SEARCH_FALLBACK';
    }
...
export async function trackTurkish(input){
  ...
  let browser;const serial=mawb.replace(/\D/g,'').slice(3),debug={prefix:'235',airline:'Turkish Cargo',url:URL,stage:'OPEN'};
  try{
    const mod=await import('puppeteer-core');const puppeteer=mod.default||mod,launch=await browserConfig();
    ...
    for(let i=0;i<28;i++){
      await sleep(i?700:1200);const card=await tkSmartText(page,serial);
