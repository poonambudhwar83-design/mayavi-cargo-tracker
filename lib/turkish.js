...
function parseReservationRows(text='') {
  ...
  const patterns = [
    /\b(TK\s*0*\d{2,4})\b\s+(\d{1,2}[.\/-]\d{1,2}[.\/-]\d{4})\s+(\d{1,2}[.\/-]\d{1,2}[.\/-]\d{4})\s+(\d{1,2}:\d{2})\s+(\d{1,2}[.\/-]\d{1,2}[.\/-]\d{4})\s+(\d{1,2}:\d{2})\s+([A-Z]{3})\s*-\s*([A-Z]{3})\b/ig,
    /\b(TK\s*0*\d{2,4})\b[^\n]{0,80}?([A-Z]{3})\s*(?:-|TO|>)\s*([A-Z]{3})[^\n]{0,160}?(\d{1,2}[.\/-]\d{1,2}[.\/-]\d{4})\s+(\d{1,2}:\d{2})[^\n]{0,100}?(\d{1,2}[.\/-]\d{1,2}[.\/-]\d{4})\s+(\d{1,2}:\d{2})/ig
  ];
  for (const m of normalized.matchAll(patterns[0])) {
    rows.push({ flightNo:`TK${String(m[1]).replace(/\D/g,'').padStart(4,'0')}`, flightDate:dateOnly(m[2]), etaDate:dateOnly(m[3]), etaTime:m[4], etdDate:dateOnly(m[5]), etdTime:m[6], origin:m[7].toUpperCase(), destination:m[8].toUpperCase() });
  }
  if (!rows.length) for (const m of normalized.matchAll(patterns[1])) {
    rows.push({ flightNo:`TK${String(m[1]).replace(/\D/g,'').padStart(4,'0')}`, flightDate:'', origin:m[2].toUpperCase(), destination:m[3].toUpperCase(), etaDate:dateOnly(m[4]), etaTime:m[5], etdDate:dateOnly(m[6]), etdTime:m[7] });
  }
  if (!rows.length) {
    ...
    if (flight && route && times.length) {
      const eta = times.at(-1);
      const etd = times.length > 1 ? times[0] : null;
      rows.push({ flightNo:`TK${flight[1].padStart(4,'0')}`, flightDate:'', origin:route[1].toUpperCase(), destination:route[2].toUpperCase(), etaDate:dateOnly(eta[1]), etaTime:eta[2], etdDate:etd?dateOnly(etd[1]):'', etdTime:etd?etd[2]:'' });
    }
  }
  return rows;
}
...
export async function trackTurkish(input) {
  ...
  try {
    ...
    // Turkish Cargo is a client-rendered page. On some AWBs the heading appears
    // before the shipment card has finished rendering, so give the result card a
    // short stabilization window instead of reading it immediately.
    debug.scrolled = debug.cargoInfoFound ? await scrollToCargoTrackingInformation(page) : false;
    await sleep(debug.cargoInfoFound ? 2200 : 800);

    let direct = await readTkSmartCard(page, mawb);
    if (!direct && debug.cargoInfoFound) {
      await sleep(1800);
      await scrollToCargoTrackingInformation(page);
      direct = await readTkSmartCard(page, mawb);
    }
    ...
    // One final DOM read after the result has fully settled. This is deliberately
    // before OCR so normal Turkish text remains the primary backend source.
    if (!direct) {
      await sleep(2500);
      await scrollToCargoTrackingInformation(page);
      direct = await readTkSmartCard(page, mawb);
