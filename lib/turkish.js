...
async function fillAddSearch(page,mawb){
  ...
  for(const frame of page.frames()){
    ...
    if(addText)await sleep(500);
    const searchText=await clickNamed(frame,/^\s*(?:search|track|sorgula)\s*$/i);
    if(!searchText){
      const candidates=(await clickableItems(frame)).map(x=>x.text).slice(0,40);
      return{ok:false,stage:addText?'SEARCH_NOT_FOUND':'ADD_OR_SEARCH_NOT_FOUND',addText,candidates};
    }
    return{ok:true,stage:addText?'ADD_THEN_SEARCH':'DIRECT_SEARCH_AFTER_AWB',addText,searchText};
  }
