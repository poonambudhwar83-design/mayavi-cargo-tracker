...
async function clickAddOption(page, serial, timeout=9000) {
  ...
  const source = `(el,text,args) => {
    const digits = String(text || '').replace(/\\D/g,'');
    return /^Add\\s*:/i.test(text) && digits.includes(args.serial) && text.length <= 48;
  }`;
  while (Date.now() < end) {
    for (const frame of page.frames()) {
      const text = await markAndClick(frame, { serial }, source, 'turkish-add');
...
async function fillAddSearch(page, mawb) {
  ...
  for (const frame of page.frames()) {
    ...
    // Turkish Cargo shows a dynamic option such as "Add: 52703873" only after the AWB is entered.
    // Click that Add option itself; do not target a separate plus icon and do not click TK SMART.
    ...
    if (!search.ok) return { ok:false, stage:'SEARCH_BUTTON_NOT_FOUND', addMode:'CLICK_ADD_OPTION', addText:add.text };

    return {
      ok:true,
      addMode:'CLICK_ADD_OPTION',
      addText:add.text,
