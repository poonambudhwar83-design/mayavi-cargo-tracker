...
async function tkSmartText(page,serial){
  for(const frame of page.frames())try{const result=await frame.evaluate((awb)=>{
    ...
    const scan=root=>{for(const el of root.querySelectorAll('section,article,div')){const t=norm(el.innerText||el.textContent);if(t.length>=40&&t.length<=18000&&/TK SMART/i.test(t)&&t.includes(awb)&&(/PIECE\(S\)|PIECES?|\bFROM\b|\bTO\b/i.test(t)))candidates.push({el,text:t,len:t.length});if(el.shadowRoot)scan(el.shadowRoot);}};
...
async function fillAddSearch(page,mawb){
  ...
  for(const frame of page.frames()){
    ...
    let addMode='CLICK_ADD';if(!await clickText(frame,`Add: ${serial}`)){await nf.input.press('ArrowDown');await nf.input.press('Enter');addMode='ARROWDOWN_ENTER_ADD';await sleep(600);if(!await bodyHas(frame,serial)){await nf.input.click();await nf.input.press('Enter');addMode='ENTER_ADD';}}
    await sleep(900);const searchMode=await submitCargoSearch(frame);return{ok:true,stage:'ADD_THEN_CARGO_SEARCH',addMode,searchMode:searchMode||'SEARCH_NOT_CLICKED',serialVisible:await bodyHas(frame,serial)};
  }
...
async function scrapeResult(page,mawb,stage,flow,debug){
  ...
  for(let i=0;i<24;i++){
    ...
    if(i===6||i===14)for(const frame of page.frames())try{await frame.evaluate(()=>window.scrollBy({top:520,left:0,behavior:'auto'}))}catch{}
  }
  return null;
}
...
export async function trackTurkish(input){
  ...
  try{
    ...
    await page.goto(quickUrl,{waitUntil:'domcontentloaded',timeout:30000});await sleep(2500);for(const frame of page.frames())await clickText(frame,'Accept all');
    ...
    return{ok:false,reason:'TURKISH TK SMART RESULT NOT MACHINE READABLE',airline:AIRLINE,debug:{...debug,...flow,stage:'TK_SMART_UNREADABLE',quickUrl,preview:clean(finalText).slice(0,6000)}};
