...
function parseTkSmart(raw, mawb) {
  ...
  const shipmentLike = /TK\s*SMART|CARGO\s+TRACKING\s+INFORMATION|DELIVERED|\bDLV\b|\bRCS\b|\bRCF\b|\bDEP\b/i.test(s);
  if (!awbMatched && !shipmentLike) return null;
  ...
  const handler = response => {
    const task = (async () => {
      try {
        ...
        const likely = digitText.includes(serial) || /\bDLV\b|DELIVERED|\bRCF\b|\bRCS\b|\bDEP\b/i.test(flat) || /(cargo|track|shipment|awb)/i.test(url);
        if (likely) items.push({ url, text: flat.slice(0, 250000) });
      } catch {}
    })();
    pending.add(task);
...
async function fillAddSearch(page, mawb) {
  ...
  for (const frame of page.frames()) {
    ...
    await sleep(650);
    let searchMode = '';
    for (const f of page.frames()) if (!searchMode && await clickText(f, 'Search')) searchMode = 'CLICK_SEARCH';
