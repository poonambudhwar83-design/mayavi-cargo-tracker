...
export async function trackTurkish(input) {
  ...
  // Use Turkish Cargo's own shipmentTracking endpoint first. This is the
  // fastest route and avoids the carrier's interactive web verification when
  // the endpoint is available normally.
  ...
  try {
    ...
    // If the endpoint only accepts a normal Turkish Cargo browser session,
    // retry it from that official page session before using the DOM flow.
    ...
    // Existing official-page flow: fill AWB -> Add -> Search.
