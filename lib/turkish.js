...
async function clickableItems(frame){
  const out=[];
  for(const el of await frame.$$('button,input[type="submit"],input[type="button"],[role="button"],a,[onclick],[aria-label],[title]')){
    try{
      const m=await el.evaluate(e=>{const r=e.getBoundingClientRect();return{text:String(e.innerText||e.value||e.getAttribute('aria-label')||e.getAttribute('title')||'').replace(/\s+/g,' ').trim(),disabled:Boolean(e.disabled),visible:r.width>3&&r.height>3}});
      if(m.visible&&!m.disabled&&m.text)out.push({el,text:m.text});
    }catch{}
  }
  return out;
}
async function clickNamed(frame,rx){
  for(const x of await clickableItems(frame))if(rx.test(x.text)){await x.el.click({delay:60});return x.text;}
  return'';
}
async function scrollToDetails(page){
  for(const frame of page.frames()){
    try{
      await frame.evaluate(async()=>{
        const wait=ms=>new Promise(r=>setTimeout(r,ms));
        for(let y=0;y<document.documentElement.scrollHeight;y+=650){window.scrollTo(0,y);await wait(120);}
...
async function fillAddSearch(page,mawb){
  ...
  for(const frame of page.frames()){
    ...
    let addText=await clickNamed(frame,/^\s*add\s*$/i);
    if(!addText)addText=await clickNamed(frame,/\badd\b.*(?:awb|air waybill|shipment)|(?:awb|air waybill|shipment).*\badd\b/i);
    let addMode='BUTTON';
    if(!addText){
      await page.keyboard.press('Enter');
      addText='ENTER_AS_ADD';addMode='ENTER';
    }
    await sleep(650);
    let searchText=await clickNamed(frame,/search/i);
    if(!searchText)searchText=await clickNamed(frame,/track|sorgula/i);
    if(!searchText){
      const afterEnter=clean(await frame.evaluate(()=>document.body?.innerText||''));
      if(afterEnter.includes(serial)||afterEnter.includes(digits))return{ok:true,stage:'ENTER_TRIGGERED_RESULT',addText,addMode,searchText:''};
      const candidates=(await clickableItems(frame)).map(x=>x.text).slice(0,40);
      return{ok:false,stage:'SEARCH_NOT_FOUND',addText,addMode,candidates};
    }
    return{ok:true,stage:'ADD_THEN_SEARCH',addText,addMode,searchText};
  }
...
export async function trackTurkish(input){
  ...
  try{
    ...
    for(const frame of page.frames())try{await clickNamed(frame,/accept all/i)}catch{}
    ...
    await scrollToDetails(page);
    for(let i=0;i<16;i++){
      await sleep(i?700:1000);
      if(i===3||i===8)await scrollToDetails(page);
      for(let n=network.length-1;n>=0;n--){const p=parseStructured(network[n].body,mawb);if(p?.notFound)return{ok:false,notFound:true,reason:'NO SHIPMENT RECORD',airline:AIRLINE,debug:{...debug,...flow,stage:'NO_RECORD',source:'network'}};if(p?.useful)return{ok:true,airline:AIRLINE,shipment:p.shipment,debug:{...debug,...flow,stage:'SUCCESS_AFTER_SCROLL',source:'network',networkUrl:network[n].url}};}
      const pageNow=await pageText(page);const p=parsePage(pageNow,mawb);if(p?.notFound)return{ok:false,notFound:true,reason:'NO SHIPMENT RECORD',airline:AIRLINE,debug:{...debug,...flow,stage:'NO_RECORD',source:'page'}};if(p?.useful)return{ok:true,airline:AIRLINE,shipment:p.shipment,debug:{...debug,...flow,stage:'SUCCESS_AFTER_SCROLL',source:'page',pageSample:clean(pageNow).slice(0,7000)}};
    }
    const finalText=await pageText(page);
    return{ok:false,reason:'TURKISH RESULT NOT MACHINE READABLE',airline:AIRLINE,debug:{...debug,...flow,stage:'UNREADABLE_AFTER_SCROLL',networkUrls:network.slice(-8).map(x=>x.url),preview:clean(finalText).slice(0,7000)}};
