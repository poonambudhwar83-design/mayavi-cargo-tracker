...
function flight(v=''){
  const m=clean(v).toUpperCase().match(/\bTK[-\s]?(\d{2,4})\b/);
  return m?`TK${m[1]}`:'';
}
...
function parsePage(text,mawb){
  ...
  if(status==='ARRIVED'){
    const block=(s.match(/(?:DELIVERED\s*-?\s*[A-Z]{3}|SHIPMENT DELIVERED|\bDLV\b)[\s\S]{0,360}/i)||[])[0]||s;
  ...
  return {
    ...
    shipment:{
      ...
      status,
      officialTracker:URL,
      source:'Turkish Cargo official result card'
    }
  };
}
...
async function pageText(page){
  ...
  for(const f of page.frames()){
    try{const t=await f.evaluate(()=>document.body?.innerText||'');if(t)out.push(t)}catch{}
  }
  return out.join('\n');
}
...
async function clickExact(frame,label){
  try{
    return await frame.evaluate((wanted)=>{
      ...
      if(!el)return'';
      el.click();
      return String(el.innerText||el.value||wanted).trim();
    },label);
  }catch{return''}
}
...
async function clickAddSuggestion(frame,serial){
  try{
    return await frame.evaluate((awb)=>{
      ...
      const els=[...document.querySelectorAll('[role="option"],[role="menuitem"],li,button,div,span')];
      const el=els.find(e=>{
        const r=e.getBoundingClientRect();
...
async function scrollLittle(page){
  for(const f of page.frames()){
    try{await f.evaluate(()=>window.scrollBy({top:520,left:0,behavior:'auto'}))}catch{}
  }
  await sleep(500);
}
...
async function fillAddSearch(page,mawb){
  ...
  for(const frame of page.frames()){
    ...
    if(pf){
      await pf.input.click({clickCount:3});
      await page.keyboard.press('Backspace');
      await pf.input.type(prefix,{delay:35});
    }
    ...
    const added=await clickAddSuggestion(frame,serial);
    if(!added){
...
export async function trackTurkish(input){
  ...
  try{
    ...
    await sleep(1800);
    for(const frame of page.frames()){
      try{await clickExact(frame,'Accept all')}catch{}
    }

    const before=await pageText(page);
    if(/captcha|verify you are human|access denied|cloudflare|security check/i.test(before)){
      return {ok:false,reason:'OFFICIAL SITE BLOCKED AUTOMATION',airline:AIRLINE,debug:{...debug,stage:'BLOCKED'}};
    }
    ...
    for(let i=0;i<22;i++){
      ...
      const parsed=parsePage(text,mawb);
      ...
      if(parsed?.useful&&(/TK SMART|\bDLV\b|DELIVERED|\bFROM\s+[A-Z]{3}\s*-/i.test(text))){
        return {ok:true,airline:AIRLINE,shipment:parsed.shipment,debug:{...debug,...flow,stage:'SUCCESS_AFTER_SCROLL',pageSample:clean(text).slice(0,8000)}};
      }
    }

    const finalText=await pageText(page);
    return {ok:false,reason:'TURKISH RESULT NOT MACHINE READABLE',airline:AIRLINE,debug:{...debug,...flow,stage:'UNREADABLE_AFTER_SCROLL',preview:clean(finalText).slice(0,8000)}};
  }catch(e){
