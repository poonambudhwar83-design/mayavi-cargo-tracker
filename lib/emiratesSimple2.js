...
export async function trackEmirates(mawb){
  ...
  try{
    ...
    const searched=await clickSearchNative(page);await sleep(6500);
    ...
    if(!shipmentId&&noMatchingRecord(startText,network))return{ok:false,notFound:true,reason:'EMIRATES NO MATCHING RECORD',officialTracker:page.url(),debug:{stage:'no-record',filled,entered,searched,url:page.url(),bodySample:startText.slice(0,3000),networkUrls:[...new Set(network.map(n=>n.url))].slice(-12),networkSamples:network.slice(-4).map(n=>({url:n.url,body:n.body.slice(0,700)}))}};
    ...
    if(!shipmentId){
      ...
      if(looksLikeShipment){
        const publicShowDetails=await clickText(page,/^Show Details$/i);if(publicShowDetails)await sleep(1400);
        const publicTrackingClick=await clickText(page,/^Tracking Details$/i);if(publicTrackingClick)await sleep(1800);
        const detailedText=await combinedText(page);if(detailedText&&(detailedText.includes(digits)||detailedText.includes(digits.slice(3))))startText=detailedText;
