...
export async function trackWithBrowser(input){
  ...
  try{
    ...
    const fill=await page.evaluate(({mawb,prefix,serial})=>{
      ...
      let prefixInput=inputs.find(e=>/prefix/.test(desc(e))), numberInput=inputs.find(e=>/(awb|airway|document|tracking|shipment).*(number|no|serial)|documentnumber|awbnumber/.test(desc(e))&&!/prefix/.test(desc(e)));
      if(prefixInput&&numberInput){set(prefixInput,prefix);set(numberInput,serial);return{filled:true,mode:'split'};}
      let best=inputs.map(e=>({e,score:(/(awb|airway|tracking|shipment|document)/.test(desc(e))?5:0)+(/number|no|serial/.test(desc(e))?2:0)})).sort((a,b)=>b.score-a.score)[0];
      if(best?.e){set(best.e,mawb);return{filled:true,mode:'single'};}return{filled:false};
    },{mawb,prefix:mawb.slice(0,3),serial:mawb.slice(4)}).catch(()=>({filled:false}));
    ...
    if(fill.filled){
      ...
      const clicked=await page.evaluate(()=>{
        ...
        const target=els.filter(visible).map(e=>({e,t:(e.innerText||e.value||e.getAttribute('aria-label')||'').trim()})).find(x=>/^(track|search|submit|go)$|track shipment|track cargo|track awb|search shipment/i.test(x.t));
        if(target){target.e.click();return target.t;}return'';
      }).catch(()=> '');
      if(clicked){await Promise.race([page.waitForNetworkIdle({idleTime:700,timeout:6500}).catch(()=>{}),new Promise(r=>setTimeout(r,6500))]);}
    ...
    if(useful(shipment))return{ok:true,shipment,screenshotBase64,debug:{stage:'SUCCESS',url,filled:fill.filled,mode:fill.mode||''}};
    return{ok:false,reason:'BROWSER OPENED AIRLINE PAGE BUT NO VERIFIED SHIPMENT FIELDS FOUND',officialTracker:url,screenshotBase64,pageText:text.slice(0,6000),debug:{stage:'NO_FIELDS',filled:fill.filled,mode:fill.mode||''}};
