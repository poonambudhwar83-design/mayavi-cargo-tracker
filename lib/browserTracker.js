...
function etihadFromText(text='',mawb='',airline={}){
  ...
  const fullDigits=String(mawb).replace(/\D/g,'');
  const serial=fullDigits.slice(3);
  const compact=flat.replace(/\D/g,'');
  const hasShipmentEvidence=compact.includes(fullDigits)||compact.includes(serial)||/(Pieces?|Pcs?|Weight|Shipment\s+arrived|Received\s+from\s+flight|Delivered|Flight\s+No|Origin|Destination)/i.test(flat);
  if(!hasShipmentEvidence||/Enter a valid Airway bill prefix/i.test(flat)){
    return{mawb,carrierCode:'EY',airlineName:'Etihad Cargo',origin:'',destination:'',bags:'',pieces:'',weight:'',flightNo:'',arrivalDate:'',arrivalTime:'',arrivalIsActual:false,status:'TRACKING',officialTracker:airline?.url||'',source:'Etihad Cargo official shipment page',arrivalSource:'etihad-no-result',arrivalSnippet:flat.slice(0,1500)};
  }
  ...
  if(!origin)origin=airports[0]||'';if(!destination)destination=airports.length>1?airports[airports.length-1]:'';
  ...
  if(!arrivalDate||!arrivalTime){const actual=(flat.match(/(?:Actual Arrival|Arrival|ETA|Expected Arrival|Scheduled Arrival)[\s\S]{0,220}/i)||[])[0]||'';arrivalDate=arrivalDate||parseDate(actual);arrivalTime=arrivalTime||parseTime(actual);}
...
export async function trackWithBrowser(input){
  ...
  try{
    ...
    let fill=await page.evaluate(({mawb,prefix,serial,digits})=>{
      ...
      const set=(e,v)=>{e.focus();const d=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value');d?.set?.call(e,v);e.dispatchEvent(new Event('input',{bubbles:true}));e.dispatchEvent(new Event('change',{bubbles:true}));e.dispatchEvent(new KeyboardEvent('keyup',{bubbles:true,key:'1'}));e.blur();};
      if(prefix==='607'){
        const etihad=inputs.find(e=>/airway|awb|shipment|enter your airway bill/.test(desc(e)))||inputs[0];
        if(etihad){set(etihad,digits);return{filled:true,mode:'etihad-full-awb',value:digits};}
      }
    ...
    },{mawb,prefix,serial,digits}).catch(()=>({filled:false}));
    ...
    let clicked='';if(fill.filled){await sleep(700);clicked=await page.evaluate(()=>{const els=[...document.querySelectorAll('button,input[type="submit"],a,[role="button"]')];const visible=e=>{const s=getComputedStyle(e),r=e.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&r.width>20&&r.height>10&&!e.disabled};const candidates=els.filter(visible).map(e=>({e,t:(e.innerText||e.value||e.getAttribute('aria-label')||e.getAttribute('title')||'').replace(/\s+/g,' ').trim()}));const target=candidates.find(x=>/^track$/i.test(x.t)||/^(search|submit|go)$/i.test(x.t)||/track\s+shipment|track\s+cargo|track\s+awb|search\s+shipment/i.test(x.t));if(target){target.e.click();return target.t;}return'';}).catch(()=> '');if(clicked){await Promise.race([page.waitForNetworkIdle({idleTime:900,timeout:10000}).catch(()=>{}),sleep(10000)]);await sleep(prefix==='607'?3500:1200);}else await sleep(3000);}
    ...
    if(useful(shipment))return{ok:true,shipment,screenshotBase64,debug:{stage:'SUCCESS',url,filled:fill.filled,mode:fill.mode||'',value:fill.value||'',clicked,arrivalSource,arrivalSnippet,pageText:text.slice(0,5000)}};
    return{ok:false,reason:'BROWSER OPENED AIRLINE PAGE BUT NO VERIFIED SHIPMENT FIELDS FOUND',officialTracker:url,screenshotBase64,pageText:text.slice(0,6000),debug:{stage:'NO_FIELDS',filled:fill.filled,mode:fill.mode||'',value:fill.value||'',clicked,arrivalSource,arrivalSnippet,pageText:text.slice(0,5000)}};
