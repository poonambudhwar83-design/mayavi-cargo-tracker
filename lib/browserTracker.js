...
function officialUrl(mawb,airline){
  ...
  if(prefix==='607')return`https://www.etihadcargo.com/en/e-services/shipment-tracking?awb=${digits}`;
...
async function etihadFrameFlow(page,{digits,serial}){
  const result={filled:false,mode:'etihad-cathay-style',value:'',frame:'',clicked:'',verified:false,attempts:[]};
  const values=[serial,digits,`607-${serial}`];
  for(const value of values){
    for(const frame of page.frames()){
      try{
        const r=await frame.evaluate(({value,serial,digits})=>{
          const visible=e=>{const s=getComputedStyle(e),b=e.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&b.width>20&&b.height>10&&!e.disabled};
          const desc=e=>`${e.name||''} ${e.id||''} ${e.placeholder||''} ${e.getAttribute('aria-label')||''} ${e.getAttribute('title')||''}`.toLowerCase();
          const inputs=[...document.querySelectorAll('input,textarea,[contenteditable="true"],[role="textbox"]')].filter(visible);
          const scored=inputs.map(e=>({e,score:(/airway|awb/.test(desc(e))?10:0)+(/shipment|track/.test(desc(e))?4:0)+(/enter/.test(desc(e))?2:0)})).sort((a,b)=>b.score-a.score);
          const input=scored[0]?.e;if(!input)return{ok:false,reason:'no-input'};
          const set=(e,v)=>{if('value'in e){const d=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value');if(d?.set)d.set.call(e,v);else e.value=v;}else e.textContent=v;e.dispatchEvent(new Event('input',{bubbles:true}));e.dispatchEvent(new Event('change',{bubbles:true}));e.dispatchEvent(new Event('blur',{bubbles:true}));e.dispatchEvent(new KeyboardEvent('keyup',{bubbles:true,key:'0'}));};
          set(input,value);
          const actual=String('value'in input?input.value:input.textContent||'');
          const buttons=[...document.querySelectorAll('button,input[type="submit"],input[type="button"],a,[role="button"]')].filter(visible);
          const btn=buttons.find(e=>/^track$/i.test((e.innerText||e.value||e.textContent||e.getAttribute('aria-label')||'').trim()));
          return{ok:true,actual,button:Boolean(btn),buttonText:btn?(btn.innerText||btn.value||btn.textContent||'').trim():'',htmlPrefix:(document.body?.innerText||'').slice(0,1200)};
        },{value,serial,digits});
        result.attempts.push({value,frame:frame.url(),...r});
        if(!r?.ok)continue;
