...
export async function trackWithBrowser(input){
  ...
  const network=[];
  try{
    browser=await launchBrowser();const page=await browser.newPage();await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/149 Safari/537.36');
    ...
    await page.goto(url,{waitUntil:'domcontentloaded',timeout:22000});await sleep(prefix==='607'?4200:1800);await acceptCookies(page);await sleep(700);
    ...
    if(useful(shipment))return{ok:true,shipment,screenshotBase64,debug:{stage:'SUCCESS',url,filled:fill.filled,mode:fill.mode||'',value:fill.value||'',verified:Boolean(fill.verified),clicked,arrivalSource,arrivalSnippet,pageText:text.slice(0,7000),attempts:fill.attempts||[],network:network.map(x=>({url:x.url,status:x.status,sample:x.body.slice(0,1200)})).slice(-12)}};
    return{ok:false,reason:'BROWSER OPENED AIRLINE PAGE BUT NO VERIFIED SHIPMENT FIELDS FOUND',officialTracker:url,screenshotBase64,pageText:text.slice(0,7000),debug:{stage:'NO_FIELDS',filled:fill.filled,mode:fill.mode||'',value:fill.value||'',verified:Boolean(fill.verified),clicked,arrivalSource,arrivalSnippet,pageText:text.slice(0,7000),attempts:fill.attempts||[],network:network.map(x=>({url:x.url,status:x.status,sample:x.body.slice(0,1200)})).slice(-12)}};
