...
function etihadFlightPlan(flat=''){
  ...
  const airports=[...block.matchAll(/\b([A-Z]{3})\b/g)].map(m=>m[1]).filter(x=>!['THE','AND','FOR','PCS','KGS','AWB','HUB','OUT'].includes(x));
  const origin=airports[0]||'',destination=airports.length>1?airports[airports.length-1]:'';
...
function etihadFromText(text='',mawb='',airline={}){
  ...
  if(arrived.length){const a=arrived[arrived.length-1];arrivalDate=parseDate(a[1]);arrivalTime=parseTime(a[2]);arrivalIsActual=true;arrivalSource='etihad-shipment-arrived';if(a[3])destination=a[3].toUpperCase();}
  ...
  return{mawb,carrierCode:'EY',airlineName:'Etihad Cargo',origin,destination,bags:pieces,pieces,weight,flightNo,arrivalDate,arrivalTime,arrivalIsActual,status,officialTracker:airline?.url||'',source:'Etihad Cargo official shipment result page',arrivalSource,arrivalSnippet:flat.slice(0,1800)};
}
