...
function etihadFromText(text='',mawb='',airline={}){
  ...
  if(!arrivalDate||!arrivalTime){const actualBlock=(flat.match(/(?:actualArrival(?:DateTime)?|actual arrival|arrivedAt|\bARR\b|\bRCF\b|received from flight)[\s\S]{0,260}/i)||[])[0]||'';const d=parseDate(actualBlock),t=parseTime(actualBlock);if(d||t){arrivalDate=d;arrivalTime=t;arrivalIsActual=true;}}
  if(!arrivalDate||!arrivalTime){const estimatedBlock=(flat.match(/(?:estimatedArrival(?:DateTime)?|estimated arrival|expected arrival|scheduled arrival|\bETA\b)[\s\S]{0,260}/i)||[])[0]||'';const d=parseDate(estimatedBlock),t=parseTime(estimatedBlock);if(d||t){arrivalDate=arrivalDate||d;arrivalTime=arrivalTime||t;arrivalIsActual=false;}}
