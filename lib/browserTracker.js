...
function statusFromText(text=''){
  ...
  if(/\bRCF\b|RECEIVED FROM FLIGHT|\bARRIVED\b|\bLANDED\b|ACTUAL ARRIVAL|\bARR\b/.test(s))return'ARRIVED';
...
function etihadFlightPlan(flat=''){
  const s=String(flat||'').replace(/\s+/g,' ').trim();
  const block=(s.match(/Flight\s*plan[\s\S]{0,700}?(?=Tracking\s+history|Want\s+to\s+create|$)/i)||[])[0]||'';
  if(!block)return null;
  const timedStations=[...block.matchAll(/\b([A-Z]{3})\s+(\d{1,2}:\d{2})\b/g)].map(m=>m[1].toUpperCase());
  const airports=[...block.matchAll(/\b([A-Z]{3})\b/g)].map(m=>m[1]).filter(x=>!['THE','AND','FOR','PCS','KGS','AWB','HUB','OUT','NEW','MON','TUE','WED','THU','FRI','SAT','SUN','JAN','FEB','MAR','APR','MAY','JUN','JUL','AUG','SEP','OCT','NOV','DEC'].includes(x));
  const origin=timedStations[0]||airports[0]||'',destination=timedStations.length>1?timedStations.at(-1):(airports.length>1?airports.at(-1):'');
  const dateTimes=[...block.matchAll(/\b(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)?\s*(\d{1,2})\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)[A-Za-z]*\s*(20\d{2})?\s*(?:,|at)?\s*(\d{1,2}:\d{2})\b/gi)];
  let arrivalDate='',arrivalTime='';
  if(dateTimes.length){
    const m=dateTimes[dateTimes.length-1];
    const year=m[3]||((s.match(/\b(20\d{2})\b/)||[])[1])||String(new Date().getUTCFullYear());
    const mon=String(m[2]).toUpperCase().replace('SEPT','SEP');
    arrivalDate=`${year}-${MONTHS[mon]||''}-${pad(m[1])}`;
    arrivalTime=parseTime(m[4]);
  }else{
    const tail=block.slice(Math.max(0,block.length-220));
    arrivalTime=parseTime(tail);
    const dm=tail.match(/\b(\d{1,2})\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)[A-Za-z]*\b/i);
    if(dm){const year=((s.match(/\b(20\d{2})\b/)||[])[1])||String(new Date().getUTCFullYear());const mon=String(dm[2]).toUpperCase().replace('SEPT','SEP');arrivalDate=`${year}-${MONTHS[mon]||''}-${pad(dm[1])}`;}
  }
  return{origin,destination,arrivalDate,arrivalTime};
}
...
function etihadFromText(text='',mawb='',airline={}){
  ...
  const pieces=piecesPair[1]&&piecesPair[2]?`${piecesPair[1]}/${piecesPair[2]}`:(first(flat,/(?:Pieces?|Pcs?|pieceCount|totalPieces|bags?)['"\s:_-]{0,20}(\d{1,6})/i)||'');
  const weight=weightPair[1]&&weightPair[2]?`${weightPair[1].replace(/,/g,'')}/${weightPair[2].replace(/,/g,'')}`:(first(flat,/(?:Gross\s*Weight|Weight|grossWeight|chargeableWeight)['"\s:_-]{0,20}([\d,.]+)/i)||'').replace(/,/g,'');
  ...
  const plan=etihadFlightPlan(flat);
  let origin=first(flat,/(?:origin|originAirport|fromStation|departureAirport)['"\s:_-]{0,30}([A-Z]{3})\b/i).toUpperCase();
  let destination=first(flat,/(?:destination|destinationAirport|toStation|arrivalAirport)['"\s:_-]{0,30}([A-Z]{3})\b/i).toUpperCase();
  const route=upper.match(/\b([A-Z]{3})\s+(?:TO|→|->|-)\s+([A-Z]{3})\b/);if(route){origin=origin||route[1];destination=destination||route[2];}
  if(plan?.origin)origin=plan.origin;if(plan?.destination)destination=plan.destination;
  ...
  const eyDelivered=[...flat.matchAll(/(\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)[A-Za-z]*\s+20\d{2})\s+(\d{1,2}:\d{2})\s*,\s*([A-Z]{3})\s+Shipment delivered/gi)];
  const eyDeparted=[...flat.matchAll(/(\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)[A-Za-z]*\s+20\d{2})\s+(\d{1,2}:\d{2})\s*,\s*([A-Z]{3})\s+Shipment departed[\s\S]{0,180}?from\s+([A-Z]{3})\s+to\s+([A-Z]{3})\s+on\s+(EY\d{2,4})/gi)];
  const historyOriginDeparture=[...eyDeparted].reverse().find(x=>String(x[3]||'').toUpperCase()===String(x[4]||'').toUpperCase())||eyDeparted.at(-1)||null;
  if(historyOriginDeparture?.[4])origin=String(historyOriginDeparture[4]).toUpperCase();
  if(eyDelivered[0]?.[3])destination=String(eyDelivered[0][3]).toUpperCase();
  let arrivalDate='',arrivalTime='',arrivalIsActual=false,arrivalSource='etihad-result-page';
  const arrived=[...flat.matchAll(/(\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)[A-Za-z]*\s+20\d{2})\s+(\d{1,2}:\d{2})\s*,?\s*([A-Z]{3})?\s*(?:Shipment\s+arrived|Shipment\s+received\s+from\s+flight|Received\s+from\s+flight)/gi)];
  const finalArrived=arrived.find(a=>destination&&String(a[3]||'').toUpperCase()===destination)||arrived[0]||null;
  if(finalArrived){const a=finalArrived;arrivalDate=parseDate(a[1]);arrivalTime=parseTime(a[2]);arrivalIsActual=true;arrivalSource='etihad-shipment-arrived';if(a[3])destination=a[3].toUpperCase();}
  if(!arrivalDate||!arrivalTime){const actualBlock=(flat.match(/(?:actualArrival(?:DateTime)?|actual arrival|arrivedAt|\bARR\b|\bRCF\b|received from flight)[\s\S]{0,260}/i)||[])[0]||'';const d=parseDate(actualBlock),t=parseTime(actualBlock);if(d||t){arrivalDate=d;arrivalTime=t;arrivalIsActual=true;arrivalSource='etihad-actual-arrival';}}
  if(!arrivalDate||!arrivalTime){const estimatedBlock=(flat.match(/(?:estimatedArrival(?:DateTime)?|estimated arrival|expected arrival|scheduled arrival|\bETA\b)[\s\S]{0,260}/i)||[])[0]||'';const d=parseDate(estimatedBlock),t=parseTime(estimatedBlock);if(d||t){arrivalDate=arrivalDate||d;arrivalTime=arrivalTime||t;arrivalIsActual=false;arrivalSource='etihad-estimated-arrival';}}
  if((!arrivalDate||!arrivalTime)&&plan&&(plan.arrivalDate||plan.arrivalTime)){
    arrivalDate=arrivalDate||plan.arrivalDate;arrivalTime=arrivalTime||plan.arrivalTime;arrivalIsActual=false;arrivalSource='etihad-flight-plan';
  }
  const originDeparture=eyDeparted.find(x=>String(x[3]||'').toUpperCase()===origin&&String(x[4]||'').toUpperCase()===origin)||historyOriginDeparture;
  const status=/your shipment has been delivered|shipment delivered/i.test(flat)?'DELIVERED':statusFromText(flat);
  return{mawb,carrierCode:'EY',airlineName:'Etihad Cargo',origin,destination,bags:pieces,pieces,weight,flightNo,departureDate:originDeparture?parseDate(originDeparture[1]):'',departureTime:originDeparture?parseTime(originDeparture[2]):'',departureFlightNo:originDeparture?.[6]||'',departureOrigin:originDeparture?String(originDeparture[4]||origin).toUpperCase():origin,departureDestination:originDeparture?String(originDeparture[5]||'').toUpperCase():'',departureIsActual:Boolean(originDeparture),departureTimeSource:originDeparture?'Etihad tracking history origin Shipment departed event':'',arrivalDate,arrivalTime,arrivalIsActual,status,officialTracker:airline?.url||'',source:'Etihad Cargo official shipment result page',arrivalSource,arrivalSnippet:flat.slice(0,1800)};
}
...
async function launchBrowser(){
  ...
  const options={args:[...chromium.args,'--no-sandbox','--disable-setuid-sandbox','--disable-blink-features=AutomationControlled','--lang=en-US,en'],defaultViewport:{width:1440,height:1100,deviceScaleFactor:1},executablePath,headless:'shell'};
  let lastError;
...
async function etihadSameOriginFetch(page,digits){
  try{
    return await page.evaluate(async awb=>{
      try{
        const r=await fetch(`/en/e-services/track-shipment?awb=${encodeURIComponent(awb)}`,{method:'GET',credentials:'include',cache:'no-store',headers:{'Accept':'text/html,application/xhtml+xml,application/json;q=0.9,*/*;q=0.8','X-Requested-With':'XMLHttpRequest'}});
        const text=await r.text();
        return{ok:r.ok,status:r.status,url:r.url,text:text.slice(0,140000)};
      }catch(e){return{ok:false,status:0,url:'',text:'',error:String(e?.message||e)}}
    },digits);
  }catch(e){return{ok:false,status:0,url:'',text:'',error:String(e?.message||e)}}
}
...
async function etihadFrameFlow(page,{digits,serial}){
  ...
  const values=[serial,digits,`${digits.slice(0,3)}-${serial}`];
  for(const frame of page.frames()){
    try{
      ...
      for(const value of values){
        ...
        await sleep(200);
        await best.type(value,{delay:110});
        const actual=await best.evaluate(e=>String(e.value||e.textContent||'')).catch(()=> '');
        result.attempts.push({value,frame:frame.url(),actual,mode:'click-clear-key-by-key'});
        if(actual!==value)continue;
        result.filled=true;result.value=value;result.frame=frame.url();
        await sleep(1200);
        ...
        if(clicked)result.clicked=clicked;
        await Promise.race([page.waitForNetworkIdle({idleTime:800,timeout:7000}).catch(()=>{}),sleep(7000)]);
        await sleep(1200);
        let merged='';for(const f of page.frames()){try{merged+=`\n${await f.evaluate(()=>document.body?.innerText||'')}`;}catch{}}
        if(/your shipment|shipment delivered|shipment arrived|tracking history|\d+\s*\/\s*\d+\s*Pcs|\d+\s*\/\s*\d+\s*Kg|received from flight|\bEY\s*\d{2,4}\b/i.test(merged)&&!/Enter a valid Airway bill prefix/i.test(merged)){result.verified=true;return result;}
        await best.press('Enter').catch(()=>{});
        await Promise.race([page.waitForNetworkIdle({idleTime:800,timeout:5000}).catch(()=>{}),sleep(5000)]);
        await sleep(700);
        merged='';for(const f of page.frames()){try{merged+=`\n${await f.evaluate(()=>document.body?.innerText||'')}`;}catch{}}
        if(/your shipment|shipment delivered|shipment arrived|tracking history|received from flight|\bEY\s*\d{2,4}\b/i.test(merged)&&!/Enter a valid Airway bill prefix/i.test(merged)){result.verified=true;return result;}
      }
    }catch{}
  }
  return result;
...
export async function trackWithBrowser(input){
  ...
  try{
    ...
    if(prefix==='607')page.on('response',async response=>{try{const u=response.url();const ct=(response.headers()['content-type']||'').toLowerCase();if(!/(json|text|javascript|xml)/.test(ct)&&!/track|shipment|awb|cargo|event|milestone|status/i.test(u))return;const body=await response.text();if(!body||body.length>250000)return;if(body.includes(serial)||body.includes(digits)||/shipment|airway|awb|milestone|flight|arrival|departure|pieces|weight/i.test(body))network.push({url:u,status:response.status(),body:body.slice(0,120000)});}catch{}});
    await page.goto(url,{waitUntil:'domcontentloaded',timeout:22000,referer:'https://www.etihadcargo.com/'});await sleep(prefix==='607'?4200:1800);await acceptCookies(page);await sleep(700);
    ...
    if(prefix==='607'){
      etihadFetch=await etihadSameOriginFetch(page,digits);
      const fetchedText=etihadFetch?.text||'';
      const fetchedUseful=/your shipment|shipment delivered|shipment arrived|tracking history|\d+\s*\/\s*\d+\s*Pcs|\d+\s*\/\s*\d+\s*Kg|received from flight|\bEY\s*\d{2,4}\b/i.test(fetchedText)&&!/page took off without us|invalid request|enter a valid airway bill/i.test(fetchedText);
      if(fetchedUseful){bodyBefore+=`\nETIHAD_SAME_ORIGIN_RESULT\n${fetchedText}`;fill={filled:true,mode:'etihad-same-origin-fetch',value:digits,verified:true};}
      else{
    ...
    let text=bodyBefore;for(const f of page.frames()){try{text+=`\n${await f.evaluate(()=>document.body?.innerText||'')}`;}catch{}}
    if(prefix==='607'&&network.length)text+=`\nETIHAD_NETWORK_CAPTURE\n${network.map(x=>x.body).join('\n')}`;
    ...
    if(useful(shipment))return{ok:true,shipment,screenshotBase64,debug:{stage:'SUCCESS',url,filled:fill.filled,mode:fill.mode||'',value:fill.value||'',verified:Boolean(fill.verified),clicked,arrivalSource,arrivalSnippet,pageText:text.slice(0,7000),attempts:fill.attempts||[],sameOriginFetch:etihadFetch?{status:etihadFetch.status,url:etihadFetch.url,sample:(etihadFetch.text||'').slice(0,1200)}:null,network:network.map(x=>({url:x.url,status:x.status,sample:x.body.slice(0,1200)})).slice(-12)}};
    return{ok:false,reason:'BROWSER OPENED AIRLINE PAGE BUT NO VERIFIED SHIPMENT FIELDS FOUND',officialTracker:url,screenshotBase64,pageText:text.slice(0,7000),debug:{stage:'NO_FIELDS',filled:fill.filled,mode:fill.mode||'',value:fill.value||'',verified:Boolean(fill.verified),clicked,arrivalSource,arrivalSnippet,pageText:text.slice(0,7000),attempts:fill.attempts||[],sameOriginFetch:etihadFetch?{status:etihadFetch.status,url:etihadFetch.url,sample:(etihadFetch.text||'').slice(0,1200)}:null,network:network.map(x=>({url:x.url,status:x.status,sample:x.body.slice(0,1200)})).slice(-12)}};
