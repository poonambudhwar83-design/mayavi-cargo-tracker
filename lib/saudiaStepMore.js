...
async function englishFirst(page){
  const clicked=await page.evaluate(()=>{
    ...
    if(!el)return false;const target=el.closest('button,a,[role="button"]')||el;target.dataset.mayaviEnglish='1';return true;
  }).catch(()=>false);
  if(clicked){await page.click('[data-mayavi-english="1"]').catch(()=>page.evaluate(()=>document.querySelector('[data-mayavi-english="1"]')?.click()).catch(()=>{}));await new Promise(r=>setTimeout(r,700));}
...
async function clickMore(page){
  const ok=await page.evaluate(()=>{
    ...
    const text=e=>(e.innerText||e.textContent||e.getAttribute?.('aria-label')||e.title||'').replace(/\s+/g,' ').trim();
    let el=[...document.querySelectorAll('button,[role="button"],a,[tabindex],div,span')].filter(vis).find(e=>/^\+?\s*more\s+information$/i.test(text(e))||/more\s+information/i.test(text(e))||/更多信息/.test(text(e)));
    if(el)el=el.closest('button,[role="button"],a,[tabindex]')||el;
    if(!el)return false;el.dataset.mayaviMore='1';return true;
  }).catch(()=>false);
  if(!ok)return false;
...
export async function saudiaStepMore(input){
  const mawb=normalizeMawb(input);if(!mawb||!mawb.startsWith('065-'))return{ok:false,reason:'INVALID SAUDIA MAWB'};let browser;
  try{
    ...
    await page.goto(URL,{waitUntil:'domcontentloaded',timeout:28000});await new Promise(r=>setTimeout(r,1700));
    ...
    const digits=mawb.replace(/\D/g,'');await page.click('[data-mayavi-awb="1"]',{clickCount:3}).catch(()=>{});await page.keyboard.press('Control+A').catch(()=>{});await page.keyboard.press('Backspace').catch(()=>{});await page.type('[data-mayavi-awb="1"]',digits,{delay:25});
    const filled=await page.$eval('[data-mayavi-awb="1"]',e=>e.value).catch(()=> '');
    const arrowClicked=await clickArrow(page);if(!arrowClicked)return{ok:false,stage:'ARROW_NOT_FOUND',mawb,filled,englishClicked,arrowClicked:false};
    await new Promise(r=>setTimeout(r,1300));
    const moreClicked=await clickMore(page);if(moreClicked)await new Promise(r=>setTimeout(r,900));
    const text=await page.evaluate(()=>document.body?.innerText||'').catch(()=> '');
    return{ok:Boolean(moreClicked),stage:moreClicked?'MORE_INFORMATION_CLICKED':'MORE_INFORMATION_NOT_FOUND',mawb,filled,englishClicked,arrowClicked:true,moreClicked,pageText:text.slice(0,2400)};
