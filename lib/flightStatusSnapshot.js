...
function parseTripText(text,{flightNo,origin,destination,date}){
  ...
  const depWindowRx=new RegExp(`${escapeRx(label)}[\\s\\S]{0,900}?\\b${escapeRx(origin)}\\b[\\s\\S]{0,650}?\\b${escapeRx(destination)}\\b`,'i');
  const depWindow=(flat.match(depWindowRx)||[])[0]||'';
  if(depWindow){
