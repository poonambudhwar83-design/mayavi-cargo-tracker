...
function parseTripText(text,{flightNo,origin,destination,date}){
  ...
  if(card){
    ...
    if(arrivalTime)return{status:'ARRIVED',arrivalDate:date,arrivalTime,arrivalIsActual:true,source:'Flight-status screenshot fallback',debugPattern:'DATED_ARRIVED_CARD',debugSnippet:card[0].slice(0,1300)};
  }
  ...
  if(row){
    ...
    if(times.length)return{status:'ARRIVED',arrivalDate:date,arrivalTime:times[times.length-1],arrivalIsActual:true,source:'Flight-status screenshot fallback',debugPattern:'HISTORY_ROW',debugSnippet:row[0].slice(0,1300)};
  }
  ...
  for(const i of starts){
    ...
    if(arrivalTime)candidates.push({arrivalTime,score:(new RegExp(`\\b${f}\\b`,'i').test(c)?3:0)+6,snippet:c.slice(0,1300)});
  }
  if(candidates.length){candidates.sort((a,b)=>b.score-a.score);return{status:'ARRIVED',arrivalDate:date,arrivalTime:candidates[0].arrivalTime,arrivalIsActual:true,source:'Flight-status screenshot fallback',debugPattern:'TARGET_DATE_DESTINATION_SNIPPET',debugSnippet:candidates[0].snippet};}
  return null;
}
