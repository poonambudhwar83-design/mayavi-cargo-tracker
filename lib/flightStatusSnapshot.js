...
async function flightStatsDepartureFallback({flightNo='',date='',destination=''}){
    ...
    if(idx>=0){
      ...
      if(!shown||shown.toLowerCase()===targetLabel.toLowerCase()){
        ...
        if(departureTime&&(!wantedDestination||!departureDestination||departureDestination===wantedDestination)){
          return{status:actual?'DEPARTED':'SCHEDULED',departureDate:date,departureTime,departureIsActual:Boolean(actual),departureOrigin,departureDestination:departureDestination||wantedDestination,source:'FlightStats departure fallback',debugPattern:'FLIGHTSTATS_DEPARTURE',debugSnippet:block.slice(0,900)};
        }
      }
    }
...
async function flightInfoScheduleFallback({flightNo='',date='',destination=''}){
  ...
  const parsePlain=(raw='')=>{
    ...
    if(m){const departureTime=normalizeTime(m[1]);if(departureTime&&(!wantedDestination||m[3].toUpperCase()===wantedDestination))return{status:'SCHEDULED',departureDate:date,departureTime,departureIsActual:false,departureOrigin:m[2].toUpperCase(),departureDestination:m[3].toUpperCase(),source:'Flight.info dated schedule fallback',debugPattern:'FLIGHTINFO_DATED_SCHEDULE',debugSnippet:m[0].slice(0,500)};}
    if(f.startsWith('TK')){
