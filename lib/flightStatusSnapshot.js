...
async function flightRadarScheduleFallback({flightNo='',date='',destination=''}){
  ...
  try{
    ...
    const at=plain.toLowerCase().indexOf(label.toLowerCase());
    if(at<0)return null;
    const block=plain.slice(Math.max(0,at-120),Math.min(plain.length,at+700));
    ...
    if(wantedDestination&&depDestination&&depDestination!==wantedDestination)return null;
    ...
    return{
      ...
      debugPattern:'FLIGHTRADAR24_SCHEDULE',
