...
function plusDays(date='',days=1){
  const m=String(date||'').match(/^(20\d{2})-(\d{2})-(\d{2})$/);if(!m)return date;
  const d=new Date(Date.UTC(Number(m[1]),Number(m[2])-1,Number(m[3])+days));
  return `${d.getUTCFullYear()}-${pad(d.getUTCMonth()+1)}-${pad(d.getUTCDate())}`;
}
function scheduleArrivalDate(date='',departureTime='',arrivalTime='',origin='',destination=''){
  if(!date||!departureTime||!arrivalTime||!origin||!destination)return date;
  const dep=localAirportTimeToIst(date,departureTime,origin);
  let arrDate=date,arr=localAirportTimeToIst(arrDate,arrivalTime,destination);
  if(!dep||!arr)return date;
  const depStamp=Date.parse(`${dep.date}T${dep.time}:00Z`);
  let arrStamp=Date.parse(`${arr.date}T${arr.time}:00Z`);
  if(Number.isFinite(depStamp)&&Number.isFinite(arrStamp)&&arrStamp<depStamp){
    arrDate=plusDays(date,1);
  }
  return arrDate;
}
async function routeScheduleFallback({flightNo='',date='',origin='',destination=''}){
  ...
  const fm=f.match(/^([A-Z0-9]{2})0*(\d{1,4})$/);
  if(!fm||!dm||!/^[A-Z]{3}$/.test(o)||!/^[A-Z]{3}$/.test(d))return null;
  const url=`https://2lnr.com/routes/${o.toLowerCase()}-${d.toLowerCase()}`;
  try{
    const r=await fetch(url,{headers:{'user-agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/149 Safari/537.36','accept':'text/html,application/xhtml+xml','accept-language':'en-US,en;q=0.9'},cache:'no-store',redirect:'follow',signal:AbortSignal.timeout(9000)});
    if(!r.ok)return null;
    const plain=cleanHtml(await r.text());
    const month=MONTHS[Number(dm[2])-1],day=Number(dm[3]);
    const dateRx=`(?:Sun|Mon|Tue|Wed|Thu|Fri|Sat)?\\s*,?\\s*${escapeRx(month)}\\s+${day}(?:\\s+${dm[1]})?`;
    const flightRx=`${escapeRx(fm[1])}\\s*0*${Number(fm[2])}`;
    const rowRx=new RegExp(`${dateRx}[\\s\\S]{0,100}?\\b${flightRx}\\b[\\s\\S]{0,120}?(\\d{1,2}:\\d{2})[\\s\\S]{0,70}?(\\d{1,2}:\\d{2})`,'i');
    const row=plain.match(rowRx);if(!row)return null;
    const departureTime=normalizeTime(row[1]),arrivalTime=normalizeTime(row[2]);
    if(!departureTime||!arrivalTime)return null;
    const arrivalDate=scheduleArrivalDate(date,departureTime,arrivalTime,o,d);
    return{
      status:'SCHEDULED',
      departureDate:date,
      departureTime,
      ...
      departureOrigin:o,
      departureDestination:d,
      departureTimeZone:'LOCAL',
      scheduledArrivalDate:arrivalDate,
      scheduledArrivalTime:arrivalTime,
      scheduledArrivalTimeZone:'LOCAL',
      arrivalDate,
      arrivalTime,
      arrivalIsActual:false,
      arrivalTimeZone:'LOCAL',
      arrivalTimeSource:'Published route schedule (airport local time)',
      source:'Published route schedule fallback',
      debugPattern:'ROUTE_SCHEDULE_MATCH',
      debugSnippet:row[0].slice(0,700),
      url
    };
  }catch{return null;}
}
  ...
  const parsePlain=(raw='')=>{
    ...
    for(const m of plain.matchAll(rx)){
      ...
      const pairRx=new RegExp(
        `${escapeRx(departureTime)}\\s+[^()]{0,120}\\(${escapeRx(departureOrigin)}\\)[^0-9]{0,40}(\\d{1,2}:\\d{2})\\s+[^()]{0,120}\\(${escapeRx(departureDestination)}\\)`,
        'i'
      );
...
export async function trackFlightScheduleFast({flightNo='',date='',origin='',destination=''}){
  ...
  const hasArrival=Boolean(stats?.arrivalTime||stats?.scheduledArrivalTime||schedule?.arrivalTime||schedule?.scheduledArrivalTime||radar?.arrivalTime||radar?.scheduledArrivalTime);
  if(!hasArrival&&origin&&destination){
    route=await routeScheduleFallback({flightNo,date,origin,destination}).catch(()=>null);
  }
  const result=stats||schedule||radar||route;
  if(!result)return{ok:false,reason:'NO FAST FLIGHT SCHEDULE DATA'};
  const arrivalSource=stats?.arrivalTime?stats:schedule?.arrivalTime||schedule?.scheduledArrivalTime?schedule:radar?.arrivalTime||radar?.scheduledArrivalTime?radar:route?.arrivalTime?route:null;
  const merged={...route,...radar,...schedule,...stats};
  if(arrivalSource){
    merged.arrivalDate=arrivalSource.arrivalDate||arrivalSource.scheduledArrivalDate||'';
    merged.arrivalTime=arrivalSource.arrivalTime||arrivalSource.scheduledArrivalTime||'';
    merged.arrivalIsActual=arrivalSource.arrivalIsActual===true;
    merged.arrivalTimeZone=arrivalSource.arrivalTimeZone||arrivalSource.scheduledArrivalTimeZone||'';
    merged.arrivalTimeSource=arrivalSource.arrivalTimeSource||arrivalSource.source||'Fast flight schedule';
    merged.scheduledArrivalDate=arrivalSource.scheduledArrivalDate||arrivalSource.arrivalDate||'';
    merged.scheduledArrivalTime=arrivalSource.scheduledArrivalTime||arrivalSource.arrivalTime||'';
    merged.scheduledArrivalTimeZone=arrivalSource.scheduledArrivalTimeZone||arrivalSource.arrivalTimeZone||'';
  }
  return{ok:true,...merged,source:[stats?.source,schedule?.source,radar?.source,route?.source].filter(Boolean).join(' + ')||'Fast flight schedule'};
}
