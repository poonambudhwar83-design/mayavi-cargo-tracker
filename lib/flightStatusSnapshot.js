...
async function flightStatsDepartureFallback({flightNo='',date=''}){
  ...
  try{
    ...
    const before=plain.slice(Math.max(0,idx-500),idx+100);
    const block=plain.slice(idx,idx+1000);
    const airportMatches=[...before.matchAll(/\b([A-Z]{3})\b/g)].map(m=>m[1]).filter(x=>!['THE','AND','FOR'].includes(x));
    const departureOrigin=airportMatches.at(-1)||'';
    ...
    return{status:actual?'DEPARTED':'SCHEDULED',departureDate:date,departureTime,departureIsActual:Boolean(actual),departureOrigin,source:'FlightStats departure fallback',debugPattern:'FLIGHTSTATS_DEPARTURE',debugSnippet:block.slice(0,900),url};
  }catch{return null;}
}
