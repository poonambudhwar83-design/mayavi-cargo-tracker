...
async function flightInfoScheduleFallback({flightNo='',date='',destination=''}){
  ...
  const parsePlain=(raw='')=>{
    ...
    if(m){const departureTime=normalizeTime(m[1]),scheduledArrivalTime=normalizeTime(m[4]);if(departureTime&&(!wantedDestination||m[3].toUpperCase()===wantedDestination))return{status:'SCHEDULED',departureDate:date,departureTime,departureIsActual:false,departureOrigin:m[2].toUpperCase(),departureDestination:m[3].toUpperCase(),scheduledArrivalDate:date,scheduledArrivalTime,source:'Flight.info dated schedule fallback',debugPattern:'FLIGHTINFO_DATED_SCHEDULE',debugSnippet:m[0].slice(0,500)};}
    if(f.startsWith('TK')){
...
export async function trackFlightScheduleFast({flightNo='',date='',destination=''}){
  ...
  const result=stats||schedule;
  if(!result)return{ok:false,reason:'NO FAST FLIGHT SCHEDULE DATA'};
  return{ok:true,...schedule,...stats,source:[stats?.source,schedule?.source].filter(Boolean).join(' + ')||'Fast flight schedule'};
}
