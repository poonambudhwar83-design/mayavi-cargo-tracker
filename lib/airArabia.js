...
async function chooseAwbType(frame){
  try{
    const chosen=await frame.evaluate(()=>{
      ...
      for(const sel of selects){
        const options=[...sel.options];const opt=options.find(o=>/\b(?:awb|mawb|air\s*waybill)\b/i.test(`${o.textContent||''} ${o.value||''}`))||options.find(o=>o.value&&!/^\s*(?:select|choose)/i.test(o.textContent||''));
  ...
  try{
    const opened=await frame.evaluate(()=>{
      ...
      const target=els.find(e=>/select|awb|air\s*waybill|tracking/i.test(String(e.innerText||e.textContent||e.getAttribute('aria-label')||'')))||els[0];
      if(!target)return false;target.click();return true;
    });
    if(opened){await sleep(250);const picked=await frame.evaluate(()=>{const visible=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>4&&r.height>4&&s.display!=='none'&&s.visibility!=='hidden'};const options=[...document.querySelectorAll('[role="option"],li,.select2-results__option,.choices__item')].filter(visible);const e=options.find(x=>/\b(?:awb|mawb|air\s*waybill)\b/i.test(String(x.innerText||x.textContent||'')));if(!e)return'';const t=String(e.innerText||e.textContent||'').trim();e.click();return t;}).catch(()=>'');if(picked)return picked;}
  }catch{}
  return'';
}
  ...
  for(const frame of page.frames()){
    try{
      const t=await frame.evaluate(()=>{
        ...
        const x=candidates.find(o=>/^next$/i.test(o.t))||candidates.find(o=>/view\s+details|shipment\s+details|more\s+details|continue/i.test(o.t));
...
export async function trackAirArabia(mawb){
  ...
  try{
    ...
    // Exact required flow: Air Arabia link -> choose AWB -> enter MAWB -> Submit -> Next/details -> screenshot -> extract fields.
    const form=await fillAndSubmit(page,mawb);
    if(!form.filled)return{ok:false,reason:'AIR ARABIA TRACKING INPUT NOT FOUND',officialTracker:START,debug:{stage:'input'}};
    await Promise.race([page.waitForNetworkIdle({idleTime:900,timeout:9000}).catch(()=>{}),sleep(9000)]);await sleep(1000);
    let resultText=await allText(page);
    ...
    const serial=digits.slice(3);let nextClicked='';
    ...
    if(!details.awbMatched){
      return{ok:false,reason:'AIR ARABIA RESULT DID NOT MATCH MAWB',officialTracker:START,debug:{stage:'details',form,nextClicked,resultUrl:page.url(),bodySample:resultText.slice(0,3500)}};
    }
    const screenshot=await page.screenshot({type:'jpeg',quality:82,fullPage:false,encoding:'base64'}).catch(()=>null);
    if(!screenshot)return{ok:false,reason:'AIR ARABIA DETAILS SCREENSHOT FAILED',officialTracker:START,debug:{stage:'screenshot',form,nextClicked}};
    ...
    if(!useful(details))return{ok:false,reason:'AIR ARABIA DETAILS FOUND NO VERIFIED SHIPMENT FIELDS',officialTracker:START,screenshotCaptured:true,screenshotVerified:true,debug:{stage:'parse',form,nextClicked,resultUrl:page.url(),bodySample:resultText.slice(0,5000)}};
    return{ok:true,shipment:details,screenshotCaptured:true,screenshotVerified:true,screenshotOcrUsed:false,debug:{stage:'done',startUrl:START,resultUrl:page.url(),form,nextClicked:nextClicked||'',awbMatched:true,screenshot:true,arrivalEvidence:details.arrivalEvidence||'',bodySample:resultText.slice(0,4500)}};
