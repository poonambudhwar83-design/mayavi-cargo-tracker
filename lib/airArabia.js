...
async function fillAndSubmit(page,mawb){
  ...
  for(const frame of page.frames()){
    ...
    const value=/^\s*514\s*$/.test(selected)?serial:(max===8?serial:(max===11?digits:formatted));
...
export async function trackAirArabia(mawb){
  ...
  try{
    ...
    let resultText=await allText(page);const preNextSample=resultText.slice(0,4500);
    ...
    if(!details.awbMatched){
      return{ok:false,reason:'AIR ARABIA RESULT DID NOT MATCH MAWB',officialTracker:START,debug:{stage:'details',form,nextClicked,preNextSample,resultUrl:page.url(),bodySample:resultText.slice(0,4500)}};
    }
    ...
    if(!useful(details))return{ok:false,reason:'AIR ARABIA DETAILS FOUND NO VERIFIED SHIPMENT FIELDS',officialTracker:START,screenshotCaptured:true,screenshotVerified:true,debug:{stage:'parse',form,nextClicked,preNextSample,resultUrl:page.url(),bodySample:resultText.slice(0,5000)}};
