...
function parseResult(raw,mawb){
  ...
  const route=upper.match(/\bORIGIN\b\s*[:\-]?\s*([A-Z]{3})\b[\s\S]{0,180}?\bDESTINATION\b\s*[:\-]?\s*([A-Z]{3})\b/)
  ...
  const arrivalBlock=(text.match(/(?:Actual Arrival|Arrived|Arrival|ATA)[\s\S]{0,260}/i)||[])[0]||'';
  const etaBlock=(text.match(/(?:Estimated Arrival|Expected Arrival|Scheduled Arrival|ETA)[\s\S]{0,260}/i)||[])[0]||'';
  ...
  if((!origin||!destination)&&flightNo){
    const fm=upper.indexOf(flightNo);if(fm>=0){const w=upper.slice(Math.max(0,fm-220),fm+360);const codes=[...w.matchAll(/\b[A-Z]{3}\b/g)].map(x=>x[0]).filter(x=>!['AWB','PCS','KGS'].includes(x));if(codes.length>=2){origin=origin||codes[0];destination=destination||codes[codes.length-1];}}
  }
...
export async function trackSaudiaWithBrowser(input){
  ...
  try{
    ...
    const setup=await page.evaluate((mawb)=>{
      ...
      if(!awb)return{filled:false,clicked:''};set(awb,mawb);
      const els=[...document.querySelectorAll('button,input[type="submit"],[role="button"],a')].filter(visible);
      const txt=e=>(e.innerText||e.value||e.getAttribute('aria-label')||e.title||'').replace(/\s+/g,' ').trim();
      const btn=els.find(e=>/track\s*shipment/i.test(txt(e))||/追踪货物/.test(txt(e)))||els.find(e=>/货物追踪/.test(txt(e))&&e.tagName==='BUTTON');
      if(btn){const t=txt(btn);btn.click();return{filled:true,clicked:t};}return{filled:true,clicked:''};
    },mawb);
    if(!setup.clicked)return{ok:false,reason:'SAUDIA TRACK SHIPMENT BUTTON NOT FOUND',officialTracker:URL,debug:{stage:'NO_BUTTON',setup}};
    ...
    const segmentClick=await page.evaluate(()=>{
      ...
      const els=[...document.querySelectorAll('button,[role="button"],a,tr')].filter(visible);
      const txt=e=>(e.innerText||e.getAttribute('aria-label')||e.title||'').replace(/\s+/g,' ').trim();
      const cands=els.map(e=>({e,t:txt(e)})).filter(x=>/segment|航段/i.test(x.t)||/\bSV[-\s]?\d{2,4}\b/i.test(x.t));
      if(!cands.length)return'';
      const target=cands[cands.length-1];target.e.click();return target.t.slice(0,180);
    }).catch(()=> '');
    if(segmentClick){await new Promise(r=>setTimeout(r,1500));await page.waitForNetworkIdle({idleTime:700,timeout:5000}).catch(()=>{});}
    ...
    return{ok:false,reason:'SAUDIA SEGMENT RESULT OPENED BUT NO VERIFIED FIELDS FOUND',officialTracker:URL,screenshotBase64,debug:{stage:'NO_FIELDS',setup,segmentClick,pageText:translate(text).slice(0,9000)}};
