...
function parseDate(s=''){
  ...
  m=x.match(/\b(\d{1,2})[-\s]+(JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)[A-Z]*[-\s,]+(\d{2})\b/);
  if(m)return `20${m[3]}-${MONTHS[m[2]]}-${pad(m[1])}`;
  return '';
}
...
function shipmentHeaderStatus(flat='',mawb=''){
  ...
  const window=idx>=0?flat.slice(idx,idx+650):flat.slice(0,850);
  if(/\bDelivered\b/i.test(window))return 'DELIVERED';
  if(/\bDelayed\b|\bLate\b|Offload|Exception/i.test(window))return 'DELAYED';
  if(/\bDeparted\b|\bAirborne\b|\bDEP\b/i.test(window))return 'DEPARTED';
  if(/\bIn Transit\b/i.test(window))return 'IN TRANSIT';
  if(/\bArrived\b/i.test(window)&&!/Estimated\s+Arrival|Scheduled\s+Arrival/i.test(window))return 'ARRIVED';
...
function scheduledArrival(flat='',destination=''){
  ...
  const rx=new RegExp(`(?:Estimated|Scheduled)\\s+Arrival\\s*\\(\\s*${esc}\\s*\\)\\s*([^|]{0,100})`,'i');
...
function actualDestinationArrival(flat='',destination=''){
  ...
  const arrivedRx=new RegExp(`Arrived\\s+in\\s+${esc}\\s+on\\s+QR\\s*[- ]?(\\d{2,4})\\s*\\/\\s*(\\d{1,2}[-\\s]${mon}[A-Za-z]*[-\\s]20\\d{2})[\\s\\S]{0,240}?${day},?\\s*(\\d{1,2}\\s+${mon}[A-Za-z]*\\s+20\\d{2})\\s+(\\d{1,2}:\\d{2})`,'ig');
  const arrived=[...flat.matchAll(arrivedRx)];
  if(arrived.length){
    const m=arrived.at(-1);
    return{arrivalDate:parseDate(m[3])||parseDate(m[2]),arrivalTime:parseTime(m[4]),arrivalIsActual:true,source:'Qatar destination actual arrival event',snippet:m[0],index:m.index||0};
  }
  const fallbackRx=new RegExp(`(?:Received\\s+in|Delivered\\s+in)\\s+${esc}\\b[\\s\\S]{0,220}?${day},?\\s*(\\d{1,2}\\s+${mon}[A-Za-z]*\\s+20\\d{2})\\s+(\\d{1,2}:\\d{2})`,'ig');
  const fallback=[...flat.matchAll(fallbackRx)].map(m=>({arrivalDate:parseDate(m[1]),arrivalTime:parseTime(m[2]),arrivalIsActual:true,source:/Delivered/i.test(m[0])?'Qatar delivered-event arrival fallback':'Qatar received-at-destination arrival fallback',snippet:m[0],index:m.index||0}));
  return fallback.filter(x=>x.arrivalDate||x.arrivalTime).sort((a,b)=>a.index-b.index).at(-1)||null;
}

function actualOriginDeparture(flat='',origin=''){
  if(!origin)return null;
  const esc=origin.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
  const mon='(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)';
  const day='(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)[a-z]*';
  const rx=new RegExp(`Departed\\s+from\\s+${esc}\\s+on\\s+QR\\s*[- ]?(\\d{2,4})\\s*\\/\\s*(\\d{1,2}[-\\s]${mon}[A-Za-z]*[-\\s]20\\d{2})[\\s\\S]{0,240}?${day},?\\s*(\\d{1,2}\\s+${mon}[A-Za-z]*\\s+20\\d{2})\\s+(\\d{1,2}:\\d{2})`,'ig');
  const matches=[...flat.matchAll(rx)];
  if(!matches.length)return null;
  const m=matches.at(-1);
  return{departureDate:parseDate(m[3])||parseDate(m[2]),departureTime:parseTime(m[4]),departureFlightNo:`QR${m[1].padStart(4,'0')}`,departureOrigin:origin.toUpperCase(),departureDestination:'',departureIsActual:true,source:'Qatar origin actual departure event',snippet:m[0]};
}

function scheduledOriginDeparture(flat='',origin=''){
  if(!origin)return null;
  const esc=origin.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
  const mon='(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)';
  const rx=new RegExp(`\\b${esc}\\b\\s+([A-Z]{3})(?:\\s+[A-Z]{3}){0,2}\\s+(\\d{1,2}[-\\s]${mon}[-\\s](?:20\\d{2}|\\d{2}))\\s+(\\d{1,2}:\\d{2})[\\s\\S]{0,160}?\\bQR\\s*[- ]?(\\d{2,4})\\b`,'i');
  const m=flat.match(rx);if(!m)return null;
  const departureDate=parseDate(m[2]),departureTime=parseTime(m[3]);
  if(!departureDate&&!departureTime)return null;
  return{departureDate,departureTime,departureFlightNo:`QR${m[4].padStart(4,'0')}`,departureOrigin:origin.toUpperCase(),departureDestination:m[1].toUpperCase(),departureIsActual:false,source:'Qatar origin scheduled departure',snippet:m[0]};
}
...
function parseShipment(text,mawb){
  ...
  const headerStatus=shipmentHeaderStatus(flat,mawb);
  const sched=scheduledArrival(flat,destination);
  let actual=actualDestinationArrival(flat,destination);

  // Qatar rule: the official header is authoritative. If it clearly says IN TRANSIT,
  // an Estimated/Scheduled Arrival at DEL is never an actual arrival event.
  if(headerStatus==='IN TRANSIT')actual=null;

  const arrival=actual||sched||{arrivalDate:'',arrivalTime:'',arrivalIsActual:false,source:'',snippet:''};
  let status=headerStatus;
  if(headerStatus==='DELIVERED')status='DELIVERED';
  else if(actual)status='ARRIVED';
  else if(headerStatus==='DEPARTED')status='DEPARTED';
  else if(headerStatus==='IN TRANSIT')status='IN TRANSIT';
  else if(status==='ARRIVED'&&!arrival.arrivalIsActual)status='IN TRANSIT';

  const flightNo=finalFlightNo(flat,destination);
  const scheduledDeparture=scheduledOriginDeparture(flat,origin);
  const actualDeparture=actualOriginDeparture(flat,origin);
  const departure=actualDeparture?{...(scheduledDeparture||{}),...actualDeparture,departureDestination:scheduledDeparture?.departureDestination||actualDeparture.departureDestination||''}:(scheduledDeparture||{});
  ...
  return {
    ...
    departureDate:departure.departureDate||'',departureTime:departure.departureTime||'',departureFlightNo:departure.departureFlightNo||'',departureOrigin:departure.departureOrigin||origin||'',departureDestination:'',departureIsActual:departure.departureIsActual===true,departureTimeSource:departure.source||'',
    ...
    status,source:'Qatar Cargo dedicated official browser adapter',departureEvidence:departure.snippet||'',arrivalEvidence:arrival.snippet
  };
}
...
export async function trackQatar(input){
  ...
  try{
    ...
    if(useful(shipment))return{ok:true,airline:AIRLINE,shipment,screenshotCaptured:Boolean(screenshotBase64),screenshotVerified:Boolean(screenshotBase64),screenshotBase64,debug:{stage:'SUCCESS',url,setup,clicked,departureEvidence:shipment.departureEvidence||'',arrivalEvidence:shipment.arrivalEvidence||'',textSample:clean(text).slice(0,5000)}};
