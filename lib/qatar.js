...
export async function trackQatar(input){
  ...
  try{
    ...
    if(!String(text).replace(/\D/g,'').includes(serial)){
      const filled=await page.evaluate(({serial})=>{const visible=e=>{const s=getComputedStyle(e),r=e.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&r.width>20&&r.height>10&&!e.disabled};const inputs=[...document.querySelectorAll('input')].filter(visible);const set=(e,v)=>{const d=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value');d?.set?.call(e,v);e.dispatchEvent(new Event('input',{bubbles:true}));e.dispatchEvent(new Event('change',{bubbles:true}));};const prefix=inputs.find(e=>/prefix/i.test(`${e.name} ${e.id} ${e.placeholder} ${e.getAttribute('aria-label')||''}`)),num=inputs.find(e=>/document|awb|airway|shipment/i.test(`${e.name} ${e.id} ${e.placeholder} ${e.getAttribute('aria-label')||''}`)&&e!==prefix);if(prefix)set(prefix,'157');if(num)set(num,serial);return Boolean(num);},{serial}).catch(()=>false);
      if(filled){await page.evaluate(()=>{const els=[...document.querySelectorAll('button,input[type="submit"],a,[role="button"]')];const b=els.find(e=>/track|search|submit/i.test((e.innerText||e.value||e.getAttribute('aria-label')||'').trim()));if(b)b.click();}).catch(()=>{});await new Promise(r=>setTimeout(r,4500));text=await page.evaluate(()=>document.body?.innerText||'').catch(()=> '');}
    }
    if(/captcha|verify you are human|security check|cloudflare|turnstile|access denied/i.test(text))return{ok:false,reason:'QATAR SECURITY CHECK REQUIRES MANUAL CHECK',airline:AIRLINE,officialTracker:url,debug:{stage:'CAPTCHA'}};
    ...
    if(useful(shipment))return{ok:true,airline:AIRLINE,shipment,screenshotCaptured:Boolean(screenshotBase64),screenshotVerified:Boolean(screenshotBase64),screenshotBase64,debug:{stage:'SUCCESS',url,arrivalEvidence:shipment.arrivalEvidence||'',textSample:clean(text).slice(0,5000)}};
    return{ok:false,reason:'QATAR OFFICIAL PAGE RETURNED NO VERIFIED SHIPMENT DATA',airline:AIRLINE,officialTracker:url,screenshotBase64,debug:{stage:'NO_DATA',textSample:clean(text).slice(0,5000)}};
