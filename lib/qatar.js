...
function destinationFlightNo(text=''){
  const flat=clean(text);
  const startMatch=/(?:Estimated Arrival|Arrived\s*\(|Received at Destination|Awaiting Confirmation)/i.exec(flat);
  const rest=flat.slice(startMatch?.index||0);
  const endMatch=/(?:Tracking Details|Shipment not received|Flight Schedules information|Book your next shipment)/i.exec(rest);
  const block=endMatch?rest.slice(0,endMatch.index):rest.slice(0,1800);
  const matches=[...block.matchAll(/\bQR\s*[- ]?(\d{2,4})\b/gi)];
  if(matches.length)return`QR${matches[matches.length-1][1]}`;
  const all=[...flat.matchAll(/\bQR\s*[- ]?(\d{2,4})\b/gi)];
  return all.length?`QR${all[0][1]}`:'';
}
function parseShipment(text,mawb){
  ...
  const flightNo=destinationFlightNo(flat);
  const arr=arrivalFromText(flat,flightNo);let status=statusFromText(flat);if(arr.status)status=arr.status;
  if(!arr.arrivalIsActual&&status!=='DELIVERED'&&status!=='DELAYED'&&((arr.arrivalDate||arr.arrivalTime)||/\bESTIMATED ARRIVAL\b|\bSCHEDULED ARRIVAL\b|AWAITING CONFIRMATION/.test(upper)))status='IN TRANSIT';
  if(status==='ARRIVED'&&!arr.arrivalIsActual)status='IN TRANSIT';
...
export async function trackQatar(input){
  ...
  try{
    ...
    if(!clicked){
      const selectors=['button::-p-text(Track Shipment(s))','::-p-aria(Track Shipment(s))','::-p-text(Track Shipment(s))'];
      for(const selector of selectors){
        try{await page.locator(selector).click({timeout:3500});clicked=`shadow:${selector}`;break;}catch{}
      }
    }

    if(clicked){await Promise.race([page.waitForNetworkIdle({idleTime:800,timeout:10000}).catch(()=>{}),new Promise(r=>setTimeout(r,10000))]);await new Promise(r=>setTimeout(r,1400));}
    ...
    if(!shipment.arrivalIsActual&&(shipment.arrivalDate||shipment.arrivalTime)&&shipment.status!=='DELAYED'&&shipment.status!=='DELIVERED')shipment.status='IN TRANSIT';
