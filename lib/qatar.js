...
function actualDestinationArrival(flat='',destination=''){
  ...
  const esc=destination.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
  const patterns=[
    new RegExp(`(?:Arrived|Landed|Received from flight|RCF|Delivered)\\s*(?:at|in)?\\s*${esc}\\b[^.!]{0,140}`,'ig'),
    new RegExp(`\\b${esc}\\b[^.!]{0,35}(?:Arrived|Landed|Received from flight|RCF|Delivered)\\b[^.!]{0,140}`,'ig')
  ];
  const candidates=[];
  for(const rx of patterns){
    for(const m of flat.matchAll(rx)){
      const s=m[0];
      if(/Estimated\s+Arrival|Scheduled\s+Arrival|ETA\b/i.test(s))continue;
      const d=parseDate(s),t=parseTime(s);
      if(d||t)candidates.push({arrivalDate:d,arrivalTime:t,arrivalIsActual:true,source:'Qatar destination actual arrival event',snippet:s,index:m.index||0});
    }
  }
...
function actualOriginDeparture(flat='',origin=''){
  ...
  const esc=origin.replace(/[.*+?^${}()|[\]\\]/g,'\\function finalFlightNo(flat='',destination=''){');
  const rx=new RegExp(`Departed\\s+from\\s+${esc}\\s+on\\s+QR\\s*[- ]?(\\d{2,4})\\s*\\/\\s*(\\d{1,2}[-\\s](?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[A-Za-z]*[-\\s]20\\d{2})[^.!]{0,180}?(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)?[a-z]*,?\\s*(\\d{1,2}\\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[A-Za-z]*\\s+20\\d{2})\\s+(\\d{1,2}:\\d{2})`,'ig');
  const matches=[...flat.matchAll(rx)];
  if(matches.length){
    const m=matches.at(-1);
    return{departureDate:parseDate(m[3])||parseDate(m[2]),departureTime:parseTime(m[4]),departureFlightNo:`QR${m[1].padStart(4,'0')}`,departureOrigin:origin.toUpperCase(),departureIsActual:true,source:'Qatar origin actual departure event',snippet:m[0]};
  }
  const simpler=new RegExp(`Departed\\s+from\\s+${esc}\\s+on\\s+QR\\s*[- ]?(\\d{2,4})[^.!]{0,220}`,'ig');
  const candidates=[];
  for(const m of flat.matchAll(simpler)){
    const d=parseDate(m[0]),t=parseTime(m[0]);
    if(d||t)candidates.push({departureDate:d,departureTime:t,departureFlightNo:`QR${m[1].padStart(4,'0')}`,departureOrigin:origin.toUpperCase(),departureIsActual:true,source:'Qatar origin actual departure event',snippet:m[0],index:m.index||0});
  }
  return candidates.sort((a,b)=>a.index-b.index).at(-1)||null;
}
...
function parseShipment(text,mawb){
  ...
  const departure=actualOriginDeparture(flat,origin)||{};
