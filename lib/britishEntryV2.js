...
function packageWeight(text=''){
  const s=String(text);
  const p=s.match(/\b(\d{1,6})\s*(?:Package\/s|Packages?|Piece\/s|Pieces?|PCS)\b/i);
  if(!p)return{pieces:'',weight:''};
  // A number after Package/s is not a weight unless IAG labels it kg/kgs.
  // This prevents "25 Package/s 0.00 Total" from becoming a false 0 kg.
  const w=s.match(/\b([\d,.]+)\s*(?:kg|kgs)\b/i);
  return{pieces:p[1]||'',weight:String(w?.[1]||'').replace(/,/g,'')};
}
...
function inferJourney(routes=[],header=null){
  // The shipment header is the master route (for example DEL -> JFK).
  // Individual movement rows may currently contain only the first leg (DEL -> LHR).
  if(header?.origin&&header?.destination)return{origin:header.origin,destination:header.destination};
