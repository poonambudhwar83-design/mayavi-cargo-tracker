...
function routeFlightLegs(text='',events=[]){
  const fallbackYear=(events.find(e=>/^20\d{2}-/.test(String(e?.date||'')))?.date||String(new Date().getUTCFullYear())).slice(0,4);
  const out=[];
  const rx=/\b(VN\s?\d{2,4})\s*\/\s*(\d{1,2})(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\b/gi;
  for(const m of String(text||'').matchAll(rx)){
    const mon=MONTH[String(m[3]||'').slice(0,3).toUpperCase()]||'';
    if(!mon)continue;
    const row={flightNo:String(m[1]).replace(/\s+/g,'').toUpperCase(),flightDate:`${fallbackYear}-${mon}-${pad(m[2])}`,raw:m[0]};
    if(!out.some(x=>x.flightNo===row.flightNo&&x.flightDate===row.flightDate))out.push(row);
  }
  return out;
}
...
function parseEvents(text=''){
  ...
  const events=[];
  const labels='Booked|Booking On Hold|Received from shipper|Pre-manifested on flight|Manifested on flight|Transferred to another flight|Departed|Flight departed|Arrived|Flight arrived|Received from flight|Physically received from|Consignee\\/?Agent notified of arrival|Delivered';
...
function statusFromLatest(label=''){
  ...
  if(/transferred to another flight|depart/i.test(label))return'IN TRANSIT';
...
function buildShipment(text,mawb){
  ...
  const routeFlights=routeFlightLegs(text,events);
  const routeFinal=routeFlights.at(-1)||{};
  ...
  const shipment={mawb,carrierCode:'VN',airlineName:'Vietnam Airlines Cargo',officialTracker:TRACK_URL,origin:route.origin,destination:route.destination,bags:latestQty.pieces||'',pieces:latestQty.pieces||'',weight:latestQty.weight||'',flightNo:currentFlight.flightNo||departed?.flightNo||routeFinal.flightNo||'',flightDate:currentFlight.flightDate||departed?.flightDate||routeFinal.flightDate||'',finalFlightNo:routeFinal.flightNo||currentFlight.flightNo||'',finalFlightDate:routeFinal.flightDate||currentFlight.flightDate||'',finalFlightDestination:route.destination||'',bookingDate:booked[0]?.date||'',departureDate:departed?.date||'',departureTime:departed?.time||'',departureFlightNo:departed?.flightNo||currentFlight.flightNo||'',departureOrigin:route.origin||'',departureDestination:route.destination||'',departureIsActual:Boolean(departed),departureTimeSource:departed?'Vietnam CHAMP final Departed event':'',arrivalDate:actual?.date||'',arrivalTime:actual?.time||'',arrivalIsActual:Boolean(actual),actualArrival:actual?.date?`${actual.date}T${actual.time||'00:00'}:00`:null,eta:null,status:statusFromLatest(latest.label),source:'Vietnam Airlines CHAMP official Track & Trace'};
  return{shipment,events,route,routeFlights,useful:Boolean(events.length||route.origin||route.destination||routeFlights.length)};
}
...
export async function trackVietnam(input){
  ...
  try{
    ...
    const text=await page.evaluate(()=>document.body?.innerText||''),built=buildShipment(text,mawb);lastDebug={stage:'CHAMP_RESULT',submitted,eventCount:built.events?.length||0,events:built.events?.slice(-12)||[],routeFlights:built.routeFlights||[],bodySample:text.slice(0,7000)};
