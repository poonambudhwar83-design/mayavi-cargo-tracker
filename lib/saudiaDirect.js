...
function redactPostData(raw=''){
  ...
  try{
    ...
    for(const k of Object.keys(j))if(/captcha|recaptcha|token/i.test(k))j[k]='[REDACTED]';
...
export async function trackSaudiaDirect(input){
  ...
  try{
    ...
    for(const value of formats){
      ...
      attempts.push({value,clicked:clicked||'enter',invalidCaptcha,eventCount:parsed?.events?.length||0,networkUrls:networkUrls.filter(x=>/\/apis\/api\/eservices\/track-shipment/i.test(x)).slice(-5),requestBody:requestBodies.slice(-1)[0]||'',textSample:translate(visible).slice(-2500)});
      if(parsed?.shipment){
        const screenshotBase64=await page.screenshot({type:'jpeg',quality:70,fullPage:false,encoding:'base64'}).catch(()=>null);
        return{ok:true,shipment:parsed.shipment,screenshotBase64,debug:{stage:'SUCCESS',attempts,eventCount:parsed.events.length,arrivalEvent:parsed.arrivalEvent?.text?.slice(0,700)||'',latestEvent:parsed.latestOperational?.text?.slice(0,700)||''}};
      }
