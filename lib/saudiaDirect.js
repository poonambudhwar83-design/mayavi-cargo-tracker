...
function normalizeResult(result){
  if(result?.ok&&result?.shipment){
    ...
    const partPending=shipment.isPartLoad===true||current==='PART ARRIVED';

    // Saudia part-load rule: while any part is still pending, Mayavi must stay IN TRANSIT.
    // Only after the complete load has arrived can the shipment become ARRIVED.
    if(partPending){
...
function isActiveMovement(shipment={}){
  ...
  if(shipment.isPartLoad===true||status==='PART ARRIVED')return true;
...
function isTerminalMovement(shipment={}){
  const status=String(shipment.status||'').trim().toUpperCase();
  if(shipment.isPartLoad===true||status==='PART ARRIVED')return false;
...
function chooseFreshest(results=[]){
  ...
  // Any source confirming a pending part load keeps the master IN TRANSIT.
  const pendingPart=good.filter(r=>r.shipment?.isPartLoad===true||String(r.shipment?.status||'').toUpperCase()==='PART ARRIVED');
    ...
    return {
      ...
      debug:{
        ...
        candidates:all.filter(r=>r?.ok&&r?.shipment).map(r=>({
          ...
          flightNo:r.shipment.flightNo||'',flightDate:r.shipment.flightDate||'',arrivalDate:r.shipment.arrivalDate||'',arrivalTime:r.shipment.arrivalTime||'',isPartLoad:r.shipment.isPartLoad===true
        }))
      }
    };
  }
