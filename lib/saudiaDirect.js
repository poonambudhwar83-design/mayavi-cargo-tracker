...
async function applyFowFlightRule(input,shipment={}){
  const s={...shipment};
  ...
  if(flightDest==='DEL'){
    try{
      const sal=await trackSaudiaViaSal(input);
      const live=sal?.shipment;
...
export async function trackSaudiaDirect(input){
  ...
  if(strong?.ok&&strong?.shipment){
    ...
    shipment=await applyFowFlightRule(input,shipment);
    return{...strong,shipment,officialTracker:PUBLIC_URL,debug:{...(strong.debug||{}),sourceSelection:'SAL_FULL_CHRONOLOGY_PRIMARY',fowFlightRule:true}};
  }
