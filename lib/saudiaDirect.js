...
function timelineEvents(raw=''){
  ...
  return dedup.map((a,i)=>({date:a.date,time:a.time,text:text.slice(a.index,i+1<dedup.length?dedup[i+1].index:Math.min(text.length,a.index+900)).trim(),source:a.source}));
}
function eventFields(event={}){
  const text=String(event.text||''),upper=text.toUpperCase();
  const flight=upper.match(/\bSV[-\s]?(\d{2,4})\b/);
  const dest=upper.match(/\bTO\b[\s\S]{0,180}?\(([A-Z]{3})\)/)||upper.match(/\bDESTINATION\b\s*[:\-]?\s*([A-Z]{3})\b/)||upper.match(/,\s*([A-Z]{3})\b/);
...
function parseResult(raw,mawb){
  ...
  const arrivalEvent=latest.find(e=>isArrivalEvent(e.text)&&(eventFields(e).destination===headerDestination||!headerDestination))||latest.find(e=>isArrivalEvent(e.text))||null;
  ...
  const verified=events.length>0||Boolean(arrivalDate&&arrivalTime&&destination)||Boolean(flightNo&&destination)||status!=='TRACKING';
  return verified?{shipment,events,arrivalEvent,latestOperational}:null;
}

function redactPostData(raw=''){
  let s=String(raw||'');
  try{const j=JSON.parse(s);for(const k of Object.keys(j))if(/captcha|recaptcha|token/i.test(k))j[k]='[REDACTED]';return JSON.stringify(j).slice(0,1200)}catch{}
  return s.replace(/((?:g-recaptcha-response|captcha|recaptcha|token)[^=&:\"]*[=:\"]+)[^&\"\s]+/gi,'$1[REDACTED]').slice(0,1200);
}
async function deepText(frame){
  return frame.evaluate(()=>{const parts=[];const add=root=>{try{const t=root?.innerText||root?.textContent||'';if(t)parts.push(t);for(const e of root?.querySelectorAll?.('*')||[]){if(e.shadowRoot)add(e.shadowRoot)}}catch{}};add(document.body);return parts.join('\n')}).catch(()=> '');
}
async function snapshotText(page){const out=[];for(const frame of page.frames()){const t=await deepText(frame);if(t)out.push(t)}return out.join('\n')}
async function crawlTimeline(page){
  ...
  for(let i=0;i<24;i++){
    ...
    await page.evaluate(()=>{
      ...
      for(const e of [...document.querySelectorAll('*')].filter(e=>e.scrollHeight>e.clientHeight+180)){try{e.scrollTop=Math.min(e.scrollHeight,e.scrollTop+Math.max(420,e.clientHeight*0.85))}catch{}}
    }).catch(()=>{});
    await new Promise(r=>setTimeout(r,260));if(stable>=5)break;
  }
  ...
  await new Promise(r=>setTimeout(r,500));captures.push(await snapshotText(page));return captures.join('\n');
}
...
async function clickResultPlus(page,mawb){
  const serial=String(mawb).replace(/\D/g,'').slice(3),digits=String(mawb).replace(/\D/g,'');
  const result=await page.evaluate(({serial,digits})=>{
    ...
    const text=e=>(e.innerText||e.textContent||e.getAttribute?.('aria-label')||e.getAttribute?.('title')||'').replace(/\s+/g,' ').trim();
    const all=[...document.querySelectorAll('body *')];
    let anchor=all.find(e=>visible(e)&&((text(e).replace(/\D/g,'').includes(digits))||(serial&&text(e).replace(/\D/g,'').includes(serial))));
    if(anchor){
      let card=anchor;
      for(let i=0;i<7&&card?.parentElement;i++,card=card.parentElement){const r=card.getBoundingClientRect();if(r.width>260&&r.height>45&&r.height<500){anchor=card;break}}
    }
    const cardRect=anchor?.getBoundingClientRect?.();
    const clickables=[...document.querySelectorAll('button,[role="button"],a,[tabindex],svg')].filter(visible).map(e=>{
      const clickable=e.closest('button,[role="button"],a,[tabindex]')||e;const r=clickable.getBoundingClientRect();const t=text(clickable);return{e:clickable,r,t};
    });
    let chosen=clickables.find(x=>/^\+$/.test(x.t)||/plus|expand|details|show more|open/i.test(`${x.t} ${x.e.getAttribute?.('aria-label')||''} ${x.e.getAttribute?.('title')||''}`));
    if(!chosen&&cardRect){
      chosen=clickables
        .filter(x=>x.r.width>=16&&x.r.width<=90&&x.r.height>=16&&x.r.height<=90&&x.r.left>=cardRect.left+cardRect.width*0.55&&x.r.right<=cardRect.right+90&&Math.abs((x.r.top+x.r.height/2)-(cardRect.top+cardRect.height/2))<Math.max(90,cardRect.height*.55))
        .sort((a,b)=>Math.abs(b.r.right-cardRect.right)-Math.abs(a.r.right-cardRect.right))[0];
    }
    if(!chosen&&cardRect){
      const x=Math.max(cardRect.left+10,cardRect.right-34),y=cardRect.top+cardRect.height/2;let el=document.elementFromPoint(x,y);el=el?.closest?.('button,[role="button"],a,[tabindex]')||el;if(el&&visible(el))chosen={e:el,r:el.getBoundingClientRect(),t:'coordinate'};
    }
    if(!chosen)return{clicked:false,reason:'plus-not-found',cardFound:Boolean(cardRect)};
    chosen.e.setAttribute('data-mayavi-plus','1');return{clicked:true,label:chosen.t||chosen.e.getAttribute?.('aria-label')||chosen.e.getAttribute?.('title')||'plus-icon',cardFound:Boolean(cardRect)};
  },{serial,digits}).catch(()=>({clicked:false,reason:'evaluate-error'}));
  if(!result.clicked)return result;
  await page.click('[data-mayavi-plus="1"]').catch(async()=>{await page.evaluate(()=>document.querySelector('[data-mayavi-plus="1"]')?.click()).catch(()=>{})});
  await new Promise(r=>setTimeout(r,1000));return result;
}
async function makePage(browser){
  const page=await browser.newPage();const version=await browser.version().catch(()=> 'HeadlessChrome/140.0.0.0');const chrome=(version.match(/(?:Chrome|Chromium)\/([\d.]+)/)||[])[1]||'140.0.0.0';
  await page.setUserAgent(`Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${chrome} Safari/537.36`);await page.setExtraHTTPHeaders({'Accept-Language':'en-US,en;q=0.9'});return page;
}
...
export async function trackSaudiaDirect(input){
  ...
  const digits=mawb.replace(/\D/g,''),serial=digits.slice(3),formats=[digits,mawb,serial];let browser;const networkBodies=[],networkUrls=[],requestBodies=[];
  try{
    ...
    page.on('request',req=>{try{if(/\/apis\/api\/eservices\/track-shipment/i.test(req.url()))requestBodies.push(redactPostData(req.postData()||''))}catch{}});
    ...
    for(const value of formats){
      ...
      await page.click('[data-mayavi-awb="1"]',{clickCount:3}).catch(()=>{});await page.keyboard.press('Control+A').catch(()=>{});await page.keyboard.press('Backspace').catch(()=>{});await page.type('[data-mayavi-awb="1"]',value,{delay:45}).catch(()=>{});await new Promise(r=>setTimeout(r,650));
      ...
      const plus=invalidCaptcha?{clicked:false,reason:'captcha-blocked'}:await clickResultPlus(page,mawb);
      const visible=await crawlTimeline(page),combined=`${visible}\n${networkBodies.join('\n')}`,parsed=parseResult(combined,mawb);
      attempts.push({value,clicked:clicked||'enter',plusClicked:Boolean(plus?.clicked),plusLabel:plus?.label||'',plusReason:plus?.reason||'',invalidCaptcha,eventCount:parsed?.events?.length||0,networkUrls:networkUrls.filter(x=>/\/apis\/api\/eservices\/track-shipment/i.test(x)).slice(-5),requestBody:requestBodies.slice(-1)[0]||'',textSample:translate(visible).slice(-2500)});
      if(parsed?.shipment){const screenshotBase64=await page.screenshot({type:'jpeg',quality:70,fullPage:false,encoding:'base64'}).catch(()=>null);return{ok:true,shipment:parsed.shipment,screenshotBase64,debug:{stage:'SUCCESS',attempts,eventCount:parsed.events.length,plusClicked:Boolean(plus?.clicked),arrivalEvent:parsed.arrivalEvent?.text?.slice(0,700)||'',latestEvent:parsed.latestOperational?.text?.slice(0,700)||''}}}
      if(invalidCaptcha)break;
    }
    const screenshotBase64=await page.screenshot({type:'jpeg',quality:70,fullPage:false,encoding:'base64'}).catch(()=>null),captchaBlocked=attempts.some(a=>a.invalidCaptcha);
    return{ok:false,reason:captchaBlocked?'SAUDIA CAPTCHA VALIDATION REQUIRED — SERVER AUTOMATION REJECTED':'SAUDIA RESULT DID NOT EXPOSE VERIFIED TIMELINE DATA',officialTracker:URL,screenshotBase64,debug:{stage:captchaBlocked?'CAPTCHA_REQUIRED':'NO_FIELDS',attempts}};
