...
function chooseFreshest(results=[]){
  ...
  if(active.length){
    ...
    // A newer departure/manifest movement must replace an older arrival snapshot.
    // This is especially important for Saudia part-loads that move onward after an earlier partial arrival.
    if(
...
export async function trackSaudiaDirect(input){
  // Do not stop at the first successful source. Saudia sources can lag each other,
  // so compare all available sources and prefer the newest operational movement.
  ...
  const chosen=chooseFreshest([sal,tm,china,connect,direct]);
  if(chosen?.ok&&chosen?.shipment){
    return {
      ...
      debug:{
        ...
        candidates:[sal,tm,china,connect,direct].filter(r=>r?.ok&&r?.shipment).map(r=>({
          source:r.shipment.source||'',status:r.shipment.status||'',sourceStatus:r.shipment.sourceStatus||'',
          flightNo:r.shipment.flightNo||'',flightDate:r.shipment.flightDate||'',arrivalDate:r.shipment.arrivalDate||''
        }))
      }
    };
  }
