...
function normalizeShipment(input={}){
  ...
  const ad=normalizeSaudiaDate(s.arrivalDate||'');if(ad)s.arrivalDate=ad;
...
export async function trackSaudiaDirect(input){
  // 1) SAL: best automatic operational source once movement begins.
  let sal;
  ...
  // 2) Official/public Saudia pages: automatic read only; no CAPTCHA bypass,
  // no screen sharing and no user interaction. The adapter tries the public
  // carrier pages and their supported AWB query forms directly.
  let official;
  try{official=await trackSaudia(input);}catch(e){official={ok:false,reason:e?.message||'SAUDIA PUBLIC SOURCE FAILED'};}
  if(official?.ok&&official?.shipment){
    return {...official,shipment:normalizeShipment(official.shipment),officialTracker:PUBLIC_URL,debug:{...(official.debug||{}),sourceSelection:'OFFICIAL_PUBLIC_AUTOMATIC_FALLBACK',salReason:sal?.reason||''}};
  }

  return {
    ok:false,
    reason:'SAUDIA AUTOMATIC SOURCES HAVE NO READABLE DATA YET',
    officialTracker:PUBLIC_URL,
    debug:{sourceSelection:'AUTO_ONLY_NO_DATA',salReason:sal?.reason||'',officialReason:official?.reason||''}
  };
}
