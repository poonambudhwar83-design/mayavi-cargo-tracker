...
async function applyFowFlightRule(input,shipment={},salShipment=null){
  const s={...shipment};
  const movementCode=sourceCode(s);
  // Once SAL has a real final-leg movement (MAN/FOW/DEP), Mayavi should keep
  // the scheduled arrival date/time for that same flight. DEP is especially
  // important: departure confirmation must not leave the arrival clock blank.
  if(!/^(MAN|FOW|DEP)$/.test(movementCode))return s;
