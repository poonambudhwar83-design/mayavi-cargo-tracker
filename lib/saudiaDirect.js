...
function saudiaSchedule(shipment={}){
  const flight=String(shipment.flightNo||'').toUpperCase().replace(/[^A-Z0-9]/g,'');
  const dest=String(shipment.destination||'').toUpperCase();
  // SV760 is RUH -> DEL. Scheduled departure is 10:10 Riyadh (UTC+3), arrival 17:00 Delhi (IST).
  if(flight==='SV760'&&dest==='DEL')return {departureUtc:'07:10',arrivalTime:'17:00'};
  return null;
}
...
function applyFlightPlan(shipment={}){
  const out={...shipment};
  const normalizedFlightDate=normalizeSaudiaDate(out.flightDate||'');
  const schedule=saudiaSchedule(out);
  if(normalizedFlightDate){
    out.flightDate=normalizedFlightDate;
    // Until an actual destination arrival exists, Saudia's current flight date is Mayavi's planned arrival date.
    if(out.arrivalIsActual!==true)out.arrivalDate=normalizedFlightDate;
  }
  if(schedule&&out.arrivalIsActual!==true){
    out.arrivalTime=schedule.arrivalTime;
    out.arrivalTimeSource='Saudia current flight plan / SV760 schedule';
  }

  const sourceStatus=String(out.sourceStatus||'').trim().toUpperCase();
  const hasActualDeparture=/^(DEP|RCF|ARR|DLV)$/.test(sourceStatus)||out.departureIsActual===true;
  const terminal=out.arrivalIsActual===true||String(out.status||'').toUpperCase()==='ARRIVED'||String(out.status||'').toUpperCase()==='DELIVERED';
  if(normalizedFlightDate&&schedule&&!hasActualDeparture&&!terminal){
    const depMs=Date.parse(`${normalizedFlightDate}T${schedule.departureUtc}:00Z`);
    if(Number.isFinite(depMs)&&Date.now()>depMs){
      out.status='DELAYED';
      out.delayReason='Saudia flight date/departure time passed but no actual departure is reported';
      out.arrivalIsActual=false;
    }
  }
  return out;
}
...
function eventEpoch(shipment={}){
  ...
  for(const value of values){
    const normalized=normalizeSaudiaDate(value)||value;
    const t=Date.parse(String(normalized));
...
function enrichMissingArrival(chosen,results=[]){
  ...
  let shipment=applyFlightPlan({...chosen.shipment});
  if(shipment.arrivalTime)return {...chosen,shipment};
  const flight=String(shipment.flightNo||'').toUpperCase().replace(/[^A-Z0-9]/g,'');
  const date=String(shipment.arrivalDate||'');
  const candidates=results.filter(r=>r?.ok&&r?.shipment?.arrivalTime);
  const exact=candidates.find(r=>{const s=r.shipment;const f=String(s.flightNo||'').toUpperCase().replace(/[^A-Z0-9]/g,'');const d=String(s.arrivalDate||'');return Boolean(flight&&f&&flight===f&&date&&d&&date===d);});
  const sameFlight=candidates.find(r=>{const f=String(r.shipment.flightNo||'').toUpperCase().replace(/[^A-Z0-9]/g,'');return Boolean(flight&&f&&flight===f&&(!date||!r.shipment.arrivalDate||String(r.shipment.arrivalDate)===date));});
  const donor=exact||sameFlight;
  if(!donor)return {...chosen,shipment};
...
export async function trackSaudiaDirect(input){
  ...
  if(chosen?.ok&&chosen?.shipment){
    return {...chosen,debug:{...(chosen.debug||{}),sourceSelection:'LATEST_OPERATIONAL_MOVEMENT',candidates:all.filter(r=>r?.ok&&r?.shipment).map(r=>({source:r.shipment.source||'',status:r.shipment.status||'',sourceStatus:r.shipment.sourceStatus||'',flightNo:r.shipment.flightNo||'',flightDate:r.shipment.flightDate||'',arrivalDate:r.shipment.arrivalDate||'',arrivalTime:r.shipment.arrivalTime||'',isPartLoad:hasPendingPartLoad(r.shipment)}))}};
  }
  return {...(direct||{}),debug:{...(direct?.debug||{}),sal:{reason:sal?.reason||'',stage:sal?.debug?.stage||'',sample:sal?.debug?.sample||''},trackingMorePublic:{reason:tm?.reason||'',stage:tm?.debug?.stage||'',sample:tm?.debug?.sample||'',network:tm?.debug?.network||[]},china:{reason:china?.reason||'',stage:china?.debug?.stage||'',sample:china?.debug?.sample||'',network:china?.debug?.network||[]},connect:{reason:connect?.reason||'',stage:connect?.debug?.stage||'',auth:connect?.debug?.auth||'',sample:connect?.debug?.sample||''}},reason:direct?.reason||sal?.reason||tm?.reason||china?.reason||connect?.reason||'SAUDIA TRACKING FAILED'};
}
