...
function mapSaudiaStatus(code='',fallback='TRACKING',destination=''){
  ...
  // Saudia official page uses RCF = Received From Flight. For our Delhi imports,
  // this means the cargo has been received from the arriving flight at DEL.
  // Do not apply this rule to RCF at any other country/station.
...
function normalizeResult(result){
  if(result?.ok&&result?.shipment){
    ...
    // Delhi-specific official milestone rule:
    // DEP -> IN TRANSIT, then RCF at DEL -> ARRIVED.
    if(sourceStatus==='RCF'&&del){
    ...
    // If the cargo page has no usable status, but a Delhi-bound flight source has
    // already verified the flight's actual arrival, Mayavi should show ARRIVED.
    // This is intentionally restricted to DEL and requires actual-arrival evidence.
    if((!sourceStatus||sourceStatus==='TRACKING')&&del&&shipment.arrivalIsActual===true){
      shipment.status='ARRIVED';
    }
...
function isActiveMovement(shipment={}){
  ...
  // RCF at DEL is terminal for our Delhi import flow, not active movement.
