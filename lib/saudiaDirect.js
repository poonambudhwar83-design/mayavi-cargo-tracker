...
async function applyFowFlightRule(input,shipment={},salShipment=null){
  ...
  if(flightDest==='DEL'){
    try{
      const live=salShipment||((await trackSaudiaViaSal(input))?.shipment);
...
export async function trackSaudiaDirect(input){
  ...
  if(strong?.ok&&strong?.shipment){
    ...
    if(!salSummary?.ok){try{salSummary=await trackSaudiaViaSalBrowser(input);}catch{}}
    ...
    shipment=await applyFowFlightRule(input,shipment,salSummary?.shipment||null);
    return{...strong,shipment,officialTracker:PUBLIC_URL,debug:{...(strong.debug||{}),sourceSelection:'SAL_FULL_CHRONOLOGY_PRIMARY',summaryBackfill:Boolean(salSummary?.ok&&salSummary?.shipment),summaryBackfillSource:salSummary?.shipment?.source||'',fowFlightRule:true}};
  }
  ...
  if(salBrowser?.ok&&salBrowser?.shipment)return{...salBrowser,shipment:normalizeShipment(salBrowser.shipment),officialTracker:PUBLIC_URL,debug:{...(salBrowser.debug||{}),sourceSelection:'SAL_BROWSER_FALLBACK',strongReason:strong?.reason||'',salReason:sal?.reason||''}};
