...
function buildFlightLegs(events=[]){
  ...
  for(const leg of map.values()){
    ...
    legs.push({
      ...
      date:dt.date||eventDateTime(manifest||{}).date||eventDateTime(arr||{}).date||'',
...
function parseShipment(payload,mawb){
  ...
  const via=chooseVia(events,origin,destination,originDeparture)||legs.find(l=>l.destination&&l.destination!==origin&&l.destination!==destination)?.destination||'';
