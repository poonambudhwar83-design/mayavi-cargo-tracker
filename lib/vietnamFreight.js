...
function chooseDeparture(events,flightNo,origin=''){
  ...
  let candidates=events.filter(e=>/\bDEPARTED\b|FLIGHT\s+DEPARTED|\bDEP\b|DEPARTURE/.test(eventText(e)));
  if(wantedFlight){
    const sameFlight=candidates.filter(e=>flightOf(e)===wantedFlight);
  ...
  let to='';
  for(const value of [event?.destination,event?.to,event?.arrivalStation,event?.destinationCode,event?.flightDestination,event?.toStation]){
    const s=clean(value).toUpperCase();
...
function chooseArrival(events,flightNo){
  const arrivals=events.filter(e=>/RECEIVED FROM FLIGHT|PHYSICALLY RECEIVED|\bARRIVED\b|\bLANDED\b|\bRCF\b|NOTIFIED OF ARRIVAL|CONSIGNEE\/AGENT NOTIFIED OF ARRIVAL/.test(eventText(e)));
  ...
  const departed=events.filter(e=>/DEPARTED|\bDEP\b|DEPARTURE/.test(eventText(e))&&(!f||flightOf(e)===f)&&seq(e)<seq(event)).sort((a,b)=>seq(b)-seq(a))[0];
  if(departed&&dt.date){
...
export async function trackVietnamFreight(input){
  ...
  try{
    ...
    const eventSummary=rawEvents.map(e=>({
      ...
      station:eventStation(e),dateTime:eventDateTime(e)
    })).filter(e=>e.text||e.flight||e.station||e.dateTime.date||e.dateTime.time).slice(-20);
    console.log('vietnam_freight_result',mawb,'flight',shipment.flightNo||'','departure',shipment.departureDate||'',shipment.departureTime||'','arrival',shipment.arrivalDate||'',shipment.arrivalTime||'','status',shipment.status||'');
