...
function parseShipment(text='',mawb=''){
  ...
  // IAG's current result page can show pieces as "25 Package/s 0.00 Total"
  // without a kg unit. Keep the package count, but never treat an unlabelled
  // "0.00 Total" as cargo weight.
  const totalWithKg=flat.match(/\b(\d{1,6})\s+Package\/s\s+([\d,.]+)\s*(?:kg|kgs)\b(?:\s+Total\b)?/i)
    ||flat.match(/\b(\d{1,6})\s+(?:PCS|PIECES?|PACKAGES?)\s*[-–]?\s*([\d,.]+)\s*(?:kg|kgs)\b/i);
  const packageOnly=flat.match(/\b(\d{1,6})\s+(?:Package\/s|Packages?|Piece\/s|Pieces?|PCS)\b/i);
  const pieces=totalWithKg?String(totalWithKg[1]||''):(packageOnly?String(packageOnly[1]||''):first(flat,[/\b(?:PIECES?|PCS|PACKAGES?)\b\s*[:\-]?\s*(\d{1,6})/i]));
  const weight=totalWithKg?String(totalWithKg[2]||'').replace(/,/g,''):first(flat,[/\b(?:GROSS\s*WEIGHT|WEIGHT)\b\s*[:\-]?\s*([\d,.]+)\s*(?:KG|KGS)\b/i]).replace(/,/g,'');
  ...
  if(origin){
    // IAG renders the event date once as a section heading. For older/completed
    // shipments the DEPARTED row may contain only its event time (e.g. 05:34),
    // so recover the nearest preceding DD/MM/YYYY heading instead of requiring
    // date + time immediately before DEPARTED.
    const depEventRx=new RegExp('(\\d{2}:\\d{2})\\s+DEPARTED\\s+[A-Z][A-Z .\\\'-]{1,60}?\\s+'+origin+'\\s+to\\s+','i');
    const dm=depEventRx.exec(flat);
    if(dm){
      departureTime=parseTime(dm[1]);
      const before=flat.slice(0,dm.index);
      const dates=[...before.matchAll(/\b(\d{2}\/\d{2}\/20\d{2})\b/g)];
      if(dates.length)departureDate=parseDate(dates[dates.length-1][1]);
    }
  }
