...
function parseShipment(text='',mawb=''){
  ...
  if(/AWB\s+INVALID|PLEASE\s+ENTER\s+A\s+VALID\s+AWB/i.test(flat)&&!/\bRCS\b|\bDEP\b|\bARR\b|\bRCF\b|\bNFD\b|\bDLV\b/i.test(flat))return null;
  ...
  const weight=first(flat,[/\b(?:GROSS\s*WEIGHT|WEIGHT)\b\s*[:\-]?\s*([\d,.]+(?:\s*\/\s*[\d,.]+)?)\s*(?:KG|KGS)?/i]).replace(/[\s,]+/g,'');
  ...
  const useful=Boolean((origin&&destination)||pieces||weight||flightNo||bookingDate||arrivalDate||arrivalTime||/\bRCS\b|\bDEP\b|\bARR\b|\bRCF\b|\bNFD\b|\bDLV\b/i.test(flat));
  return useful?{mawb,carrierCode:'BA',airlineName:'British Airways / IAG Cargo',officialTracker:AIRLINE_URL,origin,destination,bags:pieces,pieces,weight,flightNo,bookingDate,arrivalDate,arrivalTime,arrivalIsActual,status,arrivalSource,source:'IAG Cargo official Track & Trace'}:null;
}
...
async function fillReal(page,mawb){
  ...
  for(const f of page.frames()){
    try{
      ...
      let n=await f.$('#awb_cod');if(!n)n=await f.$('input[name="awb_cod"]');if(!n)n=await f.$('input[name="awb.cod"]');
      let mode='';
      if(p&&n){await realType(p,prefix);await realType(n,serial);mode='split';}
      else{
        const handles=await f.$$('input');let one=null;
        for(const h of handles){const meta=await h.evaluate(e=>({type:(e.type||'text').toLowerCase(),desc:`${e.id||''} ${e.name||''} ${e.placeholder||''} ${e.getAttribute('aria-label')||''}`,visible:!!(e.offsetWidth||e.offsetHeight||e.getClientRects().length)}));if(!meta.visible||['hidden','checkbox','radio','submit','button'].includes(meta.type))continue;if(/XXX-YYYYYYYY|AWB|AIR\s*WAYBILL|AIRWAY|SHIPMENT|TRACKING/i.test(meta.desc)){one=h;break;}}
        if(!one)continue;await realType(one,mawb);mode='single';
      }
      await sleep(500);
      ...
      if(n)await n.press('Enter').catch(()=>{});return{filled:true,clicked:'Enter',mode,frame:f.url(),value:mawb};
    }catch{}
  }
