...
function parseShipment(text='',mawb=''){
  ...
  const finalArrival=(destination&&arrivals.find(m=>String(m[2]).toUpperCase()===destination))||arrivals[0]||null;
  const finalDelivered=(destination&&delivered.find(m=>String(m[2]).toUpperCase()===destination))||delivered[0]||null;
  const finalReceived=(destination&&received.find(m=>String(m[2]).toUpperCase()===destination))||received[0]||null;
  ...
  return useful?{mawb,carrierCode:'BA',airlineName:'British Airways / IAG Cargo',officialTracker:AIRLINE_URL,origin,destination,bags:pieces,pieces,weight,flightNo,bookingDate,arrivalDate,arrivalTime,arrivalIsActual,status,arrivalSource,deliveredDate,deliveredTime,source:'IAG Cargo official Track & Trace'}:null;
}
