const AIRPORT_TZ = {
  ...
  CGK:'Asia/Jakarta', SGN:'Asia/Ho_Chi_Minh', HAN:'Asia/Ho_Chi_Minh', VTE:'Asia/Vientiane', KTI:'Asia/Phnom_Penh',
...
function markedIst(zone='',source=''){
  return String(zone||'').toUpperCase()==='IST'||/\bIST\b|ASIA\/KOLKATA|INDIA STANDARD TIME/i.test(String(source||''));
}
function markedUtc(zone='',source=''){
  const z=String(zone||'').trim().toUpperCase();
  return z==='UTC'||z==='GMT'||z==='Z'||/\bUTC\b|\bGMT\b|COORDINATED UNIVERSAL TIME/i.test(String(source||''));
}
function utcTimeToIst(date='',time=''){
  ...
  return {date:`${parts.year}-${parts.month}-${parts.day}`,time:`${parts.hour}:${parts.minute}`,timeZone:'IST'};
}

export function normalizeShipmentTimesToIst(row={}){
  const out={...row};

  const depZone=String(out.departureTimeZone||'').toUpperCase();
  if(!markedIst(depZone,out.departureTimeSource||out.source)){
    const depDate=String(out.departureDate||out.flightDate||'').trim();
    const depTime=String(out.departureTime||'').trim();
    const dep=markedUtc(depZone,out.departureTimeSource||out.source)
      ? utcTimeToIst(depDate,depTime)
      : localAirportTimeToIst(depDate,depTime,out.departureOrigin||out.origin||'');
    if(dep){
      out.departureDate=dep.date;
      out.departureTime=dep.time;
      out.departureTimeZone='IST';
      out.departureTimeSource=`${out.departureTimeSource||out.source||'Official source'}; ${out.departureOrigin||out.origin||'origin'} local time converted to IST`;
    }
  }

  const schedZone=String(out.scheduledArrivalTimeZone||'').toUpperCase();
  if(!markedIst(schedZone,out.scheduledArrivalTimeSource||out.source)){
    const schedDate=String(out.scheduledArrivalDate||'').trim();
    const schedTime=String(out.scheduledArrivalTime||'').trim();
    const sched=markedUtc(schedZone,out.scheduledArrivalTimeSource||out.source)
      ? utcTimeToIst(schedDate,schedTime)
      : localAirportTimeToIst(schedDate,schedTime,out.destination||'');
    if(sched){
      out.scheduledArrivalDate=sched.date;
      out.scheduledArrivalTime=sched.time;
      out.scheduledArrivalTimeZone='IST';
    }
  }

  const arrZone=String(out.arrivalTimeZone||'').toUpperCase();
  if(!markedIst(arrZone,out.arrivalTimeSource||out.source)){
    const arrDate=String(out.arrivalDate||'').trim();
    const arrTime=String(out.arrivalTime||'').trim();
    const arr=markedUtc(arrZone,out.arrivalTimeSource||out.source)
      ? utcTimeToIst(arrDate,arrTime)
      : localAirportTimeToIst(arrDate,arrTime,out.destination||'');
    if(arr){
  ...
  if(out.actualArrivalDate&&out.actualArrivalTime&&!markedIst(out.actualArrivalTimeZone,out.actualArrivalTimeSource||out.arrivalTimeSource||out.source)){
    const actual=markedUtc(out.actualArrivalTimeZone,out.actualArrivalTimeSource||out.arrivalTimeSource||out.source)
      ? utcTimeToIst(String(out.actualArrivalDate),String(out.actualArrivalTime))
      : localAirportTimeToIst(String(out.actualArrivalDate),String(out.actualArrivalTime),out.destination||'');
    if(actual){
      out.actualArrivalDate=actual.date;
      out.actualArrivalTime=actual.time;
      out.actualArrivalTimeZone='IST';
    }
  }
...
export function normalizeExportTimesToIst(row={}){
  return normalizeShipmentTimesToIst(row);
}
