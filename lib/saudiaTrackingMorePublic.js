...
export async function trackSaudiaViaTrackingMorePublic(input){
  ...
  let browser;
  const bodies=[];
  const debug={stage:'OPEN',network:[]};
  try{
    ...
    page.on('response',async r=>{try{
      ...
      if(body&&body.length<800000){
        bodies.push(body);
        if(/awb|aircargo|track|shipment|flight|status/i.test(`${u} ${body}`)) debug.network.push({url:u,status:r.status(),sample:clean(body).slice(0,600)});
      }
    }catch{}});
    ...
    const found=await page.evaluate(()=>{
      ...
      const x=xs.find(e=>/awb|tracking|waybill|297-72328572/i.test(`${e.placeholder||''} ${e.name||''} ${e.id||''} ${e.getAttribute('aria-label')||''}`))||xs[0];
      if(!x) return null;
      x.dataset.mayaviTmAwb='1';
      return {x:x.placeholder||'',all:xs.slice(0,8).map(e=>({placeholder:e.placeholder||'',name:e.name||'',id:e.id||''}))};
    }).catch(()=>null);
    debug.inputs=found;
    ...
    await page.keyboard.type(mawb,{delay:35});
    await sleep(350);
    ...
    debug.action=action;
    if(action){
      await page.click('[data-mayavi-tm-track="1"]',{delay:100}).catch(()=>{});
    }else{
    ...
    while(Date.now()<end){
      ...
      if(shipment) return {ok:true,shipment,officialTracker:URL,debug:{...debug,stage:'TM_PUBLIC_SUCCESS'}};
      await sleep(500);
    }
