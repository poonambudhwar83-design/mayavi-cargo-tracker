...
const SEARCH_URL='https://www.trackingmore.com/id/aircargo';
...
async function collectResult(page,bodies,mawb,debug,timeout=30000){
  const end=Date.now()+timeout;
  let combined='';
  while(Date.now()<end){
    combined=`${await page.evaluate(()=>document.body?.innerText||'').catch(()=> '')}\n${bodies.join('\n')}`;
    const shipment=parse(combined,mawb);
    if(shipment) return {shipment,combined};
    await sleep(500);
  }
  return {shipment:null,combined};
}
...
export async function trackSaudiaViaTrackingMorePublic(input){
  ...
  try{
    ...
    page.on('response',async r=>{try{
      ...
      if(!/trackingmore/i.test(u)||!/json|text|javascript|html/i.test(ct)) return;
      const body=await r.text();
      if(body&&body.length<800000){
        bodies.push(body);
        if(/awb|aircargo|track|shipment|flight|status/i.test(`${u} ${body}`)) debug.network.push({url:u,status:r.status(),sample:clean(body).slice(0,700)});
      }
    }catch{}});

    // First try the public generic AWB result URL directly.
    const directUrl=`${SEARCH_URL}?awb=${encodeURIComponent(mawb)}`;
    await page.goto(directUrl,{waitUntil:'domcontentloaded',timeout:35000}).catch(()=>{});
    await sleep(2500);
    let result=await collectResult(page,bodies,mawb,debug,9000);
    if(result.shipment) return {ok:true,shipment:result.shipment,officialTracker:URL,debug:{...debug,stage:'TM_DIRECT_RESULT'}};

    // Then use Saudia's carrier-specific public page. This field asks for the 8-digit AWB serial,
    // so enter 39009725 rather than the full 065-39009725.
    bodies.length=0;
    ...
    await sleep(1800);
    result=await collectResult(page,bodies,mawb,debug,22000);
    if(result.shipment) return {ok:true,shipment:result.shipment,officialTracker:URL,debug:{...debug,stage:'TM_PUBLIC_SUCCESS'}};
    return {ok:false,reason:'TRACKINGMORE PUBLIC RESULT NOT READABLE',officialTracker:URL,debug:{...debug,stage:'RESULT_TIMEOUT',sample:clean(result.combined).slice(0,2600),finalUrl:page.url()}};
  }catch(e){
