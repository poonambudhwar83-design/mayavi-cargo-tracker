...
const BASE='https://saudiacargo.com/e-services/track-shipment';
const AIRLINE={name:'Saudia Cargo',iata:'SV',url:BASE};
...
function parse(html,mawb){
  ...
  const mentions=text.includes(mawb)||text.includes(digits)||text.includes(serial);
  if(/no shipment|not found|invalid awb|no result/i.test(text))return{notFound:true};
  if(!mentions)return null;

  const route=text.match(/(?:Origin|From)\s*[:\-]?\s*([A-Z]{3})[\s\S]{0,100}?(?:Destination|To)\s*[:\-]?\s*([A-Z]{3})/i)
    ||text.match(/\b([A-Z]{3})\s*(?:-|→|>)\s*([A-Z]{3})\b/);
  const origin=airport(route?.[1]||pick(text,/(?:Origin|From)\s*[:\-]?\s*([A-Z]{3})/i));
  const destination=airport(route?.[2]||pick(text,/(?:Destination|To)\s*[:\-]?\s*([A-Z]{3})/i));
  const pieces=pick(text,/(?:Pieces?|Pcs?|No\.? of Pieces|Number of Pieces)\s*[:\-]?\s*(\d{1,6})/i)||pick(text,/\b(\d{1,6})\s*(?:PCS|Pieces?)\b/i);
  const weight=(pick(text,/(?:Gross Weight|Weight)\s*[:\-]?\s*([\d,.]+)\s*(?:KG|KGS)?/i)||pick(text,/\b([\d,.]+)\s*(?:KG|KGS)\b/i)).replace(/,/g,'');
  const flights=[...text.matchAll(/\bSV[-\s]?(\d{2,4})\b/gi)];
  const flightNo=flights.length?`SV${flights[flights.length-1][1]}`:'';
  let arrival=dateTime((text.match(/(?:Actual Arrival|Arrived|Arrival Time|ATA)[\s\S]{0,120}/i)||[])[0]||'');
  let arrivalIsActual=Boolean(arrival.date);
  if(!arrival.date)arrival=dateTime((text.match(/(?:Estimated Arrival|Expected Arrival|Scheduled Arrival|ETA)[\s\S]{0,150}/i)||[])[0]||'');
  const st=status(text);
  const shipment={mawb,carrierCode:'SV',airlineName:'Saudia Cargo',origin,destination,bags:pieces,pieces,weight,flightNo,arrivalDate:arrival.date,arrivalTime:arrival.time,arrivalIsActual,status:st,officialTracker:BASE,source:'Saudia Cargo official tracking'};
...
export async function trackSaudia(input){
  ...
  const values=[mawb,digits];
  let last='';
  for(const value of values){
    const url=`${BASE}?awbNumber=${encodeURIComponent(value)}`;
    try{
      const r=await fetch(url,{headers:{'user-agent':'Mozilla/5.0','accept':'text/html,application/xhtml+xml','accept-language':'en-US,en;q=0.9'},cache:'no-store',redirect:'follow',signal:AbortSignal.timeout(10000)});
      last=`HTTP ${r.status}`;if(!r.ok)continue;
      const html=await r.text();const p=parse(html,mawb);
      if(p?.notFound)continue;
      if(p?.useful)return{ok:true,airline:AIRLINE,shipment:p.shipment,debug:{stage:'SUCCESS',source:'saudia-public',url}};
    }catch(e){last=e?.message||String(e);}
  }
  return{ok:false,reason:'SAUDIA PUBLIC TRACKER RETURNED NO VERIFIED SHIPMENT DATA',airline:AIRLINE,debug:{stage:'NO_DATA',last}};
}
