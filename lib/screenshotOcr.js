...
function bookingDateFromText(flat=''){
  const explicit=windowAround(flat,/BOOKING\s*DATE|BOOKED\s*(?:ON|AT)?|BOOKING\s*CONFIRMED/i,40,260);
  let d=parseDate(explicit);if(d)return d;
  const accepted=windowAround(flat,/RECEIVED\s+FROM\s+SHIPPER|\bRCS\b|\bACCEPTED\b/i,60,260);
  d=parseDate(accepted);return d||'';
}
