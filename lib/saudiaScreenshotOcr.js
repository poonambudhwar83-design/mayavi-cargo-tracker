...
export function parseSaudiaScreenshotText(text='',input=''){
  ...
  const sourceStatus=first(upper,/\bSTATUS\s*:?\s*(DLV|XXX|BKD|ARR|DEP|RCF|MAN|RCS|DLY)\b/i).toUpperCase();
  const destination=first(upper,/\bDESTINATION\s*:?\s*([A-Z]{3})\b/i).toUpperCase();
  const pieces=first(upper,/\bTOTAL\s*(?:NUMBER\s*OF\s*)?PIECES\s*:?\s*(\d{1,6})\b/i);
  const weight=first(upper,/\bWEIGHT\s*:?\s*([\d,.]+)\s*(?:KG|KGS?)\b/i).replace(/,/g,'');
  const fd=first(upper,/\bFLIGHT\s*NO\.?\s*:?\s*SV\s*[- ]?(\d{1,4})\b/i);
  const flightNo=fd?`SV${fd}`:'';
  const flightDate=first(upper,/\bFLIGHT\s*DATE\s*:?\s*([0-3]?\d[A-Z]{3}\d{2,4})\b/i).toUpperCase();
  ...
  const shipment={
    ...
    sourceStatus,status,
    // Summary DLV/BKD/XXX date is not treated as flight arrival. ETA/actual arrival is mapped separately.
  ...
  const useful=mawbMatches&&Boolean(destination||pieces||weight||flightNo||sourceStatus);
...
export async function readSaudiaScreenshot({mawb,screenshotBase64,timeoutMs=35000}){
  ...
  try{
    ...
    if(!parsed.useful)return{ok:false,reason:'NO VERIFIED SAUDIA FIELDS FOUND IN SUBMIT RESULT CARD SCREENSHOT',debug:parsed};
