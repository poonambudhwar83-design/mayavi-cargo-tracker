...
export async function trackKuwait(input){
  ...
  try{
    ...
    const route=routeFromFlightTable(extracted.tables);
    ...
    const useful=Boolean(origin||destination||pieces||weight||flightNo||bookingDate||departureDate||arrivalDate);
    if(!useful)return{ok:false,reason:'KUWAIT DETAILS SECTION RETURNED NO VERIFIED FIELDS',officialTracker:CARGO_URL,screenshotBase64,debug:{fill,clicked,ops,route,textSample:extracted.text.slice(0,6000)}};
    ...
    return{
      ...
      debug:{stage:'KUWAIT_DETAILS_DEP_PLUS_7',fill,clicked,ops,route,departureDate,departureTime,calculatedArrival,textSample:extracted.text.slice(0,6000)}
    };
  }catch(e){
