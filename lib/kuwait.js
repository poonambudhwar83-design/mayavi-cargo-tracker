...
function time24(value=''){
  ...
  let h=Number(m[1]);if(m[3].toUpperCase()==='AM'&&h===12)h=0;if(m[3].toUpperCase()==='PM'&&h<12)h+=12;return`${pad(h)}:${m[2]}`;
}
  ...
  for(const table of tables){
    ...
    for(let i=0;i<rows.length;i++){
      ...
      for(let j=i+1;j<Math.min(rows.length,i+5);j++){
        const cells=rows[j].cells||[];if(!cells.length)continue;const mapped=pickMapped(headers,cells);
...
async function acceptCookies(page){
  await page.evaluate(()=>{const text=e=>(e?.innerText||e?.value||e?.getAttribute?.('aria-label')||'').trim();const b=[...document.querySelectorAll('button,[role="button"],a')].find(x=>/accept cookies|accept all|agree|allow all/i.test(text(x)));if(b)b.click();}).catch(()=>{});
}
...
async function lookupFlightAware(browser,flightNo=''){
  ...
  try{
    ...
    await page.goto(FLIGHTAWARE_FLEET_URL,{waitUntil:'domcontentloaded',timeout:22000});await new Promise(r=>setTimeout(r,1300));await acceptCookies(page);
    const setup=await page.evaluate(({flightNo})=>{
      ...
      const d=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value');d?.set?.call(input,flightNo);input.dispatchEvent(new Event('input',{bubbles:true}));input.dispatchEvent(new Event('change',{bubbles:true}));
    ...
    const uniqueAirports=[];for(const a of extracted.airportLinks||[]){if(a.iata&&!uniqueAirports.includes(a.iata)&&!['KAC','KUW','IST','EDT','UTC'].includes(a.iata))uniqueAirports.push(a.iata);}
    ...
    const times=[...panel.matchAll(/\b(\d{1,2}:\d{2}\s*(?:AM|PM))\b/gi)].map(m=>time24(m[1]));
    ...
    const arrivalIsActual=status==='ARRIVED';
    return{ok:Boolean(destination||arrivalDate||arrivalTime||status),destination,flightOrigin:origin,departureDate,departureTime,arrivalDate,arrivalTime,arrivalIsActual,status,setup,url:extracted.url,panelSample:panel.slice(0,4500)};
...
export async function trackKuwait(input){
  ...
  try{
    ...
    await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/149 Safari/537.36');
    await page.goto(CARGO_URL,{waitUntil:'domcontentloaded',timeout:22000});await new Promise(r=>setTimeout(r,1500));await acceptCookies(page);
    const fill=await page.evaluate(({prefix,serial})=>{
      ...
      const p=inputs.find(e=>/prefix/.test(desc(e)));let a=inputs.find(e=>/awb/.test(desc(e))&&!/prefix/.test(desc(e)));if(!a)a=inputs.find(e=>e!==p)||inputs[0];set(p,prefix);set(a,serial);
    ...
    const clicked=await page.evaluate(()=>{const text=e=>(e?.innerText||e?.value||e?.getAttribute?.('aria-label')||'').replace(/\s+/g,' ').trim();const b=[...document.querySelectorAll('button,input[type="submit"],a,[role="button"]')].find(e=>/^submit$/i.test(text(e)));if(b){b.click();return true;}return false;});
    if(!clicked)return{ok:false,reason:'KUWAIT SUBMIT BUTTON NOT FOUND',officialTracker:CARGO_URL,debug:{fill}};
    await Promise.race([page.waitForNetworkIdle({idleTime:900,timeout:10000}).catch(()=>{}),new Promise(r=>setTimeout(r,9000))]);await new Promise(r=>setTimeout(r,1300));
    ...
    const tableHit=deriveFromTables(extracted.tables),ops=operationalRows(extracted.tables),rowHit=deriveFromRows(extracted.tables),latest=ops[0]||{},rcs=ops.find(r=>r.code==='RCS')||{},m=tableHit?.mapped||{};
    const detailSource=latest.details||rcs.details||rowHit?.raw||'';
    ...
    const flightLookup=flightNo?await lookupFlightAware(browser,flightNo):{ok:false,reason:'NO FLIGHT NUMBER FROM CARGO DETAILS'};
    const status=flightLookup.status||cargoStatus(`${latest.code} ${latest.details||''}`);
    const shipment={mawb,carrierCode:'KU',airlineName:'Kuwait Airways Cargo',officialTracker:CARGO_URL,origin,pieces,bags:pieces,weight,flightNo,bookingDate,destination:flightLookup.destination||'',arrivalDate:flightLookup.arrivalDate||'',arrivalTime:flightLookup.arrivalTime||'',arrivalIsActual:Boolean(flightLookup.arrivalIsActual),status,source:'Kuwait Airways cargo Details + FlightAware flight status'};
    ...
    if(!useful)return{ok:false,reason:'KUWAIT TWO-LINK FLOW RETURNED NO VERIFIED FIELDS',officialTracker:CARGO_URL,screenshotBase64,debug:{fill,clicked,ops,rowHit,flightLookup,textSample:extracted.text.slice(0,6000)}};
    return{ok:true,shipment,screenshotBase64,screenshotCaptured:Boolean(screenshotBase64),screenshotVerified:true,debug:{stage:'KUWAIT_CARGO_PLUS_FLIGHTAWARE',fill,clicked,ops,rowHit,flightLookup,textSample:extracted.text.slice(0,6000)}};
