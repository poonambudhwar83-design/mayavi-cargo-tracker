...
function buildShipment(extracted,mawb){
  ...
  if(rcf.code==='RCF'&&rcfDate&&rcfTime){
    ...
    // Before DEP the cargo cards genuinely show N/A. Keep the official flight
    // date so the scheduled-flight fallback can supply the ETA without guessing.
...
async function kuwaitFlightStatsSchedule({flightNo='',flightDate='',origin='',destination=''}){
  ...
  try{
    ...
    const arrivalBlock=(plain.match(/Flight Arrival Times[\s\S]{0,500}/i)||[])[0]||'';
    const arrivalDate=parseDate((arrivalBlock.match(/(\d{1,2}-[A-Z]{3}-20\d{2})/i)||[])[1]||'')||
      (()=>{const q=arrivalBlock.match(/(\d{1,2})-([A-Z]{3})-(20\d{2})/i);return q?parseDate(q[0]):'';})();
    const actual=timeFrom((arrivalBlock.match(/Actual[\s\S]{0,60}?((?:[01]?\d|2[0-3]):[0-5]\d)/i)||[])[1]||'');
    const estimated=timeFrom((arrivalBlock.match(/Estimated[\s\S]{0,60}?((?:[01]?\d|2[0-3]):[0-5]\d)/i)||[])[1]||'');
    const scheduled=timeFrom((arrivalBlock.match(/Scheduled[\s\S]{0,60}?((?:[01]?\d|2[0-3]):[0-5]\d)/i)||[])[1]||'');
...
export async function trackKuwaitexport async function trackKuwait(input){
