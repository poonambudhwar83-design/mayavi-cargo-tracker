...
function time24(v=''){
  const m=String(v||'').match(/(\d{1,2}):(\d{2})\s*(AM|PM)/i);if(!m)return'';
  let h=+m[1];const ap=m[3].toUpperCase();if(ap==='AM'&&h===12)h=0;if(ap==='PM'&&h<12)h+=12;
  return`${pad(h)}:${m[2]}`;
}
...
function cargoStatus(v=''){
  const s=String(v||'').toUpperCase();
  if(/ARRIVED|LANDED|DELIVERED|\bDLV\b|RECEIVED FROM FLIGHT|\bRCF\b/.test(s))return'ARRIVED';
...
function liveStatus(v=''){
  ...
  if(/CANCELLED|CANCELED|\bDELAYED\b/.test(s))return'DELAYED';
  if(/EXPECTED TO DEPART|SCHEDULED|AT ORIGIN|BOARDING|DEPARTURE SCHEDULED/.test(s))return'BOOKED';
  return'';
}
function opRows(tables=[]){
  for(const t of tables){
    const rows=t.rows||[];if(!rows.length)continue;
    const h=(rows[0].cells||[]).map(x=>String(x).toLowerCase()).join(' | ');
    if(!(h.includes('operational status')&&h.includes('airport')&&h.includes('details')))continue;
...
async function acceptCookies(page){
  await page.evaluate(()=>{
    ...
    const b=[...document.querySelectorAll('button,a,[role="button"]')].find(x=>/accept cookies|accept all|agree|allow all/i.test(txt(x)));if(b)b.click();
  }).catch(()=>{});
}
...
function parseLivePanel(extracted={}){
  const panel=extracted.panelText||extracted.pageText||'';
  const codes=[];
  for(const m of panel.toUpperCase().matchAll(/\b([A-Z]{3})\b/g)){
    const c=m[1];if(!['KAC','KUW','IST','EDT','UTC','AM','PM'].includes(c)&&!codes.includes(c))codes.push(c);
  }
  ...
  const times=[...panel.matchAll(/(\d{1,2}:\d{2}\s*(?:AM|PM))/gi)].map(m=>time24(m[1])).filter(Boolean);
  const status=liveStatus(panel)||liveStatus(extracted.pageText);
  return{
    ok:Boolean(codes[1]||dates[0]||times[0]||status),
    flightOrigin:codes[0]||'',destination:codes[1]||'',
    departureDate:dates[0]||'',arrivalDate:dates[1]||dates[0]||'',
    departureTime:times[0]||'',arrivalTime:times[1]||times[0]||'',
    status,arrivalIsActual:status==='ARRIVED',url:extracted.url,
    panelSample:panel.slice(0,4500)
  };
}
async function exactHistoryForDate(page,kac,flightDate=''){
  if(!flightDate)return{ok:false,reason:'NO CARGO FLIGHT DATE'};
  const url=`https://www.flightaware.com/live/flight/${kac}/history`;
  ...
  const rows=await page.evaluate(()=>{
    ...
    return[...document.querySelectorAll('tr')].map(tr=>({cells:[...tr.querySelectorAll('th,td')].map(clean)})).filter(r=>r.cells.length);
  });
  for(const row of rows){
    const cells=row.cells||[];
    if(parseDate(cells[0]||'')!==flightDate)continue;
    const rowText=cells.join(' | ');
    const routeCodes=[...rowText.toUpperCase().matchAll(/\(([A-Z]{3})\s*\/\s*[A-Z0-9]{4}\)/g)].map(m=>m[1]);
    const times=[...rowText.matchAll(/(\d{1,2}:\d{2}\s*(?:AM|PM))/gi)].map(m=>time24(m[1])).filter(Boolean);
    const departureTime=times[0]||'',arrivalTime=times[1]||'';
    const offset=Number(rowText.match(/\(\+(\d+)\)/)?.[1]||0);
    let status='BOOKED';
    ...
    return{
      ...
      flightOrigin:routeCodes[0]||'',destination:routeCodes[1]||'',
      departureDate:flightDate,departureTime,
      arrivalDate:addDays(flightDate,offset),arrivalTime,
      status,arrivalIsActual:status==='ARRIVED',url,historyRow:rowText
    };
  }
  return{ok:false,matchedFlightDate:false,reason:`NO HISTORY ROW FOR ${flightDate}`,url};
}
async function lookupFlightAware(browser,flightNo='',flightDate=''){
  if(!flightNo)return{ok:false,reason:'NO FLIGHT NUMBER'};
  const page=await browser.newPage();
  const num=String(flightNo).replace(/^KU/i,''),kac=`KAC${num}`;
  try{
    ...
    const extracted=await page.evaluate(({kac,flightNo})=>{
      ...
      return{panelText:clean(panel),pageText:clean(document.body).slice(0,12000),url:location.href};
    },{kac,flightNo});
    const live=parseLivePanel(extracted);
    ...
    return{ok:false,reason:`FLIGHT NUMBER MATCHED BUT FLIGHT DATE DID NOT MATCH ${flightDate}`,setup,livePage:live,history};
...
export async function trackKuwait(input){
  ...
  try{
    ...
    const fill=await page.evaluate(({prefix,serial})=>{
      ...
      const p=inputs.find(e=>/prefix/.test(desc(e)));let a=inputs.find(e=>/awb/.test(desc(e))&&!/prefix/.test(desc(e)));if(!a)a=inputs.find(e=>e!==p)||inputs[0];
      set(p,prefix);set(a,serial);return{prefixFound:Boolean(p),awbFound:Boolean(a),prefixValue:p?.value||'',awbValue:a?.value||''};
    },{prefix,serial});
    ...
    const extracted=await page.evaluate(()=>{
      const tables=[...document.querySelectorAll('table')].map(table=>({rows:[...table.querySelectorAll('tr')].map(tr=>({cells:[...tr.querySelectorAll('th,td')].map(td=>(td.innerText||td.textContent||'').replace(/\s+/g,' ').trim()).filter(Boolean)})).filter(r=>r.cells.length)})).filter(t=>t.rows.length);
      return{text:(document.body?.innerText||'').replace(/\s+/g,' ').trim(),tables};
    });
    const ops=opRows(extracted.tables);
    const rcs=ops.find(r=>r.code==='RCS')||{};
    const flightEvent=ops.find(r=>/PRE|DEP/.test(r.code)&&cleanFlight(r.details))||ops.find(r=>cleanFlight(r.details))||ops[0]||{};
    const latest=ops[0]||{};
    const detailSource=flightEvent.details||rcs.details||'';
    const origin=rcs.airport||flightEvent.airport||latest.airport||'';
    const pieces=piecesFrom(detailSource)||piecesFrom(rcs.details)||'';
    const weight=weightFrom(detailSource)||weightFrom(rcs.details)||'';
    const flightNo=cleanFlight(detailSource)||cleanFlight(rcs.details)||'';
    const bookingDate=parseDate(rcs.dateText)||'';
    const flightDate=parseDate(flightEvent.dateText)||bookingDate;
    ...
    if(!useful)return{ok:false,reason:'KUWAIT TWO-LINK FLOW RETURNED NO VERIFIED FIELDS',officialTracker:CARGO_URL,screenshotBase64,debug:{fill,clicked,ops,flightDate,flightLookup,textSample:extracted.text.slice(0,6000)}};
    return{ok:true,shipment,screenshotBase64,screenshotCaptured:Boolean(screenshotBase64),screenshotVerified:true,debug:{stage:'KUWAIT_CARGO_PLUS_FLIGHTAWARE_EXACT_DATE',fill,clicked,ops,flightDate,flightLookup,textSample:extracted.text.slice(0,6000)}};
