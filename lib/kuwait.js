...
function cargoStatus(rows=[]){
  ...
  if(/PRE[-\s]?MANIFEST/.test(joined))return'PRE-MANIFESTED';
  return'BOOKED';
}
...
function buildShipment(extracted,mawb){
  ...
  const preManifested=/PRE[-\s]?MANIFESTED|PRE[-\s]?MANIFEST/i.test(extracted.text||'');
  ...
  if(preManifested&&dep.code!=='DEP'&&rcf.code!=='RCF')status='PRE-MANIFESTED';
  ...
  if(rcf.code==='RCF'&&rcfDate&&rcfTime){
    ...
    if(status!=='PRE-MANIFESTED')status=/BOOKED|CONFIRMED/i.test(flight.status||'')?'BOOKED':status;
  }
  ...
  const shipment={
    ...
    remarks:status==='PRE-MANIFESTED'?'Pre-Manifested / departure not confirmed':'',
    source:rcfDate&&rcfTime?'Kuwait Airways RCF card':
      calculatedArrival.date?'Kuwait Airways cargo cards · DEP + 7 hours':
      preManifested?'Kuwait Airways cargo cards · Pre-Manifested · departure not confirmed':
