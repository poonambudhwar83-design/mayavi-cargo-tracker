...
export async function trackKuwait(input){
  ...
  try{
    ...
    const destination=cleanCode(m.destination)||((/RCF|ARR|DLV/.test(latest.code))?latest.airport:'');
    ...
    if(!useful)return{ok:false,reason:'KUWAIT DETAILS TABLE FOUND BUT NO VERIFIED FIELDS EXTRACTED',officialTracker:URL,screenshotBase64,debug:{fill,clicked,tableHit,ops,rowHit,tables:extracted.tables.slice(0,12),textSample:extracted.text.slice(0,7000)}};
    return{ok:true,shipment,screenshotBase64,screenshotCaptured:Boolean(screenshotBase64),screenshotVerified:true,debug:{stage:'KUWAIT_DETAILS_TABLE',fill,clicked,tableHit,ops,rowHit,tables:extracted.tables.slice(0,12),textSample:extracted.text.slice(0,7000)}};
