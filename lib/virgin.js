...
function parseVirgin(summaryText='',trackingText='',mawb=''){
  ...
  if(destination){
    ...
    const eta=tracking.match(new RegExp('\\b'+d+'\\b[\\s\\S]{0,220}?ETA\\s+(\\d{1,2}:\\d{2})\\s*,?\\s*(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)[a-z]*,?\\s*([A-Za-z]{3})\\s+(\\d{1,2})\\s*,?\\s*(20\\d{2})','i'));
    if(eta){arrivalTime=timeOnly(eta[1]);arrivalDate=dateOnly(eta[2]+' '+eta[3]+' '+eta[4],year);arrivalSource='TRACKING DETAILS DESTINATION ETA';}
  }
  const preferred=preferredCardArrival(flat,destination,year);
  if(preferred.date&&preferred.time){arrivalDate=preferred.date;arrivalTime=preferred.time;arrivalSource=preferred.label;}
