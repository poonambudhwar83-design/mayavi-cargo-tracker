...
function parseVirgin(summaryText='',trackingText='',mawb=''){
  ...
  const side=virginSideFields(flat,origin,destination,year);
  ...
  let arrivalDate=side.arrivalDate||'',arrivalTime=side.arrivalTime||'',arrivalIsActual=false,arrivalSource=(side.arrivalDate||side.arrivalTime)?'DESTINATION SIDE':'';
