...
function near(text='',rx,before=180,after=500){const flat=clean(text),m=flat.match(rx);if(!m)return{date:'',time:''};const at=m.index||0,w=flat.slice(Math.max(0,at-before),Math.min(flat.length,at+after));return{date:dateOnly(w),time:timeOnly(w),text:w};}
...
function parseVirgin(text='',mawb=''){
  ...
  const simpleRoute=flat.match(/\b([A-Z]{3})\s*(?:→|->|–|—|-)\s*([A-Z]{3})\b/);if(simpleRoute){origin=origin||simpleRoute[1];destination=destination||simpleRoute[2];}
  ...
  const bookingLabel=near(flat,/\b(?:Booking\s*Date|Booked\s*(?:On|At))\b/i);
  const bookingDate=bookingLabel.date||'';
  ...
  if((!arrivalDate||!arrivalTime)&&destination){
    const idx=upper.lastIndexOf(destination.toUpperCase());
    if(idx>=0){
      const w=flat.slice(Math.max(0,idx-240),Math.min(flat.length,idx+260));
      arrivalDate=arrivalDate||dateOnly(w);
  ...
  if(!arrivalDate){
    const bookedFlight=flat.match(/Booked\s+on\s+Flight\s+VS[- ]?\d{1,4}\s*,?\s*(\d{1,2}\s+[A-Za-z]{3}\s+20\d{2})/i);
    if(bookedFlight)arrivalDate=dateOnly(bookedFlight[1]);
  }
  ...
  return{shipment:{mawb,carrierCode:'VS',airlineName:'Virgin Atlantic Cargo',officialTracker:URL,origin,destination,bags:pieces,pieces,weight,flightNo,bookingDate,arrivalDate,arrivalTime,arrivalIsActual,status,source:'Virgin Atlantic Cargo Track Cargo → Tracking Details → Show Details'}};
}
    ...
    for(const frame of page.frames()){
      try{
        const hit=await frame.evaluate(srcs=>{
          ...
          clickable.scrollIntoView({block:'center',behavior:'auto'});clickable.click();return matches[0].t;
        ...
        for(const h of handles){
          try{
            const m=await h.evaluate(e=>{
              ...
              const parent=e.closest('label,div,section,form,td');
              const desc=`${e.placeholder||''} ${e.name||''} ${e.id||''} ${e.getAttribute('aria-label')||''} ${e.title||''} ${parent?.innerText||''}`.replace(/\s+/g,' ').trim().toLowerCase();
              return{visible:r.width>10&&r.height>8&&s.display!=='none'&&s.visibility!=='hidden'&&!e.disabled&&!e.readOnly,desc,max:e.maxLength||-1};
            });
            if(m.visible)candidates.push({h,m});
          }catch{}
        }
        const exact=candidates.find(x=>/doc\.?\s*no\.?/.test(x.m.desc));if(exact)return{frame,handle:exact.h,mode:'doc-no'};
        const awb=candidates.find(x=>/(awb|air\s*waybill|document)/.test(x.m.desc)&&!/prefix/.test(x.m.desc));if(awb)return{frame,handle:awb.h,mode:'doc-no'};
        if(candidates.length===1)return{frame,handle:candidates[0].h,mode:'doc-no'};
      }catch{}
    }
    await sleep(350);
  }
...
async function typeDocNo(page,mawb){
  const digits=digitsOnly(mawb),field=await findDocField(page,15000);if(!field)return{ok:false};
  const h=field.handle;
  await h.focus();await h.click({clickCount:3}).catch(()=>{});
  ...
  await h.type(digits,{delay:55});await sleep(350);
  const value=await h.evaluate(e=>e.value||'').catch(()=>'');
  return{ok:true,mode:'doc-no',value,pressedEnter:false};
}
...
export async function trackVirgin(input){
  ...
  try{
    ...
    const trackCargo=await clickText(page,[/^Track\s+cargo$/i],9000);
    debug.flow.push({step:'TRACK_CARGO',clicked:Boolean(trackCargo),text:trackCargo});
    if(!trackCargo)return{ok:false,reason:'VIRGIN ATLANTIC TRACK CARGO TILE NOT FOUND',officialTracker:URL,debug};
    await sleep(1400);

    const fill=await typeDocNo(page,mawb);
    ...
    const search=await clickText(page,[/^Search$/i],9000);
    debug.flow.push({step:'SEARCH',clicked:Boolean(search),text:search});
    if(!search)return{ok:false,reason:'VIRGIN ATLANTIC SEARCH BUTTON NOT FOUND',officialTracker:URL,debug};

    let text=await waitForText(page,/Tracking\s+Details|Search\s+Results|Booked\s+on\s+Flight|No\s+(?:shipment|result|record)|Not\s+found|Invalid/i,25000);
    debug.afterSearch=clean(text).slice(0,7000);
    ...
    const trackingDetails=await clickText(page,[/^Tracking\s+Details$/i],12000);
    debug.flow.push({step:'TRACKING_DETAILS',clicked:Boolean(trackingDetails),text:trackingDetails});
    if(!trackingDetails)return{ok:false,reason:'VIRGIN ATLANTIC TRACKING DETAILS BUTTON NOT FOUND',officialTracker:URL,debug:{...debug,stage:'DETAIL_BUTTON_MISSING'}};
    await sleep(1200);
    text=await waitForText(page,/Your\s+order\s+is\s+confirmed|Shipment\s+is\s+not\s+ready|Show\s+Details|Flight\s+Details|Tracking\s+Details/i,18000);

    const showDetails=await clickText(page,[/^Show\s+Details$/i],10000);
    debug.flow.push({step:'SHOW_DETAILS',clicked:Boolean(showDetails),text:showDetails});
    if(showDetails)await sleep(1200);
    text=await pageText(page);
    debug.afterShowDetails=clean(text).slice(0,12000);
    ...
    if(parsed?.shipment)return{ok:true,shipment:parsed.shipment,officialTracker:URL,screenshotBase64,screenshotCaptured:Boolean(screenshotBase64),screenshotVerified:Boolean(screenshotBase64),debug:{...debug,stage:showDetails?'SHOW_DETAILS_SUCCESS':'TRACKING_DETAILS_SUCCESS',sample:clean(text).slice(0,12000)}};
    return{ok:false,reason:'VIRGIN ATLANTIC TRACKING DETAILS NOT READABLE',officialTracker:URL,screenshotBase64,screenshotCaptured:Boolean(screenshotBase64),debug:{...debug,stage:'NO_FIELDS',sample:clean(text).slice(0,12000)}};
