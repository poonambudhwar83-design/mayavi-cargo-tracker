...
async function browserConfig(){
  for(const p of [process.env.CHROME_EXECUTABLE_PATH,'/usr/bin/chromium','/usr/bin/chromium-browser','/usr/bin/google-chrome','/usr/bin/google-chrome-stable'].filter(Boolean)){
    if(fs.existsSync(p))return{executablePath:p,args:['--no-sandbox','--disable-setuid-sandbox','--disable-dev-shm-usage','--disable-gpu','--no-zygote','--disable-extensions','--disable-background-networking','--disable-default-apps','--disable-sync','--disable-translate','--disk-cache-size=1','--media-cache-size=1']};
  }
  const mod=await import('@sparticuz/chromium');
  const chromium=mod.default||mod;
  return{executablePath:await chromium.executablePath(),args:[...chromium.args,'--disable-extensions','--disable-background-networking','--disable-default-apps','--disable-sync','--disable-translate','--disk-cache-size=1','--media-cache-size=1']};
}
...
function finalDestinationEstimate(text='',destination=''){
  const flat=clean(text),dest=String(destination||'').toUpperCase();if(!flat||!dest)return{date:'',time:''};
  const year=yearFrom(flat),d=esc(dest);
  let m=flat.match(new RegExp(`\\b[A-Z]{3}\\s+${d}\\s+ETD\\s+\\d{1,2}:\\d{2}\\s*,\\s*ETA\\s+(\\d{1,2}:\\d{2})\\s*,\\s*(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)[a-z]*,?\\s*([A-Za-z]{3}\\s+\\d{1,2},?\\s+20\\d{2})`,'i'));
  if(m)return{date:dateOnly(m[2],year),time:timeOnly(m[1])};
  m=flat.match(new RegExp(`Expected\\s+to\\s+arrive\\s+at\\s+${d}\\s+on\\s+[^,]*,\\s*(\\d{1,2}\\s+[A-Za-z]{3}\\s+20\\d{2})\\s+(\\d{1,2}:\\d{2}(?:\\s*(?:AM|PM))?)`,'i'));
  if(m)return{date:dateOnly(m[1],year),time:timeOnly(m[2])};
  m=flat.match(new RegExp(`\\b${d}\\b\\s+(\\d{1,2}:\\d{2}\\s*(?:AM|PM))\\s*\\|\\s*(\\d{1,2}\\s+[A-Za-z]{3})(?:\\s+20\\d{2})?\\s+(\\d{1,2}:\\d{2}\\s*(?:AM|PM))\\s*\\|\\s*(\\d{1,2}\\s+[A-Za-z]{3})(?:\\s+20\\d{2})?`,'i'));
  if(m)return{date:dateOnly(m[4],year),time:timeOnly(m[3])};
  return{date:'',time:''};
}
function preferredCardArrival(text='',destination='',fallbackYear=''){
  const flat=clean(text),dest=String(destination||'').toUpperCase();if(!flat||!dest)return{date:'',time:'',label:''};
  const d=esc(dest),year=fallbackYear||yearFrom(flat);
  for(const item of [{label:'CONFIRMED',rx:'Confirmed'},{label:'EXPECTED',rx:'Expected'},{label:'ON TIME',rx:'On\\s*Time'}]){
    const m=flat.match(new RegExp(`\\b${item.rx}\\b[\\s\\S]{0,700}?\\b${d}\\b\\s+(\\d{1,2}:\\d{2}\\s*(?:AM|PM))\\s*(?:\\||,)?\\s*(\\d{1,2}\\s+[A-Za-z]{3})(?:\\s*,?\\s*(20\\d{2}))?`,'i'));
    if(m){const y=m[3]||year;return{date:dateOnly(`${m[2]} ${y}`,y),time:timeOnly(m[1]),label:item.label};}
  }
  return{date:'',time:'',label:''};
}


function virginSideFields(text='',origin='',destination='',fallbackYear=''){
  const flat=clean(text),org=String(origin||'').toUpperCase(),dest=String(destination||'').toUpperCase(),year=fallbackYear||yearFrom(flat);
  if(!flat||!org||!dest)return{bookingDate:'',bookingTime:'',arrivalDate:'',arrivalTime:''};
  const om=flat.match(new RegExp('\\b'+esc(org)+'\\b','i'));if(!om)return{bookingDate:'',bookingTime:'',arrivalDate:'',arrivalTime:''};
  const from=(om.index||0),after=flat.slice(from+org.length),dm=after.match(new RegExp('\\b'+esc(dest)+'\\b','i'));if(!dm)return{bookingDate:'',bookingTime:'',arrivalDate:'',arrivalTime:''};
  const to=from+org.length+(dm.index||0),left=flat.slice(from,to),right=flat.slice(to,Math.min(flat.length,to+900));
  return{bookingDate:dateOnly(left,year),bookingTime:timeOnly(left),arrivalDate:dateOnly(right,year),arrivalTime:timeOnly(right)};
}

function virginOriginDeparture(text='',origin='',fallbackYear=''){
  const flat=clean(text),org=String(origin||'').toUpperCase(),year=fallbackYear||yearFrom(flat);
  const empty={departureDate:'',departureTime:'',departureFlightNo:'',departureOrigin:org,departureDestination:'',departureIsActual:false,departureTimeSource:''};
  if(!flat||!org)return empty;

  let departureFlightNo='',departureDestination='',bookedDate='';
  const booked=flat.match(new RegExp(`Booked\\s+on\\s+Flight\\s+VS[- ]?(\\d{1,4})\\s*,\\s*(\\d{1,2}\\s+[A-Za-z]{3}\\s+20\\d{2})\\s*,\\s*${esc(org)}-([A-Z]{3})\\b`,'i'));
  if(booked){
    departureFlightNo=`VS${booked[1].padStart(4,'0')}`;
    bookedDate=dateOnly(booked[2],year);
    departureDestination=booked[3].toUpperCase();
  }

  const actual=flat.match(new RegExp(`\\bDEP\\s+${esc(org)}\\s*,\\s*(\\d{1,2}:\\d{2}\\s*(?:AM|PM))\\s*,\\s*([A-Za-z]{3}\\s+\\d{1,2}\\s*,\\s*20\\d{2})`,'i'));
  if(actual){
    return{
      departureDate:dateOnly(actual[2],year),
      departureTime:timeOnly(actual[1]),
      departureFlightNo,
      departureOrigin:org,
      departureDestination,
      departureIsActual:true,
      departureTimeSource:'Virgin Tracking Details origin DEP'
    };
  }

  const destPart=departureDestination?esc(departureDestination):'[A-Z]{3}';
  const etd=flat.match(new RegExp(`\\b${esc(org)}\\s+${destPart}\\s+ETD\\s+(\\d{1,2}:\\d{2})(?:\\s*(AM|PM))?\\s*,?\\s*ETA\\s+\\d{1,2}:\\d{2}(?:\\s*(?:AM|PM))?\\s*,?\\s*(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)[a-z]*,?\\s*([A-Za-z]{3})\\s+(\\d{1,2})\\s*,?\\s*(20\\d{2})`,'i'));
  if(etd){
    return{
      departureDate:dateOnly(`${etd[3]} ${etd[4]} ${etd[5]}`,year)||bookedDate,
      departureTime:timeOnly(etd[1]+(etd[2]?` ${etd[2]}`:'')),
      departureFlightNo,
      departureOrigin:org,
      departureDestination,
      departureIsActual:false,
      departureTimeSource:'Virgin Tracking Details origin ETD'
    };
  }

  const card=flat.match(new RegExp(`\\b${esc(org)}\\s+(?:Confirmed|Expected|On\\s*Time)\\s+VS\\s*[- ]?(\\d{1,4})\\s+([A-Z]{3})\\s+(\\d{1,2}:\\d{2}\\s*(?:AM|PM))\\s*\\|\\s*(\\d{1,2}\\s+[A-Za-z]{3})(?:\\s*,?\\s*(20\\d{2}))?`,'i'));
  if(card){
    const y=card[5]||year;
    return{
      departureDate:dateOnly(`${card[4]} ${y}`,y)||bookedDate,
      departureTime:timeOnly(card[3]),
      departureFlightNo:`VS${card[1].padStart(4,'0')}`,
      departureOrigin:org,
      departureDestination:card[2].toUpperCase(),
      departureIsActual:false,
      departureTimeSource:'Virgin Tracking Details origin card'
    };
  }

  return{...empty,departureDate:bookedDate,departureFlightNo,departureDestination,departureTimeSource:bookedDate?'Virgin origin booking leg date':''};
}

function parseVirgin(summaryText='',trackingText='',mawb=''){
  const flat=clean(summaryText),tracking=clean(trackingText),evidence=clean(`${flat} ${tracking}`),upper=evidence.toUpperCase();if(!evidence)return null;
  if(/NO\s+(?:SHIPMENT|RESULT|RECORD)|NOT\s+FOUND|INVALID\s+(?:AWB|DOCUMENT|DOC)|NO\s+DATA/i.test(evidence))return{notFound:true};
  const digits=digitsOnly(mawb),serial=digits.slice(3),allDigits=digitsOnly(evidence),year=yearFrom(evidence);
  if(!allDigits.includes(digits)&&!allDigits.includes(serial)&&!/TRACKING\s+DETAILS|DOC\.?\s*NO\.?|JRN\s*NO/i.test(evidence))return null;
  ...
  // The left/right route cards live on the Tracking Details page, not the summary page.
  // Never derive these four fields from summary text because it can repeat/mix dates.
  const side=virginSideFields(tracking,origin,destination,year);
  let bookingDate=side.bookingDate||'',bookingTime=side.bookingTime||'';
  const bkd=tracking.match(/\bBKD\b[^,]*,\s*(\d{1,2}:\d{2}\s*(?:AM|PM))\s*,\s*([A-Za-z]{3}\s+\d{1,2}\s*,\s*20\d{2})/i);
  if(bkd&&!bookingDate){bookingTime=bookingTime||timeOnly(bkd[1]);bookingDate=dateOnly(bkd[2],year);}
  if(!bookingDate){const booking=near(tracking,/\bBooking\s*Date\b/i);bookingDate=booking.date||'';bookingTime=bookingTime||booking.time||'';}

  let arrivalDate=side.arrivalDate||'',arrivalTime=side.arrivalTime||'',arrivalIsActual=false,arrivalSource=(side.arrivalDate||side.arrivalTime)?'TRACKING DETAILS DESTINATION SIDE':'';
  if(destination){
    const d=esc(destination);
    // Use the LAST destination ETA in Tracking Details/Milestones: Virgin updates ETA after departure.
    const etaRx=new RegExp('\\b'+d+'\\b[\\s\\S]{0,180}?ETA\\s+(\\d{1,2}:\\d{2})(?:\\s*(AM|PM))?\\s*,?\\s*(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)[a-z]*,?\\s*([A-Za-z]{3})\\s+(\\d{1,2})\\s*,?\\s*(20\\d{2})','ig');
    const etas=[...tracking.matchAll(etaRx)];
    if(etas.length){
      const eta=etas.at(-1), rawTime=eta[1]+(eta[2]?' '+eta[2]:'');
      arrivalTime=timeOnly(rawTime);arrivalDate=dateOnly(eta[3]+' '+eta[4]+' '+eta[5],year);arrivalSource='TRACKING DETAILS LATEST DESTINATION ETA';
    }
  }
  const preferred=preferredCardArrival(flat,destination,year);
  if(!arrivalDate&&!arrivalTime&&preferred.date&&preferred.time){arrivalDate=preferred.date;arrivalTime=preferred.time;arrivalSource=preferred.label;}
  let actualArrivalDate='',actualArrivalTime='';
  if(destination){
    const arrRx=new RegExp(`\\bARR\\s+${esc(destination)}\\s*,\\s*(\\d{1,2}:\\d{2}\\s*(?:AM|PM))\\s*,\\s*([A-Za-z]{3}\\s+\\d{1,2}\\s*,\\s*20\\d{2})`,'i');
    const arr=tracking.match(arrRx);
    if(arr){
      actualArrivalTime=timeOnly(arr[1]);
      actualArrivalDate=dateOnly(arr[2],year);
      arrivalIsActual=true;
      // Actual ARR milestone is authoritative for an arrived shipment.
      // Never keep a scheduled/summary card time once destination ARR exists.
      if(actualArrivalDate)arrivalDate=actualArrivalDate;
      if(actualArrivalTime)arrivalTime=actualArrivalTime;
      arrivalSource='ARR';
    }
  }
  if(!arrivalDate||!arrivalTime){
    const eta=finalDestinationEstimate(tracking,destination);
    arrivalDate=eta.date;arrivalTime=eta.time;arrivalSource=arrivalSource||'ETA';
  }

  const departure=virginOriginDeparture(tracking,origin,year);

  let flightNo='';
  if(destination){
    const destEsc=esc(destination);
    const arrFlight=new RegExp(`Arrived\\s+at\\s+${destEsc}\\s+on\\s+Flight\\s+VS[- ]?(\\d{1,4})`,'i');
    const am=tracking.match(arrFlight);if(am)flightNo=`VS${am[1].padStart(4,'0')}`;
    if(!flightNo){const bookedFinal=new RegExp(`Booked\\s+on\\s+Flight\\s+VS[- ]?(\\d{1,4})[^.]{0,160}[-–—]${destEsc}\\b`,'i');const bm=tracking.match(bookedFinal);if(bm)flightNo=`VS${bm[1].padStart(4,'0')}`;}
  }
  if(!flightNo){const flights=[...tracking.toUpperCase().matchAll(/\bVS\s*[- ]?(\d{1,4})\b/g)];if(flights.length)flightNo=`VS${flights.at(-1)[1].padStart(4,'0')}`;}

  let status='BOOKED';
  if(/\bDLV\b|\bDELIVERED\b|PROOF\s+OF\s+DELIVERY/.test(upper))status='ARRIVED';
  else if(destination&&new RegExp(`\\bARR\\s+${esc(destination)}\\b`,'i').test(tracking))status='ARRIVED';
  else if(/\bDELAY(?:ED)?\b|\bLATE\b|\bOFFLOAD(?:ED)?\b|\bEXCEPTION\b/.test(upper))status='DELAYED';
  else if(/\bDEP\b|\bDEPARTED\b|\bAIRBORNE\b|\bIN\s+FLIGHT\b/.test(upper))status='DEPARTED';
  else if(/\bIN\s+TRANSIT\b/.test(upper))status='IN TRANSIT';

  const useful=Boolean((origin&&destination)||pieces||weight||flightNo||bookingDate||arrivalDate||arrivalTime);
  if(!useful)return null;
  return{shipment:{mawb,carrierCode:'VS',airlineName:'Virgin Atlantic Cargo',officialTracker:URL,origin,destination,bags:pieces,pieces,weight,flightNo,bookingDate,bookingTime,departureDate:departure.departureDate,departureTime:departure.departureTime,departureFlightNo:departure.departureFlightNo,departureOrigin:departure.departureOrigin,departureDestination:departure.departureDestination,departureIsActual:departure.departureIsActual,departureTimeSource:departure.departureTimeSource,arrivalDate,arrivalTime,arrivalIsActual,status,arrivalSource,actualArrivalDate,actualArrivalTime,source:`Virgin Atlantic Cargo → ${departure.departureTimeSource||'origin leg'} + ${arrivalSource||'Tracking Details'} arrival`}};
}
...
export async function trackVirgin(input){
  ...
  const profileDir=`/tmp/mayavi-virgin-${process.pid}-${Date.now()}-${digitsOnly(mawb).slice(-5)}`;
  try{
    fs.rmSync(profileDir,{recursive:true,force:true});
    const mod=await import('puppeteer-core'),puppeteer=mod.default||mod,launch=await browserConfig();
    browser=await puppeteer.launch({...launch,headless:true,userDataDir:profileDir,defaultViewport:{width:1440,height:1100}});
    ...
    const summaryText=text;debug.afterShowDetails=clean(summaryText).slice(0,18000);
    ...
    const nav=await waitForDetailNavigation(page,beforeUrl,20000);debug.detailUrl=nav.url;debug.afterTrackingDetails=clean(nav.text).slice(0,18000);
    ...
    const trackingText=nav.text;

    const parsed=parseVirgin(summaryText,trackingText,mawb),screenshotBase64=await page.screenshot({type:'jpeg',quality:68,fullPage:false,encoding:'base64'}).catch(()=>null);
    if(parsed?.shipment)return{ok:true,shipment:parsed.shipment,officialTracker:URL,screenshotBase64,screenshotCaptured:Boolean(screenshotBase64),screenshotVerified:Boolean(screenshotBase64),debug:{...debug,stage:'SHOW_DETAILS_THEN_TRACKING_SUCCESS',detailSample:clean(`${summaryText} ${trackingText}`).slice(0,18000)}};
    return{ok:false,reason:'VIRGIN ATLANTIC TRACKING DETAILS NOT READABLE',officialTracker:URL,screenshotBase64,debug:{...debug,stage:'NO_FIELDS',detailSample:clean(`${summaryText} ${trackingText}`).slice(0,18000)}};
  }catch(error){
