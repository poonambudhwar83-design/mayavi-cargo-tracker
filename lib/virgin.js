...
function parseVirgin(summaryText='',trackingText='',mawb=''){
  ...
  if(!bookingDate){const booking=near(tracking,/\bBooking\s*Date\b/i);bookingDate=booking.date||'';}
  ...
  return{shipment:{mawb,carrierCode:'VS',airlineName:'Virgin Atlantic Cargo',officialTracker:URL,origin,destination,bags:pieces,pieces,weight,flightNo,bookingDate,arrivalDate,arrivalTime,arrivalIsActual,status,arrivalSource,actualArrivalDate,actualArrivalTime,source:`Virgin Atlantic Cargo → ${arrivalSource||'Tracking Details'} priority arrival + Tracking Details status`}};
}
