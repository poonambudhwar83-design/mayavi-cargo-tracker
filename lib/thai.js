...
function parseShipment(text,mawb){
  ...
  return{mawb,carrierCode:'TG',airlineName:'THAI Cargo',origin,destination,pieces,bags:pieces,weight,flightNo:finalLeg?.flightNo||'',flightDate:finalLeg?.flightDate||'',bookingDate:booking?.date||'',bookingTime:booking?.time||'',arrivalDate,arrivalTime,arrivalIsActual,status,officialTracker:OFFICIAL,source:'THAI Cargo CHORUS public tracking',provider:'THAI Cargo CHORUS'};
}
