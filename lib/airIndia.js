...
function parseSummary(text='',mawb=''){
  ...
  const origin=(flat.match(originRx)||[])[1]?.toUpperCase()||'';
  const destination=(flat.match(destRx)||[])[1]?.toUpperCase()||'';
  const destSegment=destination?flat.match(new RegExp(`\\b${destination}\\s+\\d{1,2}\\s+(?:${months})[A-Z]*,\\s*\\d{1,2}:\\d{2}\\s*\\(A\\)`,'i'))?.[0]||'':'';
  const arrivalDate=parseDate(destSegment),arrivalTime=parseTime(destSegment);
  const serial=digitsOnly(mawb).slice(3),awbMatched=Boolean(serial&&digitsOnly(flat).includes(serial));
  return{mawb:formatted||mawb,carrierCode:'AI',airlineName:'Air India Cargo',origin,destination,bags:pieces,pieces,weight,flightNo:'',arrivalDate,arrivalTime,arrivalIsActual:Boolean(arrivalDate||arrivalTime),status:(arrivalDate||arrivalTime)?'ARRIVED':'TRACKING',awbMatched};
}
...
function parseActivity(text='',fallback={}){
  ...
  const timeline=timelineIndex>=0?flat.slice(timelineIndex):flat;
  let arrival=null;
  ...
  const delivered=dest&&new RegExp(`\\bDELIVERY\\s+Delivered[\\s\\S]{0,420}?\\bat\\s+${dest}\\b`,'i').test(timeline);
...
export async function trackAirIndia(mawb){
  ...
  try{
    ...
    // Exact user-required flow: Air India link -> MAWB -> Next -> details screen -> screenshot -> fill tracker.
    ...
    // Screenshot is mandatory and is captured before any tracker row is returned.
    ...
    // Open Activity View only to verify exact flight and final status/time from the same shipment screen.
    ...
    shipment.source=activityClicked?'Air India Cargo details-screen screenshot + Activity View verification':'Air India Cargo details-screen screenshot + same-screen extraction';

    if(!useful(shipment))return{ok:false,reason:'AIR INDIA DETAILS FOUND NO VERIFIED SHIPMENT FIELDS',officialTracker:page.url(),screenshotCaptured:true,screenshotVerified:true,debug:{stage:'parse',summarySample:summaryText.slice(0,3000),activitySample:activityText.slice(0,3500)}};
