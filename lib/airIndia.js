...
function parseActivity(text='',fallback={}){
  ...
  if(!manifested&&manifestEvents.length&&Number(totalPieces||0)>0){
    ...
    manifestGroup=[...groups.values()].find(group=>{
      const p=group.reduce((n,m)=>n+Number(m[1]||0),0);
      const w=group.reduce((n,m)=>n+Number(String(m[2]||'').replace(/,/g,'')),0);
      return p===Number(totalPieces||0)&&(!Number(totalWeight||0)||Math.abs(w-Number(totalWeight||0))<1);
    })||[];
