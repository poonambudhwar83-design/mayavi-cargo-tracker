...
function parseSummary(text='',mawb=''){
  ...
  const originMatch=flat.match(originRx),destMatch=flat.match(destRx),scheduledArrivalMatch=flat.match(scheduledArrivalRx),simpleDestMatch=flat.match(simpleDestRx),flightRouteMatch=flat.match(flightRouteRx),routeMatch=flat.match(routeRx);
  ...
  const origin=routeOrigin||originMatch?.[1]?.toUpperCase()||'';
...
function parseActivity(text='',fallback={}){
  ...
  const departure=timeline.match(/\bDEPARTURE\s+Departed\s+(\d{1,6})\s+pcs\s+([\d,.]+)\s+kg[\s\S]{0,160}?\bon\s+([A-Z0-9]{2})[- ]?(\d{2,4})[A-Z]?\s*,\s*(\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[A-Za-z]*\s+20\d{2})\s+(\d{1,2}:\d{2})\s*\(A\)[\s\S]{0,60}?\bat\s+([A-Z]{3})\b/i);
  if(departure){
    return{origin:String(departure[7]).toUpperCase(),destination:String(fallback.destination||'').toUpperCase(),bags:departure[1],pieces:departure[1],weight:String(departure[2]).replace(/,/g,''),flightNo:`${String(departure[3]).toUpperCase()}${departure[4]}`,flightDate:parseDate(departure[5]),departureTime:parseTime(departure[6]),arrivalIsActual:false,status:'IN TRANSIT',departureEvidence:clean(departure[0])};
  }
  if(/\bDEPARTURE\s+Departed\b/i.test(timeline))return{weight:verifiedWeight,status:'IN TRANSIT'};

  // Freight On Hand (FOW) / Freight on Hand reflects the currently available
  // physical pieces and weight. Prefer it over the historical BOOKED quantity.
  const freightOnHand=timeline.match(/\b(?:FREIGHT\s+ON\s+HAND|FOW)\b[\s\S]{0,180}?(\d{1,6})\s+pcs\s+([\d,.]+)\s+kg\b/i)
    ||timeline.match(/\b(?:FREIGHT\s+ON\s+HAND|FOW)\b[\s\S]{0,220}?\b(\d{1,6})\s+(?:pcs|pieces)\b[\s\S]{0,80}?\b([\d,.]+)\s*kg\b/i);
  if(freightOnHand){
    const flight=timeline.match(/\b(?:AI|IX)\s*[- ]\s*\d{2,4}\b/i)?.[0]?.replace(/[-\s]/g,'').toUpperCase()||fallback.flightNo||'';
    return{
      origin:String(fallback.origin||'').toUpperCase(),
      destination:String(fallback.destination||'').toUpperCase(),
      bags:freightOnHand[1],pieces:freightOnHand[1],
      weight:String(freightOnHand[2]).replace(/,/g,''),
      flightNo:flight,
      arrivalIsActual:false,
      status:'BOOKED',
      activityMilestone:'FREIGHT ON HAND',
      freightOnHandEvidence:clean(freightOnHand[0])
    };
  }

  // MANIFESTED is a pre-departure event, but it contains the current operating flight,
  // pieces/weight, planned flight date and station. Read those fields instead of
  // falling through to a generic BOOKED status. This also supports IX (Air India Express)
  // flight numbers shown inside the Air India Cargo portal.
  const manifested=timeline.match(/\bMANIFESTED\s+Manifested\s+(\d{1,6})\s+pcs\s+([\d,.]+)\s+kg\s+for\s+([A-Z0-9]{2})[- ]?(\d{2,4})[A-Z]?\s*,\s*(\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[A-Za-z]*\s+20\d{2})\s+at\s+([A-Z]{3})\b/i);
  if(manifested){
    return{
      origin:String(fallback.origin||manifested[6]).toUpperCase(),
      destination:String(fallback.destination||'').toUpperCase(),
      bags:manifested[1],pieces:manifested[1],
      weight:String(manifested[2]).replace(/,/g,''),
      flightNo:`${String(manifested[3]).toUpperCase()}${manifested[4]}`,
      flightDate:parseDate(manifested[5]),
      arrivalIsActual:false,
      status:'BOOKED',
      activityMilestone:'MANIFESTED',
      manifestEvidence:clean(manifested[0])
    };
  }
  ...
  const bookedEvent=timeline.match(/\bBOOKED\s+Booked\s+\d{1,6}\s+pcs\s+[\d,.]+\s+kg\s+at\s+([A-Z]{3})\s+for\s+([A-Z0-9]{2})[- ]?(\d{2,4})[A-Z]?\s*,\s*(\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[A-Za-z]*\s+20\d{2})[\s\S]{0,160}?\b([A-Z]{3})\s*[•·]\s*(\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[A-Za-z]*\s+20\d{2})\s*,\s*(\d{1,2}:\d{2})(?::\d{2})?/i);
  if(bookedEvent)return{origin:String(fallback.origin||bookedEvent[1]).toUpperCase(),destination:String(fallback.destination||'').toUpperCase(),flightNo:`${String(bookedEvent[2]).toUpperCase()}${bookedEvent[3]}`,flightDate:parseDate(bookedEvent[4]),bookingDate:parseDate(bookedEvent[6]),bookingTime:parseTime(bookedEvent[7]),bookingDateSource:'Air India BOOKED event timestamp',weight:verifiedWeight,status:'BOOKED'};
  const booked=timeline.match(/\bBOOKED\s+Booked\s+\d{1,6}\s+pcs\s+[\d,.]+\s+kg\s+at\s+([A-Z]{3})\s+for\s+([A-Z0-9]{2})[- ]?(\d{2,4})[A-Z]?\s*,\s*(\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[A-Za-z]*\s+20\d{2})/i);
  if(booked)return{origin:String(fallback.origin||booked[1]).toUpperCase(),destination:String(fallback.destination||'').toUpperCase(),flightNo:`${String(booked[2]).toUpperCase()}${booked[3]}`,flightDate:parseDate(booked[4]),weight:verifiedWeight,status:'BOOKED'};
...
async function airIndiaFlightFallback(shipment={},flightDate=''){
  ...
  const m=flight.match(/^AI0*(\d{2,4})$/);const d=String(flightDate||'').match(/^(20\d{2})-(\d{2})-(\d{2})$/);
  if(!m||!d||!shipment.origin||!shipment.destination)return null;
  const url=`https://www.flightstats.com/v2/flight-tracker/AI/${Number(m[1])}?year=${d[1]}&month=${Number(d[2])}&date=${Number(d[3])}`;
  try{
    ...
    const scheduled=(block.match(/Scheduled\s+(\d{1,2}:\d{2})\s+(?:IST|UTC|GMT|[A-Z]{3})\b/i)||[])[1]||'';
    ...
    return{arrivalDate:flightDate,arrivalTime,arrivalIsActual:Boolean(arrived&&actual),status:arrived&&actual?'ARRIVED':'IN TRANSIT',flightStatusUrl:url,flightStatusSource:'FlightStats destination schedule/status'};
  }catch{return null;}
}
...
export async function trackAirIndia(mawb){
  ...
  try{
    ...
    // Activity View is authoritative. Never promote a BOOKED/MANIFESTED load from flight timing alone.
    if(shipment.status!=='BOOKED'&&!shipment.arrivalIsActual&&shipment.flightNo&&shipment.origin&&shipment.destination&&!shipment.arrivalTime&&activity.flightDate){
      flightFallback=await airIndiaFlightFallback(shipment,activity.flightDate);
      if(flightFallback)shipment=merge(shipment,flightFallback);
    }
