...
export async function trackAirIndia(mawb){
  ...
  try{
    ...
    // Exact user-required flow: enter MAWB -> Next -> result/details screen -> screenshot.
    ...
    const resultScreenshotBase64=await page.screenshot({type:'jpeg',quality:72,fullPage:false,encoding:'base64'}).catch(()=>null);

    // Fill core tracker fields from the visible result screen.
    const route=routeFromTracking(tracking),pw=piecesWeight(tracking);

    // Activity View is used only to verify the latest real event/flight; header milestone labels are ignored.
    let activity='';let activityClicked=false;
    const activitySel='[data-testid="tabs-panel__tab-activityView"]';
    if(await page.$(activitySel)){await page.click(activitySel);activityClicked=true;await sleep(1600);activity=await bodyText(page);}else activity=tracking;
    ...
    const shipment={
      ...
      status,source:'Air India Cargo Portal result-screen screenshot + verified Activity View'
    };
    if(!useful(shipment))return{ok:false,reason:'AIR INDIA PORTAL FOUND NO VERIFIED SHIPMENT FIELDS',officialTracker:page.url(),screenshotCaptured:Boolean(resultScreenshotBase64),debug:{stage:'parse',trackingSample:tracking.slice(0,2600),activitySample:activity.slice(0,3500)}};
    return{ok:true,shipment,screenshotCaptured:Boolean(resultScreenshotBase64),screenshotVerified:Boolean(resultScreenshotBase64),screenshotOcrUsed:false,debug:{stage:'done',url:page.url(),activityClicked,awbMatched,resultScreenshotCaptured:Boolean(resultScreenshotBase64),trackingSample:tracking.slice(0,2600),activitySample:activity.slice(0,3500),finalArrival:finalArrival||null}};
