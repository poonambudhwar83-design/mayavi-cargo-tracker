...
function parseSummary(text='',mawb=''){
  ...
  const flightRouteRx=/\bAI\s*[- ]\s*(\d{2,4})\b[\s\S]{0,220}?\b([A-Z]{3})\s*[–-]\s*([A-Z]{3})\b/i;
  ...
  const routeOrigin=(flightRouteMatch?.[2]||routeMatch?.[1]||'').toUpperCase();
  const routeDestination=(flightRouteMatch?.[3]||routeMatch?.[2]||'').toUpperCase();
  ...
  const flightNo=flightRouteMatch?`AI${flightRouteMatch[1]}`:'';
