...
function parseSummary(text='',mawb=''){
  ...
  const arrivalDate=destMatch?(parseDate(destMatch[2])||parseShortDate(destMatch[2])):scheduledArrivalMatch?parseDate(scheduledArrivalMatch[1]):'';
  const arrivalTime=destMatch?parseTime(destMatch[3]):scheduledArrivalMatch?parseTime(scheduledArrivalMatch[2]):'';
  ...
  return{mawb:formatted||mawb,carrierCode:'AI',airlineName:'Air India Cargo',origin,destination,bags:pieces,pieces,weight,flightNo,arrivalDate,arrivalTime,arrivalIsActual,status:destination?'BOOKED':'TRACKING',awbMatched};
}
...
function parseActivity(text='',fallback={}){
  ...
  // All pre-departure Air India milestones stay BOOKED. ETA alone must not promote them.
