...
function parseActivity(text='',fallback={}){
  ...
  if(arrival){
    const arrivedPieces=Number(arrival[1]||0),masterPieces=Number(totalPieces||0);
    const partial=masterPieces>0&&arrivedPieces>0&&arrivedPieces<masterPieces;
    const departureOrigin=(timeline.match(/\bDEPARTURE\s+Departed[\s\S]{0,420}?\bat\s+([A-Z]{3})\b/i)||[])[1]||'';
    return{origin:String(fallback.origin||departureOrigin||'').toUpperCase(),destination:String(arrival[7]).toUpperCase(),bags:fallback.bags||fallback.pieces||arrival[1],pieces:fallback.pieces||fallback.bags||arrival[1],weight:fallback.weight||String(arrival[2]).replace(/,/g,''),flightNo:`${String(arrival[3]).toUpperCase()}${arrival[4]}`,flightDate:parseDate(arrival[5]),arrivalDate:parseDate(arrival[5]),arrivalTime:parseTime(arrival[6]),arrivalIsActual:true,status:partial?'PART ARRIVED':'ARRIVED',arrivedPieces:String(arrival[1]),arrivedWeight:String(arrival[2]).replace(/,/g,''),remainingPieces:partial?String(masterPieces-arrivedPieces):'0',...masterBooking,arrivalEvidence:clean(arrival[0])};
  }
...
export async function trackAirIndia(mawb){
  ...
  try{
    ...
    if(shipment.status==='PART DEPARTED'&&shipment.remainingPieces){
