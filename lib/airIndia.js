...
function parseSummary(text='',mawb=''){
  ...
  // current shipment quantity. Do not let an older BOOKED event overwrite it.
  const topPieces=(flat.match(/\b(?:Pieces|Pcs)\s*:?\s*(\d{1,6})\b/i)||flat.match(/\b(\d{1,6})\s*pcs\b[\s\S]{0,90}?\b(?:Weight|Wt)\b/i)||[])[1]||'';
  const topWeight=((flat.match(/\b(?:Weight|Wt)\s*:?\s*([\d,.]+)\s*(?:kg|kgs)?\b/i)||flat.match(/\b(?:Pieces|Pcs)\s*:?\s*\d{1,6}\b[\s\S]{0,90}?\b([\d,.]+)\s*kg\b/i)||[])[1]||'').replace(/,/g,'');
  const pieces=topPieces||(flat.match(/\b098-\d{8}\b\s+(\d{1,6})\s+pcs\b/i)||flat.match(/\b(\d{1,6})\s+pcs\b/i)||[])[1]||'';
  const weight=topWeight||((flat.match(/\b098-\d{8}\b\s+\d{1,6}\s+pcs\s+([\d,.]+)\s+kg\b/i)||flat.match(/\b([\d,.]+)\s+kg\b/i)||[])[1]||'').replace(/,/g,'');
...
function parseActivity(text='',fallback={}){
  ...
  const fow=timeline.match(/\b(?:FREIGHT\s+ON\s+HAND|FOW)\b\s*(?:Freight\s+on\s+hand\s*)?(\d{1,6})\s+(?:pcs|pieces)\s+([\d,.]+)\s*kg\b/i);
  ...
  if(fow||manifested){
    const pieces=String(fow?.[1]||manifested?.[1]||fallback.pieces||fallback.bags||'');
    const weight=String(fow?.[2]||manifested?.[2]||fallback.weight||'').replace(/,/g,'');
