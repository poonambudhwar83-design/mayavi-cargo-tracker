...
function parseSummary(text='',mawb=''){
  ...
  const scheduledArrivalRx=new RegExp(`\\bArrival:\\s*(\\d{1,2}\\s+(?:${months})[A-Z]*\\s+20\\d{2})\\s+(\\d{1,2}:\\d{2})\\s*\\(S\\)`,'i');
  const simpleDestRx=/\b([A-Z]{3})\s*-\s*Tracking\s+View\b/i;
  const routeRx=/\b([A-Z]{3})\s*[–-]\s*([A-Z]{3})\b/;
  const originMatch=flat.match(originRx),destMatch=flat.match(destRx),scheduledArrivalMatch=flat.match(scheduledArrivalRx),simpleDestMatch=flat.match(simpleDestRx),routeMatch=flat.match(routeRx);
  const origin=originMatch?.[1]?.toUpperCase()||routeMatch?.[1]?.toUpperCase()||'';
  const destination=destMatch?.[1]?.toUpperCase()||simpleDestMatch?.[1]?.toUpperCase()||routeMatch?.[2]?.toUpperCase()||'';
  ...
  return{mawb:formatted||mawb,carrierCode:'AI',airlineName:'Air India Cargo',origin,destination,bags:pieces,pieces,weight,flightNo:'',arrivalDate,arrivalTime,arrivalIsActual,status:destination?'IN TRANSIT':'TRACKING',awbMatched};
}
...
function parseActivity(text='',fallback={}){
  ...
  if(booked)return{origin:String(booked[1]).toUpperCase(),destination:String(fallback.destination||'').toUpperCase(),flightNo:`${String(booked[2]).toUpperCase()}${booked[3]}`,flightDate:parseDate(booked[4]),weight:verifiedWeight,status:'BOOKED'};
...
export async function trackAirIndia(mawb){
  ...
  try{
    ...
    if(!shipment.arrivalIsActual&&(shipment.arrivalDate||shipment.arrivalTime)&&shipment.status!=='DELAYED')shipment.status='IN TRANSIT';
