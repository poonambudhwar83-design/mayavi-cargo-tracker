...
function parseActivity(text='',fallback={}){
  ...
  if(destinationArrivals.length){
    ...
    return{
      ...
      bags:masterPieces?pieceParts.join('+'):fallback.bags||fallback.pieces||latest[1],
      pieces:masterPieces?`${pieceParts.join('+')}/${masterPieces}`:pieceParts.join('+'),
...
export async function trackAirIndia(mawb){
  ...
  try{
    ...
    if(shipment.status!=='PART ARRIVED'&&shipment.status!=='ARRIVED'){
