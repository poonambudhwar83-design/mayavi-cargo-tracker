...
function parseActivity(text='',fallback={}){
  ...
  if(destinationArrivals.length){
    ...
    const partEvents=[];const partSeen=new Set();for(const m of [...dated,...departedForParts]){const key=[m[1],String(m[2]).replace(/,/g,''),m[3],m[4],parseDate(m[5])].join('|');if(!partSeen.has(key)&&Number(m[1]||0)<masterPieces){partSeen.add(key);partEvents.push(m);}}if(nextManifest&&Number(nextManifest[1]||0)<masterPieces){const key=[nextManifest[1],String(nextManifest[2]).replace(/,/g,''),nextManifest[3],nextManifest[4],parseDate(nextManifest[5])].join('|');if(!partSeen.has(key)){partSeen.add(key);partEvents.push(nextManifest);}}
