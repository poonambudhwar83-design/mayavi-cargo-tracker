...
async function airIndiaFlightFallback(shipment={},flightDate=''){
  ...
  try{
    ...
    return{arrivalDate:flightDate,arrivalTime,arrivalIsActual:Boolean(arrived&&actual),status:arrived?'ARRIVED':shipment.status||'IN TRANSIT',flightStatusUrl:url,flightStatusSource:'FlightStats destination schedule/status'};
  }catch{return null;}
}
...
export async function trackAirIndia(mawb){
  ...
  try{
    ...
    // ETA is required before departure too, because Mayavi's mail time is ETA - 5 hours.
    // As soon as Air India gives us the booked flight/date, resolve its destination ETA.
