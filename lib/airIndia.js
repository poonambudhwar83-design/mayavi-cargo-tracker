...
function parseVisibleResult(text='',mawb=''){
  ...
  const header=flat.match(/\b098-\d{8}\b\s+(\d{1,6})\s+pcs\s+([\d,.]+)\s+kg[\s\S]{0,260}?\b([A-Z]{3})\b[\s\S]{0,140}?\bAccepted\b[\s\S]{0,320}?\b([A-Z]{3})\b[\s\S]{0,120}?Tracking\s+View/i);
  ...
  const flights=[...upper.matchAll(/\b([A-Z0-9]{2})\s*[- ]?(\d{2,4})[A-Z]?\b/g)].map(m=>`${m[1]}${m[2]}`);
  const flightNo=flights.length?flights[flights.length-1]:'';

  // Air India uses (A) beside actual departure/arrival times. This is tied to the flight segment, unlike the generic milestone labels.
  const actual=around(flat,/ARRIVAL\s*:\s*[\s\S]{0,90}?\(A\)|ACTUAL\s+ARRIVAL|\bATA\b|RECEIVED\s+FROM\s+FLIGHT|\bRCF\b|\bLANDED\b|ARRIVAL\s+OF\s+SHIPMENT/i);
  ...
  }else if(/\bDLV\b|DELIVERED\s+TO\s+CONSIGNEE/i.test(flat))status='DELIVERED';
  else if(actual&&(arrivalDate||arrivalTime||/\bRCF\b|RECEIVED\s+FROM\s+FLIGHT|\bLANDED\b|ACTUAL\s+ARRIVAL|\bATA\b|\(A\)/i.test(actual)))status='ARRIVED';
  else if(/\bDEP\b|IN\s+TRANSIT|AIRBORNE|IN\s+FLIGHT|DEPARTURE\s*:/i.test(flat))status='IN TRANSIT';
...
export async function trackAirIndia(mawb){
  ...
  try{
    ...
    // Exact required flow: this Air India link -> MAWB -> Next -> details screen -> screenshot -> extract -> Mayavi tracker.
    ...
    if(!awbMatched)return{ok:false,reason:'AIR INDIA RESULT DID NOT MATCH MAWB',officialTracker:page.url(),debug:{stage:'details',bodySample:details.slice(0,2800)}};

    // Screenshot is captured before tracker fields are extracted.
    ...
    const shipment={...parseVisibleResult(details,mawb),officialTracker:page.url(),source:'Air India Cargo details-screen screenshot + same-screen extraction'};
    if(!useful(shipment))return{ok:false,reason:'AIR INDIA DETAILS FOUND NO VERIFIED SHIPMENT FIELDS',officialTracker:page.url(),screenshotCaptured:true,screenshotVerified:true,debug:{stage:'parse',bodySample:details.slice(0,3400)}};
    return{ok:true,shipment,screenshotCaptured:true,screenshotVerified:true,screenshotOcrUsed:false,debug:{stage:'done',url:page.url(),awbMatched:true,bodySample:details.slice(0,3400),screenExtraction:true}};
