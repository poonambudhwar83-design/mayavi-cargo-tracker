...
export async function trackAirIndia(mawb){
  ...
  try{
    ...
    if(shipment.departureIsActual&&shipment.departureDate&&shipment.departureTime&&String(shipment.departureTimeZone||'').toUpperCase()!=='IST'){
      const istDeparture=utcDateTimeToIst(shipment.departureDate,shipment.departureTime);
      if(istDeparture){shipment.departureDate=istDeparture.arrivalDate;shipment.departureTime=istDeparture.arrivalTime;shipment.departureTimeZone='IST';shipment.departureTimeSource=(shipment.departureTimeSource||'Air India Activity View')+'; converted from UTC to IST';}
    }
