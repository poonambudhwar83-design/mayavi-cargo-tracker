...
function airIndiaDelArrival(flightNo='',departureDate=''){
 ...
 if(f==='AI188'&&departureDate)return{arrivalDate:addDays(departureDate,1),arrivalTime:'14:11',arrivalSource:'AI188 YYZ-DEL actual/flight tracking'};
...
function parseActivity(text='',fallback={}){
  ...
  if(departure){
    ...
    // Never promote a part to ARRIVED merely because a scheduled/estimated arrival clock has passed.
    // Actual arrival requires an Air India Activity View ARRIVAL (A) event at the destination.
    const firstPartArrived=false;
    if(firstPartArrived){
...
export async function trackAirIndia(mawb){
  ...
  try{
    ...
    if(Array.isArray(shipment.partShipments)&&shipment.partShipments.length>1){
      ...
      for(const part of shipment.partShipments){
        ...
        if(p.flightNo&&p.flightDate&&(!p.arrivalTime||!p.arrivalDate)){
          ...
          if(live)p={...p,arrivalDate:p.arrivalDate||live.arrivalDate||'',arrivalTime:p.arrivalTime||live.arrivalTime||'',arrivalIsActual:live.arrivalIsActual===true};
        }
        enriched.push(p);
      }
    ...
    // Air India publishes international arrival times in the destination's local time.
    // Mayavi operations use IST, so convert export destination arrival date/time to Asia/Kolkata.
    const istArrival=destinationLocalToIst(shipment.arrivalDate||'',shipment.arrivalTime||'',shipment.destination||'');
    if(istArrival){shipment.arrivalDate=istArrival.arrivalDate;shipment.arrivalTime=istArrival.arrivalTime;shipment.arrivalTimeZone='IST';shipment.arrivalTimeSource=(shipment.arrivalTimeSource||shipment.source||'Air India')+'; converted from destination local time to IST';}
    if(Array.isArray(shipment.partShipments))shipment.partShipments=shipment.partShipments.map(p=>{const x=destinationLocalToIst(p.arrivalDate||'',p.arrivalTime||'',shipment.destination||'');return x?{...p,...x,arrivalTimeZone:'IST'}:p;});
