...
function parseActivity(text='',fallback={}){
  ...
  if(fow||manifested){
    ...
    return{
      ...
      bookingDate:fowStamped?parseDate(fowStamped[4]):'',
      bookingTime:fowStamped?parseTime(fowStamped[5]):'',
      bookingDateSource:fowStamped?'Air India Freight On Hand event timestamp':'',
  ...
  const bookedRx=/\bBOOKED\s+Booked\s+(\d{1,6})\s+pcs\s+([\d,.]+)\s+kg\s+at\s+([A-Z]{3})\s+for\s+([A-Z0-9]{2})[- ]?(\d{2,4})[A-Z]?\s*,\s*(\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[A-Za-z]*\s+20\d{2})[\s\S]{0,160}?\b([A-Z]{3})\s*[•·]\s*(\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[A-Za-z]*\s+20\d{2})\s*,\s*(\d{1,2}:\d{2})(?::\d{2})?/gi;
  const bookedEvent=[...timeline.matchAll(bookedRx)].find(m=>sameLoad(m[1],m[2]))||null;
