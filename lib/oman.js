...
function parseTime(s=''){const m=String(s).match(/\b([01]?\d|2[0-3]):([0-5]\d)(?::[0-5]\d)?\b/);return m?`${pad(m[1])}:${m[2]}`:'';}
...
function statusFrom(text=''){
  ...
  const last=(s.match(/LAST ACTIVITY[\s\S]{0,220}/)||[])[0]||'';
  const x=last||s;
...
function parseShipment(text,mawb){
  ...
  const bookedWin=lastWindow(flat,/\bBOOKED\b/ig,60,360);const bookingDate=parseDate(bookedWin);
...
export async function trackOman(input){
  ...
  try{
    ...
    const shipment=parseShipment(text,mawb);if(!useful(shipment))return{ok:false,reason:'OMAN AIR SMARTKARGO RESULT HAS NO SHIPMENT FIELDS',airline:AIRLINE,officialTracker:OFFICIAL,debug:{stage:'NO_FIELDS',resultUrl:page.url(),textSample:clean(text).slice(0,5000)}};
