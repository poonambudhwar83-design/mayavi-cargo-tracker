...
async function fetchFlightArrival({flightNo='',origin='',destination='',date=''}){
  ...
  try{
    ...
    return{arrivalDate:date,arrivalTime,arrivalIsActual:Boolean(actual),status:actual?'ARRIVED':'IN TRANSIT',source:actual?'Oman flight actual arrival':'Oman final-flight scheduled/estimated arrival',url};
  }catch{return null;}
}
...
export async function trackOman(input){
  ...
  try{
    ...
    if(!shipment.arrivalDate||!shipment.arrivalTime){const fa=await fetchFlightArrival({flightNo:shipment.flightNo,origin:shipment.finalLegOrigin,destination:shipment.finalLegDestination,date:shipment.finalFlightDate});if(fa){shipment={...shipment,arrivalDate:fa.arrivalDate,arrivalTime:fa.arrivalTime,arrivalIsActual:fa.arrivalIsActual,scheduledArrivalDate:fa.arrivalDate,scheduledArrivalTime:fa.arrivalTime,status:fa.status,source:`${shipment.source} + ${fa.source}`,arrivalSourceUrl:fa.url};}}
