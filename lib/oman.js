...
export async function trackOman(input){
  ...
  try{
    ...
    await page.goto(OFFICIAL,{waitUntil:'domcontentloaded',timeout:15000});await new Promise(r=>setTimeout(r,900));
    ...
    const shipment=parseShipment(combined,mawb);const screenshotBase64=await page.screenshot({type:'jpeg',quality:60,fullPage:false,encoding:'base64'}).catch(()=>null);
    if(useful(shipment))return{ok:true,airline:AIRLINE,shipment,screenshotCaptured:Boolean(screenshotBase64),screenshotVerified:Boolean(screenshotBase64),screenshotBase64,debug:{stage:'SUCCESS',setup,submit,captured:captured.length,textSample:clean(combined).slice(0,5000)}};
    return{ok:false,reason:'OMAN AIR OFFICIAL PAGE RETURNED NO VERIFIED SHIPMENT DATA',airline:AIRLINE,officialTracker:OFFICIAL,screenshotBase64,debug:{stage:'NO_DATA',setup,submit,captured:captured.length,textSample:clean(combined).slice(0,5000)}};
