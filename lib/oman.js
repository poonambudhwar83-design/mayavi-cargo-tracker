...
function parseShipment(text,mawb){
  ...
  if(finalEvent?.type==='ARRIVED'){
    arrivalDate=finalEvent.eventDate||finalEvent.flightDate;arrivalTime=finalEvent.eventTime||finalEvent.eta;arrivalIsActual=Boolean(arrivalDate||arrivalTime);
  }else if(finalEvent?.type==='DEPARTED'){
    arrivalDate=finalEvent.flightDate;arrivalTime=finalEvent.eta;arrivalIsActual=false;
  }else if(expected){
  ...
  let status=statusFrom(flat,destination);
  if(expected&&!finalEvent){
    status=leg?.flightDate&&expected.flightDate&&expected.flightDate<leg.flightDate?'EARLY ARRIVAL':'IN TRANSIT';
  }
  return{mawb,carrierCode:'WY',airlineName:AIRLINE.name,origin,destination,bags:pieces,pieces,weight,flightNo,bookingDate,arrivalDate,arrivalTime,arrivalIsActual,scheduledArrivalDate,scheduledArrivalTime,status,officialTracker:OFFICIAL,source:'Oman Air Cargo SmartKargo exact AWB result'};
}
