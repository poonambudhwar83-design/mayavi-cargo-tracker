...
function parse(text='',mawb='',source=''){
  ...
  // Reject directory/help pages with no shipment-specific evidence.
...
export async function trackEtihadAlternate(input){
  ...
  }catch(e){return{ok:false,reason:e?.message||'ETIHAD ALTERNATE BROWSER FAILED'};}finally{if(browser)await browser.close().catch(()=>{});}
}
