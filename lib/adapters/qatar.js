...
function parseDateTime(raw='') {
  ...
  const mo = monthNumber(m[2]); if (!mo) return {date:'',time:''};
...
function parseQatarText(raw,mawb) {
  ...
  let actual = Boolean(arrival.date);
...
async function fillAndSubmit(page,mawb) {
  ...
  return page.evaluate(({prefix,serial}) => {
    const allDeep=(selector)=>{
      ...
      const walk=root=>{
        if (!root?.querySelectorAll) return;
        for (const el of root.querySelectorAll(selector)) if (!seen.has(el)) {seen.add(el);out.push(el);}
        for (const el of root.querySelectorAll('*')) if (el.shadowRoot) walk(el.shadowRoot);
      };
      walk(document); return out;
    };
    const visible=el=>{const r=el.getBoundingClientRect();return r.width>3&&r.height>3&&!el.disabled;};
    const inputs=allDeep('input[type="text"]').filter(visible);
    const prefixInput=inputs.find(el=>Number(el.maxLength)===3&&(el.value==='157'||!el.value)) || null;
    const numberInput=inputs.find(el=>el!==prefixInput&&!el.readOnly&&visible(el)) || null;
    const button=allDeep('button,input[type="submit"],[role="button"]')
      .find(el=>visible(el)&&/track shipment/i.test(String(el.innerText||el.value||el.getAttribute('aria-label')||'')));

    if (!numberInput) return {ok:false,reason:'QATAR_AWB_NUMBER_INPUT_NOT_FOUND',inputCount:inputs.length};
    if (!button) return {ok:false,reason:'QATAR_TRACK_BUTTON_NOT_FOUND',inputCount:inputs.length};
    ...
    if (prefixInput) setValue(prefixInput,prefix);
    setValue(numberInput,serial);
    button.click();
    return {ok:true,prefixMode:prefixInput?'input':'page-default-157',serialLength:numberInput.value.length,button:String(button.innerText||button.value||'').trim()};
  },{prefix,serial});
}
...
export async function trackQatar(inputMawb) {
  ...
  let browser; const debug={stage:'QATAR_OPEN',airline:airline.name,officialUrl:QATAR_URL};
  try {
