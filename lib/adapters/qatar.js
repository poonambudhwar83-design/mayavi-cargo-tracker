...
async function clearAndType(page, handle, value) {
...
async function fillAndSubmit(page,mawb) {
  ...
  const prefixField=visible.find(x=>x.meta.maxLength===3&&(x.meta.value==='157'||!x.meta.value)) || null;
  const numberField=visible.find(x=>x!==prefixField&&!x.meta.readOnly) || null;
  if (!numberField) return {ok:false,reason:'QATAR_AWB_NUMBER_INPUT_NOT_FOUND',inputCount:visible.length};

  let prefixValue='157';
  if (prefixField && prefixField.meta.value!=='157') prefixValue=await clearAndType(page,prefixField.handle,prefix);
  else if (prefixField) prefixValue=prefixField.meta.value;

  const typedValue=await clearAndType(page,numberField.handle,serial);
  if (typedValue!==serial) {
    return {ok:false,reason:'QATAR_AWB_VALUE_DID_NOT_STICK',prefixMode:prefixField?'input':'page-default-157',serialLength:typedValue.length};
  }
  ...
  if (!trackButton) return {ok:false,reason:'QATAR_TRACK_BUTTON_NOT_FOUND',serialLength:typedValue.length};

  await trackButton.click({delay:70});
  return {ok:true,prefixMode:prefixField?'input':'page-default-157',prefixValue,serialLength:typedValue.length,button:buttonText};
}
