...
async function fillAndSubmit(page,mawb) {
  ...
  const valueToType=prefixField ? serial : digits;
  const typedValue=await clearAndType(page,numberField.handle,valueToType);
  if (typedValue!==valueToType) {
    return {
      ok:false,
      reason:'QATAR_AWB_VALUE_DID_NOT_STICK',
      prefixMode:prefixField?'separate-prefix':'single-full-mawb',
      enteredLength:typedValue.length,
      expectedLength:valueToType.length
    };
  }
  ...
  if (!trackButton) return {ok:false,reason:'QATAR_TRACK_BUTTON_NOT_FOUND',enteredLength:typedValue.length};
