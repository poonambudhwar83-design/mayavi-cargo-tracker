...
async function fillAndSubmit(page, mawb) {
  ...
  return page.evaluate(({ prefix, serial }) => {
    ...
    const visible = el => {
      ...
      return rect.width > 3 && rect.height > 3 && !el.disabled;
    };

    const textInputs = findAllDeep(document, 'input[type="text"]').filter(visible);
    const prefixInput = textInputs.find(el => Number(el.maxLength) === 3 && (el.value === '157' || !el.value));
    const numberInput = textInputs.find(el => el !== prefixInput && !el.readOnly && visible(el));
    const buttons = findAllDeep(document, 'button,input[type="submit"],[role="button"]');
    const button = buttons.find(el => visible(el) && /track shipment/i.test(String(el.innerText || el.value || el.getAttribute('aria-label') || '')));

    if (!prefixInput || !numberInput) {
      return {
        ok:false,
        reason:'QATAR_AWB_INPUTS_NOT_FOUND',
        inputCount:textInputs.length,
        fields:textInputs.map(el => ({ maxLength:Number(el.maxLength), value:String(el.value || '').slice(0,3) }))
      };
    }
