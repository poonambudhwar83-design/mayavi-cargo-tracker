...
function parseKuwait(text, mawb) {
  ...
  // 3) For future flights where DEP/RCF are still N/A, read the flight card.
  // Prefer an explicit scheduled arrival; otherwise calculate from the card's
  // scheduled departure time. This makes new 229 MAWBs work without new code.
  if (!arrival && cardArrival?.date && cardArrival?.time) {
    arrival = addHours(cardArrival.date, cardArrival.time, 0);
    sourceRule = 'flight card scheduled arrival';
  }
  if (!arrival && cardDeparture?.date && cardDeparture?.time) {
    departureDate = cardDeparture.date;
    departureTime = cardDeparture.time;
    arrival = addSevenHours(departureDate, departureTime);
    sourceRule = 'flight card scheduled departure + 7 hours';
  }

  // Last-resort legacy schedule only when the Kuwait card itself does not expose
  // a machine-readable time. This is no longer the primary future-flight logic.
  if (!arrival && flightNo && rowDate) {
    ...
    if (scheduledDepartureTime) {
      ...
      sourceRule = 'legacy schedule fallback + 7 hours';
    }
  }
