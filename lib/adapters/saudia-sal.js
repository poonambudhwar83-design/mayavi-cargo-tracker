...
function plannedSaudiaArrival(flightNo,flightDate,origin,destination){
  ...
  if(f==='SV760'&&(!o||o==='RUH')&&d==='DEL'&&flightDate)return{date:flightDate,time:'17:00',source:'SV760 scheduled RUH-DEL arrival'};
...
function analyse(events,meta={}){
  ...
  const destArr=ordered.filter(e=>e.code==='ARR'&&(!destination||e.station===destination));
  const arrived=total(destArr);
  ...
  const latestArr=destArr.at(-1);const latestMove=active?.events?.filter(e=>['FOW','DEP'].includes(e.code)).at(-1);const flightNo=active?.flightNo||latestArr?.flightNo||'';const flightDate=active?.flightDate||latestArr?.flightDate||'';const planned=latestArr?.date?{date:'',time:'',source:''}:plannedSaudiaArrival(flightNo,flightDate,meta.origin,meta.destination);return{master,arrived,remaining,status,loadStatus,discrepancy,flightNo,flightDate,arrivalDate:latestArr?.date||planned.date||'',arrivalTime:latestArr?.time||planned.time||'',arrivalPlanned:Boolean(!latestArr?.date&&planned.date),arrivalPlanSource:planned.source,actualArrival:latestArr?.date?`${latestArr.date}T${latestArr.time||'00:00'}:00`:null,latestMove,history};
}
