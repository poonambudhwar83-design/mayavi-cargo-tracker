    ...
    for(const frame of page.frames()){
      try{
        const marked=await frame.evaluate(awb=>{
          ...
          const matches=[...document.querySelectorAll('mat-option,.mat-mdc-option,.mat-option,[role="option"],li,[class*="option"],[class*="autocomplete"],button,a,div,span,p')]
            .filter(el=>visible(el)&&/^Add\\s*:/i.test(norm(el.innerText||el.textContent||''))&&norm(el.innerText||el.textContent||'').replace(/\\D/g,'').includes(awb));
          if(!matches.length)return false;
          matches.sort((a,b)=>norm(a.innerText||a.textContent||'').length-norm(b.innerText||b.textContent||'').length);
          const leaf=matches[0], option=leaf.closest('mat-option,.mat-mdc-option,.mat-option,[role="option"],li,[class*="option"],[class*="autocomplete"]')||leaf;
          option.setAttribute('data-mayavi-turkish-add','1'); option.scrollIntoView({block:'center'}); return true;
        },serial);
        if(!marked)continue;
        const h=await frame.$('[data-mayavi-turkish-add="1"]');
        if(h){await h.click({delay:100});await sleep(650);return true;}
      }catch{}
    }
    await sleep(300);
  }
...
export async function trackTurkishLive(input) {
  ...
  try {
    ...
    const addClicked = await clickAddOption(page, serial, 9000);
    await sleep(900);
