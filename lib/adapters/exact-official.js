...
function parseAirIndiaTimeline(raw='',mawb='',config=null) {
  ...
  if(latest.code==='DEPARTED') status='IN TRANSIT';
    ...
    status='ARRIVED';
    if(latest.date){ actual=true; arrival={date:latest.date,time:latest.time}; }
    ...
    if(actualArrival){ actual=true; arrival={date:actualArrival.date,time:actualArrival.time}; }
  }
