...
function parseAirIndiaTimeline(raw='',mawb='',config=null) {
  ...
  for(let i=0;i<lines.length;i++){
    ...
    events.push({index:i,code:m[1].toUpperCase().replace(/\s+/g,' '),pieces:pcs,weight:wt,flightNo:flight?('AI'+flight):'',station,date:dt.date,time:dt.time,block});
  }
  if(!events.length) return null;

  const priority=['ARRIVED','RCF','DELIVERED','DLV','DEPARTED','MANIFESTED','ACCEPTED','EXECUTED','FREIGHT ON HAND'];
  let latest=null;
  for(const p of priority){ latest=events.find(e=>e.code===p); if(latest) break; }
  if(!latest) latest=events[0];

  const manifested=events.find(e=>e.code==='MANIFESTED');
  const accepted=events.find(e=>e.code==='ACCEPTED');
  const executed=events.find(e=>e.code==='EXECUTED');
  const master=manifested||accepted||executed||latest;

  let status='TRACKING', actual=false, arrival={date:'',time:''};
  if(['ARRIVED','RCF'].includes(latest.code)){ status='ARRIVED'; actual=true; arrival={date:latest.date,time:latest.time}; }
  else if(['DELIVERED','DLV'].includes(latest.code)){ status='DELIVERED'; actual=true; arrival={date:latest.date,time:latest.time}; }
  else if(latest.code==='DEPARTED'){ status='IN TRANSIT'; }
  ...
  return {useful:true,shipment:shipmentObject(mawb,config,{origin:master.station||'',destination:'',pieces:master.pieces||'',weight:master.weight||'',flightNo:master.flightNo||'',arrival,actual,status,source:'official Air India activity timeline'})};
}
