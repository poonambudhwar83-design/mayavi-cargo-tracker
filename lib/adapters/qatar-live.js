...
async function submit(page,mawb){
  ...
  let p=visible.find(x=>x.m.max===3||x.m.val==='157'||/prefix/i.test(x.m.ph)); let n=visible.find(x=>x!==p&&(x.m.max===8||/awb|air waybill|shipment|number/i.test(x.m.ph)));
...
export async function trackQatarLive(input){
  ...
  try{
    ...
    const network=[]; page.on('response',async r=>{try{const u=r.url(),ct=String(r.headers()['content-type']||'');if(!/(track|shipment|cargo|aura|apex|croamis)/i.test(u)&&!/json|text/i.test(ct))return;const body=await r.text();if(body&&body.length<250000&&/(157|51777880|shipment|awb|origin|destination|arrival|pieces|weight|RCF|DLV|DEP)/i.test(body))network.push(body)}catch{}});
