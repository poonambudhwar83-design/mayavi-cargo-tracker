...
async function submit(page,mawb){
  ...
  if(serialValue!==serial){
    await sleep(250);
    const rendered=clean(await deepText(page));
    tokenized=rendered.includes(serial);
    if(!tokenized)return{ok:false,reason:'QATAR_NUMBER_NOT_ACCEPTED',enteredValue:serialValue};
  }
