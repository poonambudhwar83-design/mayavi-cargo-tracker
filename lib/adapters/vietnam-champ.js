...
function parseEvents(text = '') {
  ...
  // Older working Mayavi parser: CHAMP can render the event label on one line
  // and its date/quantity/flight details on the following lines. Preserve that
  // layout as a second parsing pass so VN fields do not go blank when CHAMP
  // changes whitespace/DOM grouping.
  for (let i=0;i<rows.length;i+=1) {
    const label = rows[i].match(/^(Booked|Booking On Hold|Received from shipper|Pre-manifested on flight|Manifested on flight|Departed|Flight departed|Arrived|Flight arrived|Received from flight|Consignee\\\/?Agent notified of arrival|Delivered)$/i)?.[1];
    if (!label) continue;
    const joined = clean(`${rows[i]} ${rows[i+1]||''} ${rows[i+2]||''}`);
    const dt = joined.match(/\\b(\\d{1,2}\\s+[A-Za-z]{3,9}\\s+\\d{4})\\s*-\\s*(\\d{1,2}:\\d{2})\\b/i);
    if (!dt) continue;
    const qty = joined.match(/\\b(\\d{1,6})\\s+pieces?\\s+([\\d,.]+)\\s*Kg\\b/i);
    const flight = joined.match(/\\b(VN\\s?\\d{2,4})\\s+(\\d{2}[A-Z]{3}\\d{2})\\b/i);
    const event = {
      label: clean(label),
      date: dateFromLong(dt[1]),
      time: dt[2].padStart(5,'0'),
      pieces: qty ? qty[1] : '',
      weight: qty ? qty[2].replace(/,/g,'') : '',
      flightNo: flight ? flight[1].replace(/\\s+/g,'').toUpperCase() : '',
      flightDate: flight ? dateFromFlightToken(flight[2]) : '',
    ...
    if (!events.some(e => e.label.toLowerCase()===event.label.toLowerCase() && e.date===event.date && e.time===event.time && e.flightNo===event.flightNo)) events.push(event);
  }
