...
function parseFlightPlanArrival(block='',fullText=''){
  const dates=[...String(block).matchAll(/\b(\d{1,2})\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)[A-Za-z]*\b/gi)];
  const times=[...String(block).matchAll(/\b([01]?\d|2[0-3]):([0-5]\d)\b/g)];
  if(!dates.length||!times.length)return{date:'',time:''};
  const d=dates[dates.length-1];
  const t=times[times.length-1];
  const year=(String(fullText).match(/\b(20\d{2})\b/)||[])[1]||String(new Date().getUTCFullYear());
  const mon=String(d[2]).toUpperCase();
  return{date:`${year}-${MONTH[mon]||''}-${pad(d[1])}`,time:`${pad(t[1])}:${t[2]}`};
}
function parseEtihadPage(text='',mawb=''){
  ...
  const planArrival=parseFlightPlanArrival(flightPlan,flat);
