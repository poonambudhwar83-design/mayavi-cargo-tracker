...
const CONFIG = {
  '235': { prefixLabel:/prefix|awb code/i, numberLabel:/awb|air waybill/i, button:/search|track/i },
  ...
  '160': { prefixLabel:/prefix|airline code/i, numberLabel:/awb|shipment|air waybill/i, button:/track|search/i },
...
function statusFrom(text=''){
  ...
  if (/\bRCS\b|booked|manifested|accepted/i.test(text)) return 'BOOKED';
  return 'TRACKING';
}
...
function parseText(text,mawb,airline){
  ...
  let arrival=parseDateTime((s.match(/(?:actual arrival|arrived|landed|\bARR\b)[\s\S]{0,180}/i)||[])[0]||'');
  let arrivalIsActual=Boolean(arrival.date);
  if(!arrival.date)arrival=parseDateTime((s.match(/(?:estimated arrival|expected arrival|scheduled arrival|\bETA\b)[\s\S]{0,220}/i)||[])[0]||'');
...
async function fillAndSubmit(page,mawb,cfg){
  ...
  for(const frame of page.frames()){
    ...
    await numberField.input.click({clickCount:3});await page.keyboard.press('Backspace');await numberField.input.type(value,{delay:25});
    const buttons=await frame.$$('button,input[type="submit"],input[type="button"],[role="button"]');
    for(const b of buttons){try{const m=await b.evaluate(el=>{const r=el.getBoundingClientRect();return{visible:r.width>4&&r.height>4&&!el.disabled,text:String(el.innerText||el.value||el.getAttribute('aria-label')||'').trim()}});if(m.visible&&cfg.button.test(m.text)){await b.click({delay:50});return true}}catch{}}
...
export async function trackMawb(input){
  ...
  try{
    ...
    await page.goto(airline.url,{waitUntil:'domcontentloaded',timeout:30000}); await sleep(1800);
    ...
    for(let i=0;i<14;i++){await sleep(i?750:1200);const text=await pageText(page);const parsed=parseText(text,mawb,airline);if(parsed.notFound)return{ok:false,notFound:true,reason:'NO SHIPMENT RECORD',airline,debug:{...debug,stage:'NO_RECORD'}};if(parsed.useful)return{ok:true,airline,shipment:parsed.shipment,debug:{...debug,stage:'SUCCESS'}};}
    return{ok:false,reason:'RESULT NOT MACHINE READABLE',airline,debug:{...debug,stage:'UNREADABLE',preview:clean(await pageText(page)).slice(0,600)}};
