...
async function clickMore(page,mawb){
  ...
  const ok=await page.evaluate(({digits,serial})=>{
    ...
    if(row)x=[...row.querySelectorAll('button,[role="button"],a,[tabindex],span,div')].filter(vis).find(e=>moreRx.test(txt(e)))||null;
    if(!x){
      const candidates=all.filter(e=>moreRx.test(txt(e))).map(e=>({e,r:e.getBoundingClientRect()}));
...
async function ocrScreenshot(base64,mawb){
  if(!base64)return{shipment:null,text:''};
  let worker;
  try{
    worker=await createWorker('eng');
    const r=await Promise.race([
      worker.recognize(Buffer.from(base64,'base64')),
      new Promise((_,reject)=>setTimeout(()=>reject(new Error('SAUDIA OCR TIMEOUT')),22000))
    ]);
    const text=r?.data?.text||'';
    return{shipment:parseInfo(text,mawb),text:clean(text).slice(0,1500)};
  }catch{return{shipment:null,text:''};}
  ...
  try{
    ...
    page.on('response',async r=>{
      try{
        ...
        if(network.length>=40||(!/track|shipment|awb|cargo|flight/i.test(u)&&!/json/i.test(ct)))return;
    ...
    if(!complete(shipment)){
      moreClicked=await clickMore(page,mawb);
      if(moreClicked){
        await sleep(1400);await translateEnglish(page);
        await page.evaluate(()=>window.scrollBy(0,Math.max(260,window.innerHeight*.3))).catch(()=>{});
        await sleep(350);
        text=await infoText(page);
        combined=`${text}\n${network.map(x=>x.text).join('\n')}`;
        shipment=mergeCore(shipment,parseInfo(combined,mawb));
      }
    }

    const screenshotBase64=await page.screenshot({type:'png',fullPage:false,encoding:'base64'}).catch(()=>null);
    let ocrText='';
    if(!complete(shipment)&&screenshotBase64){
      const ocr=await ocrScreenshot(screenshotBase64,mawb);
      if(ocr.shipment)shipment=mergeCore(shipment,ocr.shipment);
      ocrText=ocr.text;
    }

    if(!useful(shipment))return{ok:false,reason:'SAUDIA RESULT DID NOT SHOW PIECES / WEIGHT / FLIGHT',officialTracker:URL,screenshotBase64,debug:{stage:'NO_CORE_FIELDS',moreClicked,sample:clean(text).slice(0,1800),ocrText,networkUrls:network.map(x=>x.url).slice(-15)}};
    return{ok:true,shipment,screenshotBase64,debug:{stage:complete(shipment)?'ALL_3_CORE_FIELDS_FOUND':'PARTIAL_CORE_FIELDS_FOUND',moreClicked,sample:clean(text).slice(0,1800),ocrText,networkUrls:network.map(x=>x.url).slice(-15)}};
