...
function parseInfo(text='',mawb=''){
  const s=clean(text);
  const pieces=first(s,/(?:Total\s+(?:number\s+of\s+)?pieces|Number\s+of\s+pieces|Pieces?|Pcs?)\s*[:#\-]?\s*(\d{1,6})\b/i)
...
async function resultRowText(page,mawb,mark=false){
  ...
  return page.evaluate(({digits,serial,mark})=>{
    ...
    const rows=all.filter(v=>v.t.length>=10&&v.t.length<=3000).filter(v=>{const nd=v.t.replace(/\D/g,'');const hasAwb=nd.includes(digits)||nd.includes(serial);const hasFields=/(Total\s+number\s+of\s+pieces|pieces?|pcs?|More\s+information|件数|更多信息|附加信息)/i.test(v.t);return hasAwb&&hasFields;}).sort((a,b)=>a.t.length-b.t.length);
...
export async function trackSaudiaCardStep(input){
  ...
  try{
    ...
    page.on('response',async r=>{try{const u=r.url(),ct=r.headers()['content-type']||'';if(network.length>=60||(!/track|shipment|awb|cargo|flight/i.test(u)&&!/json/i.test(ct)))return;const t=await r.text();if(t&&t.length<220000){network.push({url:u,text:t});if(/\/apis\/api\/eservices\/track-shipment/i.test(u))saudiaApiText=t;}}catch{}});
    ...
    if(!rowText){
      ...
      return{ok:false,reason:'SAUDIA VERIFIED AWB RESULT CARD NOT FOUND',officialTracker:URL,screenshotBase64,debug:{stage:'NO_VERIFIED_AWB_RESULT',apiSample:clean(saudiaApiText).slice(0,5000),networkUrls:network.map(x=>x.url).slice(-15)}};
    }

    let shipment=parseInfo(rowText,mawb);
    let moreClicked=false,detail='';
    if(!complete(shipment)){
      moreClicked=await clickMore(page);
      if(moreClicked){await sleep(1300);await translateEnglish(page);detail=await detailsText(page);shipment=mergeCore(shipment,parseInfo(detail,mawb));}
    }
    const screenshotBase64=await page.screenshot({type:'png',fullPage:false,encoding:'base64'}).catch(()=>null);
    if(!useful(shipment))return{ok:false,reason:'SAUDIA VERIFIED RESULT DID NOT SHOW PIECES / WEIGHT / FLIGHT',officialTracker:URL,screenshotBase64,debug:{stage:'VERIFIED_RESULT_NO_CORE_FIELDS',moreClicked,row:clean(rowText).slice(0,1200),detail:clean(detail).slice(0,1200),apiSample:clean(saudiaApiText).slice(0,5000),networkUrls:network.map(x=>x.url).slice(-15)}};
    return{ok:true,shipment,screenshotBase64,debug:{stage:complete(shipment)?'ALL_3_VERIFIED_CORE_FIELDS_FOUND':'PARTIAL_VERIFIED_CORE_FIELDS_FOUND',moreClicked,row:clean(rowText).slice(0,1200),detail:clean(detail).slice(0,1200),apiSample:clean(saudiaApiText).slice(0,5000),networkUrls:network.map(x=>x.url).slice(-15)}};
