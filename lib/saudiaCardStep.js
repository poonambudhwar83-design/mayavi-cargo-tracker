...
export async function trackSaudiaCardStep(input){
  ...
  try{
    ...
    if(!useful(shipment))return{ok:false,reason:'SAUDIA RESULT DID NOT SHOW PIECES / WEIGHT / FLIGHT',officialTracker:URL,screenshotBase64,debug:{stage:'NO_CORE_FIELDS',moreClicked,sample:clean(text).slice(0,1800),networkUrls:network.map(x=>x.url).slice(-15)}};
    return{ok:true,shipment,screenshotBase64,debug:{stage:complete(shipment)?'ALL_3_CORE_FIELDS_FOUND':'PARTIAL_CORE_FIELDS_FOUND',moreClicked,sample:clean(text).slice(0,1800),networkUrls:network.map(x=>x.url).slice(-15)}};
