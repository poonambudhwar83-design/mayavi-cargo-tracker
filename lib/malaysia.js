  ...
  try{
    ...
    if(!shipment){
      ...
      if(!submitted){
        ...
        try{
          const mainEntry=network.find(x=>/\/assets\/main-[^/]+\\.js/i.test(String(x?.url||'')));
          const mainText=String(mainEntry?.body||'');
          const chunkName=(mainText.match(/MaskargoShipmentTracking-[A-Za-z0-9_-]+\\.js/)||[])[0]||'';
          if(chunkName){
            const chunkUrl=new URL(chunkName,mainEntry.url).href;
            const chunkText=await page.evaluate(async u=>{const r=await fetch(u,{credentials:'include',cache:'no-store'});return await r.text()},{u:chunkUrl}).catch(()=> '');
