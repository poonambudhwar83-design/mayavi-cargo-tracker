...
export async function trackMalaysia(input){
  ...
  try{
    ...
    lastDebug={stage:'MASKARGO_DIRECT_RESULT',deepUrl,submitted,endpointCandidates,endpointSnippets,resultUrl:page.url(),expandedHistory,network:network.map(x=>({url:x.url,status:x.status,sample:clean(x.body).slice(0,900)})).slice(-10),tables:extracted.tables.slice(0,8),textSample:clean(extracted.text).slice(0,8500)};
