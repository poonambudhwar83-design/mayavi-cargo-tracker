...
function endpointCallSnippetsFromNetwork(network=[]){
  const out=[];
  const needles=['/bin/mh/revamp/maskargo/shipment/tracking','shipmentTrackingApiEndpoint','shipment-tracking'];
  for(const n of network){
    ...
    for(const needle of needles){
      ...
      while(at>=0&&out.length<20){
        const start=Math.max(0,at-1200),end=Math.min(body.length,at+2200);
        const snippet=clean(body.slice(start,end));
        if(snippet&&!out.some(x=>x.snippet===snippet))out.push({url:n.url||'',needle,snippet});
...
function malaysiaComponentApiSnippets(network=[]){
  const out=[];
  const entries=network.filter(n=>/MaskargoShipmentTracking-.*\.js/i.test(String(n?.url||'')));
  const needles=['shipmentTrackingApiEndpoint','/bin/mh/revamp/maskargo/shipment/tracking','prefixNumber','awbNumber','prefix','awb'];
  for(const n of entries){
    const body=String(n?.body||'');
    for(const needle of needles){
      const lower=body.toLowerCase(), target=needle.toLowerCase();
      let at=lower.indexOf(target),count=0;
      while(at>=0&&count<8&&out.length<30){
        const snippet=clean(body.slice(Math.max(0,at-3200),Math.min(body.length,at+7000)));
        if(snippet&&!out.some(x=>x.snippet===snippet))out.push({url:n.url||'',needle,snippet});
        at=lower.indexOf(target,at+target.length);count++;
      }
    }
  }
  return out;
...
export async function trackMalaysia(input){
  ...
  try{
    ...
    const network=[],requestTrace=[];
    page.on('request',request=>{try{
      const u=request.url();
      if(/\/bin\/mh\/revamp\/maskargo\/shipment\/tracking|shipment.?tracking|awb/i.test(u)){
        requestTrace.push({url:u,method:request.method(),postData:request.postData()||'',headers:request.headers()});
        if(requestTrace.length>30)requestTrace.shift();
      }
    }catch{}});
    page.on('response',async response=>{try{
      ...
      if(body.length>240000){
        const lower=body.toLowerCase(),slices=[];
        const needles=['shipmenttrackingapiendpoint','/bin/mh/revamp/maskargo/shipment/tracking','prefixnumber','awbnumber','shipment tracking','shipmenttracking'];
        for(const needle of needles){
          let at=lower.indexOf(needle);
          let count=0;
          while(at>=0&&count<8){
            slices.push(body.slice(Math.max(0,at-1800),Math.min(body.length,at+3600)));
            at=lower.indexOf(needle,at+needle.length);count++;
          }
        }
        if(!slices.length)return;
        captured=slices.join('\n---MASKARGO_BUNDLE_HIT---\n');
      }
      if(captured.includes(serial)||captured.replace(/\D/g,'').includes(digits)||/shipment.?tracking|shipment details|flight details|awb|pieces|weight|airport departure|airport arrival/i.test(captured))network.push({url:u,status:response.status(),body:captured.slice(0,180000)});
    }catch{}});
    const deepUrl=trackingUrl(serial);
    // Open the exact working MASkargo deep link first. The query parameters are
    // prefixNumber=232 and awbNumber=<last 8 digits>, matching the official UI.
    await page.goto(deepUrl,{waitUntil:'domcontentloaded',timeout:30000});
    await sleep(3200);await acceptCookies(page);await sleep(700);
    await Promise.race([page.waitForNetworkIdle({idleTime:900,timeout:9000}).catch(()=>{}),sleep(5000)]);

    // Read a result rendered directly from the deep link before trying to type
    // into the form. This avoids failing when MASkargo renders a result view
    // where the input controls are no longer present.
    let extracted=await extractAll(page,network);
    let shipment=parseShipment(extracted,mawb);
    let submitted={ok:true,mode:'deep-link',prefixValue:'232',awbValue:serial,clicked:'DIRECT_URL',frameUrl:page.url()};

    if(!shipment){
      submitted=(await fillMalaysiaFormByTyping(page,serial))||await fillExactMalaysiaForm(page,serial);
      if(!submitted){
        const frameDiagnostics=[];
        for(const frame of page.frames()){try{
          frameDiagnostics.push(await frame.evaluate(()=>{
            ...
            const inputs=[...document.querySelectorAll('input,textarea,select')].map(e=>({
              ...
              value:e.value||'',aria:e.getAttribute('aria-label')||'',maxLength:e.maxLength||0
            })).slice(0,40);
            const buttons=[...document.querySelectorAll('button,a,[role="button"]')].map(e=>norm(e.innerText||e.textContent||e.value||e.getAttribute('aria-label')||'')).filter(Boolean).filter(x=>/shipment|track|search|awb|cargo/i.test(x)).slice(0,40);
            const scripts=[...document.scripts].map(s=>s.src||norm(s.textContent||'').slice(0,240)).filter(Boolean).slice(0,80);
            const metas=[...document.querySelectorAll('meta')].map(m=>({name:m.name||m.getAttribute('property')||'',content:m.content||''})).filter(x=>/model|page|api|endpoint|cargo|shipment/i.test((x.name||'')+' '+(x.content||''))).slice(0,30);
            const html=document.documentElement?.outerHTML||'';
            const needles=['shipmentTrackingApiEndpoint','/bin/mh/revamp/maskargo/shipment/tracking','shipment-tracking'];
            const hints=[];
            for(const needle of needles){const at=html.indexOf(needle);if(at>=0)hints.push(norm(html.slice(Math.max(0,at-900),Math.min(html.length,at+1800))))}
            return{url:location.href,inputs,buttons,scripts,metas,hints};
          }));
        }catch{}}
        let componentDebug=null;
        try{
          const mainEntry=network.find(x=>String(x?.url||'').includes('/assets/main-'));
          const candidates=endpointCandidatesFromNetwork(network);
          const chunkCandidate=candidates.find(v=>String(v||'').includes('MaskargoShipmentTracking-')&&String(v||'').includes('.js'))||'';
          const chunkName=String(chunkCandidate||'').split('/').pop()||'';
          if(chunkName&&mainEntry?.url){
            const chunkUrl=new URL(chunkName,mainEntry.url).href;
            const chunkText=await page.evaluate(async u=>{const r=await fetch(u,{credentials:'include',cache:'no-store'});return await r.text()},chunkUrl).catch(()=> '');
            const compact=String(chunkText||'');
            const lower=compact.toLowerCase(),needles=['shipmenttrackingapiendpoint','/bin/','shipment/tracking','shipment-tracking','prefixnumber','awbnumber','axios','fetch('];
            const hints=[];
            for(const needle of needles){
              let at=lower.indexOf(needle.toLowerCase()),count=0;
              while(at>=0&&count<10){
                hints.push(clean(compact.slice(Math.max(0,at-2200),Math.min(compact.length,at+5200))));
                at=lower.indexOf(needle.toLowerCase(),at+needle.length);count++;
              }
            }
            componentDebug={chunkUrl,size:compact.length,hints:[...new Set(hints)].slice(0,30)};
          }
        }catch(e){componentDebug={error:String(e?.message||e)}}
        return{ok:false,reason:'MASKARGO DEEP LINK RETURNED NO VERIFIED DETAILS AND FORM SUBMISSION FAILED',officialTracker:deepUrl,debug:{
          stage:'DIRECT_AND_FORM_FAILED',deepUrl,frames:page.frames().map(f=>f.url()),
          endpointCandidates:endpointCandidatesFromNetwork(network),endpointSnippets:endpointCallSnippetsFromNetwork(network),
          componentApiSnippets:malaysiaComponentApiSnippets(network),requestTrace,
          network:network.map(x=>({url:x.url,status:x.status,sample:clean(x.body).slice(0,1800)})).slice(-20),
          frameDiagnostics,componentDebug,textSample:clean(extracted.text).slice(0,8000)
        }};
      }
      await Promise.race([page.waitForNetworkIdle({idleTime:900,timeout:12000}).catch(()=>{}),sleep(9000)]);
      await sleep(900);
      extracted=await extractAll(page,network);
      shipment=parseShipment(extracted,mawb);
    }

    // If the result has a collapsed history, open it before final extraction.
    ...
    await sleep(700);
    extracted=await extractAll(page,network);
    shipment=parseShipment(extracted,mawb)||shipment;
    const endpointCandidates=endpointCandidatesFromNetwork(network);
    const endpointSnippets=endpointCallSnippetsFromNetwork(network);
    lastDebug={stage:'MASKARGO_DIRECT_RESULT',deepUrl,submitted,endpointCandidates,endpointSnippets,componentApiSnippets:malaysiaComponentApiSnippets(network),requestTrace,resultUrl:page.url(),expandedHistory,network:network.map(x=>({url:x.url,status:x.status,sample:clean(x.body).slice(0,1800)})).slice(-20),tables:extracted.tables.slice(0,8),textSample:clean(extracted.text).slice(0,8500)};
