...
const trackingUrl=serial=>`https://www.maskargo.com/en/shipment-tracking.html?prefixNumber=232&awbNumber=${encodeURIComponent(serial)}`;
...
async function fillMalaysiaFormByTyping(page,serial){
  for(const frame of page.frames()){try{
    ...
    await prefix.click({clickCount:3}).catch(()=>{});
...
async function malaysiaPageConfig(page){
  return page.evaluate(async()=>{
    try{
      const model=await fetch('/en/shipment-tracking.model.json',{credentials:'include',cache:'no-store'}).then(r=>r.json());
      const hits=[];
      const walk=(v,path='root')=>{
        if(!v||hits.length>50)return;
        if(Array.isArray(v)){v.forEach((x,i)=>walk(x,`${path}[${i}]`));return}
        if(typeof v==='object'){for(const [k,x] of Object.entries(v)){const p=`${path}.${k}`;if(/endpoint|api|shipmentTracking/i.test(k)||typeof x==='string'&&/shipment|tracking|api/i.test(x))hits.push({path:p,key:k,value:typeof x==='string'?x.slice(0,800):''});walk(x,p)}}
      };
      walk(model);
...
export async function trackMalaysia(input){
  ...
  try{
    ...
    lastDebug={stage:'MASKARGO_DIRECT_RESULT',deepUrl,submitted,pageConfig,resultUrl:page.url(),expandedHistory,network:network.map(x=>({url:x.url,status:x.status,sample:clean(x.body).slice(0,900)})).slice(-10),tables:extracted.tables.slice(0,8),textSample:clean(extracted.text).slice(0,8500)};
