...
function inputs(html=''){return[...String(html).matchAll(/<input\b[^>]*>/gi)].map(m=>{const tag=m[0];return{name:attr(tag,'name'),id:attr(tag,'id'),type:(attr(tag,'type')||'text').toLowerCase(),value:attr(tag,'value'),placeholder:attr(tag,'placeholder')};});}
...
export async function trackIndigo(input){
  ...
  try{
    ...
    if(!get.ok||/Access Denied|permission to access/i.test(getHtml))return{ok:false,reason:`INDIGO OFFICIAL GET BLOCKED (${get.status})`,officialTracker:URL,debug:{stage:'GET_BLOCKED',status:get.status,preview:clean(getHtml).slice(0,1800)}};
    ...
    const trackInput=list.find(i=>i.name&&['submit','button'].includes(i.type)&&/track/i.test(`${i.value} ${i.name} ${i.id}`));
    if(!prefixInput||!awbInput)return{ok:false,reason:'INDIGO FORM FIELDS NOT FOUND',officialTracker:URL,debug:{stage:'FORM_FIELDS',inputs:list.filter(i=>i.type!=='hidden').slice(0,30)}};
    ...
    if(!post.ok||/Access Denied|permission to access/i.test(html))return{ok:false,reason:`INDIGO OFFICIAL POST BLOCKED (${post.status})`,officialTracker:URL,debug:{stage:'POST_BLOCKED',status:post.status,prefixField:prefixInput.name,awbField:awbInput.name,trackField:trackInput?.name||'',preview:clean(html).slice(0,1800)}};
    const shipment=parseIndigoTracking(html,mawb),debug=shipment._debug;delete shipment._debug;
    if(useful(shipment))return{ok:true,shipment,officialTracker:URL,adapter:'IndiGo CarGo direct SmartKargo form',debug:{stage:'SUCCESS_DIRECT_FORM',status:post.status,prefixField:prefixInput.name,awbField:awbInput.name,trackField:trackInput?.name||'',...debug}};
    return{ok:false,reason:'INDIGO OFFICIAL FORM RETURNED NO VERIFIED SHIPMENT FIELDS',officialTracker:URL,debug:{stage:'NO_FIELDS',status:post.status,prefixField:prefixInput.name,awbField:awbInput.name,trackField:trackInput?.name||'',...debug}};
  }catch(e){return{ok:false,reason:`INDIGO DIRECT TRACKING ERROR: ${e?.message||e}`,officialTracker:URL,debug:{stage:'ERROR'}};}
}
