...
function parseIndigoTracking(html='',mawb=''){
  ...
  const flightNo=flights.length?flights.at(-1)[1].replace(/\s+/g,''):'';
  ...
  const departureFlight=(bestDepartureRow.match(/\b(6E\s*0*\d{1,4})\b/i)||[])[1]?.replace(/\s+/g,'')||'';
  ...
  if(!best&&dest){const at=upper.lastIndexOf(`ARRIVED AT ${dest}`);if(at>=0){const w=text.slice(Math.max(0,at-650),at+800);for(const p of dmyPairs(w))if(!best||p.stamp>best.stamp)best=p;bestRow=w;}}
  ...
  let status='TRACKING';
  if(dest&&new RegExp(`(?:LAST\\s+ACTIVITY[\\s\\S]{0,260}?)?DELIVERED\\s+AT\\s+${dest}\\b`,'i').test(text))status='DELIVERED';
  ...
  const shipment={mawb,carrierCode:'6E',airlineName:'IndiGo CarGo',origin,destination,bags:pieces,pieces,weight,flightNo,departureDate:bestDeparture?.date||'',departureTime:bestDeparture?.time||'',departureFlightNo:departureFlight||flightNo,departureOrigin:origin,departureDestination:destination,departureIsActual:Boolean(bestDeparture),departureTimeSource:bestDeparture?'IndiGo SmartKargo origin Departed event':'',arrivalDate,arrivalTime,arrivalIsActual,status,officialTracker:URL,source:'IndiGo CarGo official SmartKargo tracker',arrivalTimeSource:arrivalIsActual?'IndiGo final-destination Arrived event':'',_debug:{textSample:text.slice(0,7000),departureRows:departureRows.slice(-10),departureEvidence:bestDepartureRow.slice(0,1400),arrivalRows:arrivalRows.slice(-10),arrivalEvidence:bestRow.slice(0,1400)}};
  return applyFlightArrivalFallback(shipment,text);
}
