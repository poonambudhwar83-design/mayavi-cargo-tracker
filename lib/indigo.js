...
function parseIndigoTracking(html='',mawb=''){
  ...
  const flightDate=flightDateFromText(text,flightNo);
  ...
  for(const row of bookingRows){
    for(const p of dmyPairs(row)){
      if(!bookingEvent||p.stamp<bookingEvent.stamp)bookingEvent=p;
    }
  }
