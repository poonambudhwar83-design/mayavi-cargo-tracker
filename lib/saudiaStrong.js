...
function flight(v=''){const raw=clean(v).toUpperCase().replace(/\s+/g,'');const m=raw.match(/^(?:SV|SVA)?0*(\d{1,4}[A-Z]?)$/);if(m)return`SV${m[1]}`;const embedded=raw.match(/SV0*(\d{1,4}[A-Z]?)/);return embedded?`SV${embedded[1]}`:'';}
...
function reconcile(data){const headerExpectedDate=flightDateOnly(data?.Date||data?.ExpectedArrivalDate||data?.EstimatedArrivalDate||'');const destination=clean(data?.Destination).toUpperCase(),origin=clean(data?.Origin).toUpperCase(),totalPieces=n(data?.TotalPieces),totalWeight=n(data?.TotalWeight);const raw=(Array.isArray(data?.Parts)?data.Parts:[]).flatMap(p=>Array.isArray(p?.Events)?p.Events:[]);const events=raw.map(ev).sort((a,b)=>a.epoch-b.epoch||a.sequence-b.sequence);const rcs=events.find(x=>x.code==='RCS'&&x.pieces);const rcsRaw=rcs?.raw||{};const rcsText=[rcsRaw?.Message,rcsRaw?.Remarks,rcsRaw?.Description,rcsRaw?.EventDescription].map(clean).join(' ');const rcsFlight=flight(rcsRaw?.Flight_Number)||flight(rcsText);const masterPieces=rcs?.pieces||totalPieces,masterWeight=rcs?.weight||totalWeight;
  ...
  const fully=masterPieces>0&&arrived.pieces>=masterPieces&&(masterWeight<=0||arrived.weight>=masterWeight-.5);let status='TRACKING',loadStatus='',discrepancy='';let selected=arrivals.at(-1)||events.at(-1)||{};
  ...
  const resolvedFlightNo=active?.flightNo||actual?.flightNo||selected.flightNo||rcsFlight||'';
  const resolvedFlightDate=active?.flightDate||actual?.flightDate||selected.flightDate||'';
  const fowToDelhi=selected?.code==='FOW'&&selected?.destination==='DEL';
  const currentDelhiPlan=!fully&&destination==='DEL'&&Boolean(resolvedFlightNo)&&Boolean(resolvedFlightDate)&&/^(BKD|RCS|MAN|FOW|DEP)$/.test(selected?.code||'');
  ...
  const expectedDate=fowToDelhi?(selected.plannedArrivalDate||flightDateOnly(resolvedFlightDate)):(currentDelhiPlan?flightDateOnly(resolvedFlightDate):(destination==='DEL'&&!fully&&/^(RCS|BKD|SIM)$/.test(selected?.code||'')?headerExpectedDate:''));
  const expectedTime=fowToDelhi?(selected.plannedArrivalTime||''):(!fully&&destination==='DEL'&&resolvedFlightNo==='SV760'&&expectedDate?'17:00':'');
